...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f9/v3/XVGj7pNlRGCHrpaTly0fnA/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T085050Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=99853776EC67AD52EB3B1035422F86FA57E2D0AE771D825182EB6940C946F7C8)
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;[GSNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/spatial-recon-spatialrender#gsnode)&gt; | Promise对象，返回3DGS模型对应的GSNode。 |
...
if (renderContext != null) {
  ...
  let scene = Scene.load().then(async (scene: Scene) => {
    ...
    let gsNodeext: spatialRender.GSNode =
      await spatialRender.GSPlugin.loadGSNode(
        scene,
        { uri, offset },
        scene.root,
      );
...
if (renderContext != null) {
  ...
  let scene = Scene.load().then(async (scene: Scene) => {
    ...
    let gsNodeext: spatialRender.GSNode =
      await spatialRender.GSPlugin.loadGSNode(scene, setting, scene.root);
  });
}
```
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: Effect = await rf.createEffect({
      effectId: spatialRender.GSPlugin.RETRO_EFFECT_ID,
    });
    // 获取复古效果参数（颜色数量）的当前值
    let colNum = effect.getPropertyValue(
      spatialRender.RetroEffectParams.COLOR_NUM,
    );
    // 设置复古效果参数（颜色数量）为4
    let res = effect.setPropertyValue(
      spatialRender.RetroEffectParams.COLOR_NUM,
      4,
    );
    // 创建摄像机
    let camera = await rf.createCamera(
      { name: 'gsCam', path: '//gsCam' },
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: spatialRender.RetroEffect = (await rf.createEffect({
      effectId: spatialRender.GSPlugin.RETRO_EFFECT_ID,
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: Effect = await rf.createEffect({
      effectId: spatialRender.GSPlugin.COMIC_EFFECT_ID,
    });
    // 获取漫画效果参数（线条阈值）的当前值
    let threshold = effect.getPropertyValue(
      spatialRender.ComicEffectParams.LINE_THRESHOLD,
    );
    // 设置漫画效果参数（线条阈值）为0.5
    let res = effect.setPropertyValue(
      spatialRender.ComicEffectParams.LINE_THRESHOLD,
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: spatialRender.ComicEffect = (await rf.createEffect({
      effectId: spatialRender.GSPlugin.COMIC_EFFECT_ID,
    })) as spatialRender.ComicEffect;
    // 创建摄像机
    let camera = await rf.createCamera(
      { name: 'gsCam', path: '//gsCam' },
      { renderingPipeline: RenderingPipelineType.FORWARD },
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: Effect = await rf.createEffect({
      effectId: spatialRender.GSPlugin.OBRA_DINN_EFFECT_ID,
    });
    // 获取黑白bit效果参数（噪声强度）的当前值
    let noiseStrength = effect.getPropertyValue(
      spatialRender.ObraDinnEffectParams.NOISE_STRENGTH,
    );
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: spatialRender.ObraDinnEffect = (await rf.createEffect({
      effectId: spatialRender.GSPlugin.OBRA_DINN_EFFECT_ID,
    })) as spatialRender.ObraDinnEffect;
    // 创建摄像机
    let camera = await rf.createCamera(
      { name: 'gsCam', path: '//gsCam' },
      { renderingPipeline: RenderingPipelineType.FORWARD },
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: Effect = await rf.createEffect({
      effectId: spatialRender.GSPlugin.COLOR_EDITING_EFFECT_ID,
    });
    // 获取颜色编辑效果参数（曝光度）的当前值
    let exposure = effect.getPropertyValue(
      spatialRender.ColorEditingEffectParams.EXPOSURE,
    );
    // 设置颜色编辑效果参数（曝光度）为0.5
    let res = effect.setPropertyValue(
      spatialRender.ColorEditingEffectParams.EXPOSURE,
...
if (renderContext != null) {
  ...
  Scene.load().then(async (scene: Scene) => {
    ...
    let effect: spatialRender.ColorEditingEffect = (await rf.createEffect({
      effectId: spatialRender.GSPlugin.COLOR_EDITING_EFFECT_ID,
    })) as spatialRender.ColorEditingEffect;
    // 创建摄像机
    let camera = await rf.createCamera(
      { name: 'gsCam', path: '//gsCam' },
      { renderingPipeline: RenderingPipelineType.FORWARD },
