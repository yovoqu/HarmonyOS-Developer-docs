...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| inputs | Record<string, number \| Vec2 \| Vec3 \| Vec4 \| Image> | 是 | 一个字符串到值的映射，用于设置着色器输入。 |
...
| blend20+ | Blend | 否 | 是 | 材质的透明效果设置，默认值为undefined。 |
...
| clearCoat | MaterialProperty | 否 | 否 | 透明图层，用于在材质表面叠加一层具有反光特性的透明图层，可模拟车漆、碳纤、被水打湿的表面等材质的光泽表现。 |
...
| environmentRotation23+ | Quaternion | 否 | 是 | 环境光的旋转，默认为undefined，接收参数需为归一化后的四元数。 模型约束： 此接口仅可在Stage模型下使用。 |
