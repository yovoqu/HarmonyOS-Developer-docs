  ...
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene) => {
        ...
        let shader: Shader = await sceneFactory.createShader(
  ...
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene) => {
        ...
        let sceneNodeParameter: SceneNodeParameters = {
          name: 'empty_node',
...
function createShaderResource(): Promise<Shader> {
  ...
  const renderResourceFactory: RenderResourceFactory =
    renderContext.getRenderResourceFactory();
  ...
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene) => {
        ...
        // 创建相机
        let camera: Camera =
          await sceneFactory.createCamera(sceneCameraParameter);
        resolve(camera);
      })
      .catch((error: Error) => {
        console.error('Scene load failed:', error);
  ...
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene) => {
        ...
        // 创建相机
        let camera: Camera = await sceneFactory.createCamera(
          nodeParameter,
          camParameter,
        );
        resolve(camera);
  ...
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene) => {
        ...
        // 创建平行光
        let light: Light = await sceneFactory.createLight(
          sceneLightParameter,
...
function createMaterialPromise(): Promise<Material> {
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene) => {
        let sceneFactory: SceneResourceFactory = result.getResourceFactory();
        let sceneMaterialParameter: SceneResourceParameters = {
          name: 'material',
        };
        // 创建材质
        let material: Material = await sceneFactory.createMaterial(
          sceneMaterialParameter,
          MaterialType.SHADER,
        );
        resolve(material);
      })
  ...
  return new Promise((resolve, reject) => {
    ...
    scene
      .then(async (result: Scene | undefined) => {
        ...
        // 根据立方体几何数据创建网格资源
        let meshRes = await sceneFactory.createMesh(
          { name: 'MeshName' },
          cubeGeom,
        );
        console.info('TEST createGeometryPromise');
        // 根据场景节点参数和网格资源创建几何对象
        let geometry: Geometry = await sceneFactory.createGeometry(
          { name: 'GeometryName' },
          meshRes,
        );
        resolve(geometry);
...
function registerResourcePath(): void {
  ...
  Scene.load($rawfile('shaders/custom_shader/custom_material_sample.shader'))
    .then(() => {
      ...
      return renderContext.registerResourcePath(
        'myproto',
        'OhosRawFile://shaders/custom_shader/',
      );
    })
    .then((result) => {
      if (result) {
...
async function loadModelFromAbsolutePath(
...
function ImportNodeTest() {
  Scene.load().then(async (result: Scene | undefined) => {
    ...
    // 加载场景资源，支持.gltf和.glb格式，路径和文件名可根据项目实际资源自定义
    Scene.load($rawfile('gltf/AnimatedCube/glTF/AnimatedCube.glb')).then(
      async (extScene: Scene) => {
        let extNode = extScene.getNodeByPath(
          'rootNode_/Unnamed Node 1/AnimatedCube',
        );
...
function ImportSceneTest() {
  Scene.load().then(async (result: Scene | undefined) => {
    ...
    let content = await result
      .getResourceFactory()
      .createScene($rawfile('gltf/DamagedHelmet/glTF/DamagedHelmet.glb'));
...
function RenderFrameTest() {
  // 加载场景资源，支持.gltf和.glb格式，路径和文件名可根据项目实际资源自定义
  Scene.load($rawfile('gltf/DamagedHelmet/glTF/DamagedHelmet.glb')).then(
    async (result: Scene | undefined) => {
      if (!result) {
        return;
      }
...
function CloneNode() {
  // 加载场景资源，支持.gltf和.glb格式，路径和文件名可根据项目实际资源自定义
  Scene.load($rawfile('gltf/CubeWithFloor/glTF/AnimatedCube.gltf')).then(
    async (result: Scene) => {
      let node = result.getNodeByPath(
        'rootNode_/Unnamed Node 1/AnimatedCube',
      ) as Node;
