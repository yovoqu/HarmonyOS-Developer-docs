...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-graphics-displaysync
...
| 类型 | 说明 |
| --- | --- |
| DisplaySync | 返回DisplaySync对象实例，用于设置帧率范围、注册帧回调函数以及控制回调的启动和停止。 |
...
| callback | Callback&lt;IntervalInfo&gt; | 是 | 订阅帧变化的回调函数。IntervalInfo包含timestamp（当前帧到达时间）和targetTimestamp（下一帧预期到达时间）两个属性，单位均为纳秒。 |
...
| --- | --- | --- | --- |
| type | 'frame' | 是 | 设置回调的类型（只能是'frame'类型）。 |
| callback | Callback&lt;IntervalInfo&gt; | 否 | 传入调用on('frame')时注册的回调函数，用于取消订阅该回调函数。必须在已通过on('frame')注册回调后使用。 |
...
> [!NOTE]
> start接口会将DisplaySync关联到UI上下文和窗口。若在非UI页面或异步回调中调用 start ，可能获取到错误的UI上下文，导致 start 功能异常，进而导致回调函数无法执行以及期望帧率范围无法生效。 此时可使用 runScopedTask 接口指定UI上下文，确保 start 在正确的上下文中执行。
