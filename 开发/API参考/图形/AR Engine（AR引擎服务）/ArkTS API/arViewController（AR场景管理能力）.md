...
let context: arViewController.ARViewContext =
...
Scene.load()
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to load scene. Code is ${err.code}, message is ${err.message}.`,
    );
  });
```
  ...
  async onFrameUpdate(
    ...
    if (arSession) {
      ...
      if (!frame) {
        ...
        console.info(
          `Succeeded in getting arSession.frame = ${frame.timestamp}`,
        );
        await frame.release();
      }
    } else {
