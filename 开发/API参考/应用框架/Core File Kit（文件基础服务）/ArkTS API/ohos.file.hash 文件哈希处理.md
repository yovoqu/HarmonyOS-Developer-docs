...
function hashFileWithStream() {
  ...
  rs.on('data', (emitData) => {
    ...
    hs.update(
      new Uint8Array(data?.split('').map((x: string) => x.charCodeAt(0)))
        .buffer,
    );
  });
  rs.on('close', async () => {
...
const hs = hash.createHash('sha256');
hs.update(
  new Uint8Array('1234567890'?.split('').map((x: string) => x.charCodeAt(0)))
    .buffer,
);
hs.update(
  new Uint8Array('abcdefg'?.split('').map((x: string) => x.charCodeAt(0)))
    .buffer,
);
const hashResult = hs.digest();
