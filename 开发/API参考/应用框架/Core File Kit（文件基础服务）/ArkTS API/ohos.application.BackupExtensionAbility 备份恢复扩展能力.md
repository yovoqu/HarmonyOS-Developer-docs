...
class BackupExt extends BackupExtensionAbility {
  onBackupEx(backupInfo: string): string {
    try {
      ...
      console.error(
        `BackupExt error. Code:${err.code}, message:${err.message}`,
      );
    }
    return '';
  }
...
class BackupExt extends BackupExtensionAbility {
  ...
  async onRestoreEx(
    ...
    try {
      ...
      console.error(
        `onRestoreEx error. Code:${err.code}, message:${err.message}`,
      );
    }
    return '';
  }
...
class BackupExt extends BackupExtensionAbility {
  async onRelease(scenario: number): Promise<void> {
    try {
      ...
      console.error(
        `onRelease failed with error. Code: ${error.code}, message: ${error.message}`,
      );
    }
  }
}
