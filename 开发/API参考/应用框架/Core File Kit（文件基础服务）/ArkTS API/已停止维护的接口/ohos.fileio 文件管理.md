...
fileio
  .chown(filePath, stat.uid, stat.gid)
  .then(() => {
...
let stat = fileio.statSync(filePath);
fileio
  .lchown(filePath, stat.uid, stat.gid)
  .then(() => {
    console.info('chown succeed');
  })
  .catch((err: BusinessError) => {
...
let watcher = fileio.createWatcher(
  filePath,
  1,
  (err: BusinessError, event: number) => {
    console.info('event: ' + event + 'errmsg: ' + JSON.stringify(err));
  },
);
watcher.stop().then(() => {
...
ss.read(
  arrayBuffer,
  option,
  (err: BusinessError, readResult: fileio.ReadOut) => {
