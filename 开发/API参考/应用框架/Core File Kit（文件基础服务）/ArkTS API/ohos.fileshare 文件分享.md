...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-fileshare
  ...
  try {
    ...
    fileShare
      ...
      .catch((err: BusinessError<Array<fileShare.PolicyErrorResult>>) => {
        console.error(
          'persistPermission failed with error message: ' +
            err.message +
            ', error code: ' +
            err.code,
        );
        if (err.code == 13900001 && err.data) {
          for (let i = 0; i < err.data.length; i++) {
            ...
            console.error(
              'error reason : ' + JSON.stringify(err.data[i].message),
            );
          }
        }
      });
...
async function revokePermissionExample() {
  try {
    ...
    fileShare
      ...
      .catch((err: BusinessError<Array<fileShare.PolicyErrorResult>>) => {
        console.error(
          'revokePermission failed with error message: ' +
            err.message +
            ', error code: ' +
            err.code,
        );
...
async function activatePermissionExample() {
  try {
    ...
    let policies: Array<fileShare.PolicyInfo> = [policyInfo];
    fileShare
      .activatePermission(policies)
      .then(() => {
        console.info('activatePermission successfully');
      })
      .catch(async (err: BusinessError<Array<fileShare.PolicyErrorResult>>) => {
        console.error(
          'activatePermission failed with error message: ' +
            err.message +
            ', error code: ' +
            err.code,
        );
        if (err.code == 13900001 && err.data) {
          for (let i = 0; i < err.data.length; i++) {
            ...
            if (
              err.data[i].code ==
              fileShare.PolicyErrorCode.PERMISSION_NOT_PERSISTED
            ) {
...
async function deactivatePermissionExample() {
  try {
    ...
    let policies: Array<fileShare.PolicyInfo> = [policyInfo];
    fileShare
      .deactivatePermission(policies)
      .then(() => {
        console.info('deactivatePermission successfully');
      })
      .catch((err: BusinessError<Array<fileShare.PolicyErrorResult>>) => {
        console.error(
          'deactivatePermission failed with error message: ' +
            err.message +
            ', error code: ' +
            err.code,
        );
    ...
    let err: BusinessError = error as BusinessError;
    console.error(
      'deactivatePermission failed with err: ' + JSON.stringify(err),
    );
  }
}
```
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;Array&lt;boolean&gt;&gt; | Promise对象。返回true表示有持久化授权；false表示��具有持久化授权。 |
...
async function checkPersistentPermissionExample() {
  try {
    ...
    let policies: Array<fileShare.PolicyInfo> = [policyInfo];
    fileShare
      .checkPersistentPermission(policies)
      .then(async (data) => {
        let result: Array<boolean> = data;
        for (let i = 0; i < result.length; i++) {
          console.info(
            'checkPersistentPermission result: ' + JSON.stringify(result[i]),
          );
          if (!result[i]) {
      ...
      .catch((err: BusinessError<Array<fileShare.PolicyErrorResult>>) => {
        console.error(
          'checkPersistentPermission failed with error message: ' +
            err.message +
            ', error code: ' +
            err.code,
        );
    ...
    let err: BusinessError = error as BusinessError;
    console.error(
      'checkPersistentPermission failed with err: ' + JSON.stringify(err),
    );
  }
}
```
