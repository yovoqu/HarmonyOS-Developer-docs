...
export default class EntryAbility extends UIAbility {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    console.info('EntryAbility onCreate');
    try {
      webview.WebviewController.setHttpDns(
        webview.SecureDnsMode.AUTO,
        'https://example1.test',
      );
    } catch (error) {
      console.error(
...
1. 首先，在EntryAbility中将[UIContext](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-uicontext)存到[localStorage](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-localstorage)中。       __PREBLOCK_146__
2. 编写动态组件所需基础代码。       __PREBLOCK_147__
3. 编写用于生成字节码缓存的组件，本例中的本地Javascript资源内容通过文件读取接口读取rawfile目录下的本地文件。       __PREBLOCK_148__       JavaScript资源的获取方式也可通过[网络请求](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-http)的方式获取，但此方法获取到的HTTP响应头非标准HTTP响应头格式，需额外将响应头转换成标准HTTP响应头格式后使用。如通过网络请求获取到的响应头是e-tag，则需要将其转换成E-Tag后使用。
4. 编写业务用组件代码。       __PREBLOCK_149__
5. 编写资源配置信息。       __PREBLOCK_150__
6. 在页面中使用。       __PREBLOCK_151__
...
1. 首先，在EntryAbility中将[UIContext](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-uicontext)存到[localStorage](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-localstorage)中。       __PREBLOCK_155__
2. 编写动态组件所需基础代码。       __PREBLOCK_156__
3. 编写用于注入资源的组件代码，本例中的本地资源内容通过文件读取接口读取rawfile目录下的本地文件。       __PREBLOCK_157__
4. 编写业务用组件代码。       __PREBLOCK_158__
5. 编写资源配置信息。       __PREBLOCK_159__
6. 在页面中使用。       __PREBLOCK_160__
7. 加载的HTML网页示例。       __PREBLOCK_161__
...
export default class EntryAbility extends UIAbility {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    ...
    if (
      pageUpdateTimeNow > pageUpdateTime &&
      pageUpdateTime < pageUpdateTime1
    ) {
      // 清除指定页面的白屏插帧方案缓存
      try {
        webview.WebviewController.clearBlanklessLoadingCache([
          'https://www.example.com',
...
export default class EntryAbility extends UIAbility {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    ...
    webview.WebviewController.initializeWebEngine();
    // 设置快速销毁模式
    webview.WebviewController.setWebDestroyMode(
      webview.WebDestroyMode.FAST_MODE,
    );
    AppStorage.setOrCreate('abilityWant', want);
    console.info('EntryAbility onCreate done');
  }
...
export default class EntryAbility extends UIAbility {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    console.info('EntryAbility onCreate');
    webview.WebviewController.setActiveWebEngineVersion(
      webview.ArkWebEngineVersion.M114,
    );
    if (
...
> 不使用该方法时，默认不开启User-Agent Client Hints���能。
