...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview-aipageinteraction
...
> [!NOTE]
> command必须为JSON对象字符串。 method字段取值区分大小写，需使用 命令总览 中列出的取值。 返回值非空时为JSON字符串，应用可通过JSON.parse解析后使用。 当网页不可用、命令无法执行或无结果返回时，接口返回空字符串。 查询类命令请参见 AIPageCommand 。
...
| --- | --- | --- | --- | --- |
| click | 点击目标元素 | ClickCommand | 无 | 使目标元素响应点击事件(click event)，不关注是否产生真实的鼠标事件(mouse event)。 |
| focus | 使目标元素获取焦点 | FocusCommand | 无 | 将焦点移动到目标元素，使其可以接收键盘输入等后续交互。 |
| cursor_position | 获取当前文本插入符位置 | 无 | CursorPositionResult | 获取当前页面中文本插入符（caret）的位置，坐标相对于Web组件。 |
| type | 向目标元素输入文本 | TypeCommand | 无 | 在目标元素指定位置插入文本，支持先清空后输入。若目标元素未获取焦点，先获取焦点再输入。 |
| send_keys | 发送键盘事件 | SendKeysCommand | 无 | 向前端发送键盘事件，支持功能键、数字键、字母键、符号键、编辑键、导航键和修饰键。 |
| scroll | 页面滚动 | ScrollCommand | CommandResult | 根据坐标点和偏移量滚动页面。 |
| select | 选中下拉选项 | SelectCommand | CommandResult | 选中&lt;select&gt;标签的选项。 |
| uploadFile | 文件上传 | UploadFileCommand | CommandResult | 设置<input type='file'>标签的文件列表。 |
| setZoomLevel | 设置网页缩放比例 | SetZoomLevelCommand | CommandResult | 设置当前网页的缩放比例，等价于CTRL+Wheel缩放。 |
...
| 参数 | 类型 | 说明 |
| --- | --- | --- |
| xpath | string | 目标元素的XPath，可通过getFullDom或getLiteDom获取。 |
| nodeid | string | 目标元素的节点标识，可通过getFullDom或getLiteDom返回的id字段获取。 |
...
> [!NOTE]
> params中必须包含xpath或nodeid中的一个，用于定位目标元素。 若目标元素未获取焦点，会先使其获取焦点再执行输入操作。 index为0时表示在已有文本的最前面插入。 当clear为true时，会先清空目标元素中的全部文本，再在index位置插入text。
...
| params | - | - | Object | 是 | 命令参数。 |
| params | - | key | string | 是 | 需要发送的按键名称，取值请参见send_keys的params.key字段取值说明。发送字母时大小写敏感。 |
| params | - | keyaction | string | 是 | 按键动作类型，取值为keydown或keyup。 |
 
 
> [!NOTE]
> 发送字母时大小写敏感，例如KeyA和Keya表示不同的按键。 可通过连续发送keydown和keyup模拟完整的按键操作。
...
> [!NOTE]
> scroll命令为即发即忘模式，命令发送成功后立即返回结果，不等待手势执行完成。 连续发送的滚动命令由Chromium手势控制器排队顺序执行，不会丢失命令。 最终滚动距离可能因浮点数舍入存在微小偏差（通常小于1像素）。
...
> [!TIP]
> 选中的option包含被禁用项时，返回{"code":255}。 多选元素（<select multiple>）可传入多个索引；单选元素传入多个索引时返回错误。
...
> [!WARNING]
> 文件路径经过规范化处理（解析符号链接）。路径无法解析时返回{"code":370,"message":"failed to resolve file path: &lt;path&gt;"}。 路径存在性和可读性为非阻塞警告，不影响命令执行。警告信息附加在message字段中，格式为"success; warnings: &lt;path&gt;: &lt;reason&gt;"。 xpath匹配到非input[type=file]元素时，上传被拒绝，返回{"code":352,"message":"upload target is not file input"}。
...
> [!NOTE]
> 当应用通过 zoomControlAccess 禁用缩放时，本命令返回{"code":482}。 zoomLevel为非数字类型（字符串、数组、对象、null）时，统一按缺少字段处理，返回{"code":391}。 NaN/Infinity/-Infinity不是合法JSON数值，会在JSON解析阶段被拒绝，不会进入本命令。
