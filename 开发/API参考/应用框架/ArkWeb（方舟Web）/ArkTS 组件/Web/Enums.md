...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| Default^9+ | 0 | 优先使用未过期cache加载资源，无效或无cache时从网络获取。 |
...
| THREAT_NONE^21+ | 4 | 安全检查通过，未发现任何风险。 |
| THREAT_UNPROCESSED^21+ | 5 | 未进行安全检查。 |
...
| VIDEO_CAPTURE^10+ | TYPE_VIDEO_CAPTURE | 视频捕获资源，例如相机。 | 相机权限：ohos.permission.CAMERA。 |
| AUDIO_CAPTURE^10+ | TYPE_AUDIO_CAPTURE | 音频捕获资源，例如麦克风。 | 麦克风权限：ohos.permission.MICROPHONE。 |
| SENSOR^12+ | TYPE_SENSOR | 传感器资源，例如加速度传感器。 | 加速度传感器权限：ohos.permission.ACCELEROMETER、  陀螺仪传感器权限：ohos.permission.GYROSCOPE。 |
...
| UPDATE | 1 | 同层标签更新。 |
| DESTROY | 2 | 同层标签销毁。 |
| ENTER_BFCACHE^12+ | 3 | 同层标签进入BFCache。 |
| LEAVE_BFCACHE^12+ | 4 | 同层标签离开BFCache。 |
...
| RETURN_TO_UICONTEXT^22+ | 3 | Web组件的软键盘避让行为将跟随UIcontext设置的[KeyboardAvoidMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-e#keyboardavoidmode11)模式，Web组件不再处理组件的避让。 |
...
| --- | --- | --- |
| IMAGE | 1 | 网页元素为图像类型。 |
| LINK^20+ | 2 | 网页元素为超链接类型。 |
| TEXT^21+ | 3 | 网页元素为文本或可编辑区域类型。 |
