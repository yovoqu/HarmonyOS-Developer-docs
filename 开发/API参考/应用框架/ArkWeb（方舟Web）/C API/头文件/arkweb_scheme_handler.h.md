...
| [typedef void (*ArkWeb_OnRequestStop)(const ArkWeb_SchemeHandler* schemeHandler,const ArkWeb_ResourceRequest* resourceRequest)](#arkweb_onrequeststop) | ArkWeb_OnRequestStop | 请求完成时的回调函数。这将在IO线程上被调用。 应该使用ArkWeb_ResourceRequest_Destroy销毁resourceRequest， 并使用ArkWeb_ResourceHandler_Destroy销毁在ArkWeb_OnRequestStart中接收到的ArkWeb_ResourceHandler。 |
...
| char** value | 请求头的值（value）。调��者必须使用OH_ArkWeb_ReleaseString函数来释放这个字符串。 |
