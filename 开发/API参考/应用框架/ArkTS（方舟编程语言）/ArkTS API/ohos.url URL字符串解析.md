...
let urlObject1 = url.URL.parseURL(
...
```ts
const myURLObject = url.URL.parseURL(
...
); // Output 'https://www.example.com/test/path2/path3'
...
```ts
const urlObject = url.URL.parseURL(
...
```ts
let objectParams = new url.URLSearchParams([
  ['user1', 'abc1'],
  ['query2', 'first2'],
  ['query3', 'second3'],
]);
...
```ts
let searchParamsObject = new url.URLSearchParams(
