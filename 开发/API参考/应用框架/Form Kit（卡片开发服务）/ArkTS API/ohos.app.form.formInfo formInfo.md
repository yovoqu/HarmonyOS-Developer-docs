...
| supportDimensions | Array&lt;number&gt; | 否 | 否 | 卡片支持的规格。具体可选规格参考[FormDimension](#formdimension)  元服务API： 从API version 11开始，该接口支持在元服务中使用。 说明： 最大长度为9，数值取值范围[1, 9]的的整数的数组，数值5从API version 9开始支持，从API version 20开始废弃。 |
...
| UPDATE_FORM_REASON_KEY24+ | 'ohos.extra.param.key.update_form_reason' | [卡片更新的原因](#formupdatereason24)。 元服务API： 从API version 24开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
