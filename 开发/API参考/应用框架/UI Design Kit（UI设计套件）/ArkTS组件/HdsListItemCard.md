...
| stateControllerId | string | 否 | 是 | 指定特定[唯一标识ID](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-component-id#id)的子组件。配置[AccessibilityGroupOptions](#accessibilitygroupoptions)的容器组件进行无障碍聚合后，会将该特定标识的子组件的选中状态和状态播报文本作为聚合组件的状态和播报文本。从而聚合���幕朗读下的状态播报，避免需要对子组件单独进行聚焦。 说明： 如果聚合组件内有多个相同类型的子组件，则以组件树上该聚合组件下的第一个查找到的子组件为控制组件。 如果与stateControllerRoleType同时配置，则优先匹配ID一致的组件。 不支持跨进程嵌入式组件内的特定类型，例如：卡片、EmbededUiextension。 默认值：无指定组件 |
...
![](assets/HdsListItemCard/file-20260514164502615-0.jpg)
