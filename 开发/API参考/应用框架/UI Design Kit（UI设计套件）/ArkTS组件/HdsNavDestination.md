...
> [!NOTE]
> HdsNavDestination组件推荐配合HdsNavigation使用，作为HdsNavigation目的页面的根节点。如果单独使用HdsNavDestination，只能作为普通容器组件，不具备路由相关属性能力。 如果路由栈中间页面的生命周期发生变化，跳转之前的栈顶HdsNavDestination的生命周期(onWillShow, onShown, onHidden, onWillDisappear)与跳转之后的栈顶HdsNavDestination的生命周期(onWillShow, onShown, onHidden, onWillDisappear)均在最后触发。 不支持在HdsNavDestination上添加 zIndex 属性，会覆盖掉系统设置的层级，可能导致出现显示异常。
