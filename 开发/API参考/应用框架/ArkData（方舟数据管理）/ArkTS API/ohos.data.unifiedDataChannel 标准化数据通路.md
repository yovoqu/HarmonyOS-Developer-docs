...
let getDelayData: unifiedDataChannel.GetDelayData = (type: string) => {
  if (type == uniformTypeDescriptor.UniformDataType.PLAIN_TEXT) {
    ...
    let text = new unifiedDataChannel.UnifiedRecord(
...
let link = new unifiedDataChannel.UnifiedRecord(
...
for (let i = 0; i < records.length; i++) {
  ...
  if (types.includes(uniformTypeDescriptor.UniformDataType.PLAIN_TEXT)) {
    ...
    let hyperlink = record.getEntry(
      uniformTypeDescriptor.UniformDataType.HYPERLINK,
    ) as unifiedDataChannel.Hyperlink;
...
let hasPlainText = unifiedData.hasType(
  uniformTypeDescriptor.UniformDataType.PLAIN_TEXT,
);
let hasLink = unifiedData.hasType(
  uniformTypeDescriptor.UniformDataType.HYPERLINK,
);
```
...
let hyperlinkRecord = new unifiedDataChannel.UnifiedRecord(
  uniformTypeDescriptor.UniformDataType.HYPERLINK,
  hyperlink,
);
...
let pixelMapRecord = new unifiedDataChannel.UnifiedRecord(
...
let unifiedData = new unifiedDataChannel.UnifiedData();
let record = new unifiedDataChannel.UnifiedRecord(
  uniformTypeDescriptor.UniformDataType.OPENHARMONY_FORM,
  form,
);
record.addEntry(uniformTypeDescriptor.UniformDataType.FILE_URI, fileUri);
unifiedData.addRecord(record);
...
for (let i = 0; i < records.length; i++) {
  ...
  let fileUriRead: uniformDataStruct.FileUri = unifiedDataRecord.getEntry(
  ...
  let formRead = unifiedDataRecord.getEntry(
    uniformTypeDescriptor.UniformDataType.OPENHARMONY_FORM,
  ) as uniformDataStruct.Form;
...
for (let i = 0; i < records.length; i++) {
  let unifiedDataRecord = records[i] as unifiedDataChannel.UnifiedRecord;
  let entries: Record<string, unifiedDataChannel.ValueType> =
    unifiedDataRecord.getEntries();
  let formRead: uniformDataStruct.Form = entries[
    uniformTypeDescriptor.UniformDataType.OPENHARMONY_FORM
  ] as uniformDataStruct.Form;
  if (formRead != undefined) {
    console.info(`formName: ${formRead.formName}`);
  }
  let fileUriRead: uniformDataStruct.FileUri = entries[
    uniformTypeDescriptor.UniformDataType.FILE_URI
  ] as uniformDataStruct.FileUri;
  if (fileUriRead != undefined) {
    console.info(`oriUri: ${fileUriRead.oriUri}`);
...
for (let i = 0; i < records.length; i++) {
  ...
  if (types.includes(uniformTypeDescriptor.UniformDataType.OPENHARMONY_FORM)) {
    console.info(
      `Types include: ${uniformTypeDescriptor.UniformDataType.OPENHARMONY_FORM}`,
    );
  }
}
```
  ...
  if (error) {
    ...
    for (let i = 0; i < records.length; i++) {
      if (
        ...
        let pixelMapRecord = records[
          i
        ] as unifiedDataChannel.SystemDefinedPixelMap;
        let newArrayBuf = pixelMapRecord.rawData.buffer;
        pixelMap
          .writeBufferToPixels(newArrayBuf)
          .then(() => {
...
try {
  unifiedDataChannel.insertData(options, unifiedData, (err, key) => {
    if (err === undefined) {
      ...
      console.error(
        `Failed to insert data. code is ${err.code}, message is ${err.message} `,
      );
    }
  });
} catch (e) {
...
try {
  unifiedDataChannel
    .insertData(options, unifiedData)
    .then((key) => {
      console.info(`Succeeded in inserting data. key = ${key}`);
    })
...
try {
  unifiedDataChannel
    ...
    .then((key) => {
      ...
      let textUpdate = new unifiedDataChannel.UnifiedRecord(
        uniformTypeDescriptor.UniformDataType.PLAIN_TEXT,
        plainTextUpdate,
      );
...
try {
  unifiedDataChannel
    ...
    .then((key) => {
      ...
      try {
        unifiedDataChannel
          .updateData(updateOptions, unifiedDataUpdate)
          .then(() => {
            console.info('Succeeded in updating data.');
          })
          .catch((err: BusinessError) => {
...
try {
  unifiedDataChannel.queryData(options, (err, data) => {
    if (err === undefined) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (let j = 0; j < records.length; j++) {
          if (
            records[j]
              .getTypes()
              .includes(uniformTypeDescriptor.UniformDataType.PLAIN_TEXT)
          ) {
            let text = records[j].getEntry(
              uniformTypeDescriptor.UniformDataType.PLAIN_TEXT,
      ...
      console.error(
        `Failed to query data. code is ${err.code}, message is ${err.message} `,
      );
    }
  });
} catch (e) {
...
try {
  unifiedDataChannel.deleteData(options, (err, data) => {
    if (err === undefined) {
      ...
      console.error(
        `Failed to delete data. code is ${err.code}, message is ${err.message} `,
      );
    }
  });
} catch (e) {
...
try {
  unifiedDataChannel.setAppShareOptions(
    unifiedDataChannel.Intention.DRAG,
  ...
  let error: BusinessError = e as BusinessError;
  console.error(
    `[UDMF]setAppShareOptions throws an exception. code is ${error.code}, message is ${error.message} `,
  );
}
```
...
try {
  ...
  let error: BusinessError = e as BusinessError;
  console.error(
    `[UDMF]removeAppShareOptions throws an exception. code is ${error.code}, message is ${error.message} `,
  );
}
```
...
let plainText = new unifiedDataChannel.UnifiedRecord(
  uniformTypeDescriptor.UniformDataType.PLAIN_TEXT,
  plainTextObj,
);
let html = new unifiedDataChannel.UnifiedRecord(
...
try {
  ...
  let records: Array<unifiedDataChannel.UnifiedRecord> =
    unifiedData.getRecords();
  console.info(`Records size is ${records.length}`); // After conversion, its length must be less than 1
  if (records.length == 1) {
    let plainTextObjRead: uniformDataStruct.PlainText = records[0].getEntry(
      uniformTypeDescriptor.UniformDataType.PLAIN_TEXT,
    ) as uniformDataStruct.PlainText;
    console.info(`TextContent is ${plainTextObjRead.textContent}`);
    let htmlObjRead: uniformDataStruct.HTML = records[0].getEntry(
      uniformTypeDescriptor.UniformDataType.HTML,
    ) as uniformDataStruct.HTML;
    console.info(`HtmlContent is ${htmlObjRead.htmlContent}`);
  }
} catch (e) {
  let error: BusinessError = e as BusinessError;
  console.error(
    `Convert data throws an exception. code is ${error.code}, message is ${error.message} `,
  );
}
```
