  ...
  async onWindowStageCreate(windowStage: window.WindowStage) {
    ...
    if (store != undefined) {
      (store as relationalStore.RdbStore).executeSql(sqlCreateTable, (err) => {
        if (err) {
          console.error(
            `custom tokenizer example: ExecuteSql failed, code is ${err.code},message is ${err.message}`,
          );
          return;
        }
...
| DATA_STATUS_FIELD12+ | '#_data_status' | 用于cursor查找的结果集返回时填充的字段，返回的结果集中，该字段对应的0表示正常数据，1表示退出账号保留���据，2表示云侧同步删除，3表示退出账户删除数据。 |
