...
| int OH_RDB_SetReturningFields(OH_RDB_ReturningContext *context, const char *const fields[], int32_t len) | 设置结果集中返回的字段。 |
| int OH_RDB_SetMaxReturningCount(OH_RDB_ReturningContext *context, int32_t count) | 设置返回结果集的最大行数量。 |
| OH_Cursor *OH_RDB_GetReturningValues(OH_RDB_ReturningContext *context) | 获取数据变化的游标，默认包含1024行。 |
