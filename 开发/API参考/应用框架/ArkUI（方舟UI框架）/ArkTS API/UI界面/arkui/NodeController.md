...
> [!NOTE]
> NodeContainer 不支持跨实例复用。如果出现跨实例复用 NodeContainer ，传入 NodeContainer 的 NodeController 触发 makeNode 回调方法时，入参中的 UIContext 对象可能为undefined，此时需要开发者判断入参中的 UIContext 对象是否为undefined，防止后续使用此入参时出现 UIContext无效的JS异常 。
