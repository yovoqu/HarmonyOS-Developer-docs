...
> [!NOTE]
> 从API version 9开始支持，从API version 18开始废弃，建议使用getHostContext替代。 getHostContext 需先获取 UIContext 实例对象后再进行获取。
...
> [!NOTE]
> 直接使用getContext可能导致 UI上下文不明确 的问题，建议使用getUIContext()获取 UIContext 实例，并使用 getHostContext 调用绑定实例的getContext。
