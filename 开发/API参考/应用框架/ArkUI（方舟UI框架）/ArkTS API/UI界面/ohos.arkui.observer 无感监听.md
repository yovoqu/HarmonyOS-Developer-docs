...
| mode15+ | NavDestinationMode | 否 | 是 | NavDestination类型。 元服务API： 从API version 15开始，该接口支持在元服务中使用。 |
...
| uniqueId20+ | number | 否 | 是 | Navigation组件的uniqueId，可以通过queryNavigationInfo获取。 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
...
| axis20+ | Axis | 否 | 是 | 滚动组件的滚动方向。 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | UIAbilityContext \| UIContext | 否 | 否 | 触发生命周期的routerPage页面对应的上下文信息。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 |
