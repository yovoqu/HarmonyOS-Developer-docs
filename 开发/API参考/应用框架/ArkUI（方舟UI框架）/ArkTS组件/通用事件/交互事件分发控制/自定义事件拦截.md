...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-on-touch-intercept
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback<TouchEvent, HitTestMode> | 是 | 自定义事件拦截回调。在做触摸测试时回调此函数。通过返回值设置组件的HitTestMode。使用TouchEvent中的touches属性前，需先校验其是否为空。 |
