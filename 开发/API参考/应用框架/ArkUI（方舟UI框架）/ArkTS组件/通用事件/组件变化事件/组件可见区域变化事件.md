...
> [!NOTE]
> 从API version 20开始，该接口支持在 attributeModifier 中调用。 仅提供自身节点相对于所有祖先节点（直到window边界）的相对裁切面积与自身面积的比值及其变化趋势。 不支持兄弟组件对自身节点的遮挡计算，不支持所有祖先的兄弟节点对自身节点的遮挡计算，不支持窗口遮挡计算，不支持组件旋转计算，如 Stack 、 Z序控制 、 rotate 等。 不支持非挂树节点的可见面积变化计算。例如，预加载的节点、通过 overlay 能力挂载的自定义节点。 不支持 scale 属性，如果想要支持 scale ，则需使用 onVisibleAreaChange 22+ ，将measureFromViewport设置为true。
...
> [!NOTE]
> 此接口与 onVisibleAreaChange 接口存在如下差异：onVisibleAreaChange在每一帧都会进行可见区域比例的计算，如果注册节点太多，系统功耗存在劣化。而此接口降低了可见区域比例计算的频度，计算间隔由 VisibleAreaEventOptions 的expectedUpdateInterval参数决定。 当前接口的可见区域回调阈值默认包含0。例如，开发者设置回调阈值为[0.5]，实际生效的阈值为[0.0, 0.5]。 从API version 18开始，支持在自定义组件中调用该接口。 不支持 scale 属性，如果想要支持 scale ，则需将 VisibleAreaEventOptions 的measureFromViewport设置为true。 从API version 21开始，返回值类型由void改为T。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2b/v3/iLaG0Eu6Sta8VDpmgHP_ng/zh-cn_image_0000002659101553.gif?HW-CC-KV=V1&HW-CC-Date=20260701T014323Z&HW-CC-Expire=86400&HW-CC-Sign=896372BA6ABD8A7CB7F30185F8AD51039FDE19A044B9C83CB9EDF4B85F9FA4AC)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/74/v3/ZdqqCzmxQ8SHL7Rt3eF2Fg/zh-cn_image_0000002628862204.jpg?HW-CC-KV=V1&HW-CC-Date=20260701T014323Z&HW-CC-Expire=86400&HW-CC-Sign=D3382D13AD20FB3C9C2F848D1BE66905940EE29222D116F6101900A6CB4B672A)
