...
> [!NOTE]
> 从API version 20开始，该接口支持在 attributeModifier 中调用。 仅提供自身节点相对于所有祖先节点（直到window边界）的相对裁切面积与自身面积的比值及其变化趋势。 不支持兄弟组件对自身节点的遮挡计算，不支持所有祖先的兄弟节点对自身节点的遮挡计算，不支持窗口遮挡计算，不支持组件旋转计算，如 Stack 、 Z序控制 、 rotate 等。 不支持非挂树节点的可见面积变化计算。例如，预加载的节点、通过 overlay 能力挂载的自定义节点。 不支持 scale 属性，如需支持 scale ，则需使用 onVisibleAreaChange 22+ ，将measureFromViewport设置为true。
...
> [!NOTE]
> 仅提供自身节点相对于所有祖先节点（直到window边界）的相对裁切面积与自身面积的比值及其变化趋势。 不支持兄弟组件对自身节点的遮挡计算，不支持所有祖先的兄弟节点对自身节点的遮挡计算，不支持窗口遮挡计算，不支持组件旋转计算，如 Stack 、 Z序控制 、 rotate 等。 不支持非挂树节点的可见面积变化计算。例如，预加载的节点、通过 overlay 能力挂载的自定义节点。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | VisibleAreaEventOptions | 是 | 可见区域变化相关的配置参数，用于设置可见区域回调阈值、期望计算间隔以及可见区域计算模式。 |
| event | VisibleAreaChangeCallback \| undefined | 是 | onVisibleAreaApproximateChange事件的回调函数。当组件可见面积与自身面积的比值达到options中设置的阈值时触发该回调，可见区域比例计算间隔由options中的expectedUpdateInterval参数决定。传入undefined表示不设置该回调。 |
...
> [!NOTE]
> 此接口与 onVisibleAreaChange 接口存在如下差异：onVisibleAreaChange在每一帧都会进行可见区域比例的计算，如果注册节点太多，系统功耗存在劣化。而此接口降低了可见区域比例计算的频度，计算间隔由 VisibleAreaEventOptions 的expectedUpdateInterval参数决定。 仅提供自身节点相对于所有祖先节点（直到window边界）的相对裁切面积与自身面积的比值及其变化趋势。 不支持兄弟组件对自身节点的遮挡计算，不支持所有祖先的兄弟节点对自身节点的遮挡计算，不支持窗口遮挡计算，不支持组件旋转计算，如 Stack 、 Z序控制 、 rotate 等。 不支持非挂树节点的可见面积变化计算。例如，预加载的节点、通过 overlay 能力挂载的自定义节点。 当前接口的可见区域回调阈值默认包含0。例如，开发者设置回调阈值为[0.5]，实际生效的阈值为[0.0, 0.5]。 从API version 18开始，支持在自定义组件中调用该接口。 不支持 scale 属性。从API version 22开始，如需支持 scale ，则需将 VisibleAreaEventOptions 的measureFromViewport设置为true。 从API version 21开始，返回值类型由void改为T。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/85/v3/tc5NZeiZR1en_rH8w-iIGg/zh-cn_image_0000002685927933.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071453Z&HW-CC-Expire=86400&HW-CC-Sign=136EB73E0DA8FD45002FEDB8018C1F968BDC46A9E4E0C9A91DF2CBADBD9A76BA)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8e/v3/woRokX6BTimXRtR8GoAxXQ/zh-cn_image_0000002656008254.jpg?HW-CC-KV=V1&HW-CC-Date=20260730T071453Z&HW-CC-Expire=86400&HW-CC-Sign=C20E024F7274E44BAB41B252895C4A6EE6653E4D49ECE2F85AE26D63A187128E)
