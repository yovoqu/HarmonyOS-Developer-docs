...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| onNeedSoftkeyboardCallback | OnNeedSoftkeyboardCallback \| undefined | 是 | 事件触发时执行的回调，系统会根据回调的返回值决定是否需要键盘。 设置为undefined时，不会触发回调，输入框类组件行为等同返回true。其他组件行为等同返回false。 |
