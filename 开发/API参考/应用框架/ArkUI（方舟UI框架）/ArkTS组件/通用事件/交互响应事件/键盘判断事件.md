...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| onNeedSoftkeyboardCallback | OnNeedSoftkeyboardCallback \| undefined | 是 | 事件触发时执行的回调，系统会根据回调的返回值决定是否需要键盘。 设置为undefined时，不会触发回调，输入框类组件行为等同返回true。其他组件行为等同返回false。前提条件：组件需可获焦，否则本接口不生效。当返回值为true时，自绘制输入框需在获焦时主动调用attach方法建立输入法通信，否则点击键盘会失去响应。 |
...
![](assets/键盘判断事件/file-20260525091139886-001.gif)
