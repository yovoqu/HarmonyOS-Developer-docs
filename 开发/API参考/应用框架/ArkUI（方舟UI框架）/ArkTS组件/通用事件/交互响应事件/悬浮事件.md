...
> [!NOTE]
> 从API version 8开始支持。后续版本如有新增内容，则采用上角标单独标记该内容的起始版本。 目前支持通过外接鼠标以及触控板触发。部分手写笔（当前在华为智慧屏MateTV、MateTV Pro中使用灵犀手写笔时无法响应悬浮事件）不支持悬浮事件，具体取决于硬件能力。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| x15+ | number | 否 | 是 | 鼠标光标或手写笔位置在当前组件为基准的组件坐标系中的X坐标。 单位：vp 取值范围：[0, +∞) 元服务API： 从API version 15开始，该接口支持在元服务中使用。 |
| y15+ | number | 否 | 是 | 鼠标光标或手写笔位置在当前组件为基准的组件坐标系中的Y坐标。 单位：vp 取值范围：[0, +∞) 元服务API： 从API version 15开始，该接口支持在元服务中使用。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9a/v3/cxyKWOgMTTaVgF0dGMvvjQ/zh-cn_image_0000002677827367.png?HW-CC-KV=V1&HW-CC-Date=20260723T011952Z&HW-CC-Expire=86400&HW-CC-Sign=EE59AF2F5F75890F7904EABB70DDD502F0DF1FA9D52503540E132A0A97496F84)
