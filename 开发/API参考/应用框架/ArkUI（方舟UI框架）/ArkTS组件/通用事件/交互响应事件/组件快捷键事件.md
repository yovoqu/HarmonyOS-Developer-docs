...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string \| FunctionKey | 是 | 热键的单个字符（可以通过键盘输入的字符）或FunctionKey。 空字符串意为取消快捷键绑定；绑定了多个快捷键的组件不能解除绑定快捷键。 当value有多个字符时，不绑定组合键，先前绑定的组合键仍然有效。 |
| keys | Array&lt;ModifierKey&gt; | 是 | 热键组合。 仅当value为FunctionKey的情况下keys的值可以为空。 当keys有重复的控制键时，不绑定组合键，先前绑定的组合键仍然有效。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6d/v3/t3GdyKujRIWk4l7MxjzZ8w/zh-cn_image_0000002685927929.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071452Z&HW-CC-Expire=86400&HW-CC-Sign=5AF9DEAA98EC9FB38F0D6DF667EF5C6D55F516A6BFCBB0A1CF0FA593723BF8E5)
