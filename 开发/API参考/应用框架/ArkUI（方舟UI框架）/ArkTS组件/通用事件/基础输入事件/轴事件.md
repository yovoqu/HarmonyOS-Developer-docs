...
> [!NOTE]
> 本模块首批接口从API version 17开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。 本模块接口仅可在Stage模型下使用。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | Callback&lt;AxisEvent&gt; | 是 | 轴事件触发时执行的回调函数，用于接收AxisEvent对象，该对象包含轴事件的动作类型、坐标、滚动步长等信息。 |
...
| x | number | 否 | 否 | 鼠标光标以目标组件为基准的组件坐标系中的X坐标。 单位：vp 元服务API： 从API version 17开始，该接口支持在元服务中使用。 |
| y | number | 否 | 否 | 鼠标光标以目标组件为基准的组件坐标系中的Y坐标。 单位：vp 元服务API： 从API version 17开始，该接口支持在元服务中使用。 |
...
| propagation | Callback&lt;void&gt; | 否 | 否 | 激活事件冒泡，适用于需要将轴事件继续传递给父组件并由父组件统一处理的场景。 元服务API： 从API version 17开始，该接口支持在元服务中使用。 |
| globalDisplayX20+ | number | 否 | 是 | 鼠标光标在全局坐标系中的X坐标。 单位：vp 取值范围：(-∞, +∞) 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
| globalDisplayY20+ | number | 否 | 是 | 鼠标光标在全局坐标系中的Y坐标。 单位：vp 取值范围：(-∞, +∞) 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| axisType | AxisType | 是 | 要检测的轴类型，用于判断当前轴事件是否包含该指定轴类型。 |
...
| 类型 | 说明 |
| --- | --- |
| Coordinate2D | 鼠标光标相对于当前组件实时位置左上角的坐标。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/16/v3/mM21yq5iSbO5QupgWm71VA/zh-cn_image_0000002655848320.png?HW-CC-KV=V1&HW-CC-Date=20260730T071451Z&HW-CC-Expire=86400&HW-CC-Sign=3BCE468AB58E788B59FEA1BB596FA17BFD57E5451B99818EB8BA956DF882DF9A)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/44/v3/ThcDE9cYTbObLOMHSBxiag/zh-cn_image_0000002656008240.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071451Z&HW-CC-Expire=86400&HW-CC-Sign=C38EE7F83F745F4F016ADC73C0B77DB6E7000FFCC20ECF6B2E616BD7EE6FC2A5)
