...
> [!NOTE]
> 本模块首批接口从API version 8开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。 目前仅支持通过外接鼠标触发。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| x | number | 否 | 否 | 鼠标位置在事件响应组件为基准的组件坐标系中的X坐标。 单位：vp 元服务API： 从API version 11开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
| y | number | 否 | 否 | 鼠标位置在事件响应组件为基准的组件坐标系中的Y坐标。 单位：vp 元服务API： 从API version 11开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
| stopPropagation | () => void | 否 | 否 | 阻塞事件冒泡。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
| globalDisplayX20+ | number | 否 | 是 | 鼠标位置在全局坐标系中的X坐标。 单位：vp 取值范围：[0, +∞) 元服务API： 从API version 20开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
| globalDisplayY20+ | number | 否 | 是 | 鼠标位置在全局坐标系中的Y坐标。 单位：vp 取值范围：[0, +∞) 元服务API： 从API version 20开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
| globalDisplayX | number | 否 | 否 | 鼠标位置在全局坐标系中的X坐标。 单位：vp |
| globalDisplayY | number | 否 | 否 | 鼠标位置在全局坐标系中的Y坐标。 单位：vp |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b8/v3/A47x14IFTmaHdRG4LhxPLA/zh-cn_image_0000002659101539.gif?HW-CC-KV=V1&HW-CC-Date=20260701T014321Z&HW-CC-Expire=86400&HW-CC-Sign=6A6A8C64A154B517C9F7E2A6CC6C5D2A5A2AE7034ADB96239DBFE9BBBE2DC4BC)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7c/v3/lnbOBfZHSxSgVOZvwOj8ZA/zh-cn_image_0000002628862190.gif?HW-CC-KV=V1&HW-CC-Date=20260701T014321Z&HW-CC-Expire=86400&HW-CC-Sign=45753BAF94BB3B8922D7074DE51112A802E31E4F757FEF85BC634B55D5D80BFF)
