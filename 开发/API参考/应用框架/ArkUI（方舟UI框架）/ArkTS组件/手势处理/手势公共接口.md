...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uniqueId | number | 是 | 组件的唯一ID。可以通过getUniqueId接口获取该ID。 |
...
| hand15+ | InteractionHand | 否 | 是 | 表示事件是由左手点击还是右手点击触发。 元服务API： 从API version 15开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
| 类型 | 说明 |
| --- | --- |
| Coordinate2D | 手指位置相对于当前组件实时位置的左上角坐标。 |
