...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-gesture-blocking-enhancement
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | ShouldBuiltInRecognizerParallelWithCallback | 是 | 系统内置手势与响应链上其他组件的手势设置并行关系的回调事件，当该组件进行触摸测试时，会触发用户定义的回调来形成手势并行关系。 |
...
| 类型 | 说明 |
| --- | --- |
| GestureRecognizer | 与current识别器绑定并行关系的某个手势识别器；返回undefined表示不建立并行关系。 |
...
| exposeInnerGesture | boolean | 是 | 暴露内部手势标识。 默认值：false 说明： 如果目标组件是组合组件，将此参数设置为true时，回调中的current参数会包含组合组件内部的手势识别器。 当前仅支持Tabs，其他组件请不要设置此参数。 设置为false时，功能与原接口onGestureRecognizerJudgeBegin相同。 |
...
| current | GestureRecognizer | 是 | 当前即将响应的识别器对象。 |
| recognizers | Array&lt;GestureRecognizer&gt; | 是 | 响应链上的其他手势识别器对象。 |
| touchRecognizers20+ | Array&lt;TouchRecognizer&gt; | 否 | 响应链上的触摸识别器对象。未传入时，表示在当前手势绑定组件及其子孙组件没有可响应的触摸识别器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | BaseGestureEvent | 是 | 触摸测试结束后的基础手势事件的信息。 说明： 仅包含BaseGestureEvent的信息，不包含其子类扩展信息。 axisHorizontal和axisVertical的值为0。 |
...
| touchRecognizers | Array&lt;TouchRecognizer&gt; | 否 | 响应链上组件的触摸识别器对象。 默认值为null，表示响应链上没有触摸识别器对象。 |
...
| 类型 | 说明 |
| --- | --- |
| GestureCollectIntervention | 手势收集干预结果。 返回值不是GestureCollectIntervention枚举值时，按照GestureCollectIntervention.CONTINUE处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | ShouldRecognizerParallelWithCallback | 是 | 手势与响应链上其他组件的手势设置并行关系的回调事件，当该组件进行触摸测试时，会触发用户定义的回调来形成手势并行关系。 |
...
![](assets/手势拦截增强/file-20260514163939257-1.gif)
...
![](assets/手势拦截增强/file-20260514163939257-2.gif)
...
![](assets/手势拦截增强/file-20260514163939257-3.gif)
...
![](assets/手势拦截增强/file-20260514163939257-4.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d0/v3/5IwM9VmRS8SAks4KOUXNaw/zh-cn_image_0000002655848456.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071458Z&HW-CC-Expire=86400&HW-CC-Sign=CF06AD5D3096767E4A54DA44AC1D738A40EED5C0C4625F7288DF16F612DA8D61)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/tBLGOtNfRPO_DniBVhZvuA/zh-cn_image_0000002686087885.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071458Z&HW-CC-Expire=86400&HW-CC-Sign=B57E7CE4D0CD33240AD4C818429D6172704727ABB2B0D7D6DB99E8A7004559D1)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ce/v3/b74ECDSJRO-vv7u3dYk17g/zh-cn_image_0000002655848454.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071458Z&HW-CC-Expire=86400&HW-CC-Sign=276AC0A26076928FADE9A5248FF37246BA57CD1F013C2F31F1BCBF84F294CC71)
