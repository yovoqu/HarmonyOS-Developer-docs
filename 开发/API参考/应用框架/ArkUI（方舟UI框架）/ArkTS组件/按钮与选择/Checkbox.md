...
> [!NOTE]
> API version 11开始，Checkbox默认样式由圆角方形变为圆形。 该组件从API version 8开始支持。后续版本如有新增内容，则采用上角标单独标记该内容的起始版本。 该组件默认有 margin 间距，值为：{ top: '14px', right: '14px', bottom: '14px', left: '14px' }。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | CheckboxOptions | 否 | 配置多选框的参数。不传入该参数时，多选框使用默认配置。 |
...
| group | string | 否 | 是 | 用于指定多选框所属群组的名称（即所属CheckboxGroup的名称）。 默认值：undefined，配合CheckboxGroupOptions属性group信息为undefined的节点使用。 说明： 未配合使用CheckboxGroup组件时，此值无用。 卡片能力： 从API version 9开始，该接口支持在ArkTS卡片中使用。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| indicatorBuilder12+ | CustomBuilder | 否 | 是 | 配置多选框的选中样式为自定义组件。当需要实现非默认勾选图标的选中样式（如文字、数字、自定义图标等）时使用此参数。自定义组件与Checkbox组件为中心点对齐显示。indicatorBuilder设置为undefined/null时，默认为indicatorBuilder未设置状态，使用默认的勾选图标样式。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnCheckboxChangeCallback | 是 | 返回选中的状态，true表示已选中，false表示未选中。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Optional&lt;OnCheckboxChangeCallback&gt; | 是 | 返回选中的状态，true表示已选中，false表示未选中。 当callback的值为undefined时，不使用回调函数。 |
...
![](assets/Checkbox/file-20260514164014516-1.gif)
...
![](assets/Checkbox/file-20260514164014516-2.gif)
...
![](assets/Checkbox/file-20260514164014516-3.gif)
...
![](assets/Checkbox/file-20260514164014516-4.gif)
...
![](assets/Checkbox/file-20260514164014516-5.gif)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/14/v3/IqNKIDpERAmd_YKzfC1UXg/zh-cn_image_0000002655848578.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071503Z&HW-CC-Expire=86400&HW-CC-Sign=BB4888E75809AA59B59C455CA108EF4925B4D1783F92C4FF77B06B330A3BA61B)
