...
> [!NOTE] 说明
> 本模块首批接口从API version 9开始支持，后续版本的新增接口，采用上角标单独标记接口的起始版本。 在自定义组件内实现onMeasureSize, onPlaceChildren任一方法即视为实现自定义布局，推荐同时实现两种方法，具体参数说明可见对应接口参数说明。 从API version 20开始，在自定义布局的自定义组件中，子组件若设置了LayoutPolicy对象的fixAtIdealSize属性，表示尺寸将不受父组件约束，完全按照开发者自定义的尺寸范围布局。 自定义布局内不支持使用懒加载(包含Repeat和LazyForEach)。
...
| uniqueId^18+ | number | 否 | 是 | 系统为子组件分配的唯一标识UniqueID。 取值范围[0,+∞)。 元服务API： 从API version 18开始，该接口支持在元服务中使用。 |
...
| 类型 | 说明 |
| --- | --- |
| [DirectionalEdgesT](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#directionaledgestt12)&lt;number&gt; | 子组件的margin信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [DirectionalEdgesT](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#directionaledgestt12)&lt;number&gt; | 子组件的padding信息。 |
...
| 类型 | 说明 |
| --- | --- |
| [DirectionalEdgesT](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-types#directionaledgestt12)&lt;number&gt; | 子组件的borderWidth信息。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| uniqueId^18+ | number | 否 | 是 | 系统为子组件分配的唯一标识UniqueID。 |
...
> [!NOTE] 说明
> 从API version 9开始支持，从API version 10开始废弃，推荐使用onPlaceChildren替代。
...
> [!NOTE] 说明
> 从API version 9开始支持，从API version 10开始废弃，推荐使用onMeasureSize替代。
...
> [!NOTE] 说明
> 从API version 9开始支持，从API version 10开始废弃。建议使用Measurable或者Layoutable替代。
...
> [!NOTE] 说明
> 从API version 9开始支持，从API version 10开始废弃。建议使用getBorderWidth，getMargin和getPadding替代。
...
![](assets/自定义组件的自定义布局/file-20260525091331260-001.png)
...
![](assets/自定义组件的自定义布局/file-20260525091331260-002.png)
...
![](assets/自定义组件的自定义布局/file-20260525091331260-003.jpg)
...
![](assets/自定义组件的自定义布局/file-20260525091331260-004.jpg)