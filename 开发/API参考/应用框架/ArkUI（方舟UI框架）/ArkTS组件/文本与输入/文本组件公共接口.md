...
> [!NOTE]
> 本模块首批接口从API version 10开始支持，后续版本的新增接口，采用上角标单独标记接口的起始版本。 本模块接口仅可在Stage模型下使用。 文本类组件公共接口。
...
| askAI20+ | TextMenuItemId | 是 | 否 | 问问小艺。对选中的文本提供AI问询能力，为一级菜单项。 元服务API： 从API version 20开始，该接口支持在元服务中使用。 |
