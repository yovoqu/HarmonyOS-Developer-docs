...
> [!NOTE]
> 在当前帧更新光标位置同时调用该接口，该接口不生效。 在Search组件中，返回的位置信息是相对Search组件中搜索图标的偏移值。 在Search组件中，不输入文本时，返回值中有相对Search组件的位置信息。 返回值中的位置信息是光标相对于可编辑组件的位置。 当无法获取光标位置时（例如 TextInputController 未与 TextInput 组件绑定时），该接口返回null。
