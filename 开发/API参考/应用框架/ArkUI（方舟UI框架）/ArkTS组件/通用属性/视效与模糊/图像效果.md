...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-image-effect
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| blurRadius | Optional&lt;number&gt; | 是 | 当前组件添加内容模糊效果，入参为模糊半径，模糊半径越大越模糊，blurRadius值小于等于0时不模糊。 当blurRadius的值为undefined时，维持之前取值。从未设置该属性时，默认值为0，表示不模糊。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| rotation | Optional<number \| string> | 是 | 色相旋转效果，输入参数为旋转角度。 取值范围：(-∞, +∞) string需为数值字符串类型。 说明： 色相旋转360度会显示原始颜色。先将色相旋转180 度，然后再旋转-180度会显示原始颜色。数据类型为number时，值为90和'90deg'效果一致。 当rotation的值为undefined时，恢复为无色相旋转的效果。 |
...
| options | LinearGradientBlurOptions | 是 | 设置线性渐变模糊效果。 线性渐变参数，包含模糊程度和模糊位置数组fractionStops，及渐变模糊方向direction。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| blurRadius | Optional&lt;number&gt; | 是 | 模糊半径，模糊半径越大越模糊，为0时不模糊。 取值范围：[0, 1000] 当blurRadius的值为undefined时，恢复为渐变模糊为0的效果。 |
| options | Optional&lt;LinearGradientBlurOptions&gt; | 是 | 设置线性渐变模糊效果。 线性渐变参数，包含模糊程度和模糊位置数组fractionStops，及渐变模糊方向direction。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| effect | Optional&lt;number&gt; | 是 | 设置组件的图像球面化程度。 取值范围：[0,1]。 说明： 1. 如果effect等于0则图像保持原样，如果effect等于1则图像为完全球面化效果。在0和1之间，数值越大，则球面化程度越高。 effect < 0 或者 effect > 1为异常情况，effect < 0按0处理，effect > 1按1处理。 2. 组件阴影和外描边不支持球面效果。 3. 设置effect大于0时，组件冻屏并且把组件内容绘制到透明离屏buffer上，如果要更新组件属性则需要把effect设置为0。 当effect的值为undefined时，恢复为图像球面化程度为0的效果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| degree | Optional&lt;number&gt; | 是 | 设置组件图像亮起程度。 取值范围：[0,1]。 如果degree等于0则图像为全黑，如果degree等于1则图像为全亮效果。0到1之间数值越大，表示图像亮度越高。degree < 0 或者 degree > 1为异常情况，degree < 0按0处理，degree > 1按1处理。 当degree的值为undefined时，恢复为亮起为1的效果。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| radius | number \| Resource | 否 | 否 | 阴影模糊半径。 取值范围：[0, +∞)，API版本26.0.0开始取值范围变更为(-∞, +∞) 单位：px 说明： API版本26.0.0之前，设置小于0的值时，按值为0处理，此时不绘制阴影；从API版本26.0.0开始，设置的值即为最终取值，值为0时仍绘制阴影，设置负数值时不绘制阴影。 如需使用vp单位的数值可用vp2px进行转换。 如果radius为Resource类型，则传入的值需为number类型。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 卡片能力： 从API version 9开始，该接口支持在ArkTS卡片中使用。 |
| type10+ | ShadowType | 否 | 是 | 阴影类型。 默认值：COLOR 元服务API： 从API version 11开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
| fill11+ | boolean | 否 | 是 | 阴影是否内部填充。true表示阴影在内部填充，false表示阴影在外部填充。 默认值：false。 说明： textShadow中该字段不生效。 元服务API： 从API version 12开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
...
> [!TIP]
> 背景图片的同步加载可能会带来潜在性能问题，详情可见 Image 中说明。
...
> [!NOTE]
> 在同时设置了材质影响的通用属性发生冲突时，除阴影外，总体原则为后设置的生效，对于阴影属性取决于 ImmersiveMaterial 的applyShadow参数。 先设置 backgroundColor 属性后设置 systemMaterial 属性：backgroundColor属性被覆盖。在高算力和中算力设备上，背景色属性被清空为透明色；在低算力设备上，材质自带的背景色效果覆盖了先设置的backgroundColor属性。 先设置 systemMaterial 属性后设置 backgroundColor 属性：systemMaterial属性影响的背景色效果被覆盖，背景色属性生效为后设置的backgroundColor属性的颜色。 对于所有设备算力档位均需要材质颜色的场景，可以通过 ImmersiveMaterial 的materialColor参数承载，不再设置 backgroundColor 属性。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| material | SystemUiMaterial \| undefined | 是 | 组件的系统材质对象。设置为undefined时恢复为无材质的效果，若同时设置了材质对象影响的通用属性，会恢复至对应通用属性设置的值，冲突的属性由材质对象决定，参考ImmersiveMaterial。 |
...
![](assets/图像效果/file-20260514163913989-2.png)
...
![](assets/图像效果/file-20260514163913989-3.png)
...
![](assets/图像效果/file-20260514163913989-4.png)
...
![](assets/图像效果/file-20260514163913989-5.png)
...
![](assets/图像效果/file-20260514163913989-6.png)
...
![](assets/图像效果/file-20260514163913989-7.png)
...
![](assets/图像效果/file-20260514163913989-8.png)

 
去掉sphericalEffect的设置，效果如下：
 

![](assets/图像效果/file-20260514163913989-9.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c7/v3/_m7ZQ6SsQy-1TRSfSeN29w/zh-cn_image_0000002686087801.png?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=4A654CFCEE4D6A800825176231E9C71810EB6D8E5CFF148AAA232B8807D12936)

 
修改lightUpEffect参数值为0.2：
 

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/sw8MQ7eNT5OtcnmSZDR_lw/zh-cn_image_0000002685927973.png?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=A6947D3B6ECAFCE380368D7F8A76DBD2B0A53B5F3FBECC4AB2F6D422E95CFC04)

 
去掉lightUpEffect的设置，效果如下：
 

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b/v3/vzdb4rsMTQKRUesbkXvwQQ/zh-cn_image_0000002656008294.png?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=C0183276C6DD588B087E71E9D01C5323BF2144D72C0E1591A6FD444816A9FE91)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/H0AtBRBFRHyJLjCuxaKd0w/zh-cn_image_0000002655848374.png?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=02E52F1D5D3484163A4C2E642436A0C8FA2BED5EC65205D50C084236299DFDE9)

 
去掉pixelStretchEffect的设置，原图效果如下：
 

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f2/v3/75KGKQruRHWAx-EBeemOfA/zh-cn_image_0000002686087803.png?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=9B1AEF999E252D6FECDB2F58272AC0F744CB48E1F969D414B9AB94CC2D1D82AD)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/24/v3/lxeferYLSzSVkW6ujyTccg/zh-cn_image_0000002685927975.png?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=740B60D80B6FDA3F3C9612A1663484BCFA4E9A8481B625D1C7875B489C1FA16D)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1a/v3/OqyKR9_yS4qBQhSKAKp5Rg/zh-cn_image_0000002656008296.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071454Z&HW-CC-Expire=86400&HW-CC-Sign=9AF5527294C7F2A9CEDAABC4F6641149A81E0037C55D116935CCBECFE984A3E7)
