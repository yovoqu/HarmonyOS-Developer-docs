...
> [!TIP]
> toolbar仅支持固定标题栏，不支持悬浮标题栏（仅限三键模式）。 toolbar支持自定义组件布局，可将其置于特定分栏位置（左侧或右侧）。但需注意，当元素数量超过可用空间时，将导致布局截断或焦点框遮挡等现象，从而使部分操作项不可见或引发交互冲突。此时，元素不会自动缩略，建议合理控制元素数量。 toolbar当前仅支持单行布局，不支持多行布局，因此应避免在一个toolbar中放置多行布局的元素。 toolbar仅支持在 NavigationMode 为Split的场景中使用。 标题栏高度会根据toolbar内的 ToolBarItem 组件在有限范围内浮动： ToolBarItem 组件与标题栏默认存在4VP的margin（外边距）。 当 ToolBarItem 组件的最大高度小于等于48VP时，标题栏高度会调整为56VP，此设置适用于标题栏、工具栏、搜索栏等通用组件。 当 ToolBarItem 组件的最大高度介于48VP到56VP之间时，标题栏高度会调整为64VP，此设置适用于图标与文字同时呈现的工具栏。 当 ToolBarItem 组件的最大高度超过56VP时，标题栏高度会调整为72VP。如果 ToolBarItem 组件的最大高度超过64VP，则标题栏的高度保持为72VP，超出的区域会发生裁剪。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/38/v3/tv73yhTVTcSP9RsXlj-gpw/zh-cn_image_0000002659221623.png?HW-CC-KV=V1&HW-CC-Date=20260701T014330Z&HW-CC-Expire=86400&HW-CC-Sign=6D1F9AF3D91BDD0DC7A8D97B90CFA35A4A6F4AAB0F511D331B0465AADFBA366B)
