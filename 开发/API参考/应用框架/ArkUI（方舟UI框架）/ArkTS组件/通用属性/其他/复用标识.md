...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-reuse-id
...
> [!NOTE]
> 根据组件的不同布局形态或类型设置对应的reuseId，以提升复用匹配的精确度。最佳实践请参考组件复用- 使用reuseId标记布局发生变化的组件 。 该接口不支持在 attributeModifier 中调用。
