...
| selectedBackgroundStyle20+ | PickerBackgroundStyle | 否 | 是 | 设置选中项背景样式。 默认值： { color: \$r('sys.color.comp_background_tertiary'), borderRadius: \$r('sys.float.corner_radius_level12') } 元服务API： 从API version 20开始，该接口支持在元服务中使用。 模型约束： 此接口仅可在Stage模型下使用。 |
