...
| lunarSwitchStyle14+ | [LunarSwitchStyle](#lunarswitchstyle14对象说明) | 否 | 是 | 设置农历开关的颜色样式。          默认值：{          selectedColor: \$r('sys.color.ohos_id_color_text_primary_actived'),          unselectedColor: \$r('sys.color.ohos_id_color_switch_outline_off'),          strokeColor: Color.White          }          元服务API： 从API version 14开始，该接口支持在元服务中使用。 |
...
![](assets/日期滑动选择器弹窗%20DatePickerDialog/file-20260514164138327-0.gif)
...
![](assets/日期滑动选择器弹窗%20DatePickerDialog/file-20260514164138327-1.png)
...
![](assets/日期滑动选择器弹窗%20DatePickerDialog/file-20260514164138327-4.png)
    ...
    Column() {
      ...
      .onClick(() => {
        this.getUIContext().showDatePickerDialog({
          ...
          canLoop: this.isLoop,
          onDateAccept: (value: Date) => {
            // 保存按下确定按钮时的日期，这样当弹窗再���弹出时显示选中的是上一次确定的日期
            this.selectedDate = value;
            console.info('DatePickerDialog:onDateAccept()' + value.toString());
          }
        });
...
![](assets/日期滑动选择器弹窗%20DatePickerDialog/file-20260514164138327-10.png)
