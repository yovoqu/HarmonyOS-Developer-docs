...
| .value[3]?.i32 | 可选值，浮层的布局方向，参数类型ArkUI_Direction，默认值为ARKUI_DIRECTION_LTR。 在大部分场景下，这个参数都应该被设置成Auto，这个模式允许系统自动处理布局方向，如果在某些场景下需要保持特定的方向，设置这个属性为LTR（Left-to-Right）或者RTL（Right-to-Left）。从API version 21开始支持。 |
| .object | 用于overlay的节点树，参数类型为ArkUI_NodeHandle，默认值为nullptr。从API version 21开始支持。 |
...
| .value[3].i32 | 浮层的布局方向，参数类型ArkUI_Direction，默认值为ARKUI_DIRECTION_LTR。从API version 21开始支持。 |
| .object | 用于overlay的节点树，参数类型为ArkUI_NodeHandle。从API version 21开始支持。 |
...
| .?object | 参数类型为ArkUI_VisibleAreaEventOptions。 |
...
| --- | --- |
| .value[...].f32 | 占比数值。 |
| .object | 返回类型为ArkUI_VisibleAreaEventOptions。 |
...
| 参数项 | 描述 |
| --- | --- |
| .value[0].i32 | 走焦类型，定义在ArkUI_FocusMove。 |
| .object | 下一个焦点。参数类型为ArkUI_NodeHandle。 |
...
| 参数项 | 描述 |
| --- | --- |
| .value[0].i32 | 组件在悬停状态下的悬停效果。参数类型为ArkUI_HoverEffect。默认值为ARKUI_HOVER_EFFECT_AUTO。 |
...
| 类型 | 说明 |
| --- | --- |
| .value[0].i32 | 组件在悬停状态下的悬停效果。参数类型为ArkUI_HoverEffect。 |
...
| --- | --- |
| .string | 焦点作用域标识符。 |
| .value[0].i32 | 焦点作用域内获焦优先级。参数类型为ArkUI_FocusPriority。默认值为ARKUI_FOCUS_PRIORITY_AUTO。 |
...
| --- | --- |
| .string | 焦点作用域标识符。 |
| .value[0].i32 | 焦点作用域优先级。参数类型为ArkUI_FocusPriority。 |
...
| 参数项 | 描述 |
| --- | --- |
| .data[0].i32 | 适用于此响应区域的事件工具类型。参数类型为ArkUI_ResponseRegionSupportedTool。默认值：ARKUI_RESPONSE_REGIN_SUPPORTED_TOOL_ALL。 |
