...
| .value[0]?.i32 | 可选值，repeat参数，类型为ArkUI_ImageRepeat，默认值为ARKUI_IMAGE_REPEAT_NONE。 |
| .object | PixelMap 图片数据，参数类型为ArkUI_DrawableDescriptor。 .object参数和.string参数二选一，不可同时设置。若同时设置，仅其中一个参数生效。 |
...
| --- | --- |
| .string | 图片地址。API version 22及之前版本，支持网络图片资源地址、本地图片资源地址和Base64格式图片地址，不支持svg图片、gif和webp等类型的动图。 从API version 23开始，新增支持webp和gif类型的动图，显示动图第一帧，不支持其他类型的动图。 |
| .value[0].i32 | repeat参数，类型为ArkUI_ImageRepeat。 |
| .object | PixelMap 图片数据，类型为ArkUI_DrawableDescriptor。 .object参数和.string互斥，仅其中一个有值。 |
