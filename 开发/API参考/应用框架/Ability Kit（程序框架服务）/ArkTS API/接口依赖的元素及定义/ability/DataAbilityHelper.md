...
let mode = 'rw';
DAHelper.openFile('dataability:///com.example.DataAbility', mode).then(
  (data) => {
...
DAHelper.off('dataChange', 'dataability:///com.example.DataAbility');
```
...
DAHelper.getFileTypes('dataability:///com.example.DataAbility', 'image/*').then(
  (data) => {
...
DAHelper.insert('dataability:///com.example.DataAbility', valueBucket).then(
  (data) => {
...
let cars = new Array(
  { name: 'roe11', age: 21, salary: 20.5, blobType: 'u8' } as rdb.ValuesBucket,
...
DAHelper.batchInsert('dataability:///com.example.DataAbility', cars).then(
  (data) => {
...
predicates筛选条件为空，自定义数据库删除数据记录的���理逻辑。使用callback异步回调。
...
let da = new ohos_data_ability.DataAbilityPredicates();
DAHelper.update('dataability:///com.example.DataAbility', va, da).then(
  (data) => {
...
let cars = new Array('value1', 'value2', 'value3', 'value4');
let da = new ohos_data_ability.DataAbilityPredicates();
DAHelper.query('dataability:///com.example.DataAbility', cars, da).then(
  (data) => {
...
dataAbilityHelper
  .executeBatch('dataability:///com.example.jsapidemo.UserDataAbility', op)
  .then((data) => {
