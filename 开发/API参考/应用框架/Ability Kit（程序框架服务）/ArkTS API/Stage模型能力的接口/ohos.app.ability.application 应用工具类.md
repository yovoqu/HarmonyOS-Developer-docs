...
> [!NOTE]
> 如果当前进程已经是 主控进程 ，调用该接口无效并且不会抛出错误码。 当前进程只有运行了isolationProcess字段设为true的组件，或曾经成为过主控进程，开发者才可将其设置为备选主控进程。
...
> [!NOTE]
> 只有在进程首次执行 AbilityStage.onCreate 完成之前调用该接口，才可以返回真实的预加载类型。 AbilityStage创建完成后，应用的预加载数据将被清除，调用该接口将返回UNSPECIFIED，无法获取到真实的预加载类型。
