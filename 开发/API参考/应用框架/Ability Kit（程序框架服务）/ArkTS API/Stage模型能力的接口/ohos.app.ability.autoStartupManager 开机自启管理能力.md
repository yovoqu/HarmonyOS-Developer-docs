  ...
  onForeground() {
    try {
      autoStartupManager
        ...
        .then((isAutoStartup: boolean) => {
          console.info(
            `getAutoStartupStatusForSelf success, isAutoStartup: ${JSON.stringify(isAutoStartup)}.`,
          );
        })
        .catch((err: BusinessError) => {
          console.error(
            `getAutoStartupStatusForSelf failed, err code: ${err.code}, err msg: ${err.message}.`,
          );
        });
    } catch (err) {
      ...
      console.error(
        `getAutoStartupStatusForSelf failed, err code: ${code}, err msg: ${msg}.`,
      );
    }
  }
}
