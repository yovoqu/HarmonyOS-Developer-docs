...
![](assets/ohos.app.ability.UIAbility%20带界面的应用组件/file-20260514163707279-0.png)
...
![](assets/ohos.app.ability.UIAbility%20带界面的应用组件/file-20260514163707279-1.png)
...
- 同步回调示例如下：       __PREBLOCK_6__
- Promise异步回调示例如下：       __PREBLOCK_7__
      ...
      (err: BusinessError) => {
        if (err) {
          hilog.error(
            0x0000,
            'testTag',
            `hiAppEvent code: ${err.code}, message: ${err.message}`,
          );
          return;
        }
        hilog.info(0x0000, 'testTag', `hiAppEvent success to write event`);
    ...
    audio
      ...
      .then((data) => {
        ...
        hilog.info(
          0x0000,
          'testTag',
          `AudioRenderer Created : Success : Stream Type: SUCCESS.`,
        );
      })
      .catch((err: BusinessError) => {
        hilog.error(
          0x0000,
          'testTag',
          `AudioRenderer Created : F : ${JSON.stringify(err)}.`,
        );
      });
  }
    ...
    MyUIAbility.audioRenderer.release((err: BusinessError) => {
      if (err) {
        hilog.error(
          0x0000,
          'testTag',
          `AudioRenderer release failed, error: ${JSON.stringify(err)}.`,
        );
      } else {
...
- 应用迁移时使用同步接口进行数据保存，示例如下：       __PREBLOCK_13__
- 应用迁移时使用异步接口进行数据保存，示例如下：       __PREBLOCK_14__
...
**元服务API**：从API version 11开始，该接口支持在元服���中使用。
  ...
  onPrepareToTerminate() {
    ...
    this.context
      ...
      .then((result) => {
        ...
        console.info(
          'startAbilityForResult success, resultCode is ' + result.resultCode,
        );
      ...
      .catch((err: BusinessError) => {
        // 异常处理
        console.error(
          'startAbilityForResult failed, err:' + JSON.stringify(err),
        );
        this.context.terminateSelf();
      });
      ...
      .then((obj) => {
        ...
        caller
          ...
          .catch((callErr: BusinessError) => {
            console.error(
              `Caller.call catch error, error.code: ${callErr.code}, error.message: ${callErr.message}`,
            );
          });
      })
      .catch((err: BusinessError) => {
        console.error(
          `Caller GetCaller error, error.code: ${err.code}, error.message: ${err.message}`,
        );
      });
  }
}
    ...
    this.context
      ...
      .then((obj) => {
        ...
        caller
          ...
          .catch((callErr: BusinessError) => {
            console.error(
              `Caller.callWithResult catch error, error.code: ${callErr.code}, error.message: ${callErr.message}`,
            );
          });
      })
      .catch((err: BusinessError) => {
    ...
    this.context
      ...
      .then((obj) => {
        ...
        try {
          ...
          console.error(
            `Caller.release catch error, error.code: ${releaseErr.code}, error.message: ${releaseErr.message}`,
          );
        }
      })
      .catch((err: BusinessError) => {
    ...
    this.context
      ...
      .then((obj) => {
        ...
        try {
          ...
          console.error(
            `Caller.onRelease catch error, error.code: ${error.code}, error.message: ${error.message}`,
          );
        }
      })
      .catch((err: BusinessError) => {
    ...
    this.context
      ...
      .then((obj) => {
        ...
        try {
          ...
          console.error(
            `Caller.on catch error, error.code: ${error.code}, error.message: ${error.message}`,
          );
        }
      })
      .catch((err: BusinessError) => {
...
取消注册Callee UIAbility断开通��的监听，与[Caller.on('release')](#onrelease-1)是反向操作，当前暂未支持。
...
export default class MainUIAbility extends UIAbility {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    ...
    try {
      ...
      console.error(
        `Callee.off catch error, error.code: ${error.code}, error.message: ${error.message}`,
      );
    }
  }
}
