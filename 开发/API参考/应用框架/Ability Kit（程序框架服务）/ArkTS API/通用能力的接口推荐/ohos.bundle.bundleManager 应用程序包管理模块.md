...
| GET_BUNDLE_INFO_WITH_HAP_MODULE | 0x00000002 | 用于获取包含hapModuleInfo的bundleInfo，获取的bundleInfo不包含signatureInfo、applicationInfo、ability、extensionAbility和permission的信息。          元服务API： 从API version 11开始，该���口支持在元服务中使用。 |
...
| WEB_NATIVE_MESSAGING21+ | 32 | [WebNativeMessagingExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-web-webnativemessagingextensionability)：为开发者提供Web原生消息通信能力的ExtensionAbility。 |
...
| PARTNER_AGENT23+ | 36 | [PartnerAgentExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/is-fusionconnectivity-partneragentextensionability)：基于蓝牙通信技术，提供设备发现与设备下线的通知功能。          模型约束：此接口仅可在Stage模型下使用。 |
    ...
    (err, data) => {
      if (err) {
        ...
        hilog.info(
          0x0000,
          'testTag',
          'getProfileByAbility successfully: %{public}s',
          JSON.stringify(data),
        );
      }
    },
...
try {
  ...
  bundleManager
    .getProfileByAbility(moduleName, abilityName)
    .then((data) => {
      hilog.info(
        0x0000,
        'testTag',
...
try {
  ...
  let data = bundleManager.getProfileByAbilitySync(
    moduleName,
    abilityName,
    metadataName,
  );
    ...
    (err, data) => {
      if (err) {
        ...
        hilog.info(
          0x0000,
          'testTag',
          'getProfileByExtensionAbility successfully: %{public}s',
          JSON.stringify(data),
        );
      }
    },
...
try {
  let data = bundleManager.getProfileByExtensionAbilitySync(
    moduleName,
    extensionAbilityName,
  );
  hilog.info(
...
try {
  ...
  hilog.info(
    0x0000,
    'testTag',
    'getLaunchWant ability name: %{public}s',
    want.abilityName,
  );
  hilog.info(
    0x0000,
    'testTag',
    'getLaunchWant bundle name: %{public}s',
    want.bundleName,
  );
} catch (error) {
  ...
  bundleManager.getBundleInfo(bundleName, bundleFlags, userId, (err, data) => {
    if (err) {
      ...
      hilog.info(
        0x0000,
        'testTag',
        'getBundleInfo successfully: %{public}s',
        JSON.stringify(data),
      );
    }
  });
...
try {
  bundleManager
    .getBundleInfo(bundleName, bundleFlags, userId)
    .then((data) => {
      hilog.info(
        0x0000,
        'testTag',
...
try {
  bundleManager
    .getBundleInfo(bundleName, bundleFlags)
    .then((data) => {
      hilog.info(
        0x0000,
        'testTag',
  ...
  bundleManager.getBundleNameByUid(uid, (err, data) => {
    if (err) {
      ...
      hilog.info(
        0x0000,
        'testTag',
        'getBundleNameByUid successfully: %{public}s',
        JSON.stringify(data),
      );
    }
  });
...
try {
  bundleManager
    .getAppCloneIdentity(uid)
    .then((res) => {
      hilog.info(
        0x0000,
        'testTag',
        'getAppCloneIdentity res = %{public}s',
        JSON.stringify(res),
      );
    })
    .catch((err: BusinessError) => {
      hilog.error(
        0x0000,
        'testTag',
        'getAppCloneIdentity failed. Cause: %{public}s',
        err.message,
      );
    });
} catch (err) {
...
try {
  ...
  hilog.info(
    0x0000,
    'testTag',
    'getSignatureInfo successfully. Data: %{public}s',
    JSON.stringify(data),
  );
} catch (err) {
  let message = (err as BusinessError).message;
  hilog.error(
    0x0000,
    'testTag',
    'getSignatureInfo failed. Cause: %{public}s',
    message,
  );
}
```
...
try {
  bundleManager
    .getAbilityInfo(uri, abilityFlags)
    .then((data) => {
      hilog.info(
        0x0000,
        'testTag',
        'getAbilityInfo successfully. Data: %{public}s',
        JSON.stringify(data),
      );
    })
    .catch((err: BusinessError) => {
      ...
      hilog.error(
        0x0000,
        'testTag',
        'getAbilityInfo failed. Cause: %{public}s',
        message,
      );
    });
} catch (err) {
...
try {
  ...
  hilog.info(
    0x0000,
    'testTag',
    'getPluginBundlePathForSelf successfully. path: %{public}s',
    path,
  );
} catch (err) {
  let message = (err as BusinessError).message;
  hilog.error(
    0x0000,
    'testTag',
    'getPluginBundlePathForSelf failed. Cause: %{public}s',
    message,
  );
}
```
