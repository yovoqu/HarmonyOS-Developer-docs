...
defaultAppManager
  .isDefaultApplication(defaultAppManager.ApplicationType.BROWSER)
  .then((data) => {
    console.info(
      'Operation successful. IsDefaultApplication ? ' + JSON.stringify(data),
    );
  })
...
defaultAppManager.isDefaultApplication(
  defaultAppManager.ApplicationType.BROWSER,
...
try {
  let data = defaultAppManager.isDefaultApplicationSync(
    defaultAppManager.ApplicationType.BROWSER,
  );
  console.info(
    'Operation successful. IsDefaultApplicationSync ? ' + JSON.stringify(data),
  );
} catch (error) {
  console.error('Operation failed. Cause: ' + JSON.stringify(error));
}
