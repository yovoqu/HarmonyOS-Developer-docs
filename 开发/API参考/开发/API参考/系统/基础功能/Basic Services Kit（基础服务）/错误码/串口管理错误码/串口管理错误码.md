...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-busmanager-serial
**支持设备：** PC/2in1

> [!TIP]
> API参考中的参数说明与SDK中@ohos.busManager.serial.d.ts文件的注释保持一致。 本模块首批接口从API version 19开始支持。后续版本的新增接口，采用上标单独标记接口的起始版本。 本文档提供串口通信接口调用的错误码说明，帮助开发者快速定位和解决串口通信问题。适用于开发串口通信应用、调试串口通信异常等场景。以下仅介绍本模块特有错误码，通用错误码请参考 通用错误码说明文档 。
