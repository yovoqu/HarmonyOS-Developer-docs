...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| xmpNamespace | XMPNamespace | 是 | XMP命名空间。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| path | string | 是 | 目标XMP标签的路径。路径语法参见XMP路径语法。 |
| type | XMPTagType | 是 | XMP标签类型。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<XMPTag \| null> | Promise对象，返回指定路径下的XMP标签。若获取失败，返回空值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | (path: string, tag: XMPTag) => boolean | 是 | 同步回调函数。 返回true，表示继续遍历；返回false，表示停止遍历。 path参数是标签的唯一路径标识，遵循XMP路径语法；tag参数是对应标签的数据表示形式。 |
| rootPath | string | 否 | 遍历起始节点。默认从根节点开始，路径语法参见XMP路径语法。 |
| options | XMPEnumerateOptions | 否 | 遍历选项。 默认行为：仅遍历直接子节点，返回普通节点和限定符。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise<Record<string, XMPTag>> | Promise对象，返回XMP标签集合。 |
