...
| requestPermissions | Array&lt;string&gt; | 是 | 是 | 技能所在的模块申请的权限列表。当应用调用skillManager接口，传入的SkillInfoFlag不包含GET_SKILL_INFO_WITH_REQUEST_PERMISSIONS时，该字段将返回默认值为undefined，开发者使用时需要进行有效值判断以防代码异常。 |
