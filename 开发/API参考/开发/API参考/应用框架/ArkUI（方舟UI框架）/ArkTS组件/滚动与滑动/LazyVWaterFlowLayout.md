...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | string \| ItemFillPolicy \| undefined | 是 | LazyVWaterFlowLayout的列数、固定列宽、最小列宽值或断点填充策略。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | LengthMetrics \| undefined | 是 | 列与列的间距。 默认值：LengthMetrics.vp(0) 取值范围：[0, +∞) 设置为小于0的值时，按LengthMetrics.vp(0)处理。 方法入参为undefined时，恢复为默认值（LengthMetrics.vp(0)）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | LengthMetrics \| undefined | 是 | 行与行的间距。 默认值：LengthMetrics.vp(0) 取值范围：[0, +∞) 设置为小于0的值时，按LengthMetrics.vp(0)处理。 方法入参为undefined时，恢复为默认值（LengthMetrics.vp(0)）。 |
...
> [!TIP]
> 头部组件位于容器顶部区域，通常用于展示标题、分组说明或其他固定在内容前方的元素。 当本组件随滚动容器滚动至可视区域内，且通过 sticky 设置了header吸顶模式时，header会吸附在滚动容器可视区域顶部。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| builder | CustomBuilder \| undefined | 是 | 头部组件构造函数。 入参为undefined时，不设置头部组件，如果已有头部组件，也会被移除。 |
...
> [!NOTE]
> 尾部组件位于容器底部区域，通常用于展示补充信息、加载状态或其他固定在内容后方的元素。 当本组件随滚动容器滚动至可视区域内，且通过 sticky 设置了footer吸底模式时，footer会吸附在滚动容器可视区域底部。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| builder | CustomBuilder \| undefined | 是 | 尾部组件构造函数。 入参为undefined时，不设置尾部组件，如果已有尾部组件，也会被移除。 |
...
> [!NOTE]
> 由于浮点数计算精度，设置sticky后，在滚动过程中可能产生缝隙，可以通过 pixelRound 指定当前组件向下像素取整解决该问题。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| sticky | StickyStyle \| undefined | 是 | 头部组件和尾部组件的吸附模式。sticky属性可设置为StickyStyle.Header（头部组件吸顶）、StickyStyle.Footer（尾部组件吸底）、StickyStyle.BOTH（同时支持头部组件吸顶和尾部组件吸底）或StickyStyle.None（不启用吸附效果）。 入参为undefined时，恢复为默认值StickyStyle.None。 未通过该接口设置时，默认头部组件不吸顶、尾部组件不吸底。 |
...
> [!NOTE]
> 当父组件设置主轴方向尺寸时，LazyVWaterFlowLayout按照父组件可视区域进行懒加载。此时onVisibleIndexesChange回调中start返回当前可视区域起始位置子组件的索引值，end返回当前可视区域结束位置子组件的索引值。 当父组件未设置主轴方向尺寸时，LazyVWaterFlowLayout会被内容撑开，导致所有子组件都会被加载布局。此时onVisibleIndexesChange回调中start返回0，end返回数据源最后一个子组件的索引值。 当该组件懒加载功能因上述父组件配置条件失效时，所有子组件都会被加载布局。此时onVisibleIndexesChange回调中start返回0，end返回数据源最后一个子组件的索引值。 此处的父组件指从当前组件向上查找到的最近一个List、Scroll或WaterFlow组件；FlowItem、LazyColumnLayout、自定义组件和NodeContainer仅作为中间封装层，不作为此处的父组件，其他文档下的具体含义请参考对应内容。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnVisibleIndexesChangeCallback \| undefined | 是 | 回调函数，当可见区域内的子组件索引发生变化时触发。 入参为undefined时，取消监听。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ec/v3/ugyqdP0RQOOmkv7tWHZqow/zh-cn_image_0000002685928125.png?HW-CC-KV=V1&HW-CC-Date=20260730T071759Z&HW-CC-Expire=86400&HW-CC-Sign=530919250BC15DEF57D913F5979FA8E3F6BBA16CF91D0F0CCE727DC86733D8F0)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ed/v3/40VtXqZxRHu3XxzMHNgEfw/zh-cn_image_0000002656008448.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071759Z&HW-CC-Expire=86400&HW-CC-Sign=9DBA4A10F9AEB72025627AA1A5B13CC68F6F1C34B295CD19FC654ECCA92AC3B6)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5e/v3/e4JnD4VVQMez6NJzKoQaIw/zh-cn_image_0000002655848526.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071759Z&HW-CC-Expire=86400&HW-CC-Sign=6ABC458CEE9B3145E8485372010139D235FCFAF3F2ACDD17C8027E55194D13FD)
