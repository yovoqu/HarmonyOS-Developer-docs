...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ohos-arkui-advanced-treeviewv2
...
> [!NOTE]
> 本模块接口仅可在Stage模型下使用。 如果TreeViewV2设置 通用属性 和 通用事件 ，编译工具链会额外生成节点__Common__，并将通用属性或通用事件挂载在__Common__上，而不是直接应用到TreeViewV2本身。这可能导致开发者设置的通用属性或通用事件不生效或不符合预期，因此，不建议TreeViewV2设置通用属性和通用事件。
...
| 名称 | 类型 | 必填 | 装饰器类型 | 说明 |
| --- | --- | --- | --- | --- |
| treeControllerV2 | TreeControllerV2 | 是 | @Param | 树视图节点控制器。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| nodeParam | NodeParamV2 | 否 | 节点信息，用于指定新增节点的属性。如果不传该参数，在当前选中的节点下添加一个标题为"新建文件夹"的节点。 |
...
| parentSubTitle | ResourceStr | 是 | 父节点副标题。 |
| currentSubtitle | ResourceStr | 是 | 当前节点副标题。 |
...
| icon | ResourceStr | 否 | 是 | 图标。 默认值：空字符串。 |
| symbolIconStyle | SymbolGlyphModifier | 否 | 是 | Symbol图标样式，显示优先级大于icon，同时设置symbolIconStyle和icon，只显示Symbol图标。 默认值：undefined，表示不显示Symbol图标。 |
| selectedIcon | ResourceStr | 否 | 是 | 选中图标。 默认值：空字符串。 |
| symbolSelectedIconStyle | SymbolGlyphModifier | 否 | 是 | Symbol选中图标样式，优先级大于selectedIcon。 默认值：undefined，选中时显示与未选中一样 |
| editIcon | ResourceStr | 否 | 是 | 编辑图标。 默认值：空字符串。 |
| symbolEditIconStyle | SymbolGlyphModifier | 否 | 是 | Symbol编辑图标样式，优先级大于editIcon。 默认值：undefined，编辑时显示与非编辑态一样 |
| primaryTitle | ResourceStr | 否 | 是 | 主标题。 默认值：空字符串。 |
| secondaryTitle | ResourceStr | 否 | 是 | 副标题。 默认值：空字符串。 |
| container | OnContainerCallback | 否 | 是 | 绑定在节点上的右键子组件，子组件由@Builder修饰。 默认值：() => void |
...
| 类型 | 说明 |
| --- | --- |
| TreeListenerManagerV2 | 返回获取到的树视图组件的监听管理器单例对象。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 是 | 节点点击回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 否 | 节点点击回调函数。传入时取消对应的监听，否则取消所有节点点击监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 是 | 节点添加回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 否 | 节点添加回调函数。传入时取消对应的监听，否则取消所有节点添加监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 是 | 节点删除回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 否 | 节点删除回调函数。传入时取消对应的监听，否则取消所有节点删除监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 是 | 节点修改回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 否 | 节点修改回调函数。传入时取消对应的监听，否则取消所有节点修改监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 是 | 节点移动回调函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | OnChangedCallback | 否 | 节点移动回调函数。传入时取消对应的监听，否则取消所有节点移动监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callbackParam | CallbackParamV2 | 是 | 节点回调参数，用于传递节点事件回调的参数信息。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c7/v3/fBRGdWYRQtyLtmxapsFbtg/zh-cn_image_0000002668463276.png?HW-CC-KV=V1&HW-CC-Date=20260811T005507Z&HW-CC-Expire=86400&HW-CC-Sign=154F9C3A76F0507CDEE8618C5F2944E59CA0606FFDFA486A2B21D69CB1849A14)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9d/v3/4HZ9Sa3kRWqTBiMI-vLgRg/zh-cn_image_0000002698223157.png?HW-CC-KV=V1&HW-CC-Date=20260811T005507Z&HW-CC-Expire=86400&HW-CC-Sign=16C4B3946B274EE0076905579652FE7ED81582E1261E4D88CAA91149CE82FA5D)
