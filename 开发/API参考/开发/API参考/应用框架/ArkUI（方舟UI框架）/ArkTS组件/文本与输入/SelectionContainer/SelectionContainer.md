...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-selectioncontainer
...
> [!NOTE]
> 本组件中选中文本相关回调返回的文本内容，按照 Text 组件的从上到下显示顺序进行拼接。 本模块接口仅可在Stage模型下使用。 本组件默认布局走 Stack ，如有其他容器布局需求请在SelectionContainer内放置一个容器组件。 SelectionContainer内跨节点选中文本时不显示放大镜，也不支持 getMagnifier 主动设置放大镜。 仅Text组件中的文本内容参与跨节点选中与文本拼接。
...
> [!NOTE]
> 不支持 隐私遮罩 。 不支持 图形变换 ，跨节点场景中Text子组件不支持图形变换。
...
> [!NOTE]
> Text子组件已显式设置 copyOption 时，优先使用Text子组件的配置；未设置时，使用SelectionContainer的配置。
...
> [!NOTE]
> 该属性在跨节点场景中用于各Text子组件选中文本手柄颜色。 在跨节点场景中Text子组件 caretColor 设置无效，始终使用SelectionContainer的配置。
...
> [!NOTE]
> 该属性在跨节点场景中用于各Text子组件选中区域的高亮颜色。 Text子组件已显式设置 selectedBackgroundColor 时，优先使用Text子组件的配置；未设置时，使用SelectionContainer的配置。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isEnabled | Optional&lt;boolean&gt; | 是 | 是否开启触控反馈。 true表示开启触控反馈，false表示不开启触控反馈。 |
...
> [!NOTE]
> 该配置会影响 onWillCopy 、 onCopy 、 bindSelectionMenu 相关回调中返回的文本内容。 该配置也会影响系统内置菜单项中依赖文本拼接结果的逻辑。例如，选择两个Text节点中的文本时，若配置为SelectionContainerTextJoinStyle.NEWLINE，执行复制后两段文本之间会插入换行符；若配置为SelectionContainerTextJoinStyle.DIRECT，执行复制后两段文本会直接拼接。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| style | Optional&lt;SelectionContainerTextJoinStyle&gt; | 是 | 聚合文本拼接方式。 |
...
> [!NOTE]
> bindSelectionMenu的长按响应时长为600ms， bindContextMenu 的长按响应时长为800ms，当两者同时绑定且触发方式均为长按时，优先响应bindSelectionMenu。 自定义菜单过长时，建议内部嵌套使用 Scroll 组件，避免键盘被遮挡。 选区跨越不可复制Text时，菜单仅基于实际选中的可复制文本进行显示和处理。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| spanType | Optional&lt;TextSpanType&gt; | 是 | 选择菜单类型。用于指定选择菜单作用的文本类型范围，不同类型对应不同的菜单行为。各枚举值的含义及适用场景详见TextSpanType。 |
| content | Optional&lt;CustomBuilder&gt; | 是 | 选择菜单内容。 |
| responseType | Optional&lt;TextResponseType&gt; | 是 | 选择菜单响应类型。 |
| options | Optional&lt;SelectionContainerMenuOptions&gt; | 否 | 选择菜单选项，用于配置菜单出现、消失、显示、隐藏等事件的回调。当需要监听这些菜单事件时传入此参数，不传入时默认不监听菜单事件。 |
...
> [!NOTE]
> 当同时为当前场景设置了 bindSelectionMenu 和editMenuOptions时，优先使用bindSelectionMenu，editMenuOptions不生效。bindSelectionMenu用于完全自定义菜单风格和触发条件，由开发者定义所有菜单项；editMenuOptions用于在系统默认菜单基础上添加扩展项，触发条件不变。建议根据自定义程度需求选择。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| editMenu | Optional&lt;SelectionContainerEditMenuOptions&gt; | 是 | 自定义编辑菜单配置。 |
...
> [!NOTE]
> 回调参数数组中各项顺序与Text组件视觉顺序一致。 数组中的每一项对应一个Text子组件的选中文本。 仅包含有选中文本的Text子组件，不包含未选中Text子组件，也不包含不可复制Text的空字符串占位。
...
> [!NOTE]
> 回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由 textJoinStyle 配置决定。 返回false时，会阻止本次跨节点复制及容器级 onCopy 回调触发，但不会影响各Text子组件已独立处理完成的复制事件逻辑。
...
> [!NOTE]
> 回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由 textJoinStyle 配置决定。 仅当容器级 onWillCopy 返回true时，该回调才会触发。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onAppear | Callback&lt;string&gt; | 否 | 是 | 选择菜单出现时触发。回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由textJoinStyle配置决定。默认值为空，不触发该回调。 |
| onDisappear | Callback&lt;void&gt; | 否 | 是 | 选择菜单消失时触发。默认值为空，不触发该回调。 |
| onMenuShow | Callback&lt;string&gt; | 否 | 是 | 选择菜单显示时触发。回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由textJoinStyle配置决定。默认值为空，不触发该回调。 |
| onMenuHide | Callback&lt;string&gt; | 否 | 是 | 选择菜单隐藏时触发。回调参数为按Text组件视觉顺序拼接后的选中文本，拼接方式由textJoinStyle配置决定。默认值为空，不触发该回调。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| onCreateMenu | OnCreateMenuCallback | 否 | 是 | 每次菜单显示前触发，传入默认菜单项并返回处理后的菜单项。默认值为空，不触发该回调。 |
| onMenuItemClick | OnMenuItemClickWithTextCallback | 否 | 是 | 点击菜单项时触发，可拦截系统默认菜单执行行为。默认值为空，不触发该回调。 |
| onPrepareMenu | OnPrepareMenuCallback | 否 | 是 | 文本选中内容变化后、菜单显示前触发，可在该回调中调整菜单数据。默认值为空，不触发该回调。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6f/v3/tF7-rlRQTDKXOy6VOQGGrw/zh-cn_image_0000002698142733.png?HW-CC-KV=V1&HW-CC-Date=20260811T005507Z&HW-CC-Expire=86400&HW-CC-Sign=938278F3C76E7B2DCB7E498EBB48FD2B215F09BC309C4EE3EF00D7721E200617)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/61/v3/wg2EedbKRfCtfG-jgxoBbQ/zh-cn_image_0000002668303068.png?HW-CC-KV=V1&HW-CC-Date=20260811T005507Z&HW-CC-Expire=86400&HW-CC-Sign=3D201DB46EDD98B86F7CEE8E6CE5D565657E845D7A7E23B1E4D8EA1BCA8DF6E9)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a2/v3/hXD-YB3yRDC97IThcRVPNQ/zh-cn_image_0000002668462946.png?HW-CC-KV=V1&HW-CC-Date=20260811T005507Z&HW-CC-Expire=86400&HW-CC-Sign=7FE0FDE5AD6616AC3687B32A08B2732B7CB488F8935FE6EE668A7C45F578AE20)
