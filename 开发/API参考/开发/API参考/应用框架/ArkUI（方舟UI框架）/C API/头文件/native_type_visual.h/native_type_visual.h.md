...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-type-visual-h
...
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| ArkUI_TranslationOptions | ArkUI_TranslationOptions | 定义组件转场时平移效果的配置选项。 |
...
| ArkUI_Matrix4 | ArkUI_Matrix4 | 四阶矩阵对象，用于矩阵变换操作。 |
| ArkUI_PointF | ArkUI_PointF | 定义一个二维坐标点结构体，用于描述组件位置或偏移等坐标信息，坐标以浮点类型存储。 |
| ArkUI_Matrix4RotationOptions | ArkUI_Matrix4RotationOptions | 定义矩阵旋转变换的参数配置对象。 |
| ArkUI_Matrix4ScaleOptions | ArkUI_Matrix4ScaleOptions | 定义矩阵缩放变换的参数配置对象。 |
| ArkUI_Matrix4TranslationOptions | ArkUI_Matrix4TranslationOptions | 定义矩阵平移变换的参数配置对象。 |
...
| ArkUI_ShapeType | ArkUI_ShapeType | 定义形状类型的枚举值。 |
| ArkUI_LinearGradientDirection | ArkUI_LinearGradientDirection | 定义渐变方向枚举值。 |
...
| ArkUI_BlendApplyType | ArkUI_BlendApplyType | 定义混合模式应用于视图内容的方式的枚举值。 |
...
| 名称 | 返回值 | 描述 |
| --- | --- | --- |
| ArkUI_Matrix4ScaleOptions* OH_ArkUI_Matrix4ScaleOptions_Create() | - | 创建指向矩阵运算的缩放参数对象的指针。新创建的对象中，x、y和z方向的缩放系数默认值为1，单次矩阵变换中心点相对于组件变换中心点的x轴偏移值centerX和y轴偏移值centerY默认值为0。 |
...
| ArkUI_Matrix4RotationOptions* OH_ArkUI_Matrix4RotationOptions_Create() | - | 创建矩阵运算的旋转参数对象的指针。在新创建的对象中，centerX（单次矩阵变换中心点相对于组件变换中心点的x轴偏移值）、centerY（y轴偏移值）和旋转角度angle默认值为0。如果未指定x、y、z方向向量，则默认x=0、y=0、z=1（绕z轴旋转）；一旦指定了任意一个方向向量，其余未指定的值等同于0。 |
...
| ArkUI_ErrorCode OH_ArkUI_Matrix4TranslationOptions_SetX(ArkUI_Matrix4TranslationOptions* options, const float x) | - | 设置矩阵运算的平移对象x轴方向的平移值，单位为px。 |
| ArkUI_ErrorCode OH_ArkUI_Matrix4TranslationOptions_GetX(const ArkUI_Matrix4TranslationOptions* options, float* x) | - | 获取矩阵运算的平移对象x轴方向的平移值，单位为px。如果从未设置x的值，其默认值为0。 |
| ArkUI_ErrorCode OH_ArkUI_Matrix4TranslationOptions_SetY(ArkUI_Matrix4TranslationOptions* options, const float y) | - | 设置矩阵运算的平移对象y轴方向的平移值，单位为px。 |
| ArkUI_ErrorCode OH_ArkUI_Matrix4TranslationOptions_GetY(const ArkUI_Matrix4TranslationOptions* options, float* y) | - | 获取矩阵运算的平移对象y轴方向的平移值，单位为px。如果从未设置y的值，其默认值为0。 |
| ArkUI_ErrorCode OH_ArkUI_Matrix4TranslationOptions_SetZ(ArkUI_Matrix4TranslationOptions* options, const float z) | - | 设置矩阵运算的平移对象z轴方向的平移值，单位为px。 |
| ArkUI_ErrorCode OH_ArkUI_Matrix4TranslationOptions_GetZ(const ArkUI_Matrix4TranslationOptions* options, float* z) | - | 获取矩阵运算的平移对象z轴方向的平移值，单位为px。如果从未设置z的值，其默认值为0。 |
...
| ArkUI_Matrix4* OH_ArkUI_Matrix4_Copy(const ArkUI_Matrix4* matrix) | - | 创建四阶矩阵对象的副本。用于对同一矩阵进行操作，以获取不同的矩阵对象。 |
| ArkUI_ErrorCode OH_ArkUI_Matrix4_Invert(ArkUI_Matrix4* matrix) | - | 对输入矩阵执行逆矩阵变换，变换后将修改输入的矩阵对象。此函数将修改输入的矩阵对象。 |
...
| ArkUI_ErrorCode OH_ArkUI_Matrix4_Translate(ArkUI_Matrix4* matrix, const ArkUI_Matrix4TranslationOptions* translate) | - | 对原始矩阵应用平移变换以获取平移后的矩阵。每次平移变换都是在先前的矩阵上累积的。变换后将修改输入的矩阵对象。 |
...
| 类型 | 说明 |
| --- | --- |
| ArkUI_Matrix4* | 返回通过指定矩阵元素创建的四阶矩阵对象。如果elements指针为空，函数将返回NULL。 |
...
| 类型 | 说明 |
| --- | --- |
| ArkUI_Matrix4ScaleOptions* | 返回指向新创建的ArkUI_Matrix4ScaleOptions的指针，用于配置矩阵运算的缩放参数。 |
...
| 类型 | 说明 |
| --- | --- |
| ArkUI_Matrix4RotationOptions* | 返回指向新创建的ArkUI_Matrix4RotationOptions的指针，用于配置矩阵运算的旋转参数。 |
...
| 类型 | 说明 |
| --- | --- |
| ArkUI_Matrix4TranslationOptions* | 返回指向新创建的ArkUI_Matrix4TranslationOptions的指针，用于配置矩阵运算的平移参数。 |
...
| --- | --- |
| ArkUI_MotionPathOptions* options | 指向路径动画的运动路径配置项ArkUI_MotionPathOptions的指针。 |
| const float to | 路径动画的终点进度，取值范围为[0.0, 1.0]，且需满足to大于或等于起点进度from；否则将返回ARKUI_ERROR_CODE_PARAM_OUT_OF_RANGE错误码。 from的含义参考OH_ArkUI_MotionPathOptions_SetFrom。 |
...
| 类型 | 说明 |
| --- | --- |
| OH_ArkUI_ShadowOptions* | 指向新创建的OH_ArkUI_ShadowOptions对象的指针，用于配置阴影的模糊半径、类型、颜色和偏移量等属性。 |
...
| 参数项 | 描述 |
| --- | --- |
| OH_ArkUI_ShadowOptions* options | 指向新创建的OH_ArkUI_ShadowOptions对象的指针，用于配置阴影的模糊半径、类型、颜色和偏移量等属性。 |
