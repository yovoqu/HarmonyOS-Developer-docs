# Camera_VideoProfile

更新时间：2026-04-20 06:34:33

来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-oh-camera-camera-videoprofile
**支持设备：** Phone | PC/2in1 | Tablet | Wearable | TV

```text
typedef struct Camera_VideoProfile {...} Camera_VideoProfile
```
  

##### 概述

**支持设备：** Phone | PC/2in1 | Tablet | Wearable | TV

录像配置文件。
 
**起始版本：** 11
 
**相关模块：** [OH_Camera](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-oh-camera)
 
**所在头文件：** [camera.h](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-camera-h)
 
  

##### 汇总

**支持设备：** Phone | PC/2in1 | Tablet | Wearable | TV

  

##### 成员变量

**支持设备：** Phone | PC/2in1 | Tablet | Wearable | TV
 
| 名称 | 描述 |
| --- | --- |
| Camera_Format format | 相机格式。 |
| Camera_Size size | 图片大小。 |
| Camera_FrameRateRange range | 帧速率，单位为fps（每秒帧数）。 |
