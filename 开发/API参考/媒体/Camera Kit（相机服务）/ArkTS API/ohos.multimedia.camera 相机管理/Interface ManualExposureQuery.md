...
function getSupportedExposureDurationRange(
  ...
  try {
    ...
    console.error(
      `The getSupportedExposureDurationRange call failed. error code: ${err.code}`,
    );
  }
...
function getExposureBiasStep(photoSession: camera.PhotoSession): number {
  ...
  try {
    ...
    let err = error as BusinessError;
    console.error(
      `The getExposureBiasStep call failed. error code: ${err.code}`,
    );
  }
  return biasStep;
}
