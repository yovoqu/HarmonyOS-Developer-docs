...
function isOISModeSupported(
  ...
  try {
    ...
    console.error(
      `The isOISModeSupported call failed. error code: ${err.code}`,
    );
  }
  return isSupported;
}
...
function getSupportedOISBiasRange(
  photoSession: camera.PhotoSession,
  oisAxis: camera.OISAxes,
): Array<number> {
  ...
  try {
    ...
    let err = error as BusinessError;
    console.error(
      `The getSupportedOISBiasRange call failed. error code: ${err.code}`,
    );
  }
  return biasRange;
}
...
function getSupportedOISBiasStep(
  photoSession: camera.PhotoSession,
  oisAxis: camera.OISAxes,
): number {
  let biasStep = 0.0;
  try {
    ...
    let err = error as BusinessError;
    console.error(
      `The getSupportedOISBiasStep call failed. error code: ${err.code}`,
    );
  }
  return biasStep;
}
...
function getCurrentCustomOISBias(
  photoSession: camera.PhotoSession,
  oisAxis: camera.OISAxes,
): number {
  let bias = 0.0;
  try {
    ...
    let err = error as BusinessError;
    console.error(
      `The getCurrentCustomOISBias call failed. error code: ${err.code}`,
    );
  }
  return bias;
}
