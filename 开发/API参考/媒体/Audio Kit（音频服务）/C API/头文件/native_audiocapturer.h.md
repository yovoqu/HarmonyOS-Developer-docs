...
| OH_AudioStream_Result OH_AudioCapturer_Pause(OH_AudioCapturer* capturer) | - | 暂停输入音频流。在暂停音频后，后续需要恢复录音的场景，建议使用pause。 |
| OH_AudioStream_Result OH_AudioCapturer_Stop(OH_AudioCapturer* capturer) | - | 停止音频采集器，停止输入音频流后，如果需要彻底结束录音，建议使用stop。 |
...
| OH_AudioStream_Result OH_AudioCapturer_GetFrameSizeInCallback(OH_AudioCapturer* capturer, int32_t* frameSize) | - | 查询回调帧数。frameSize表示每次回调对应的采样帧数。 |
...
| typedef void (*OH_AudioCapturer_SensitiveRecordPermitCallback)(OH_AudioCapturer* capturer, void* userData, bool isPermitted) | OH_AudioCapturer_SensitiveRecordPermitCallback | 蜂窝通话录音场景下，风险提示语播放结束的回调函数。应用必须等待回调返回许可结果，且isPermitted为true时，方可开始蜂窝通话录音。 |
...
| --- | --- |
| OH_AudioCapturer* capturer | 指向OH_AudioStreamBuilder_GenerateCapturer创建的音频流实例。 |
| void* userData | 通过OH_AudioStreamBuilder_SetSensitiveRecordPermitCallback指向应用自定义的数据存储区域。 |
