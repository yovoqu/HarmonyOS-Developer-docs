...
```ts
let audioHapticManagerInstance: audioHaptic.AudioHapticManager =
  audioHaptic.getAudioHapticManager();
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e/v3/EYnXYkfeSd6Tjp26TpeGiQ/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084801Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=738EE9AFCFE01E4F6FB5DB81D29E74796BFD1EEF1360BFBEA1EAC2A1F1277253)
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/47/v3/WjYfTLc2QmqXSjFjDCDICA/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084801Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=1B181474348BDCAF77B18AAB7856270B14958201FBC0395CEEBFD3E64BE5B658)
单个应用最多支持同时注册128个资源，超过之后将会注��失败（返回注册的资源ID为负数）。推荐应用合理控制注册资源数量，对于不再需要使用的资源，建议及时取消注册。
...
audioHapticManagerInstance
  .registerSource(audioUri, hapticUri)
  .then((value: number) => {
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to register source. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/be/v3/mqWDv7w7RjeAYRXPgUsq6Q/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084801Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=C292E8C75005CDF870C8CA89E61ACA04068321C942396A04D00523001EB81E98)
单个应用最多支持同时注册128个资源，超过之后将会注册失败（返回注册的资源ID为负数）。推荐应用合理控制注册资源数量，对于不再需要使用的资源，建议及时取消注册。
...
// 单个应用最多支持同时注册128个资源，超过之后将会注册失败（返回注册的资源ID为负数）。推荐应用合理控制注册资源数量，对于不再需要使用的资源，建议及时取消注册。
audioHapticManagerInstance
  .registerSourceFromFd(audioFd, hapticFd)
  .then((value: number) => {
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to register source from fd. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/46/v3/pEI3u60tQqWiG8gnefAiHw/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084801Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=5A9C7D15A022A43A1AD456ACB19048BE3FAAF5249AF95D59729049BC52BEE42B)
...
audioHapticManagerInstance
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to unregister source. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
let latencyMode: audioHaptic.AudioLatencyMode =
  audioHaptic.AudioLatencyMode.AUDIO_LATENCY_MODE_FAST;
...
let options: audioHaptic.AudioHapticPlayerOptions = {
  muteAudio: false,
  muteHaptics: false,
};
let audioHapticPlayerInstance: audioHaptic.AudioHapticPlayer | undefined =
  undefined;

audioHapticManagerInstance
  .createPlayer(id, options)
  .then((value: audioHaptic.AudioHapticPlayer) => {
    audioHapticPlayerInstance = value;
    console.info('Succeeded in creating player.');
  })
  .catch((err: BusinessError) => {
    console.error(
      `Failed to create player. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
```ts
let audioHapticType: audioHaptic.AudioHapticType =
  audioHaptic.AudioHapticType.AUDIO_HAPTIC_TYPE_AUDIO;
...
audioHapticPlayerInstance
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to release. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/47/v3/HvVjgCgYQomQ1UoxgKqJvA/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084801Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=2E5A8162550AAA212B07BCFB7D30D32F2022D6631DB7C1FA08E15AA8436217C2)
该方法需在音振播放器释放前调用。
...
audioHapticPlayerInstance
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to set volume. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e4/v3/FZXgcHXCQQWn1xVKSh-kFQ/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084801Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=84D3C9E2046A791AC8B0E6B0DCDF79A2256B89E700BD340DE323E430D3EDBA1D)
该方法需在音振播放器销毁前调用。
...
audioHapticPlayerInstance
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to set loop. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
