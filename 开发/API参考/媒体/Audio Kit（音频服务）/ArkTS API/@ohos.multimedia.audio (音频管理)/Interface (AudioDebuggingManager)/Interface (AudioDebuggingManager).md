...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiodebuggingmanager
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| renderer | AudioRenderer | 是 | 目标音频播放实例。 |
| fd | number | 是 | 文件描述符，指定快照信息的写入位置。小于0或不可写时，快照信息将输出到运行日志；否则输出到fd指向的文件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| capturer | AudioCapturer | 是 | 目标录音实例。 |
| fd | number | 是 | 文件描述符，指定快照信息的写入位置。小于0或不可写时，快照信息将输出到运行日志；否则输出到fd指向的文件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| loopback | AudioLoopback | 是 | 目标耳返实例。 |
| fd | number | 是 | 文件描述符，指定快照信息的写入位置。小于0或不可写时，快照信息将输出到运行日志；否则输出到fd指向的文件。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| session | AudioSessionManager | 是 | 目标会话管理器实例。 |
| fd | number | 是 | 文件描述符，指定快照信息的写入位置。小于0或不可写时，快照信息将输出到运行日志；否则输出到fd指向的文件。 |
