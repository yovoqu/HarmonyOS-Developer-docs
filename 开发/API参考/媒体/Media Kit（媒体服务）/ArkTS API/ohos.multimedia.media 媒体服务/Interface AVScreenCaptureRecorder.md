...
> [!NOTE]
> 应用最多可添加5个水印。 需在 startRecording 接口调用前调用addWatermark接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| watermark | image.PixelMap | 是 | 水印图像，取值原则：PixelMap对象不能为空。支持透明度设置。图像格式和尺寸要求请参考image.PixelMap。 |
| config | WatermarkConfiguration | 是 | 配置视频录制水印的相关参数。各字段取值范围请参考WatermarkConfiguration定义。需在调用startRecording接口前设置。 |
