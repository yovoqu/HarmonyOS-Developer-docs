...
```ts
audioPlayer.on('timeUpdate', (seekDoneTime: number) => {
  // 设置'timeUpdate'事件回调。
...
audioPlayer.getTrackDescription(
...
audioPlayer.on('error', (error: BusinessError) => {
  // 设置'error'事件回调。
...
```ts
audioPlayer.on('timeUpdate', (newTime: number) => {
  // 设置'timeUpdate'事件回调。
  if (newTime == null) {
