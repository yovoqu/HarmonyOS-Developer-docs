...
| 名称 | 描述 |
| --- | --- |
| OH_AVRecorder_CodecMimeType mimeType | 编码器MIME类型。值与type相关，音频编码器取音频MIME类型，视频编码器取视频MIME类型。 |
| char* type | 编码器类型，audio表示音频编码器，video表示视频编码器。 |
| OH_AVRecorder_Range bitRate | 编码器支持的比特率的范围，比特率单位为比特每秒（bit/s）。音频和视频编码器均适用。 |
| OH_AVRecorder_Range frameRate | 编码器支持的视频帧率的范围，视频帧率单位为帧每秒（fps）。仅适用于视频编码器。 |
...
| OH_AVRecorder_Range channels | 编码器支持的音频声道数的范围。仅适用于音频编码器。 |
