...
| OH_AVRecorder_Profile profile | 包含音视频录制的详细参数，如编码格式、比特率、分辨率等。 |
...
| OH_AVRecorder_FileGenerationMode fileGenerationMode | 指定录制输出文件的生成模式。应用创建模式适用于需要自定义输出文件URL的录制场景；系统创建模式适用于无需指定输出文件URL的录制场景，将通过触发OH_AVRecorder_OnUri回调通知应用获取录制生成的媒体资源文件。默认值为应用创建模式。 |
| OH_AVRecorder_Metadata metadata | 包含录制媒体的附加元数据。用于为录制文件添加描述性属性，如体裁分类、视频旋转方向、地理位置及自定义参数等。默认为空。 |
