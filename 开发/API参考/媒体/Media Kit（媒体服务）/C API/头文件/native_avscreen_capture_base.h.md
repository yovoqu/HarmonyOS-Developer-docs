...
| 名称 | 描述 |
| --- | --- |
| const char * OH_SCREEN_CAPTURE_CONTENT_RECT | 获取录屏图像帧中有效内容区域信息的key。 返回值是一个int32_t数组，单位为像素（px）。数组长度为4。数组元素定义为[top, left, width, height]，分别表示矩形窗口的左上角坐标、窗口的宽度和高度。数组元素可以从OH_AVFormat_GetIntBuffer中获取。 起始版本： 26.0.0 |
