...
async function example(
  ...
  phAccessHelper.getAssets(fetchOptions, async (err, fetchResult) => {
    ...
    await photoAccessHelper.MediaAssetManager.requestImage(
      context,
      photoAsset,
      requestOptions,
      handler,
...
async function example(context: Context) {
  try {
    ...
    console.error(
      `cancelRequest failed with error: ${err.code}, ${err.message}`,
    );
  }
}
```
...
async function example(context: Context) {
  try {
    let imageFileUri: string =
      'file://com.example.temptest/data/storage/el2/base/haps/ImageFile.jpg'; // 应用沙箱动态照片的图片uri。
    let videoFileUri: string =
      'file://com.example.temptest/data/storage/el2/base/haps/VideoFile.mp4'; // 应用沙箱动态照片的视频uri。
    ...
    console.error(
      `loadMovingPhoto failed with error: ${err.code}, ${err.message}`,
    );
  }
}
```
...
async function example(context: Context) {
  ...
  phAccessHelper.getAssets(fetchOptions, async (err, fetchResult) => {
    ...
    let photoAsset: photoAccessHelper.PhotoAsset =
      await fetchResult.getFirstObject();
    await photoAccessHelper.MediaAssetManager.quickRequestImage(
      context,
      photoAsset,
      requestOptions,
      handler,
