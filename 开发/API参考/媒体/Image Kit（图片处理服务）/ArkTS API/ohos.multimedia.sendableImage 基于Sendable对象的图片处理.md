...
async function Demo() {
  ...
  sendableImage
    ...
    .then((pixelMap: sendableImage.PixelMap) => {
...
class MySequence implements rpc.Parcelable {
  ...
  unmarshalling(messageSequence: rpc.MessageSequence) {
    try {
      ...
      console.error(
        `createPixelMapFromParcel error. code is ${error.code}, message is ${error.message}`,
      );
      return false;
    }
    return true;
...
async function Demo() {
  ...
  await sendableImage
    .createPixelMap(color, opts)
    .then((srcPixelMap: sendableImage.PixelMap) => {
...
async function Demo(surfaceId: string) {
  ...
  sendableImage
    .createPixelMapFromSurface(surfaceId, region)
    .then(() => {
...
async function Demo() {
  ...
  let pixelMap: sendableImage.PixelMap = sendableImage.createPixelMapSync(
    color,
    opts,
  );
  return pixelMap;
...
async function Demo() {
  ...
  let sendablePixelMap: sendableImage.PixelMap =
    sendableImage.convertFromPixelMap(pixelMap);
...
async function Demo() {
  ...
  let sendablePixelMap: sendableImage.PixelMap =
    sendableImage.createPixelMapSync(color, opts);
  let pixelMap: image.PixelMap =
    sendableImage.convertToPixelMap(sendablePixelMap);
  return pixelMap;
}
```
...
async function Demo(pixelMap: sendableImage.PixelMap) {
  ...
  if (pixelMap != undefined) {
    pixelMap
      ...
      .catch((error: BusinessError) => {
        console.error(
          `Failed to read the image data in the area. code is ${error.code}, message is ${error.message}`,
        ); // 不符合条件则进入。
      });
  }
}
...
async function Demo(pixelMap: sendableImage.PixelMap) {
  ...
  if (pixelMap != undefined) {
    pixelMap
      ...
      .catch((error: BusinessError) => {
        console.error(
          `Failed to write pixelmap into the specified area. code is ${error.code}, message is ${error.message}`,
        );
      });
  }
}
...
async function Demo(pixelMap: sendableImage.PixelMap) {
  if (pixelMap != undefined) {
    pixelMap
      .createAlphaPixelmap()
      .then((alphaPixelMap: sendableImage.PixelMap) => {
...
async function Demo(pixelMap: sendableImage.PixelMap) {
  ...
  pixelMap
    ...
    .catch((error: BusinessError) => {
      console.error(
        `Failed to apply color space for pixelmap object. code is ${error.code}, message is ${error.message}`,
      );
    });
}
```
    ...
    sendableImage
      ...
      .then((pixelParcel: sendableImage.PixelMap) => {
        pixelParcel
          ...
          .then(async (pixelMap: sendableImage.PixelMap) => {
            this.pixel_map = pixelMap;
            pixelMap.getImageInfo().then((imageInfo: image.ImageInfo) => {
              console.info(
                'unmarshalling information h:' +
                  imageInfo.size.height +
                  'w:' +
...
async function Demo(context: Context) {
  ...
  const sendableImageSourceObj: sendableImage.ImageSource =
    sendableImage.createImageSource(path);
}
```
...
async function Demo(context: Context) {
  ...
  const sendableImageSourceObj: sendableImage.ImageSource =
    sendableImage.createImageSource(file.fd);
}
```
...
async function Demo() {
  ...
  const sendableImageSourceObj: sendableImage.ImageSource =
    sendableImage.createImageSource(buf);
}
```
...
async function Demo() {
  ...
  let receiver: sendableImage.ImageReceiver = sendableImage.createImageReceiver(
    size,
...
async function Demo() {
  ...
  img
    ...
    .catch((error: BusinessError) => {
      console.error(
        `getComponent failed code ${error.code}, message is ${error.message}`,
      );
    });
}
```
...
async function Demo() {
  ...
  img
    ...
    .catch((error: BusinessError) => {
      console.error(
        `release failed. code ${error.code}, message is ${error.message}`,
      );
    });
}
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bd/v3/wLXGbF9ASq-NEsNEJ7Mh0Q/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084904Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=9682AFD81B33AA4FB217826AC84A897A6AB18F739AF0C592FB23C31D1E28AE7A)
...
async function Demo() {
  ...
  receiver
    .readLatestImage()
    .then((img: sendableImage.Image) => {
      console.info('readLatestImage succeeded.');
    })
    .catch((error: BusinessError) => {
      console.error(
        `readLatestImage failed. code ${error.code}, message is ${error.message}`,
      );
    });
}
```
...
![图片](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ec/v3/YJ67fPWKRYG4RA0aY-y7qg/caution_3.0-zh-cn.png?HW-CC-KV=V1&amp;HW-CC-Date=20260514T084904Z&amp;HW-CC-Expire=86400&amp;HW-CC-Sign=D75A80962A5929261FE6D7A7C53BF15FAD65128DD7476101628CEF89AA0B5764)
此接口需要在[on](#on)回调触发后调用，才能正常的接收到数据。且此接口返回的[Image](#image)对象使用完毕后需要调用[release](#release-2)方法释放，释放后才可以继续接收新的数据。
...
async function Demo() {
  ...
  receiver
    ...
    .catch((error: BusinessError) => {
      console.error(
        `readNextImage failed. code ${error.code}, message is ${error.message}`,
      );
    });
}
```
