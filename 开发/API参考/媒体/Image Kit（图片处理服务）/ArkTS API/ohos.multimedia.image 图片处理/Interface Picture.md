    ...
    if (mainPixelmap != null) {
      mainPixelmap
        ...
        .then((imageInfo: image.ImageInfo) => {
          if (imageInfo != null) {
            console.info(
              'GetMainPixelmap information height:' +
...
async function GetHdrComposedPixelmap(pictureObj: image.Picture) {
  ...
  if (pictureObj != null) {
    // 图片包含Hdr图。
    let hdrComposedPixelmap: image.PixelMap =
      await pictureObj.getHdrComposedPixelmap();
    if (hdrComposedPixelmap != null) {
      hdrComposedPixelmap
        ...
        .then((imageInfo: image.ImageInfo) => {
          if (imageInfo != null) {
            console.info(
              `GetHdrComposedPixelmap information height:${imageInfo.size.height} width:${imageInfo.size.width}`,
            );
          }
        })
        .catch((error: BusinessError) => {
...
async function GetHdrComposedPixelmapWithOptions(picture: image.Picture) {
  ...
  let hdrComposedPixelmap: image.PixelMap | undefined =
    await picture.getHdrComposedPixelmapWithOptions(opt);
  ...
  hdrComposedPixelmap
    ...
    .then((imageInfo: image.ImageInfo) => {
      if (imageInfo !== null) {
        console.info(
          `GetHdrComposedPixelmapWithOptions information height:${imageInfo.size.height} width:${imageInfo.size.width}`,
        );
      }
    })
    .catch((error: BusinessError) => {
      console.error(
        `GetHdrComposedPixelmapWithOptions information failed error.code: ${error.code} ,error.message: ${error.message}`,
      );
    });
}
```
    ...
    if (gainPixelmap != null) {
      gainPixelmap
        ...
        .then((imageInfo: image.ImageInfo) => {
          if (imageInfo != null) {
            console.info(
              `GetGainmapPixelmap information height:${imageInfo.size.height} width:${imageInfo.size.width}`,
            );
          } else {
...
async function SetAuxiliaryPicture(context: Context) {
  ...
  if (pictureObj != null) {
    ...
    let auxPictureObj: image.AuxiliaryPicture | null =
      pictureObj.getAuxiliaryPicture(type);
...
async function SetPictureObjMetadata(exifContext: Context) {
  ...
  let exifImageSource: image.ImageSource = image.createImageSource(
    exifRawFile.buffer as ArrayBuffer,
    exifOps,
  );
  let exifCommodityPixelMap: image.PixelMap =
    await exifImageSource.createPixelMap();
  let exifPictureObj: image.Picture = image.createPicture(
    exifCommodityPixelMap,
  );
  ...
  if (exifPictureObj != null) {
    ...
    let exifMetaData: image.Metadata =
      await exifPictureObj.getMetadata(metadataType);
    exifPictureObj
      .setMetadata(metadataType, exifMetaData)
      .then(() => {
        console.info('Set metadata success');
      })
      .catch((error: BusinessError) => {
        console.error(
          'Failed to set metadata. error.code: ' +
...
async function GetPictureObjMetadataProperties(pictureObj: image.Picture) {
  if (pictureObj != null) {
    let metadataType: image.MetadataType = image.MetadataType.EXIF_METADATA;
    let pictureObjMetaData: image.Metadata =
      await pictureObj.getMetadata(metadataType);
