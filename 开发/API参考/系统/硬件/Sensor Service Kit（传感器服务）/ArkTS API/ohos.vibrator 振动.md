...
| attribute | VibrateAttribute | 是 | 马达振动属性，用于指定马达ID、设备ID和振动使用场景。attribute中的usage参数决定振动的场景类型，不同场景类型受系统振动开关管控规则不同，开发者需根据实际业务场景选择合适的usage值。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stopMode | VibratorStopMode | 是 | 指定的停止振动模式，用于停止对应模式的马达振动，支持两种： VIBRATOR_STOP_MODE_TIME：停止VibrateTime类型的固定时长振动； VIBRATOR_STOP_MODE_PRESET：停止VibratePreset类型的预置振动。 此接口无法停止自定义振动（VibrateFromFile和VibrateFromPattern），请使用vibrator.stopVibration10+。stopMode需与启动振动时的VibrateEffect类型对应，否则停止操作可能无效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stopMode | VibratorStopMode | 是 | 指定的停止振动模式，用于停止对应模式的马达振动，支持两种： VIBRATOR_STOP_MODE_TIME：停止VibrateTime类型的指定时长振动； VIBRATOR_STOP_MODE_PRESET：停止VibratePreset类型的预置振动。 此接口无法停止自定义振动（VibrateFromFile和VibrateFromPattern），请使用vibrator.stopVibration10+。stopMode需与启动振动时的VibrateEffect类型对应，否则停止操作可能无效。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | VibratorInfoParam | 否 | 指出需要停止振动的设备和马达信息。不传参时默认停止本地设备的全部马达振动。deviceId默认值为-1（本地设备），vibratorId默认值为0（该设备的全部马达）。deviceId和vibratorId可通过vibrator.getVibratorInfoSync或vibrator.on查询获取。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| effectId | string | 是 | 待确认的预置振动效果ID。字符串最大长度64，超出部分截取前64个字符。使用场景：不同设备预置的振动效果可能不同，需传入具体的effectId查询是否支持。取值可参考EffectId和HapticFeedback中定义的值。 |
| callback | AsyncCallback&lt;boolean&gt; | 是 | 回调函数。返回true表示设备支持该effectId，可用于startVibration；返回false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;boolean&gt; | Promise对象。返回true表示设备支持该effectId，可用于startVibration；返回false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
| 类型 | 说明 |
| --- | --- |
| boolean | 返回对象。返回true表示设备支持该effectId，可用于startVibration；返回false表示不支持，使用该effectId触发振动可能效果不佳或无法振动。 |
...
| --- | --- | --- | --- |
| effectId | string | 是 | 待确认的预置振动效果ID。字符串最大长度64，超出部分截取前64个字符。使用场景：不同设备预置的振动效果可能不同，需传入具体的effectId查询是否支持。取值可参考EffectId和HapticFeedback中定义的值。 |
| param | VibratorInfoParam | 否 | 指出需要查询的设备和马达信息。不传param时默认查询本地设备。deviceId默认值为-1（本地设备），vibratorId默认值为0（该设备的全部马达）。deviceId和vibratorId可通过vibrator.getVibratorInfoSync或vibrator.on查询获取。 |
...
| 类型 | 说明 |
| --- | --- |
| EffectInfo | 预置振动效果信息。isEffectSupported为true表示支持该效果，可用于startVibration；为false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| param | VibratorInfoParam | 否 | 指出需要查询的设备和马达信息。不传param时默认查询所有设备所有马达的信息。deviceId默认值为-1（查询全部设备），vibratorId默认值为0（查询该设备的全部马达）。 |
...
| callback | Callback&lt;VibratorStatusEvent&gt; | 是 | 回调函数。当马达设备上线或下线时触发，回调参数为VibratorStatusEvent对象，包含timestamp、deviceId、vibratorCount、isVibratorOnline等信息。回调中获取的deviceId可用于startVibration和stopVibration等接口指定目标设备。 |
...
| --- | --- | --- | --- |
| type | string | 是 | 监听类型，该值固定为vibratorStateChange，表示马达上下线状态变化事件。 |
| callback | Callback&lt;VibratorStatusEvent&gt; | 否 | 回调函数，需要注销的回调函数。不传此参数时注销所有vibratorStateChange类型的回调。使用场景：若仅需注销特定回调则传入对应callback；若需注销全部回调则不传此参数。 |
...
| deviceId | number | 否 | 否 | 设备的ID。可用于startVibration和stopVibration等接口指定目标设备。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| deviceId | number | 否 | 是 | 设备的ID。默认值：-1，表示本地设备。使用场景：在多设备场景下需指定远程设备时设置此参数；不填此参数时默认控制本地设备。从API version 19开始，设备ID可通过getVibratorInfoSync或on查询获取。 |
| vibratorId | number | 否 | 是 | 马达ID。默认值：0，表示该设备的全部马达。使用场景：在多马达设备上需指定特定马达时设置此参数；不填此参数时默认控制该设备的全部马达。从API version 19开始，马达ID可通过getVibratorInfoSync或on查询获取。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| isEffectSupported | boolean | 否 | 否 | 预置效果是否受支持。true表示支持该预置振动效果，可用于startVibration；false表示不支持，使用该effectId触发振动可能效果不佳。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| deviceId | number | 否 | 否 | 设备ID。可用于startVibration和stopVibration等接口指定目标设备。 |
| vibratorId | number | 否 | 否 | 马达ID。可用于startVibration和stopVibration等接口指定目标马达。 |
...
| options | ContinuousParam | 否 | 可选参数，用于指定长振事件的振动强度、频率、振动调节曲线和通道编号。不填时使用各参数的默认值（intensity默认100，frequency默认50，index默认0）。 |
...
| 类型 | 说明 |
| --- | --- |
| VibratorPatternBuilder | 返回已添加连续振动事件的VibratorPatternBuilder对象。可用于继续链式调用addContinuousEvent或addTransientEvent添加更多振动事件，最终通过build生成VibratorPattern对象。 |
...
| options | TransientParam | 否 | 可选参数，用于指定短振事件的振动强度、频率和通道编号。不填时使用各参数的默认值（intensity默认100，frequency默认50，index默认0）。 |
...
| 类型 | 说明 |
| --- | --- |
| VibratorPatternBuilder | 返回已添加短振事件的VibratorPatternBuilder对象。可用于继续链式调用addContinuousEvent或addTransientEvent添加更多振动事件，最终通过build生成VibratorPattern对象。 |
...
| 类型 | 说明 |
| --- | --- |
| VibratorPattern | 振动序列对象。包含振动序列的起始时间和振动事件数组，可作为VibrateFromPattern的pattern参数传入startVibration接口触发振动。 |
...
> [!NOTE]
> 由于设备存在多样性，不同的设备可能预置不同的效果，建议使用预置效果前先使用 vibrator.isSupportEffect 10+ 或 vibrator.isSupportEffectSync 接口查询当前设备是否支持该预置效果。
...
> [!NOTE]
> 由于设备存在多样性，建议使用预置效果前先使用 vibrator.isSupportEffect 10+ 或 vibrator.isSupportEffectSync 接口查询当前设备是否支持该预置效果。
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| VIBRATOR_STOP_MODE_TIME | 'time' | 停止VibrateTime类型（duration模式）的振动。需与startVibration时使用的VibrateTime类型对应。 |
| VIBRATOR_STOP_MODE_PRESET | 'preset' | 停止VibratePreset类型（预置EffectId模式）的振动。需与startVibration时使用的VibratePreset类型对应。 |
...
| 类型 | 说明 |
| --- | --- |
| VibrateTime | 按照指定时长触发马达振动。适用于仅需控制振动时长的基础场景。 元服务API： 从API version 11开始，该接口支持在元服务中使用。 |
| VibratePreset | 按照预置振动类型触发马达振动。适用于交互反馈类的短振场景，推荐使用以确保与系统整体振感反馈体验风格一致。 |
| VibrateFromFile | 按照自定义振动配置文件触发马达振动。适用于需要精细振动控制的复杂场景。 |
| VibrateFromPattern18+ | 按照自定义振动效果触发马达振动。适用于需要灵活组合振动事件的场景。 |
...
| effectId | string | 否 | 否 | 预置的振动效果ID。字符串最大长度64，超出部分截取前64个字符。使用场景：不同设备预置的振动效果可能不同，建议先通过vibrator.isSupportEffect或vibrator.isSupportEffectSync查询是否支持。取值可参考EffectId和HapticFeedback中定义的值。 |
...
| hapticFd | HapticFileDescriptor10+ | 否 | 否 | 振动配置文件的描述符。需确保文件可用且格式正确，振动序列存储格式请参考振动效果说明。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| fd | number | 否 | 否 | 资源文件描述符。可通过fileIo.open从沙箱路径获取或通过getRawFd从HAP资源获取。 |
...
| points | Array&lt;VibratorCurvePoint&gt; | 否 | 是 | 可选参数，表示振动调节曲线数组。使用场景：适用于需要精细控制振动强度和频率变化趋势的交互反馈场景。数组中元素个数最少设置4个，最大设置16个。 |
...
| events | Array&lt;VibratorEvent&gt; | 否 | 否 | 振动事件数组。由VibratorPatternBuilder的addContinuousEvent和addTransientEvent方法添加后通过build方法生成。同一VibratorPattern中多个VibratorEvent的time值不能重叠。 |
...
| points | VibratorCurvePoint[] | 否 | 是 | 可选参数，表示振动调节曲线数组。使用场景：适用于需要精细控制振动强度和频率变化趋势的交互反馈场景。数组中元素个数最少设置4个，最大设置16个。 |
...
| pattern | VibratorPattern | 否 | 否 | build方法返回的VibratorPattern对象。需先通过VibratorPatternBuilder添加振动事件后调用build方法生成。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| id | number | 否 | 是 | 马达ID。默认值：0。使用场景：当设备存在多个马达时，可通过指定不同的马达ID来选择特定马达触发振动。马达ID可以通过getVibratorInfoSync查询。不填写时默认使用马达ID为0的马达。 |
| deviceId19+ | number | 否 | 是 | 设备ID。默认值：-1，表示本地设备。使用场景：在多设备场景下需指定远程设备时设置此参数；不填写时默认控制本地设备。从API version 19开始，设备ID可以使用getVibratorInfoSync或on查询。 元服务API：从API version 19开始，该接口支持在元服务中使用。 |
| usage | Usage | 否 | 否 | 马达振动的使用场景。默认值：'unknown'。取值范围只允许在Usage提供的类型中选取。不同usage值对应不同的系统振动开关管控规则，开发者需根据实际业务场景选择合适的usage值。 元服务API：从API version 11开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| effectId | EffectId | 是 | 预置的振动效果ID。字符串最大长度64，超出部分截取前64个字符。建议先通过vibrator.isSupportEffect或vibrator.isSupportEffectSync查询是否支持。 |
...
> [!NOTE]
> 从API version 8 开始支持，从API version 9 开始废弃，建议使用 vibrator.stopVibration 替代。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| stopMode | VibratorStopMode | 是 | 马达停止指定的振动模式。需与启动振动时的模式对应：VIBRATOR_STOP_MODE_TIME用于停止时长振动，VIBRATOR_STOP_MODE_PRESET用于停止预置效果振动。 |
