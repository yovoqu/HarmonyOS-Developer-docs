...
let awareness: smartMobilityCommon.SmartMobilityAwareness =
  smartMobilityCommon.getSmartMobilityAwareness();
```
...
try {
  ...
  let awareness: smartMobilityCommon.SmartMobilityAwareness =
    smartMobilityCommon.getSmartMobilityAwareness();
  // 业务类型
  let types: smartMobilityCommon.SmartMobilityType[] = [
    smartMobilityCommon.SmartMobilityType.CAR_HOP,
  ];
  // 出行业务事件回调函数
  ...
  hilog.error(
    0x0000,
    'testTag',
    `on smart mobility event error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  hilog.error(
    0x0000,
    'testTag',
    `off smart mobility event error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  // 获取公共事件信息
  let event: smartMobilityCommon.SmartMobilityEvent =
    awareness.getSmartMobilityEvent(type, eventName);
  hilog.info(
    0x0000,
    'testTag',
    `get smart mobility event succeed, event: ${JSON.stringify(event)}`,
  );
} catch (e) {
  // 捕获接口调用异常时的错误码并做相应处理
  hilog.error(
    0x0000,
    'testTag',
    `get smart mobility event error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  // 业务类型
  let types: smartMobilityCommon.SmartMobilityType[] = [
    smartMobilityCommon.SmartMobilityType.HICAR,
  ];
  // 出行连接状态回调函数
  ...
  hilog.error(
    0x0000,
    'testTag',
    `on smart mobility status error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  hilog.error(
    0x0000,
    'testTag',
    `off smart mobility status error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  hilog.info(
    0x0000,
    'testTag',
    `get smart mobility status succeed, status: ${JSON.stringify(ret)}`,
  );
} catch (e) {
  // 捕获接口调用异常时的错误码并做相应处理
  hilog.error(
    0x0000,
    'testTag',
    `get smart mobility status error, error code: ${e?.code}`,
  );
}
```
