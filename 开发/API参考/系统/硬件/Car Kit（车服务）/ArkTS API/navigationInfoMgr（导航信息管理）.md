...
let navInfoController: navigationInfoMgr.NavigationController =
  navigationInfoMgr.getNavigationController();
```
...
try {
  ...
  hilog.error(
    0x0000,
    'testTag',
    `update navigation status error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  hilog.error(
    0x0000,
    'testTag',
    `update navigation metadata error, error code: ${e?.code}`,
  );
}
```
...
class Listener implements navigationInfoMgr.SystemNavigationListener {
  ...
  onQueryNavigationInfo(
    query: navigationInfoMgr.QueryType,
    args: Record<string, Object>,
  ): Promise<navigationInfoMgr.ResultData> {
  ...
  onReceiveNavigationCmd(
    command: navigationInfoMgr.CommandType,
    args: Record<string, Object>,
  ): Promise<navigationInfoMgr.ResultData> {
    return new Promise((resolve) => {
      let ret: navigationInfoMgr.ResultData = {
...
try {
  ...
  hilog.error(
    0x0000,
    'testTag',
    `register system navigation listener error, error code: ${e?.code}`,
  );
}
```
...
try {
  ...
  hilog.error(
    0x0000,
    'testTag',
    `unregister system navigation listener error, error code: ${e?.code}`,
  );
}
```
