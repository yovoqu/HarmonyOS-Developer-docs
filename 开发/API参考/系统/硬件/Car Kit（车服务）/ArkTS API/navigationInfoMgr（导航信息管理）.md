...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| naviTurnMode | number | 否 | 否 | 导航转向模式，十六进制，取值范围0~3E7，参考naviTurnMode，按照16进制的格式传递。 |
...
| trafficLane | string | 否 | 否 | 车道线，从最左边到最右边按序排列。通讯以四位为一个单元进行解析，每个单元对应一个车道线，根据需求可以传多个车道线，图标编码为枚举值，具体参考trafficLane。 |
