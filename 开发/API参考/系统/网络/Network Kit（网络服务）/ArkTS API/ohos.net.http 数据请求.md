...
> [!NOTE] 说明
> console.info()输出的数据中包含换行符会导致数据出现截断现象。 自API 12开始支持接收经过brotli算法压缩的HTTP响应。
...
> [!NOTE] 说明
> (1) 此接口仅支持接收5MB以内的数据，如果需要接收超过5MB的数据，则需主动在HttpRequestOptions的maxLimit中进行设置，或者使用requestInStream接口发起流式请求。自API version 23开始，本接口支持的最大接收数据量为50MB，API version 23之前仍为5MB，超过5MB会接收失败。 (2) 如需传入cookies，请开发者自行在参数options中添加。 (3) 若URL包含中文或其他语言，需先调用encodeURL(URL)编码，再发起请求。
...
> [!NOTE] 说明
> 从API version 6开始支持，从API version 8开始废弃，建议使用on("headersReceive")替代。
...
> [!NOTE] 说明
> 从API version 6开始支持，从API version 8开始废弃，建议使用off("headersReceive")替代。
...
| usingProtocol9+ | HttpProtocol | 否 | 是 | HTTP请求使用的协议版本。未指定时，由系统自动协商最适合的协议版本。若指定HTTP3，由于HTTP3协议的安全限制，需通过TlsConfig指定TLS 版本为1.3，且目标域名支持HTTP3协议，才能启用HTTP3，否则将协商降级。 |
...
| connectionExtraInfo24+ | ConnectionExtraInfo | 否 | 是 | HTTP请求交互的详细信息。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| networkProtocolName | string | 否 | 否 | request请求过程中的HTTP协议版本，如'HTTP/1.0'，'HTTP/1.1'，'HTTP/2'，'HTTP/2 over TLS'，'HTTP/3'，'Unknown/Non-HTTP'等。 |
| tlsVersion | TlsVersion | 否 | 是 | request请求过程中的TLS协议版本。只有当使用TLS协议时返回相应的TLS版本。 |
| cipherSuite | CipherSuite | 否 | 是 | request请求过程中的加密套件。只有当使用TLS协议时返回相应的加密套件。 |
...
> [!NOTE] 说明
> 将拦截器链附加到HttpRequest实例后，当该实例发起HTTP请求时，会触发已附加的拦截器链中相应类型的拦截器。 更多使用HTTP请求触发拦截器功能，可以参考HTTP拦截器功能代码示例。 HTTP拦截器相关能力仅支持HttpRequest.request接口，目前暂不支持HttpRequest.requestInStream(流式传输)接口。
...
> [!NOTE] 说明
> 推荐在网络并发等场景下使用。 当指定的网络没有激活时，系统按照指定默认网络处理。
