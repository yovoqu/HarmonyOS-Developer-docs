...
let version: urpc.FlowbufArrayElement<number> = {
  type: 'ARRAY_INT8',
  value: [1, 2, 3],
  length: 3,
};
...
urpc
  .urpcStubCreate(config, funcList)
  .then((urpcStub: urpc.UrpcStub) => {
    urpcStub.destroy();
  })
  .catch((err: BusinessError<string>) => {
    console.error(
      `Failed to create urpc stub, error code ${err.code}, error info ${err.data}`,
    );
  });
```
