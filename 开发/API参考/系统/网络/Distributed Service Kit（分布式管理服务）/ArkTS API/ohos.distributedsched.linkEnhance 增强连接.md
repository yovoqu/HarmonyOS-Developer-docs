...
try {
  ...
  let connection: linkEnhance.Connection = linkEnhance.createConnection(
    peerDeviceId,
    'demo',
  );
} catch (err) {
...
try {
  ...
  server.on(
    'connectionAccepted',
    (connection: linkEnhance.Connection): void => {
      hilog.info(
        0x0000,
        TAG,
        'serverOnCallback = ' + JSON.stringify(connection),
      );
    },
  );
  // 启动服务
...
try {
  ...
  connection.on('connectResult', (result: linkEnhance.ConnectResult): void => {
    hilog.info(
      0x0000,
      TAG,
      'clientConnectResultCallback result = ' + result.success,
    );
  });
  // 发起连接
  connection.connect();
...
try {
  ...
  hilog.info(
    0x0000,
    TAG,
    'peerDeviceId=%{public}s' + connection.getPeerDeviceId(),
  );
} catch (err) {
  hilog.error(
    0x0000,
  ...
  connection.on('connectResult', (result: linkEnhance.ConnectResult): void => {
    ...
    if (result.success) {
      ...
      hilog.info(
        0x0000,
        TAG,
        'sendData data connection peerDeviceId=%{public}s' +
          connection.getPeerDeviceId(),
      );
      connection.disconnect();
    }
