...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-oh-wifi-h
...
| 名称 | 描述 |
| --- | --- |
| WIFI_MAX_SSID_LEN 33 | OH_WifiLinkedInfo 成员变量ssid数组的长度（32 + \0）。 起始版本： 24 |
| WIFI_MAC_LEN 18 | OH_WifiLinkedInfo 成员变量macAddress数组和bssid数组的长度（17 + \0）。 起始版本： 24 |
...
| OH_WifiConnState | OH_WifiConnState | 定义WIFI连接状态。 |
| OH_WifiChannelWidth | OH_WifiChannelWidth | 定义当前连接热点的信道带宽。 |
| OH_WifiStandard | OH_WifiStandard | 当前连接热点的WIFI标准。 |
| OH_WifiCategory | OH_WifiCategory | 热点支持的最高WIFI级别。 |
| OH_WifiLinkType | OH_WifiLinkType | WIFI7连接类型。 |
...
| 名称 | 描述 |
| --- | --- |
| OH_WifiLinkedInfo | 提供WLAN连接的相关信息。可以获取当前设备已连接的WIFI热点的信息。 |
...
| Wifi_ResultCode OH_Wifi_GetLinkedInfo(OH_WifiLinkedInfo *info) | 该接口用于获取WIFI连接信息。 |
...
| connState | OH_WifiConnState | 否 | 否 | WLAN连接状态。详情见OH_WifiConnState。 |
| channelWidth | OH_WifiChannelWidth | 否 | 否 | 当前连接热点的信道带宽。详情见OH_WifiChannelWidth。 |
| wifiStandard | OH_WifiStandard | 否 | 否 | 当前连接热点的WIFI标准。详情见OH_WifiStandard。 |
| supportedWifiCategory | OH_WifiCategory | 否 | 否 | 热点支持的最高WIFI级别。详情见OH_WifiCategory。 |
| isHiLinkNetwork | bool | 否 | 否 | 热点是否支持hilink，true表示支持，false表示不支持。 |
| wifiLinkType | OH_WifiLinkType | 否 | 是 | WIFI7连接类型。详情见OH_WifiLinkType。 |
...
| 类型 | 说明 |
| --- | --- |
| Wifi_ResultCode | 返回操作结果，详细定义参见Wifi_ResultCode。 WIFI_SUCCESS 成功获取设备MAC地址。 WIFI_PERMISSION_DENIED 权限拒绝。 WIFI_NOT_SUPPORTED 不支持该能力。 WIFI_INVALID_PARAM 输入参数macAddr是空指针。 WIFI_OPERATION_FAILED 内部执行失败。 WIFI_STA_DISABLED WIFI STA模式未启用。 |
...
> [!NOTE]
> 当macType是1（设备MAC地址），获取macAddress还需申请ohos.permission.GET_WIFI_LOCAL_MAC权限（API version 8-15仅面向系统应用开放。从API version 16开始，在PC/2in1设备上面向普通应用开放，在其余设备上仍仅面向系统应用开放），无该权限时，macAddress返回为空。 如果应用申请了ohos.permission.GET_WIFI_PEERS_MAC权限，则返回结果中的bssid为真实bssid地址，否则为随机设备地址。
...
| 类型 | 说明 |
| --- | --- |
| Wifi_ResultCode | 返回操作结果，详细定义参见Wifi_ResultCode。 WIFI_SUCCESS 成功获取WIFI连接信息。 WIFI_PERMISSION_DENIED 权限拒绝。 WIFI_NOT_SUPPORTED 不支持获取连接信息。 WIFI_INVALID_PARAM 输入参数info是空指针。 WIFI_OPERATION_FAILED 内部执行失败。 WIFI_STA_DISABLED WIFI STA模式未启用。 |
