...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceAddress | [PartnerDeviceAddress](#partneragentpartnerdeviceaddress) | 是 | 应用注册的设备地址信息。          ��用需在PartnerDeviceAddress中设置bluetoothAddress字段值。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| supportBR | boolean | 否 | 是 | 该设备是否支持通过[ACL](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/terminology#acl)连接的方式发现，建立ACL连接后会认为成功发现了该设备。发现设备后，会拉起[PartnerAgentExtensionAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/is-fusionconnectivity-partneragentextensionability)进程，并调用进程中[onDeviceDiscovered](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/is-fusionconnectivity-partneragentextensionability#ondevicediscovered)方法。true表示支持通过连接的方式发现，false表示不支持通过连接的方式发现。未指定默认为false。 |
| supportBleAdvertiser | boolean | 否 | 是 | 该设备是否支持通过[BLE](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/terminology#ble)扫描的方式发现，扫描到该设备后会认为成功发现了该设备。发现设备后，会拉起PartnerAgentExtensionAbility进程，并调用进程中onDeviceDiscovered方法。true表示支持通过BLE扫描的方式发现，false表示不支持通过BLE扫描的方式发现。未指定默认为false。          注意：          选择[supportBleAdvertiser](#partneragentdevicecapability)选项，若扫描到该设备，3min内无ACL连接，会调用[onDestroyWithReason](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/is-fusionconnectivity-partneragentextensionability#ondestroywithreason)并销毁已拉起的PartnerAgentExtensionAbility进程。 |
