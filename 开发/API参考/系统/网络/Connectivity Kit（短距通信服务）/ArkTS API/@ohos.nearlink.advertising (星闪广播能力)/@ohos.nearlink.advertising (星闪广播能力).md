...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nearlink-advertising
...
| manufacturerData | ManufacturerData[] | 否 | 是 | 厂商数据。若未配置则默认不携带该字段。 |
| serviceData | ServiceData[] | 否 | 是 | 服务数据。若未配置则默认不携带该字段。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback&lt;AdvertisingStateChangeInfo&gt; | 是 | 回调函数，返回广播状态变化信息。 |
