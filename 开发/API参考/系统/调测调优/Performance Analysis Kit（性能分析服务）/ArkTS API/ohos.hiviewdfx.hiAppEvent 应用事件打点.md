...
| collectMinidump | boolean | 否 | 是 | 是否使能minidump，默认值为false。 true：在Native Crash场景同时生成minidump。 false：在Native Crash场景不生成minidump。 生成minidump日志文件以.dmp结尾，跟随APP_CRASH事件一起返回，保存在external_log字段中。 说明：该配置项为持久化配置，应用未重新设置前，值不变。 起始版本：26.0.0 元服务API： 从API版本26.0.0开始，该接口支持在元服务中使用。 |
