...
字串长度超出限制范围。
**可能原因**
- StatusBarItem中配置的hoverTips字符串长度为0或者大于128。
- 调用updateStatusBarHoverTips方法时传入的字符串长度为0或者大于128。
**处理步骤**
...
对不存在的菜单项进行更新。
**可能原因**
- 调用updateStatusBarMenuItem时，若传入的item的menuCode未在状态栏的一级菜单项中预先定义，则会导致更新失败。
- 调用updateStatusBarSubMenuItem时，若传入的item的menuCode未在状态栏的二级菜单项中预先定义，则会导致更新失败。
**处理步骤**
...
- 调用updateStatusBarMenuItem时，待更新的item的子菜单的menuCode不唯一。子菜单的各menuCode在当前菜单项中不唯一。子菜单的menuCode是其他菜单项的menuCode。子菜单的menuCode在其他菜单项的子菜单中存在。
**处理步骤**
...
传入的字符串参数的长度超过限制。
**可能原因**
- 调用addCustomCategory方法时传入的字符串长度大于512或者内容为空。
- 调用updateCustomCategory方法时传入的字符串长度大于512或者内容为空。
- 调用addQuickTask方法时传入的字符串长度大于512或者内容为空。
- 调用updateQuickTask方法时传入的字符串长度大于512或者内容为空。
**处理步骤**
...
**可能原因**
- 调用addQuickTask方法时传入的parameters参数的数组大小超过64。
- 调用updateQuickTask方法时传入的parameters参数的数组大小超过64。
**处理步骤**
