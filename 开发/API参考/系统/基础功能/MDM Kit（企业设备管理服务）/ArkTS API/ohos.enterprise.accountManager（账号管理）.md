...
try {
  ...
  console.error(
    `Failed to disallow os account addition. Code: ${err.code}, message: ${err.message}`,
  );
}
```
...
try {
  // 参数需根据实际情况进行替换
  let isDisallowed: boolean = accountManager.isOsAccountAdditionDisallowed(
    wantTemp,
    100,
  );
  console.info(
    `Succeeded in querying the os account addition or not: ${isDisallowed}`,
  );
} catch (err) {
  console.error(
    `Failed to query the os account addition or not. Code: ${err.code}, message: ${err.message}`,
  );
}
```
...
accountManager
  .addOsAccountAsync(
    wantTemp,
    'TestAccountName',
    osAccount.OsAccountType.NORMAL,
  )
  .then((info) => {
  ...
  .catch((err: BusinessError) => {
    console.error(
      `Failed to create os account. Code: ${err.code}, message: ${err.message}`,
    );
  });
```
...
async function setDomainAccountPolicy() {
  ...
  try {
    ...
    console.error(
      `Failed to set domainAccount policy. Code: ${err.code}, message: ${err.message}`,
    );
  }
  // 设置指定域账号策略
  ...
  await osAccount
    ...
    .catch((err: BusinessError) => {
      console.error(
        `Failed to get account domain info. Code: ${err.code}, message: ${err.message}`,
      );
    });
  try {
    ...
    console.error(
      `Failed to set domain account policy. Code: ${err.code}, message: ${err.message}`,
    );
  }
}
```
...
async function getDomainAccountPolicy() {
  ...
  try {
    domainAccountPolicy = accountManager.getDomainAccountPolicy(
      wantTemp,
      accountInfo,
    );
    ...
    console.error(
      `Failed to get domain account policy. Code: ${err.code}, message: ${err.message}`,
    );
  }
  // 查询指定域账号策略
  let accountInfo2: osAccount.DomainAccountInfo = {
