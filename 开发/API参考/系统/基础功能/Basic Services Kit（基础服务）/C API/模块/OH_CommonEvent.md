...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-oh-commonevent
...
| 名称 | 描述 |
| --- | --- |
| oh_commonevent.h | 提供订阅、取消订阅、发布、事件数据访问、附加信息读写、有序事件控制等全部操作函数，以及错误码枚举与核心数据类型定义。 |
| oh_commonevent_support.h | 提供系统预定义的公共事件名称常量（如 COMMON_EVENT_BATTERY_CHANGED、COMMON_EVENT_SCREEN_ON 等），供订阅时引用，本身不提供函数。 |
