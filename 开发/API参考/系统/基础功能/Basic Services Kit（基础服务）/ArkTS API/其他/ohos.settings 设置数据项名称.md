...
> [!NOTE]
> 本模块首批接口从API version 7开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。 如果访问的数据项没有获取到值，表示当前系统应用没有将该数据项的值添加到数据库。
...
> [!NOTE]
> 从 API version 8开始支持，从API version 9开始废弃，建议使用 getValueSync() 替代。
...
> [!NOTE]
> 从 API version 8开始支持，从API version 9开始废弃，建议使用 setValueSync() 替代。
