...
| AUTO_GAIN_TIME_ZONE(deprecated21) | string | 是 | 是否自动从NITZ获取时区。          - 值为true，表示自动获取。          - 值为false，表示不自动获取（该常量不支持使��）。 |
...
**需要权限**： ohos.permission.MANAGE_SETTINGS，该权限仅系统��用可用。
...
// 跳转输入法详情页面。
// 请在组件内获取context，确保this.getUIContext().getHostContext()���回结果为UIAbilityContext。
const context: Context = this.getUIContext().getHostContext() as common.UIAbilityContext;
...
try {
  ...
  console.error(
    `Failed to open the biometrics and password settings page. code: ${err?.code}, message: ${err?.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `Failed to open the NFC settings page. code: ${err?.code}, message: ${err?.message}`,
  );
}
```
