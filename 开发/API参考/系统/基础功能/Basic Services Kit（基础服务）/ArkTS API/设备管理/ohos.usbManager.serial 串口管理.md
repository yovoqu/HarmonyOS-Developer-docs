...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| portId | number | 是 | 端口号，来自getPortList返回的SerialPort对象，传入无效值时抛出错误码31400003异常。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| portId | number | 是 | 端口号，来自getPortList返回的SerialPort对象，必须使用getPortList返回的有效端口号，传入无效值时抛出错误码31400003异常。 |
...
| 类型 | 说明 |
| --- | --- |
| Readonly&lt;SerialAttribute&gt; | 返回串口的配置参数对象，包含波特率、数据位、校验位、停止位等配置信息。 |
...
| --- | --- | --- | --- |
| portId | number | 是 | 端口号，来自getPortList返回的SerialPort对象，必须使用getPortList返回的有效端口号，传入无效值时抛出错误码31400003异常。 |
| attribute | SerialAttribute | 是 | 串口配置参数对象，包含波特率（baudRate，必填）、数据位（dataBits，可选，默认8）、校验位（parity，可选，默认None）、停止位（stopBits，可选，默认1）。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| baudRate | BaudRates | 否 | 否 | 串口波特率，表示数据传输速率，单位：比特/秒 |
| dataBits | DataBits | 否 | 是 | 串口数据位，表示报文中的有效数据位数，默认值为8，单位：比特 |
| parity | Parity | 否 | 是 | 串口奇偶校验，用于检测数据传输错误，默认值为PARITY_NONE（无奇偶校验）。 |
| stopBits | StopBits | 否 | 是 | 串口停止位，表示报文结束标志，默认值为1，单位：比特 |
