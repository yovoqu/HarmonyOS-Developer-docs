...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions
...
> [!NOTE]
> 三方应用默认只能监听自身应用的安装事件。若三方应用需要监听InHouse应用的安装事件，需要被监听的InHouse应用在app.json5的 allowListenBundleChangedEvent 中配置本应用的 appIdentifier 。
...
> [!NOTE]
> 三方应用默认只能监听自身应用的卸载事件。若三方应用需要监听InHouse应用的卸载事件，需要被监听的InHouse应用在app.json5的 allowListenBundleChangedEvent 中配置本应用的 appIdentifier 。
...
> [!NOTE]
> 三方应用默认只能监听自身应用的更改事件。若三方应用需要监听InHouse应用的更新事件，需要被监听的InHouse应用在app.json5的 allowListenBundleChangedEvent 中配置本应用的 appIdentifier 。
...
> [!NOTE]
> 三方应用默认只能监听自身应用的缓存清理事件。若三方应用需要监听InHouse应用的缓存清理事件，需要被监听的InHouse应用在app.json5的 allowListenBundleChangedEvent 中配置本应用的 appIdentifier 。
...
> [!NOTE]
> 应用默认只能接收自身应用的skill变化事件。 申请ohos.permission.MANAGE_SKILL_PRIVILEGE权限后能接收自身应用和其他应用的skill变化事件。
...
#### COMMON_EVENT_BLUETOOTH_A2DPSOURCE_PLAY_STATE_CHANGE24+
...
#### COMMON_EVENT_BLUETOOTH_SCO_CONNECT_STATE_CHANGE24+
...
> [!NOTE]
> 从API version 9 开始支持，从API version 20 开始废弃，建议使用 COMMON_EVENT_BLUETOOTH_HANDSFREE_AG_CONNECT_STATE_CHANGE 替代。
...
> [!NOTE]
> 从API version 9 开始支持，从API version 20 开始废弃，建议使用 COMMON_EVENT_BLUETOOTH_A2DPSOURCE_CONNECT_STATE_CHANGE 替代。
...
> [!NOTE]
> 从API version 9 开始支持，从API version 20 开始废弃，建议使用 COMMON_EVENT_BLUETOOTH_A2DPSOURCE_AVRCP_CONNECT_STATE_CHANGE 替代。
...
> [!NOTE]
> 从API version 9 开始支持，从API version 20 开始废弃，建议使用 COMMON_EVENT_BLUETOOTH_A2DPSOURCE_CODEC_VALUE_CHANGE 替代。
...
> [!NOTE]
> 从API version 9 开始支持，从API version 20 开始废弃，建议使用 COMMON_EVENT_BLUETOOTH_REMOTEDEVICE_ACL_STATE_CHANGE 替代。
...
> [!NOTE]
> 从API version 9 开始支持，从API version 20 开始废弃，建议使用 COMMON_EVENT_BLUETOOTH_REMOTEDEVICE_PAIR_STATE_CHANGE 替代。
