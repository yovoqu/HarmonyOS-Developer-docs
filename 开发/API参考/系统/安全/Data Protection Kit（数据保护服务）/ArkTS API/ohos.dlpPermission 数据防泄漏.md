...
dlpPermission.isInSandbox().then(async (inSandbox) => {
  ...
  if (inSandbox) {
    dlpPermission
      ...
      .then((res: dlpPermission.DLPPermissionInfo) => {
...
dlpPermission.isInSandbox().then((inSandbox) => {
  // 是否在沙箱内。
  if (inSandbox) {
...
| 类型 | 说明 |
| --- | --- |
| string | 返回DLP文件扩展名。��如：原文件"text.txt"，加密后的DLP文件名为"test.txt.dlp"，返回扩展名为".dlp"。 |
...
dlpPermission
  .setSandboxAppConfig('configInfo')
  .then((res) => {
    // 设置沙箱应用配置信息。
    console.info('res', res);
  })
...
let plainFilePath: string =
  'file://docs/storage/Users/currentUser/Documents/test.txt';
let dlpFilePath: string =
  'file://docs/storage/Users/currentUser/Documents/test.txt.dlp';
plaintextFd = fileIo.openSync(plainFilePath, fileIo.OpenMode.READ_ONLY).fd;
dlpFd = fileIo.openSync(
  dlpFilePath,
...
dlpPermission
  .generateDlpFileForEnterprise(plaintextFd, dlpFd, dlpProperty, customProperty)
  .then((res) => {
...
| plaintextFd | number | 是 | ���标解密文件的fd。取值范围为[0, 231-1]。当fd小于0时，函数返回null；当fd大于231-1时，fd的值被截断。 |
...
let dlpConnManager: dlpPermission.DlpConnManager =
  new dlpPermission.DlpConnManager();
```
...
let pluginId: number = dlpPermission.DlpConnManager.registerPlugin(
  new DataCapsulePlugin(),
);
```
