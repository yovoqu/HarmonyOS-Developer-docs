...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;ObjectDetector&gt; | Promise对象，返回ObjectDetector实例，用于执行多目标识别。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| request | visionBase.Request | 是 | 图片实例。多目标识别接口仅支持传入一张图片，不支持传入多张图片。 详细内容请参考约束与限制。 |
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;ObjectDetectionResponse&gt; | Promise对象，返回多目标识别的结果。 |
