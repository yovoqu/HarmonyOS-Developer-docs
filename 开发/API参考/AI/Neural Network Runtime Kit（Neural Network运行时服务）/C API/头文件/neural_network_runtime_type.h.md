...
| OH_NN_OPS_MINIMUM = 97 | 逐元素计算两个张量的最小值，输入必须是两个张量，或一个张量和一个标量。当输入是两个张量时，数据类型不能同时是bool值，并且两个要能广播到相同的形状。当输入是一个张量和一个常量时，标量必须是常量。 输入： input1，n维张量，数据类型可以是数值类型或bool值。 input2，n维张量，数据类型可以是数值类型或bool值。 输出： output，比较结果张量，形状和数据类型和输入相同。 起始版本： 12 |
