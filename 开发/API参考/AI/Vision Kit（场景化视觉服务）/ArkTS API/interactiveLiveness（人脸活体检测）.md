...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| isSilentMode(deprecated) | DetectionMode | 否 | 否 | 表示的是人脸活体检测模式，默认动作活体检测模式。 INTERACTIVE_MODE表示动作活体检测模式。 说明： 从版本26.0.0开始废弃。 |
...
| --- | --- | --- | --- |
| config | InteractiveLivenessConfig | 是 | 跳转到人脸活体检测页面的配置项。 |
| callback | AsyncCallback<InteractiveLivenessResult \| undefined> | 是 | 回调函数。当人脸活体检测成功，err为undefined，data为获取到的InteractiveLivenessResult；否则返回错误对象， data为undefined。当前只适用于RouteRedirectionMode.BACK_MODE跳转模式。 |
