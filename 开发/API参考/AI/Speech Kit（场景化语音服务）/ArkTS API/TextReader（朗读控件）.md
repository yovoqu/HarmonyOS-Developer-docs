...
TextReader.start(readInfoList)
  ...
  .catch((e: BusinessError) => {
    console.error(
      `TextReader failed to start. Code: ${e.code}, message: ${e.message}`,
    );
  });
```
...
TextReader.stop()
  ...
  .catch((e: BusinessError) => {
    console.error(
      `TextReader failed to stop. Code: ${e.code}, message: ${e.message}`,
    );
  });
```
...
try {
  ...
  console.error(
    `TextReader failed to resetParam. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
TextReader.release()
  ...
  .catch((e: BusinessError) => {
    console.error(
      `TextReader failed to release. Code: ${e.code}, message: ${e.message}`,
    );
  });
```
...
try {
  ...
  console.error(
    `TextReader failed to pause. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `TextReader failed to resume. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `TextReader failed to hide panel. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `TextReader failed to show panel. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `TextReader failed to show Minibar. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `TextReader failed to hide Minibar. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.info(
    `Succeeded in querying read state, read state: ${JSON.stringify(readState)}`,
  );
} catch (e) {
  console.error(
    `TextReader failed to queryReadState. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
try {
  ...
  console.error(
    `TextReader failed to setArticle. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
| callback | Callback&lt;string&gt; | 是 | 点击文章或者切换文章时，执行的回调函数，参数为文章id，err为undefined，否则为错误对象。 |
...
try {
  ...
  console.error(
    `TextReader failed to set eventListener. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
| callback | Callback&lt;string&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
try {
  ...
  console.error(
    `TextReader failed to unset eventListener. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
| callback | Callback&lt;string&gt; | 是 | 点击标题时执行的回调函数，参数为文章id，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;string&gt; | 是 | 点击作者时执行的回调函数，参数为文章id，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;string&gt; | 是 | 点击通知栏时执行的回调函数，参数为文章id，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;void&gt; | 是 | 拉起面板时执行的回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;void&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;void&gt; | 是 | 收回面板时执行的回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;void&gt; | 是 | 触发stop事件时执行的回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;void&gt; | 是 | 触发release事件时执行的回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;[ReadState](#readstate)&gt; | 是 | 正在播放的文章状态变更时，执行的回调函数，err为undefined，否则为错误对象。 |
...
try {
  TextReader.on('stateChange', (state: TextReader.ReadState) => {
    console.info(
      `Succeeded in setting stateChangeListener: ${JSON.stringify(state)}`,
    );
  });
} catch (e) {
  console.error(
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'stateChange'，当前正在播放的文章状态变更时，触发该事件。 |
| callback | Callback&lt;[ReadState](#readstate)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
try {
  ...
  console.error(
    `TextReader failed to unset on(type: 'requestMore')eventListener. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
| callback | Callback&lt;void&gt; | 是 | 拉到播放列表底端或播放到文章最后一篇，触发该回调执行，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;string&gt; | 是 | 拉到播放列表底端或播放到文章最后一篇，触发该回调执行。返回的自定义参数在[StartParams](#startparams)的callbackParam属性配置，err为undefined，否则为错误对象。 |
...
try {
  TextReader.on('requestMore', (callback) => {
    console.info(
      `Succeeded in setting requestMoreListener. Callback is ${callback}`,
    );
  });
} catch (e) {
  console.error(
...
在on(type: 'requestMore')函数调用之后使用，用于注销拉到���放列表底端或播放到文章最后一篇带用户自定义参数的回调函数。注销监听后，可以重新注册。使用callback异步回调。
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'requestMore'，请求更多文章时，触发该事件监听。 |
| callback | Callback&lt;string&gt; | 是 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;[NotificationEvent](#notificationevent)&gt; | 是 | 播控中心状态发生变化时，触发该回调执行，err为undefined，否则为错误对象。 |
...
try {
  TextReader.on('eventNotification', (ne: TextReader.NotificationEvent) => {
    console.info(
      `Succeeded in setting eventNotificationListener: ${JSON.stringify(ne)}`,
    );
  });
} catch (e) {
  console.error(
...
| callback | Callback&lt;[NotificationEvent](#notificationevent)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;[PanelEvent](#panelevent)&gt; | 是 | 播放面板状态发生变化时，触发该回调执行，err为undefined，否则为错误对象。 |
...
try {
  TextReader.on('eventPanel', (pe: TextReader.PanelEvent) => {
    console.info(
      `Succeeded in setting eventPanelListener: ${JSON.stringify(pe)}`,
    );
  });
} catch (e) {
  console.error(
...
| callback | Callback&lt;[PanelEvent](#panelevent)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;Array&lt;[ListEventState](#listeventstate)&gt;&gt; | 是 | 播报列表发生相关事件时，触发该回调执行，err为undefined，否则为错误对象。 |
...
try {
  TextReader.on('eventReadList', (list: Array<TextReader.ListEventState>) => {
    list.forEach((id) => {
      console.info(
        `Succeeded in setting eventReadListListener, article id: ${id}`,
      );
    });
  });
} catch (e) {
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'eventReadList'。 |
| callback | Callback&lt;Array&lt;[ListEventState](#listeventstate)&gt;&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
| callback | Callback&lt;[ReadProgress](#readprogress)&gt; | 是 | 已播放时长发生相关事件时，触发该回调执行，err为undefined，否则为错误对象。 |
...
try {
  TextReader.on('readProgress', (readProgress: TextReader.ReadProgress) => {
    console.info(
      `Succeeded in setting readProgressListener: ${JSON.stringify(readProgress)}`,
    );
  });
} catch (e) {
  console.error(
    `TextReader failed to set readProgressListener. Code: ${e.code}, message: ${e.message}`,
  );
}
```
...
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'readProgress'。 |
| callback | Callback&lt;[ReadProgress](#readprogress)&gt; | 否 | 需要取消监听的回调函数，需与订阅时传入的回调函数是同一个，若不填，则取消当前应用监听该事件的所有回调函数，err为undefined，否则为错误对象。 |
...
try {
  ...
  console.error(
    `TextReader failed to unset readProgressListener. Code: ${e.code}, message: ${e.message}`,
  );
}
```
