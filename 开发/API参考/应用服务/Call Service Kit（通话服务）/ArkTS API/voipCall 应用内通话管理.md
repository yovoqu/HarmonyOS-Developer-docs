...
| AUDIO_EVENT_MIC_DISABLE | 5 | 静音且不允许操作麦克风。 起始版本: 6.1.1(24) |
...
voipCall.on('voipCallUiEvent', (data: voipCall.VoipCallUiEventInfo) => {
  hilog.info(
    0x0000,
    'testTag',
    `Succeeded in reading callback. CallId: ${data.callId}, voipCallUiEvent: ${data.voipCallUiEvent}`,
  );
});
```
  ...
  onCreate(): void {
    ...
    pushService.receiveMessage('VoIP', this, async (data) => {
      ...
      const fileData: Uint8Array =
        await resourceMgr.getRawFileContent('example.png');
        ...
        pixelMap.getImageInfo((err, imageInfo) => {
          if (imageInfo) {
            hilog.info(
              0x0000,
              'testTag',
              `DemoPushMessageAbility imageInfo: ${imageInfo.size.width} * ${imageInfo.size.height}.`,
            );
          }
        });
      }
...
export default class VoipExtAbility extends UIAbility {
  onCreate(): void {
    ...
    pushService.receiveMessage('VoIP', this, async (data) => {
      ...
      let callAudioEvent: voipCall.CallAudioEvent =
...
voipCall
  .reportIncomingCallError('callId123', voipCall.VoipCallFailureCause.OTHER)
  .then(() => {
    hilog.info(
      0x0000,
      'testTag',
