...
try {
  ...
  hilog.info(
    0,
    'TAG',
    'Succeeded in getting SelfSceneCode res = ' + selfSceneCode,
  );
} catch (err) {
  hilog.error(
    0,
    'TAG',
    `get self sceneCode failed.code is ${err.code}, message is ${err.message}`,
  );
}
```
...
try {
  ...
  hilog.info(
    0,
    'TAG',
    'Succeeded in getting AdsVerificationVersion res = ' + version,
  );
} catch (err) {
  hilog.error(
    0,
    'TAG',
    `get ads verification version failed.code is ${err.code}, message is ${err.message}`,
  );
}
```
