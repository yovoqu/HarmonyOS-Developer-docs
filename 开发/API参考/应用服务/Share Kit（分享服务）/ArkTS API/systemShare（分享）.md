...
| appLaunchTrustInfo | Array&lt;string&gt; | 否 | 是 | 通过配置[应用唯一标识符](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager-bundleinfo#signatureinfo)（appIdentifier字段）列表，指定可分享的目标应用名单。仅取前50个配置项，超出部分不生效。需申请权限ohos.permission.SET_SYSTEMSHARE_APPLAUNCHTRUSTLIST，该能力受限开放，仅支持企业应用限制内部数据分享到企业集团信任的应用。 起始版本： 6.0.1(21)。 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| ERROR | -1 | 发生错误。如果同时传递message���数，将弹出toast提示。 |
...
export default class TestShareAbility extends ShareExtensionAbility {
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    systemShare
      ...
      .catch((error: BusinessError) => {
        console.error(
          `Failed to getContactInfo. Code: ${error.code}, message: ${error.message}`,
        );
        // 联系人不存在或数据获取异常
        session.terminateSelf();
      });
...
export default class TestShareAbility extends ShareExtensionAbility {
  onSessionCreate(want: Want, session: UIExtensionContentSession) {
    ...
    systemShare
      ...
      .catch((error: BusinessError) => {
        console.error(
          `Failed to getWant. Code: ${error.code}, message: ${error.message}`,
        );
      });
  }
}
