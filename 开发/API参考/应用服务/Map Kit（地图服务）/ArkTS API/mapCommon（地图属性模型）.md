...
| annotations | [Text](#text)[] | 否 | 是 | 标记的注释，最小长度为1，最大长度为3。          起始版本： 5.0.3(15)          元服务API： 从��本5.0.3(15)开始，该接口支持在元服务中使用。 |
...
export class ClusterOverlayParamsMore
  implements mapCommon.ClusterOverlayParams
{
  ...
  async getCustomIcon(
    ...
    offContext.fillText(
      JSON.stringify(clusterItems[0].position.longitude).substring(0, 3),
      31,
      31,
    );
    offContext.restore();
...
// 新增轨迹点动画
let traceOverlay = await this.mapController.addTraceOverlay(
  traceOptions,
  markers,
);
```
...
**元服务API：** 从版本5.0.0(12)开始，��接口支持在元服务中使用。
...
| SATELLITE | 3 | 卫星图，显示卫星照片的地图，只支持中国。          起始版本： 6.0.0(20)          元服务API： 从���本6.0.0(20)开始，该接口支持在元服务中使用。 |
