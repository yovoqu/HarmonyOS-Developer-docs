...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| siteId | string | 否 | 是 | 地点详情页的地点ID，异常值返回401错误码。 siteId可通过MapEventManager中on(type: 'poiClick', callback: Callback<mapCommon.Poi>)方法获取。 元服务API： 从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
...
> [!NOTE]
> 以下两组参数，至少传入其中一组，如果都传入，以siteId为主。 siteId location、name
...
| transitionDuration | number | 否 | 是 | 转场动效时间，默认值：-1，单位：ms，取值范围：大于0，异常值将按照默认值-1处理，保持interpolatingSpring(0.5, 1, 328, 36)（初始速度为0.5，质量为1，刚度为1，阻尼为1）。当subWindowEnabled为true时该配置不生效。 起始版本： 6.0.2(22) 元服务API： 从版本6.0.2(22)开始，该接口支持在元服务中使用。 |
