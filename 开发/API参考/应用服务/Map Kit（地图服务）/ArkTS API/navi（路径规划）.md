...
const result = await navi.getDrivingRoutes(
...
const result = await navi.getWalkingRoutes(
  this.getUIContext().getHostContext(),
  params,
);
console.info('Succeeded in getting walking routes.');
...
const result = await navi.getCyclingRoutes(
  this.getUIContext().getHostContext(),
  params,
);
console.info('Succeeded in getting cycling routes.');
...
const result = await navi.getTransitRoutes(
  this.getUIContext().getHostContext(),
  params,
);
...
const result = await navi.getDrivingMatrix(
  this.getUIContext().getHostContext(),
  params,
);
console.info('Succeeded in getting driving matrix.');
...
const result = await navi.getWalkingMatrix(
  this.getUIContext().getHostContext(),
  params,
);
console.info('Succeeded in getting walking matrix.');
...
const result = await navi.getCyclingMatrix(
  this.getUIContext().getHostContext(),
  params,
);
console.info('Succeeded in getting cycling matrix.');
...
const result = await navi.snapToRoads(
  this.getUIContext().getHostContext(),
  params,
);
console.info('Succeeded in snapping to roads.');
...
| bearing | number | 否 | 是 | 偏转角度。 以正北方向为0度、顺时针方向为正的角度，默认值为0，取值范围：[0, 360)。超出取值范围的值会换算成取值范围内的值，比如361会被换算成1，-1换算为359。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| waypoints | Array&lt;[mapCommon.LatLng](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-common#latlng)&gt; | 否 | 是 | 途经点。最多可以输入5个途经点，异常值不处理。 |
...
| hasStairs | boolean | 否 | 是 | 此路段是否含有阶梯。 - true：含��阶梯 - false：不含有阶梯 |
