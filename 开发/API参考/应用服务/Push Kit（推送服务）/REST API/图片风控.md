...
| downloadUrl | 否 | String | 验证通过的图片地址（本接口成功时才返回）。由华为Push服务器在原始URL的基础上拼接风控校验Key生成，确保图片来源合法且未被篡改，可作为通知右侧大图标（对应Notification的image参数）、服务卡片图片（对应FormImage的url参数）等场景的图片地址。 |
