...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfviewmanage
...
let loadResult: pdfService.ParseResult =
  await pdfController.loadDocument(filePath);
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;[ScrollParam](#scrollparam)&gt; | 是 | 页面滚动回调函数监听，返回ScrollParam类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerScrollListener(
  (scrollParam: pdfViewManager.ScrollParam) => {},
);
```
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;pdfService.[ParseResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfservice#parseresult)&gt; | Promise对象，返回ParseResult类型数据。 |
...
let loadResult: pdfService.ParseResult =
  await pdfController.loadDocument(tempFilePath);
```
...
if (pdfService.ParseResult.PARSE_SUCCESS === loadResult) {
  ...
  const rect1: pdfService.PdfRect = {
    left: 20,
    top: 0,
    right: 120,
    bottom: 300,
...
let loadResult: pdfService.ParseResult =
  await pdfController.loadDocument(filePath);
if (pdfService.ParseResult.PARSE_SUCCESS === loadResult) {
  pdfController.enableAnnotation(
...
let loadResult: pdfService.ParseResult =
  await pdfController.loadDocument(filePath);
if (pdfService.ParseResult.PARSE_SUCCESS === loadResult) {
  let rectArray: Array<pdfService.PdfRect> = [
    { left: 5, top: 5, right: 250, bottom: 250 },
  ];
  let selectedRects: Array<pdfViewManager.SelectedRects> = [
    new pdfViewManager.SelectedRects(0, rectArray, 0),
  ];
  pdfController.addMarkupAnnotation(
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;Array&lt;[SelectedRects](#selectedrects)&gt;&gt; | 是 | 选中文本拖拽窗口变化回调函数监听，返回SelectedRects类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerSelectedRectsChangedListener(
  (pageRects: Array<pdfViewManager.SelectedRects>) => {},
);
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;pdfService.[PageFit](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfservice#pagefit)&gt; | 是 | 页面适配变化回调函数监听，返回PageFit类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerPageFitChangedListener(
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;[TextSelectedParam](#textselectedparam)&gt; | 是 | 页面文本被选中回调函数监听，返回选中文本TextSelectedParam类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerTextSelectedListener(
  (pageText: pdfViewManager.TextSelectedParam) => {},
);
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;[SelectedAnnotation](#selectedannotation)&gt; | 是 | 页面批注被选中回调函数监听，返回选中批注SelectedAnnotation类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerAnnotationSelectedListener(
  (pdfAnnotation: pdfViewManager.SelectedAnnotation) => {},
);
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;[ImageSelectedParam](#imageselectedparam)&gt; | 是 | 页面图片被选中回调函数监听，返回选中图片ImageSelectedParam类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerImageSelectedListener(
  (image: pdfViewManager.ImageSelectedParam) => {},
);
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;[RedirectInfo](#redirectinfo)&gt; | 是 | Click动作回调函数监听，返回RedirectInfo类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerActionClickListener(
  (redirectInfo: pdfViewManager.RedirectInfo) => {},
);
```
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| listener | [Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)&lt;[AnnotationChangedParam](#annotationchangedparam)&gt; | 是 | 批注变化时回调函数监听，返回AnnotationChangedParam类型数据。 |
...
let pdfController = new pdfViewManager.PdfController();
pdfController.registerAnnotationChangedListener(
  (annotationChange: pdfViewManager.AnnotationChangedParam) => {},
);
```
...
let redirectInfo = new pdfViewManager.RedirectInfo(
  'https://www.test.com',
  pdfViewManager.RedirectType.URI,
);
```
...
| rect | Array&lt;pdfService.[PdfRect](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfservice#pdfrect)&gt; | 是 | 是 | 批注矩形区域。 |
| points | Array&lt;pdfService.[PdfPoint](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfservice#pdfpoint)&gt; | 是 | 是 | 批注坐标。 |
...
| rectArray | Array&lt;pdfService.[PdfRect](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfservice#pdfrect)&gt; | 否 | 否 | 数组PdfRect类型。 |
...
| --- | --- | --- | --- |
| pageIndex | number | 是 | 页面索引。 |
| rectArray | Array&lt;pdfService.[PdfRect](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-arkts-pdfservice#pdfrect)&gt; | 是 | 数组PdfRect类型。 |
...
rectArray.push(rect1);
let selectedRects: pdfViewManager.SelectedRects =
  new pdfViewManager.SelectedRects(0, rectArray, 0);
```
