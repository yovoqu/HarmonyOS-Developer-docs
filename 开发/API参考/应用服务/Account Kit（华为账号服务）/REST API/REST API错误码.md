...
| 业务响应主错误码 | 业务响应子错误码 | 描述 | 解决方法 |
| --- | --- | --- | --- |
| 1101 | 20222 | token格式不正确。 | token格式不正确，可能原因： - token参数为伪造的值。 - 请求头的Content-Type为application/x-www-form-urlencoded，但实际代码调用时，未对请求body体进行URLEncode处理，可参考示例代码组装参数。 |
...
| 60180003 | code解析得到的Client ID与入参clientId不一致。 | code参数获取时的clientId与当前接口参数clientId不一致导致，请检查当前入参clientId是否与配置Client ID中的值一致。 |
