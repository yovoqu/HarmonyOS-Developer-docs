...
> [!NOTE]
> 为优化开发者体验，Account Kit推荐使用 一键登录获取华为账号绑定号码和UnionID/OpenID 新接口进行应用服务端开发，可以减少 获取用户级凭证 接口调用，接入更简单，整体时延更低。
...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access_token | 是 | String | 通过获取用户级凭证获取的Access Token。获取凭证时Authorization Code需包含quickLoginMobilePhone scope的授权。 |
