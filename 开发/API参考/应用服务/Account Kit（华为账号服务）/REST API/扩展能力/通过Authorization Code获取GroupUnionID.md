...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/57/v3/m68BO5jCR52HPHOTrIJotg/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260730T071727Z&HW-CC-Expire=86400&HW-CC-Sign=4870EEFEB706A65DA18C7314EB7D005B749B155BD07FDCB67D93DCF64B2917AE)
...
grant_type=authorization_code&code=&client_id=<client_id>&client_secret=<client_secret>&need_group_union_id=<need_group_union_id>
```
...
| 1101 | 20154 | code解析得到的Client ID与入参client_id不一致。 | 检查入参client_id是否与配置Client ID中的值一致。 |
...
| 1101 | 20182 | grant_type值不正确。 | grant_type可选值如下： - “authorization_code”：该场景用于获取用户级凭证。 - “device_code”：该场景用于扫码授权登录-获取用户级凭证。 - “refresh_token”： 该场景用于刷新用户级凭证。 - “client_credentials”：该场景用于获取应用级凭证。 当前场景请固定传”authorization_code“。 |
| 1102 | 20001 | client_id为空。 | 请按照接口参数的要求，传入正确的client_id参数。 |
| 1102 | 20151 | code为空。 | 请按照接口参数的要求，传入正确的code参数。 |
| 1102 | 20181 | grant_type为空。 | grant_type可选值如下： - “authorization_code”：该场景用于获取用户级凭证。 - “device_code”：该场景用于扫码授权登录-获取用户级凭证。 - “refresh_token”： 该场景用于刷新用户级凭证。 - “client_credentials”：该场景用于获取应用级凭证。 当前场景请固定传”authorization_code“。 |
...
| 1203 | 100301 | 参数处理异常。 | 参数处理异常，建议按照接口入参要求，检查参数是否符合规范。若仍无法解决，请通过在线提单提交问题。 |
