...
| 参数 | 是否必选 | 参数类型 | 描述 |
| --- | --- | --- | --- |
| access_token | 是 | String | 通过获取应用级凭证获取的Access Token。 说明： 通过OpenID获取UnionID时，Access Token必须由OpenID所属应用的Client ID获取。 |
...
| NSP_STATUS | 描述 | 解决方法 |
| --- | --- | --- |
| 6 | access_token已过期。access_token有效期为3600秒，超过有效期后将无法继续使用。 | 请通过获取应用级凭证重新获取新的access_token。 |
