...
| uncertaintyOfTimeSinceBoot12+ | number | 否 | 是 | 表示位置时间戳的不确定度。          元服务API： 从API version 12开始，该接��支持在元服务中使用。 |
...
let cachedLocationsCb = (
  locations: Array<geoLocationManager.Location>,
): void => {
  console.info(
    'cachedGnssLocationsChange: locations: ' + JSON.stringify(locations),
  );
};
...
let gnssStatusCb = (
  ...
  let carrierToNoiseDensitys: Array<number> =
  ...
  let carrierFrequencies: Array<number> =
    satelliteStatusInfo.carrierFrequencies;
  let satelliteConstellations:
    | Array<geoLocationManager.SatelliteConstellationCategory>
    | undefined = satelliteStatusInfo.satelliteConstellation;
  let satelliteAdditionalInfos: Array<number> | undefined =
  ...
  for (let i = 0; i < totalNumber; i++) {
    ...
    if (satelliteConstellations != undefined) {
      ...
      let satelliteConstellation: geoLocationManager.SatelliteConstellationCategory =
        satelliteConstellations[i];
    }
...
try {
  geoLocationManager
    ...
    .catch((error: BusinessError) => {
      console.error(
        'promise, getCurrentLocation: error=' + JSON.stringify(error),
      );
    });
} catch (err) {
  console.error('errCode:' + err.code + ', message:' + err.message);
...
try {
  geoLocationManager
    ...
    .catch((error: BusinessError) => {
      console.error(
        'promise, getAddressesFromLocation: error=' + JSON.stringify(error),
      );
    });
} catch (err) {
  console.error('errCode:' + err.code + ', message:' + err.message);
...
try {
  geoLocationManager.getAddressesFromLocationName(
    ...
    (err, data) => {
      if (err) {
        console.error(
          'getAddressesFromLocationName: err=' + JSON.stringify(err),
        );
      }
      if (data) {
        console.info(
          'getAddressesFromLocationName: data=' + JSON.stringify(data),
        );
      }
    },
  );
...
try {
  geoLocationManager
    ...
    .catch((error: BusinessError) => {
      console.error(
        'promise, getAddressesFromLocationName: error=' + JSON.stringify(error),
      );
    });
} catch (err) {
  console.error('errCode:' + err.code + ', message:' + err.message);
...
try {
  geoLocationManager
    ...
    .then((result) => {
      console.info(
        'promise, getCachedGnssLocationsSize: ' + JSON.stringify(result),
      );
    })
    .catch((error: BusinessError) => {
      console.error(
        'promise, getCachedGnssLocationsSize: error=' + JSON.stringify(error),
      );
    });
} catch (err) {
  console.error('errCode:' + err.code + ', message:' + err.message);
...
try {
  geoLocationManager
    ...
    .catch((error: BusinessError) => {
      console.error(
        'promise, flushCachedGnssLocations: error=' + JSON.stringify(error),
      );
    });
} catch (err) {
  console.error('errCode:' + err.code + ', message:' + err.message);
...
let requestInfo: geoLocationManager.LocationCommand = {
  scenario: 0x301,
  command: 'command_1',
};
try {
...
try {
  // 添加围栏
  geoLocationManager
    .addGnssGeofence(gnssGeofenceRequest)
    .then((id) => {
      // 围栏添加成功后返回围栏ID
    ...
    .catch((err: BusinessError) => {
      console.error(
        'addGnssGeofence failed, promise errCode:' +
...
try {
  ...
  console.info(
    'getGeofenceSupportedCoordTypes return:' +
      JSON.stringify(supportedCoordTypes),
  );
} catch (error) {
  console.error(
    'getGeofenceSupportedCoordTypes: error=' + JSON.stringify(error),
  );
}
```
...
try {
  ...
  console.error(
    'getCurrentWifiBssidForLocating: errCode' +
      error.code +
      ', errMessage' +
      error.message,
  );
...
try {
  ...
  console.error(
    'getPoiInfo errCode:' + error.code + ', errMessage:' + error.message,
  );
}
```
...
try {
  ...
  let distance = geoLocationManager.getDistanceBetweenLocations(
    location1,
    location2,
  );
  ...
  console.error(
    'getDistanceBetweenLocations: errCode' +
      error.code +
      ', errMessage' +
      error.message,
  );
...
try {
  ...
  // 02 byte    minor
  // 01 byte    tx power
  let manufactureDataBuffer: Uint8Array = new Uint8Array([
    0x02, 0x15, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99,
    0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x11, 0x22, 0x33, 0x44, 0x55,
  ]);
  let manufactureDataMaskBuffer: Uint8Array = new Uint8Array([
  ...
  let beacon: geoLocationManager.BeaconFence = {
    identifier: '11',
    beaconFenceInfoType:
      geoLocationManager.BeaconFenceInfoType.BEACON_MANUFACTURE_DATA,
  ...
  console.error(
    'addBeaconFence: errCode' + error.code + ', errMessage' + error.message,
  );
}
```
...
try {
  ...
  geoLocationManager
    ...
    .catch((error: BusinessError) => {
      console.error(
        'promise, removeBeaconFence: errCode' +
          error.code +
          ', errMessage' +
          error.message,
      );
  ...
  console.error(
    'removeBeaconFence: errCode' + error.code + ', errMessage' + error.message,
  );
}
```
...
try {
  let wlanBssidArray: Array<string> = [
    '02:1b:32:23:ea:91',
    '02:1b:32:23:ea:93',
  ];
  ...
  geoLocationManager
    .isWlanBssidMatched(wlanBssidArray, rssiThreshold, needStartScan)
    .then((res) => {
      console.info('Wlan Bssid Matched Result:' + res);
    });
} catch (error) {
  console.error(
    'isWlanBssidMatched: errCode' + error.code + ', errMessage' + error.message,
  );
}
```
...
try {
  geoLocationManager
    ...
    .catch((error: BusinessError) => {
      console.error(
        'promise, getActiveGeoFences: error=' + JSON.stringify(error),
      );
    });
} catch (error) {
  console.error(
    'getActiveGeoFences: errCode' + error.code + ', errMessage' + error.message,
  );
}
```
