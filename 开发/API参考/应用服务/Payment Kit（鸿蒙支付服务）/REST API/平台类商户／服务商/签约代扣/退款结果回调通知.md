...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-references/payment-partner-withhold-refunds-notify
...
> [!WARNING]
> 为保证回调请求的可靠性，系统具备重试机制，可能会出现重发的通知。 为保证信息合法性，开发者需要对返回的支付信息进行 SM2验签 ，验签注意事项： 需直接使用通知的完整内容进行验签。 验签前需要对返回数据进行排序拼接，sign字段是签名值，排序拼接后的待验签内容需要排除sign字段。 验签公钥使用 华为支付证书 。 退款状态需根据refundOrderStatus字段判断。
