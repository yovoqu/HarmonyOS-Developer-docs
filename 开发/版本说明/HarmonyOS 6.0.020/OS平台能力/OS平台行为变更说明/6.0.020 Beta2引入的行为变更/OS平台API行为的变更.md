...
对于公共事件COMMON_EVENT_PACKAGE_ADDED、COMMON_EVENT_PACKAGE_REMOVED、COMMON_EVENT_PACKAGE_CHANGED、COMMON_EVENT_PACKAGE_RESTARTED、COMMON_EVENT_PACKAGE_DATA_CLEARED、COMMON_EVENT_PACKAGE_CACHE_CLEARED、COMMON_EVENT_QUICK_FIX_APPLY_RESULT的订阅方增加了管控。
...
| 事件名称 | 描述 |
| --- | --- |
| [COMMON_EVENT_PACKAGE_ADDED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_package_added) | 应用安装完成的事件。 |
| [COMMON_EVENT_PACKAGE_REMOVED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_package_removed) | 应用卸载完成的事件。 |
| [COMMON_EVENT_PACKAGE_CHANGED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_package_changed) | 应用更新完成的事件。 |
| [COMMON_EVENT_PACKAGE_RESTARTED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_package_restarted) | 应用重新启动的事件。 |
| [COMMON_EVENT_PACKAGE_DATA_CLEARED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_package_data_cleared) | 应用数据清理完成的事件。 |
| [COMMON_EVENT_PACKAGE_CACHE_CLEARED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_package_cache_cleared) | 应用缓存数据清理完成的事件。 |
| [COMMON_EVENT_QUICK_FIX_APPLY_RESULT](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_quick_fix_apply_result) | 应用使能快速修复包完成的事件。 |


适配指导

如果使用上述公共事件判断应用是否安装，请改用canOpenLink接口来查询应用是否存在。
...
bindSheet半模态弹窗侧边样式默认支持避让软键盘，提升易用性。
...
默认行为变更，当半模态样式指定为SheetType的SIDE侧边样式时，若开发者期望自定义避让软键盘，则需要设置bindSheet的属性keyboardAvoidMode = SheetKeyboardAvoidMode.NONE。

若开发者期望采用半模态控件自带的避让软键盘能力，则可以设置bindSheet的属性keyboardAvoidMode = SheetKeyboardAvoidMode的其他枚举值，或者不设置属性keyboardAvoidMode，采用默认值。
...
![](assets/OS平台API行为的变更/file-20260515134822443-0.png)
...
变更前：当开发者需要自定义保存控件的图标和文本时，或者保存控件不满足约束与限制，点击保存控件时会弹出系统提示弹框。开发者可以使用tipPosition接口设置保存控件系统提示弹框展示在屏幕上的位置。
...
变更前：泰国的默认历法为佛历，沙特阿拉伯的默认历法为伊斯兰历，阿富汗和伊朗的默认历法为波斯历。例如，创建intl.DateTimeFormat时，传入地区为泰国，则调用format接口时，会使用佛历显示日期。

变更后：泰国、沙特阿拉伯、阿富汗和伊朗的默认历法均为公历。例如，创建intl.DateTimeFormat时，传入地区为泰国，则调用format接口时，会使用公历显示日期。
...
| @ohos.intl.d.ts | [intl.DateTimeFormat.prototype.formatRange](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-intl#formatrangedeprecated) |
| @ohos.i18n.d.ts | [i18n.Calendar.prototype.getDisplayName](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-i18n#getdisplayname8) |
| @ohos.i18n.d.ts | [i18n.Calendar.prototype.get](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-i18n#get8) |
