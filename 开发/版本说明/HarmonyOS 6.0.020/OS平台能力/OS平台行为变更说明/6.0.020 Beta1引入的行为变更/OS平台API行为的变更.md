...
AbilityDelegator提供的startAbility()接口参见[startAbility API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-abilitydelegator#startability9)。
...
在文件分享场景下（[Want](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-want#want)的flags字段中配置了[wantConstant.Flags.FLAG_AUTH_READ_URI_PERMISSION](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#flags)或[wantConstant.Flags.FLAG_AUTH_WRITE_URI_PERMISSION](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#flags)），应用可以通过Want的uri字段传递单个URI、或者通过[wantConstant.Params.PARAMS_STREAM](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-wantconstant#params)的Key值传递多个URI。为了确保传递给目标应用的参数合法，系统将主动擦除不满足条件的URI。
...
排查Want的flags字段中是否设置了文件URI读写Flag（wantConstant.Flags.FLAG_AUTH_READ_URI_PERMISSION或wantConstant.Flags.FLAG_AUTH_WRITE_URI_PERMISSION），并且uri字段或wantConstant.Params.PARAMS_STREAM字段中写入了非[文件URI](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-fileuri#fileuri10)。
...
> [!NOTE]
> 在ets文件中 不支持使用delete ，但可在三方库中或者js文件中使用。
