...
- 宿主的targetSdkVersion >= 20。 建议宿主应用在6.0及以上版本使用嵌入式运行元服务，5.x版本建议使用跳出式运行元服务，提供方元服务无需在嵌入式场景主动避让适配。
  ...
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam): void {
    ...
    if (want.parameters) {
      hilog.info(
        ...
        'testTag',
        '%{public}s',
        'followed host window mode' +
          want.parameters[
            'ohos.extra.atomicservice.param.key.isFollowHostWindowMode'
          ],
      );
    }
  }
