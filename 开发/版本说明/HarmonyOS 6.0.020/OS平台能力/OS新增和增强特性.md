...
- PC/2in1设备或自由多窗模式的平板设备的窗口能力增强：新增支持设置一级子窗是否支持与主窗保持相对位置不变。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setrelativepositiontoparentwindowenabled20)）
...
- 基于Web的PDF浏览能力增强：新增支持PDF文档预览回调功能。（[指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/web-pdf-preview#pdf文档预览回调功能)）
...
音视频会话新增支持发送私有数据到远端设备，支持在媒体应用（API参考）、投播控制器（API参考）、播控中心（API参考）的会话中发送私有数据到远端设备。


### Call Service Kit


voip接口新增支持wearable设备。(指南、API参考)
...
新增支持获取AI模型Dump维测数据。（API参考）


### Core File Kit


新增端云文件版本管理类。支持对端云文件的历史版本进行管理，提供获取文件历史版本信息列表的能力。（API参考）
...
- 新增支持点击状态栏图标展开二级菜单场景下，可加载动效。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/statusbar-extension-manager#statusbarmanagerquickoperation)）
...
【新增Kit】Mechanic Kit（机械设备管理服务）主要面向云台、机械臂等智能机械体配件设备提供交互控制的能力。（指南、API参考）
...
新增支持对媒体资产发生变化（API参考）和相册发生变化（API参考）的监听。


### NDK开发


新增格物服务，提供QoS感知的推理加速和资源管理优化能力。（指南）


### Network Kit


新增C API，支持网络探测（API参考）和网络跟踪路由（API参考）。


### Network Boost Kit


新增连接迁移（多网并发）功能。（指南、API参考）


### Payment Kit


新增thirdPaymentService服务，支持直接通过三方支付提供的SDK拉起三方支付收银台，实现支付的能力。（指南、API参考）


### Pen Kit


新增C API，支持新增报点预测能力。（指南、API参考）
...
- 新增Kiosk模式管理，适用于企业应用。企业应用可以使用该模式将设备锁定至单一应用。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-kioskmanager)）另见[MDM Kit相关功能](#section671133914213)。
...
新增应用评论服务，用户无需进入应用市场应用详情页，可直接在应用内进行评论。（指南、API参考）
...
- 文本组件能力增强：Text组件新增支持设置文本颜色按线性或径向渐变。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-text#shaderstyle20)）
...
- 拖拽事件能力增强：新增支持获取拖起方的包名。（[API参考-ArkTS](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#getdragsource20)、[API参考-C API](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-drag-and-drop-h#oh_arkui_dragevent_getdragsource)）
...
- 滚动组件通用接口新增支持设置划动离手时触发，并限定使用鼠标滚轮划动时不会触发。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-scrollable-common#onwillstopdragging20)）
 ...
 BuilderNode、ComponentContent新增支持查询当前对象是否设置为继承父组件中自定义组件的冻结策略。（API参考-BuilderNode、API参考-ComponentContent）XComponent的C API新增支持获取功能键按压状态信息的能力。（API参考）手势拦截新增支持由应用自定义需要阻止的手势。（API参考-ArkTS、API参考-C API）图形变换新增支持设置组件的三维变换矩阵。（API参考）弹出式菜单新增支持多个生命周期回调，如onWillAppear、onDidAppear、onWillDisappear等。（API参考）全屏模态新增参数enableSafeArea，支持适配安全区域。（API参考）自定义绘制新增绘制前景的能力。（API参考）新增C API支持在UIContext作用域内运行自定义函数的能力，基于该能力可确保在调用跨实例组件设置属性时的上下文正确性，避免跨实例接口调用失败。（指南、API参考）新增C API支持获取目标节点的uniqueId的能力（API参考），及通过uniqueId获取节点的能力（API参考）。窗口管理能力增强：
...
针对PC/2in1设备，新增支持设置是否跟随系统的能效模式，以合理保证进程的运行。（API参考）


### Core File Kit


新增备份恢复框架安全退出的回调API，可在应用备份或恢复完成时自定义执行一些额外的处理动作。（指南、API参考）


### Data Augmentation Kit


新增获取知识加工状态功能。（指南、API参考）


### Device Security Kit


新增支持审计通知类事件过滤功能，应用可通过获取设备上的安全审计数据，按需进行过滤，以支撑审计相关业务。（指南、API参考）
...
针对PC/2in1设备，新增C API支持由应用申请注入权限，包括注入按键事件、触屏事件、鼠标事件的权限。（API参考）
...
新增支持获取时区跳变规则，包括时区跳变点的时间戳和偏移量。（指南、API参考）
...
新增隔空传送分享能力，“一抓一放”实现跨端传输。（指南）


### Test Kit


UITest新增多个UI测试能力的接口，如：获取指定屏幕内的控件对象（API参考），获取控件对象所属的屏幕ID（API参考）。


### MDM Kit


新增支持针对企业设备清除应用产生的所有数据。（API参考）

新增支持将应用锁定在Kiosk模式（即通过系统层面限定设备只能运行单个应用或者一组应用）。（API参考）


### Media Kit


C API新增支持低功耗音视频播放的能力。（API参考-低功耗音频播放、API参考-低功耗视频播放）

新增支持监听SoundPool的错误事件。（API参考）


### NDK开发


新增支持使用扩展的Node-API接口在当前线程中创建、切换和销毁上下文环境。（指南）
...
配置文件app.json5新增字段appPreloadPhase，允许配置应用预加载到不同阶段。（指南）
...
新增接口having，支持筛选符合条件的分组数据。（API参考）


### ArkGraphics 2D


将原有的一批C API能力封装提供为ArkTS API，详见API参考中标记为“20+”的API。


### ArkGraphics 3D


新增支持从屏幕指定位置发射射线，检测并返回所有命中的3D物体信息的能力。（API参考）


### ArkUI


- 文本与输入组件能力增强。包括：新增文本装饰线样式，支持对文本设置删除线和下划线。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-styled-string#constructor20)）
...
- 新增C API，支持触发Span组件的长按事件（NODE_TEXT_SPAN_ON_LONG_PRESS）。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeeventtype)）
 ...
 Refresh支持设置最大下拉距离。（API参考-ArkTS API、API参考-C API）Tabs在滑动页面切换时，支持设置翻页动画曲线。（API参考）滚动类组件支持设置滚动条的起始和末尾边距。（API参考-ArkTS API、API参考-C API）Swiper组件支持在显示区域上方或前方插入或删除数据时，设置是否保持可见内容的位置不变。（API参考-ArkTS API、API参考-C API）拖拽事件支持获取事件发生时所在的屏幕ID。（API参考-ArkTS API、API参考-C API）图形变化过程中，支持设置单个方向的旋转角。（API参考）自定弹窗支持获取初始化等弹窗状态。（API参考）优化栅格布局断点，若未配置更小断点的栅格列数，系统取已配置的更大断点的栅格列数补全未配置的栅格列数。（API参考、指南）新增ToolBarItem组件，支持为窗口标题栏添加工具栏项。（API参考）新增无障碍事件的相关能力，在系统开启无障碍模式后，提供拦截无障碍事件的能力。（API参考）支持自定义开启/禁止角标显示。（API参考-ArkTS API、API参考-C API）支持查询当前ComponentContent对象是否已解除与后端实体节点的引用关系。（API参考）新增C API，支持通过百分比或具体数值形式设置组件平移（NODE_TRANSLATE_WITH_PERCENT）。（API参考）绘制类组件支持通过attributeModifier动态设置属性方法。（API参考-Canvas、API参考-Shape）CanvasRenderingContext2D支持绘制圆角矩形。（API参考）
...
新增支持查询指定的source type是否支持回声消除。（API参考）


### Basic Services Kit


新增设备类型枚举值，可用于校验deviceType的返回值。（API参考）


### Data Augmentation Kit


【新增Kit】Data Augmentation Kit（数据增强套件）提供知识库、知识检索、知识问答（RAG）能力，打造个性化智慧数据平台，实现个性化智慧体验。（指南、API参考）


### Device Certificate Kit


新增支持根据编码类型获取X509证书的颁发者名称（API参考），以及证书吊销列表的颁发者名称（API参考）。
...
【新增Kit】Enterprise Space Kit（企业数字空间服务）为企业安全管控类MDM应用提供高效、智能的数据传输能力，支持空间数据的管理与应用服务。通过严格的空间数据传输审核流程，确保数据传输的安全与合规性，实现空间数据的独立管理与隔离。（指南、API参考）
...
新增支持通行密钥服务。（指南、API参考-ArkTS API、API参考-C API）
...
【新增Kit】在应用安全隐私保护前提下，Screen Time Guard Kit（屏幕时间守护服务）为开发者提供屏幕使用时间管控、应用使用限制等开放能力，满足不同用户对时间管理多样化诉求，更好的服务终端用户。（指南、API参考）
...
新增支持多种场景下输入文本的测试能力。（API参考-1、API参考-2）
