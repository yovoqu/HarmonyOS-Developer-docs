...
BundleInfo新增buildVersion。（API参考）
...
新增支持带有卡在位状态周期性检测的NFC Tag读卡事件订阅能力。（API参考）
...
新增支持阅读分享功能，即选中文本进行分享时，分享面板同时显示图片分享和文本分享，用户可选择图片或文本进行分享。（API参考）


### Spatial Recon Kit


3D空间重建场景下，新增支持空间重建会话管理、3DGS（3D Gaussian Splatting）场景建模、任务控制、进度查询以及三维模型文件导出功能。（API参考）


## 6.1.0(23) Beta1新增关键特性


开发套件Beta (基于API 23) 新增如下关键特性，开发者可通过新增的API调用相应特性的能力。完整的API新增请参见API变更清单。
...
util模块新增接口ArkTSVM.setMultithreadingDetectionEnabled，支持开启或关闭多线程检测能力。（API参考）
...
AVCodec新增支持AV1/VP9/VP8/RV30/RV40/WVC1/DVVIDEO/RAWVIDEO/MPEG1格式的视频软解码能力。（指南）


### Basic Service Kit


上传下载的预下载模块新增支持获取对端IP地址。（API参考）
...
BLE广播报文数据AdvertiseData新增advertiseName，支持应用自定义BLE广播名称。（API参考）

Wi-Fi的P2P连接方式新增支持通过指定群组频率创建群组的能力。（API参考）


### Crypto Architecture Kit


新增支持通过私钥对象获取公钥对象的能力。（API参考）


### Enterprise Space Kit


空间管理场景下，新增支持企业账号认证、获取企业应用访问令牌、设置工作空间状态栏图标、设置工作空间本地名称功能。（指南）


### Form Kit


ArkTS卡片开发支持V2装饰器语法。（指南）


### Game Service Kit


游戏近场快传新增支持安装包传输。（指南、API参考）
...
新增查询当前设备是否支持NearLink Kit的能力。（指南、API参考）
...
- 新增配置参数pathPreference支持HTTP请求指定特定激活的网络。
 ...
 新增接口支持VPN类应用获取数据包对应的UID。（API参考）DNS解析能力支持转码，可将Unicode编码形式的主机名转换为ASCII编码形式（API参考），也可将ASCII编码形式的主机名转换为Unicode编码形式（API参考）。（指南）
...
- 新增属性overlayIcon，支持设置重叠图标。
 ...
 NotificationFlags新增属性，支持应用调整通知是否在横幅和锁屏界面显示。（API参考）


### Payment Kit


新增营销组件，开发者可在应用内实现领券、选券能力。（指南、API参考）
...
新增禁用画布缩放能力、设置滚动位置ScrollTo及监听长画布滚动位置能力、自定义长画布最大高度能力。（指南、API参考）


### Performance Analysis Kit


HiappEvent新增ArkWeb抛滑触控操作丢帧检测能力。（指南）

HiDebug新增提供添加维测信息的接口，开发者可根据业务需要将维测信息添加到崩溃日志中，若程序发生崩溃，可在崩溃日志中找到该维测信息。（指南、API参考）

AppFreeze日志中调用栈的堆栈信息增加线程状态信息。（指南）


### Preview Kit


新增C API，支持通用文件缓存加速功能，即通过缓存解码后的图片、视频等数据至磁盘，实现后续文件打开或浏览时的直接读取，减少重复解码耗时，提升用户操作流畅度。（指南）
...
默认界面扫码能力、图像识码能力和自定义界面扫码能力支持带后置相机的Wearable。（指南）


### Telephony Kit


新增VCard模块，提供电子名片的文件格式标准，支持将VCard文件导入联系人数据库和将联系人数据导出为VCard文件等。（API参考）
...
HUKS新增支持群组密钥功能，针对同一开发者开发的多个HAP应用提供跨应用密钥共享能力。（指南）
...
打包工具打包hap/hsp时，支持传入已存在的hap/hsp（--exist-src-path）和指定要保留的目录（--lib-path-retain），要保留的目录会从已存在的hap/hsp中直接拷贝到目标包，不会从源文件中重新压缩打包。当前保留目录只支持--lib-path。（指南）
