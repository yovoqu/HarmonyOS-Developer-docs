...
使用网格：新增网格组件的NDK开发指南，包括创建网格组件、创建后如何设置子组件所占行列数、如何处理滚动事件。

丰富UI开发常见问题：新增20+常见问题及对应的解决措施，帮助开发者快速完成问题定位与解决。例如：FrameNode节点运行时出现jscrash、文本超长时如何展示等。


### AVSession Kit新增桌面歌词接入指南


应用接入桌面歌词：支持音乐播放器或有歌词展示需求的音频类应用接入桌面歌词功能，可以实现将歌词以悬浮窗形式展示在系统桌面，让用户无需进入应用即可查看歌词。
...
获取目标应用的URL信息：在使用UIAbilityContext.openLink接口拉起目标应用时，需要传入目标应用的URL信息。主要介绍如何通过aa工具与bm工具，获取目标应用的URL信息。
...
状态管理V1-V2迁移指导：梳理迁移场景，将迁移场景分为组件内状态变量迁移、数据对象状态变量迁移、应用内状态变量迁移、组件复用迁移，新增内置对象迁移指导、AnimateTo使用迁移指导。


### Image Kit增加位图操作


使用PixelMap完成位图操作文档中，增加将两张宽度相同的位图纵向拼接成一张长图的开发示例。


### AVSession Kit提供音频模板功能


使用音频模板：AVSession Kit支持媒体应用通过音频模板接入播控中心，实现音视频在播控中心进行统一的界面显示和播控管理。
...
基础库常见问题：介绍基础库开发过程中的常见问题，帮助开发者快速完成问题定位与解决。例如，如何解决解析大文件xml发生的内存溢出问题。


### Camera Kit新增高性能拍照指南


优化原有拍照(ArkTS)及拍照(C/C++)文档内容，新增高性能拍照指南示例，介绍在单段式拍照时，通过设置明确的画质优先策略满足不同拍照效果要求。
...
添加图像效果（C/C++）：新增离线图像处理指导文档，介绍如何基于滤镜（Filter）设置不同的图像效果，比如设置图像模糊程度、调节图像亮度和灰度、设置图像反色等。
...
状态管理原理介绍：新增状态管理基本原理介绍，帮助开发者更好地理解其工作流程。

状态管理常见问题：新增状态管理开发过程中的常见问题，帮助开发者快速完成问题定位与解决。
...
“音频录制流管理”文档名称变更为“查询和监听其他应用录制状态”。

“共享音频输入”文档名称变更为“录音并发策略说明”。

“管理麦克风”文档名称变更为“管理麦克风静音状态”。


### Camera Kit新增相机旋转C++开发文档


适配相机旋转角度：新增适配相机旋转C++指导文档，以确保在不同屏幕状态下、在合适的方向显示相机图像。

相机旋转角度术语：介绍适配相机旋转角度C++开发过程中涉及的设备方向、镜头角度、屏幕显示角度等术语。


### AVcodec Kit文档优化


ROI视频编码：新增在Surface模式下通过NativeBuffer接口配置ROI场景。
...
窗口开发常见问题：补充20+个窗口开发过程中的常见问题与指导，包含如何实现横竖屏切换、如何获取窗口宽高、如何设置隐私窗口等。
...
实现音频输入设备路由切换：当应用有音频输入时，系统会根据音频流类型选择对应的输入设备。若默认输入设备不满足应用需求，可通过setBluetoothAndNearlinkPreferredRecordCategory或selectMediaInputDevice实现音频输入设备路由切换。

空间音频能力查询和状态订阅：空间音频是一种能够将传统立体声体验扩展到三维空间的音频技术，对于音频类播放应用，开发者可以查询到设备是否支持空间音频，以及当前设备空间音频的开关状态。
...
窗口开发常见问题：介绍窗口开发过程中的常见问题，帮助开发者快速完成问题定位与解决。
...
屏幕管理开发术语：介绍屏幕管理开发的核心概念，便于开发者了解屏幕开发相关概念及内容，更好地完成应用开发。


### 新增ArkData常见问题


ArkData常见问题：介绍ArkData开发过程中的常见问题，帮助开发者快速完成问题定位与解决。
...
UI开发常见问题：介绍UI开发时的常见问题，主要包括自定义节点常见问题和按钮与选择组件常见问题的现象和解决措施。
...
使用仿真器运行轻量级穿戴应用：介绍在DevEco Studio上通过仿真器运行和调试Lite Wearable设备应用的使用方式，包括模拟传感器、电池等。
...
日历服务实践案例：介绍了不同“一键服务”类型对应的实践场景及各字段取值建议，提供了对应的示例及案例示意图，指导快速完成各场景服务的一键直达开发，提升用户体验。
...
![](assets/文档变更说明/file-20260515135045099-0.png)
...
![](assets/文档变更说明/file-20260515135045099-1.png)
...
窗口旋转：介绍了窗口旋转与屏幕方向的关系、旋转接口规格、旋转策略及设备差异性等内容，便于开发者系统了解窗口旋转相关内容，从而进行应用窗口旋转相关的适配与开发。
...
- [UI开发调试调优](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ui-debug-optimize)：UI开发时，调试调优的适用场景与实现方法。 [UI稳定性故障调试](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ui-stability)：介绍稳定性故障的概念与分类，并提供各类稳定性问题的参考帮助，用于指导应用开发者充分利用系统提供的调试能力和工具定位各类UI相关的稳定性问题。主要包括：[UI相关应用崩溃常见问题](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-stability-crash-issues)和[UI相关应用无响应常见问题](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-stability-freeze-issues)的现象和解决方法。
- [UI显示异常调试](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-debug)：介绍UI显示异常问题的调试方法，并结合相关案例讲解具体的解决步骤。
- [UI预览](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ui-ide-previewer)：介绍如何通过DevEco Studio预览UI效果并随时调整页面布局，支持页面预览和组件预览。
- [UI调优](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ui-inspector-profiler)：介绍UI的dump和调优能力，用于提高开发效率和优化开发者体验。
- [UI高性能开发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ui-performance-overview)：介绍UI开发时，性能优化的核心思路和优化步骤。

构建渲染节点：介绍如何通过NDK接口直接构建渲染节点，包括节点树操作、属性设置及含动画的自定义绘制。 在NDK中保证多实例场景功能正常：介绍如何解决Native侧多实例场景下的组件操作问题，适用于NDK多窗口开发中不同UI实例间的交互场景。


### 优化ArkUI文档


- [添加交互响应](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-interaction-development-guide-overview)：ArkUI框架提供了丰富的交互功能，支持直接处理基本输入事件，以及由这些事件驱动的手势系统，同时支持拖拽和焦点切换等复杂交互。 [交互基础机制说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-interaction-basic-principles)：介绍[触摸事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-touch)、[鼠标事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-mouse-key)、[轴事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-axis)等指向性事件交互的基本概念和相关原理。
- [输入设备与事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/rkts-interaction-development-guide-raw-input-event)：介绍各类输入设备产生事件的适用场景和相关约束。
- [添加手势响应](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/rkts-interaction-development-guide-support-gesture)：介绍各类手势的触发方式和绑定方法，以及在手势冲突时的解决方法。
- [支持统一拖拽](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-events-drag-event)：介绍拖拽相关的概念、拖拽方式和拖拽能力（通用拖拽、多选拖拽等）的使用方法。
- [支持焦点处理](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-events-focus-event)：介绍焦点相关概念与规范，说明焦点处理过程中常用场景的适配方法。


### UI开发 (基于NDK构建UI）章节目录结构调整


将UI开发 (基于NDK构建UI）调整至“开发”-“应用框架”-“ArkUI（方舟UI框架）”目录下，调整后的目录结构更符合基于NDK接口开发UI界面的学习历程。目录结构调整不影响原页面URL链接地址，原地址可正常访问。

图3 UI开发 (基于NDK构建UI）目录调整后结构

![](assets/文档变更说明/file-20260515135045099-2.png)
...
新增HAP转HAR开发指导，HAP不支持导出接口或ArkUI组件给其他模块或应用使用，HAR支持，本页面为开发者提供该场景下通过配置项变更将HAP工程变成HAR工程的指导。
...
Longque JS Engine 提供了一批Longque JS API指导指南，旨在HarmonyOS 平台构建稳定、高性能的应用。
...
新增了Node-API常见问题汇总，方便开发者快速获取相关指导。


### 新增并发常见问题指导文档


为了帮助开发者快速定位并解决在使用TaskPool、Worker及Sendable特性时遇到的各类问题，新增并发常见问题指导文档。
...
![](assets/文档变更说明/file-20260515135045099-3.png)
...
**图4 **API变更查询
![](assets/文档变更说明/file-20260515135045099-4.png)
...
![](assets/文档变更说明/file-20260515135045099-5.png)
...
![](assets/文档变更说明/file-20260515135045099-6.png)
...
![](assets/文档变更说明/file-20260515135045099-7.png)


同时，每个接口下也会显示其支持的设备（默认开启，可一键隐藏）：


![](assets/文档变更说明/file-20260515135045099-8.png)
...
创建异步线程执行AVTranscoder视频转码：添加视频转码的开发实践，简要介绍了视频码率和分辨率的关系，帮助开发者选择合适的码率、分辨率，并提供了使用异步线程转码的开发指导。
...
将UI范式基本语法、状态管理、渲染控制节点从“入门”-“学习ArkTS语言”移至“开发”-“应用框架”-“ArkUI（方舟UI框架）”-“UI开发 (ArkTS声明式开发范式)”目录下，调整后目录结构更符合UI开发框架学习历程。
...
![](assets/文档变更说明/file-20260515135045099-9.png)
...
针对文本开发，新增提供了字体管理、文本测量、文本绘制与显示三个章节内容，各章节按实现语言分为ArkTS和C/C++对应的具体指导内容。此次指南优化上新了约15个页面，进一步丰富完善了应用在开发和布局时，针对文本元素或内容进行的排版、测量、绘制和显示等场景。
...
![](assets/文档变更说明/file-20260515135045099-10.png)
...
HarmonyOS 5.0.3(15)版本API参考文档提供API version筛选功能，开发者可便捷获取某个API version支持的接口范围。

1. 打开API参考文档，在左侧导航栏“高级筛选”下拉选项中设置“API version”版本号。
**图9 **“API version”筛选
![](assets/文档变更说明/file-20260515135045099-11.png)
2. 设置目标API version后，默认显示支持的API范围，不支持的接口在导航栏中置灰。
**图10 **导航栏筛选后效果
![](assets/文档变更说明/file-20260515135045099-12.png)
3. 选择“只看筛选内容”，导航中不支持接口将被隐藏。
**图11 **只看筛选内容效果
![](assets/文档变更说明/file-20260515135045099-13.png)


官网文档默认版本变更为“HarmonyOS 5.0.3(15)”
...
- 入口1：官网 ->开发->开发文档
**图12 **开发文档入口
![](assets/文档变更说明/file-20260515135045099-14.png)
- 入口2：官网->文档->文档中心
**图13 **文档中心入口
![](assets/文档变更说明/file-20260515135045099-15.png)
...
HarmonyOS 5.0.2(14)、 HarmonyOS 5.0.1(13)、HarmonyOS 5.0.0(12)分支官网文档版本统一收编至HarmonyOS 5.0.3(15)版本，历史版本文档停止更新。
...
> [!NOTE]
> 通过API version筛选支持接口范围可便捷了解不同API version支持接口范围，请参考[获取体验增强](#section1810915471038)。
...
围绕实际图形的绘制与显示流程，新增提供了画布的获取与绘制结果的显示、画布操作及状态、绘制效果、图元绘制四个章节内容，各章节内容下按实现语言分为ArkTS和C/C++对应的具体指导内容。

画布的获取与绘制结果的显示：介绍图形绘制中画布的获取与显示。画布为图形绘制的核心，所有绘制操作都是基于画布Canvas实现的。

画布操作及状态：介绍获取得到画布Canvas之后，可以基于画布进行的图形操作和状态处理。

绘制效果：介绍基于画布，使用画刷（Brush）或画笔（Pen）添加的绘制效果。其中画刷主要用于实现填充效果；画笔主要用于实现描边效果。

图元绘制：无论多复杂的图形，都是由基础的图元组合而来。介绍了当前支持绘制的基础图元，包括几何形状绘制（点、圆、矩形等）、图片绘制和字块绘制。

图形开发术语：介绍图形开发过程中的术语概念。
...
游戏场景感知：提供了游戏场景感知的C/C++开发指导，帮助开发者快速实现游戏与系统的交互，在系统资源有限的情况下进一步改善玩家的游戏体验。

IAP Kit

非续期订阅类型商品的购买：提供了非续期订阅类型商品的接入购买和权益发放开发指导，助力应用实现非续期订阅商品的购买。

Payment Kit

通用收银台接入：介绍了Payment Kit通用收银台业务规则、混合支付场景、纯外部支付场景，助力应用实现混合支付、纯外部支付模式。
...
应用内快捷方式加桌：介绍了在应用市场推荐场景下，如何实现应用内快捷方式加桌。
...
模块加载副作用及优化：介绍当使用ArkTS模块化时，模块的加载和执行可能会产生非预期的顶层代码执行、全局状态变化、原型链修改、导入内容未定义等副作用，并介绍如何优化，减少副作用对应用的影响。
...
时域AI超分：介绍时域AI超分的使用场景、业务流程和接入步骤，助力开发者实现超采样率和超分辨率并达到抗锯齿效果。
...
数字人民币支付场景：介绍如何接入数字人民币支付服务，快速实现应用的数字人民币支付。

Remote Communication Kit

URPC场景：介绍在URPC场景下如何实现远程函数调用。

NearLink Kit

从NearLink Kit简介、查询星闪开关状态、发送星闪广播、发起星闪扫描、SSAP连接及数据传输，全面介绍如何实现星闪设备之间的连接、数据交互。
...
在非ArkTS线程中回调ArkTS接口：不推荐使用uv_queue_work接口来实现“非JS线程向JS线程提交任务”的功能，否则存在冗余的线程创建。推荐使用Node-API接口进行线程安全开发文章中“从任意线程往JS线程提交任务”的实现方法。uv_queue_work的使用建议，请参考libuv指导文档。
...
详情请参见HarmonyOS 5.0.0 Release版本概览。
...
不依赖UI组件的全局自定义弹窗：介绍如何使用UIContext中获取到的PromptAction对象提供的openCustomDialog接口来实现自定义弹窗。
...
获取支持与帮助：详细介绍官方面向公共开发者提供的支持与帮助渠道、官网文档“意见反馈”功能。
...
- [自定义扩展](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-user-defined-modifier-V5)：介绍Modifier的自定义扩展能力，通过与UI分离的方式，对已有UI组件的属性、手势、内容进行扩展修改。 [AttributeModifier](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-user-defined-extension-attributemodifier-V5)：介绍如何通过Modifier对象动态修改属性。
- [AttributeUpdater](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-user-defined-extension-attributeupdater-V5)：介绍该属性对象不经过状态变量，直接更新对应属性的方法。

使用镜像能力：介绍镜像能力的使用场景与默认支持的组件。 支持适老化：说明适老化的使用约束与触发方式等。 粒子动画：介绍粒子动画的概念，说明粒子在颜色、透明度、大小等维度变化的实现方法。 帧动画：说明如何使用animator实现动画效果。 在应用程序中使用画中画功能：新增支持使用typeNode实现画中画功能开发的方式，提供了对应的指导内容，同时优化了文档结构。
ArkWeb
...
安全审计：介绍安全审计的使用场景、约束和限制以及接入步骤等，助力应用实现审计相关业务。

HiAI Foundation Kit

单算子应用：介绍单算子应用场景以及接入步骤，助力三方框架性能提升。

Map Kit

设置地图元素压盖顺序：介绍如何设置地图和各种覆盖物元素的层级压盖关系。

Network Boost Kit

- [连接迁移通知](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/networkboost-nethandovercallback-V5)：介绍如何实现在弱网环境下，系统发起多网迁移（WiFi蜂窝，主卡副卡等）时，给应用提供连接迁移开始和完成通知应用提供连接迁移开始和完成通知。
...
SOTER免密身份认证：介绍SOTER免密身份认证应用场景以及接入步骤，助力应用通过用户的生物特征来代替传统的密码验证，实现免密身份认证能力。
...
**图14 **文档中心版本筛选入口优化
![](assets/文档变更说明/file-20260515135045099-16.png)
...
全局取色：介绍全局取色能力的使用场景以及接入步骤，助力应用获取手写笔/手指目标位置的图像颜色信息。
...
![](assets/文档变更说明/file-20260515135045099-17.png)
...
涉诈剧本检测：介绍涉诈剧本检测的使用场景以及接入步骤，助力应用实现对用户支付或转账时存在欺诈威胁，给出有效提示或拦截。
...
![](assets/文档变更说明/file-20260515135045099-18.png)
...
本地搜索方案：介绍本地搜索的场景以及接入步骤，助力开发者快速实现“一步搜索，内容直达”。

Map Kit

弧线：介绍如何在地图上绘制弧线，包括设置弧线的位置、宽度、颜色等参数。
...
介绍通过PdfView组件实现打开PDF文档、PDF文档与图片格式互转、添加页眉页脚、添加水印、添加背景、添加批注、添加书签。

Ringtone Kit

介绍Ringtone Kit（铃声服务）的场景以及如何设置铃声。
...
从UI Design Kit简介、分层图标处理、单层图标处理，全面介绍应用如何实现跟随HarmonyOS Design System高端精致设计效果UI界面。

Weather Service Kit

介绍如何查询天气数据，用于支撑应用的天气预报、分钟级降水预报、天气预警、天气指数、天文数据等场景的实现。

Wear Engine Kit

从Wear Engine简介、开发准备、手机侧应用开发、手表侧应用开发、调测验证，全面介绍如何获取穿戴设备基础信息、应用间消息通信、发送模板化的通知到穿戴设备、获取穿戴用户状态等，实现手机与穿戴设备能力共享。

Network Boost Kit

网络场景识别：介绍应用如何订阅络场景识别，网络场景信息包括数据传输的链路类型、网络场景类型、数传策略建议、弱信号信息等。
...
文件授权访问(ArkTS)：文档下线，其中获取并使用公共目录，调整至获取并使用公共目录；通过FilePicker设置永久授权获取调整至授权持久化。
...
应用数据迁移适配指导：更新了“迁移调试”工具的获取方式，HarmonyOS NEXT Developer Beta1及之后版本，开发者需要联系华为方技术支持人员申请“迁移调试”工具，模拟进行数据迁移验证。或通过“华为开发者联盟官网”->“支持”，在线提单方式获取。

Graphics Accelerate Kit

介绍如何在Vulkan图形API平台开启超帧内插模式和超帧外插模式。
...
跨设备互通能力：介绍如何通过NDK接口实现跨设备的相机、扫描、图库访问能力。

Speech Kit

AI字幕控件：介绍如何实现AI字幕的展示。

Store Kit

隐私管理服务：介绍如何实现隐私链接查询、隐私签署状态查询、停止隐私协议和未上架应用接入隐私托管服务。
...
优化文档结构，从介绍管理AR会话、获取设备位姿、检测环境中的平面、识别平面语义、识别目标形状等原子化场景如何实现，由部分到整体，引导开发者开发AR场景的应用（AR物体摆放）。
...
![](assets/文档变更说明/file-20260515135045099-19.png)
...
获取方式1：华为开发者联盟官网-“文档”页面中，提供HarmonyOS NEXT版本、HarmonyOS 3.1/4.0及以下版本配套文档获取入口。

1. 打开“华为开发者联盟”官网，单击网页右上角“文档”按钮，进入[文档获取概览页](https://developer.huawei.com/consumer/cn/doc/)。
2. 根据文档使用场景，选择设计、HarmonyOS SDK相关主题文档。


获取方式2：华为开发者联盟官网-“开发”页面中，提供HarmonyOS NEXT版本配套文档获取入口。
...
- 指向性跳转： [（可选）使用canOpenLink判断应用是否可访问](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/canopenlink-V5)：介绍如何使用canOpenLink接口判断目标应用是否可访问。
- [使用Deep Linking实现应用间跳转](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/deep-linking-startup-V5)：介绍使用Deep Linking进行应用间跳转的实现原理和开发指导。
- [使用App Linking实现应用间跳转](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/app-linking-startup-V5)：介绍使用App Linking进行应用间跳转的适用场景、实现原理和开发指导。
- [应用间显式跳转切换link跳转适配指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/uiability-startup-adjust-V5)：提供从显式跳转到link跳转（即应用链接跳转）的适配指导。

通用意图跳转
- [通过startAbility拉起文件处理类应用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/file-processing-apps-startup-V5)：介绍如何使用startAbility接口拉起文件处理类应用，来打开特定类型的文件。

拉起系统应用：介绍拉起系统应用的方式，提供当前已支持的系统应用跳转能力清单。
ArkUI

- [使用NDK接口构建UI](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-build-ui-overview-V5)：介绍ArkUI NDK接口提供的能力，以及如何通过NDK接口创建UI界面。 [接入ArkTS界面](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-access-the-arkts-page-V5)：说明如何通过占位组件（ContentSlot）和NDK提供的UI组件接入ArkTS界面。
- [添加交互事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-add-event-V5)：介绍如何监听组件事件和绑定手势事件。
- [使用动画](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-use-animation-V5)：介绍属性动画实现组件出现/消失转场的方法。
- [开发长列表](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-loading-long-list-V5)：介绍如何通过懒加载（NodeAdapter）按需生成子组件，开发长列表界面。
- [构建弹窗](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-build-pop-up-window-V5)：说明如何通过弹窗控制器显示自定义弹窗，设置自定义弹窗的样式和内容。
- [构建自定义组件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-build-custom-components-V5)：介绍自定义测算、自定义布局和自定义绘制的实现方法。
- [嵌入ArkTS组件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/ndk-embed-arkts-components-V5)：介绍如何通过ComponentContent完成对ArkTS组件的封装，将封装对象转递到Native侧。

Router切换Navigation：介绍两种路由跳转方式的架构差异，说明各个业务场景下两种方式的能力对比，同时提供Router各个模块切换Navigation的方法。
Graphics Accelerate Kit

OpenGTX功能：介绍如何通过LTPO（动态帧率/刷新率）方案，动态调整游戏的帧率/刷新率以及设备的SOC/DDR频率。
...
![](assets/文档变更说明/file-20260515135045099-20.png)
...
![](assets/文档变更说明/file-20260515135045099-21.png)
...
![](assets/文档变更说明/file-20260515135045099-22.png)
...
- [自定义节点](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-user-defined-node-V5)：介绍ArkUI框架提供底层实体节点部分基础能力。 [自定义占位节点](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/arkts-user-defined-place-hoder-V5)：可以将自定义节点挂载在占位节点上，实现自定义节点与原生组件的混合显示。
...
旋转屏动画增强：介绍在原旋转屏动画基础上，如何配置渐隐和渐现的转场效果。 粒子动画：介绍粒子在颜色、透明度、大小、速度、加速度、自旋角度等维度变化做动画的实现方法。 事件分发：介绍触控事件的分发机制和事件响应链的收集方法。 设置主题换肤：介绍应用级和页面级的主题设置能力，并提供局部深浅色模式设置、动态换肤等功能描述。
ArkTS
...
应用跟随系统未成年人模式：介绍应用如何实现跟随系统未成年人模式。

AR Engine

命中检测与运动跟踪：介绍如何实现命中检测与运动跟踪。
...
介绍如何使用Cloud Foundation Kit在应用中开发云函数、开发云数据库、开发云存储。

Core Vision Kit

主体分割：介绍如何检测出图片中区别于背景的的前景物体或区域（即“显著主体”），并将其从背景中分离出来。

Connectivity Kit

WLAN服务开发指南：介绍WLAN服务的实现原理、模式，以及P2P模块的开发指导。

Device Certificate Kit

设备真实性证明：介绍系统提供的设备真实性证明和应用身份证明的能力。应用可校验业务请求是否来自真实设备和合法应用，帮助开发者识别黑灰产的攻击行为。

Device Security Kit

从Device Security Kit简介、服务配额、开发准备、应用设备状态检测、系统完整性检测、URL检测、（该特性在6月11日补丁版本带入）可信应用服务，全面介绍应用如何实现对某台设备上的使用状态、设备环境进行管理和检测。

Graphics Accelerate Kit

顶点标记：介绍增强模式的运动估计原理及顶点标记方法。
...
介绍如何实现接入手写套件、报点预测、一笔成形，助力应用创造更多的手写应用场景。

PDF Kit

介绍如何实现打开PDF文档、以及如何在PDF文档中添加页眉页脚、添加水印、添加背景、添加批注、添加书签。

Scan Kit

“扫码直达”服务：介绍如何接入“扫码直达”服务，确保用户可通过锁屏、控制中心等系统级的常驻入口，扫应用的二维码、条形码并跳转到应用对应服务页面。
...
支持应用归因服务：介绍如何实现登记归因来源及转化、归因结果回传。

Wallet Kit

从Wallet Kit简介、创建Wallet Kit服务、数字车钥匙、接入交通卡，全面介绍如何使用Wallet Kit在应用中接入数字车钥匙和交通卡。


### 优化文档


《DevEco Studio使用指南》中将Code Linter支持的检查规则，从表格升级为单页面的形式展示规则详情，提供正反例、配置方法、选项等信息，方便用户查阅与使用。具体请参考Code Linter代码检查规则。

ArkUI

组件导航 (Navigation)：补充页面跳转、页面返回等路由操作方法，说明页面生命周期原理与页面转场动效实现逻辑，提供系统路由表和自定义路由表定义机制与实践步骤。
...
应用安全隐私体验建议：介绍应用设计、开发及检测阶段需关注的应用全生命周期的安全、隐私行为。

应用功耗体验建议：介绍提升应用功耗热体验，如何合理使用应用全生命周期软硬件资源规范建议。

Ability Kit

应用启动框架AppStartup：介绍如何使用AppStartup来设置组件的初始化顺序以及之间的依赖关系，加速应用启动。

密码自动填充服务：介绍如何在应用中快速集成自动填充等功能，完成在登录、注册等场景下自动保存或填充用户密码。

Account Kit

获取发票抬头：介绍如何使用获取发票抬头能力，帮助应用快速打开用户发票抬头选择页或管理页，并返回用户选择的发票抬头信息。

AR Engine

从AR Engine、开发准备、平面检测，介绍如何实现运动跟踪、环境跟踪和命中检测等空间计算能力。

Call kit

向系统通知去电消息：介绍如何实现向系统通知去电消息，实现来电横幅通知的展示。

应用上报通话中的静音、扬声器事件：介绍如何实现应用上报通话中的静音、扬声器事件。

Game Service Kit

游戏场景感知功能：介绍游戏场景感知功能的使用场景、业务流程，以及如何订阅设备状态变化、上报游戏信息、取消订阅设备状态，帮助游戏应用感知游戏场景和运行状态的不同，使用不同策略调度系统资源以达到更精细化的优化效果。

Graphics Accelerate Kit

从Graphics Accelerate Kit简介、超帧功能（内插模式、外插模式）、ABR功能，全面介绍游戏应用如何快速构建超帧、ABR（自适应稳态渲染）等游戏渲染加速能力，实现游戏帧率和运行流畅度的提升、系统负载和功耗的降低等。

IAP Kit

沙盒测试：介绍如何接入IAP Kit实现在应用调测过程中，无需真实支付即可实现支付测试。

Intent Kit

优化章节组织结构，按照特性类别展示具体特性，提升开发者阅读体验。

事件推荐方案：从典型场景、卡片展示效果、接入方案，介绍接入事件推荐的步骤，助力应用将新动态主动推荐给用户。
...
文件预览：新增多文件预览，用户打开文件预览可以一次性传入多个文件，可在预览窗口选择对应文件进行查看。


Remote Communication Kit

自定义证书校验：介绍如何实现自定义服务器证书的验证。
...
应用接入状态栏：介绍应用如何接入状态栏，在状态栏中添加应用图标以及管理图标。
...
![](assets/文档变更说明/file-20260515135045099-23.png)
...
代码混淆：介绍如何使用代码混淆，降低工程被破解攻击的风险，缩短代码的类与成员的名称，减小应用的大小。
...
拦截器：介绍如何实现拦截HTTP请求和响应，并修改响应、请求内容。

Speech Kit

朗读控件：介绍应用如何实现将文本实时转化成语音并进行朗读。

Vision Kit

文档扫描：介绍应用如何实现拍摄文档并将其转换为高清扫描件。

XEngine Kit

空域GPU超分：介绍如何基于单帧输入图像，使用空间邻域信息实现超采样，并提供了示例代码。
...
根据开发旅程，优化开发指导文档结构，从Push Kit简介、开发准备、发送场景化消息到端云调试等逐步展开，提升开发者阅读体验。
...
从Health Service、接入流程、应用开发者申请资质说明、开发接入，全面介绍如何实现管理用户运动健康采样数据、锻炼记录、健康记录以及读取实时三环数据。
...
签约代扣：介绍如何实现直连商户通过预签约、申请免密代扣。

Store Kit

应用市场更新：介绍应用市场升级检测、显示升级对话框接口，以及如何实现检测应用是否有待更新版本，并升级弹框提示用户更新。
