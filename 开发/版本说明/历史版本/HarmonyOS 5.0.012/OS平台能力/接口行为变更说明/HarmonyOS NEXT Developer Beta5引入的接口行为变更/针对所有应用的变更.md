...
变更后：使用autoSync配置为true的键值型数据库，进行数据增删改操作时，可自动触发向同一组网环境内处于多端协同状态下的设备进行跨设备数据同步。
...
键值型数据库的跨设备自动同步功能只支持在跨设备协同场景下使用。详见键值型数据库跨设备同步开发指导。
...
开发者可以使用日历 API，创建延时类提醒，详见Calendar Kit开发指南。

方案二：

开发者若需要使用后台代理提醒能力，发送延时类提醒，需要提供如下信息，申请方式详见代理提醒开发指南。
