...
开发者在使用addComponentContent前需要判断父节点的isModifiable是否为true，不支持isModifiable为false的FrameNode节点使用addComponentContent新增子节点。需要在声明式组件中动态添加内容时，可以通过占位节点NodeContainer、ContentSlot进行操作。
...
开发者需要申请加入可信应用服务开通白名单，等待审核通过后，在AppGallery Connect上开通可信应用服务，详细步骤可参考指导文档。
