...
- 关闭v8的JIT功能： 编译时关闭：在编译v8的GN文件里面加入v8_jitless = true选项
...
为满足开发者对性能的要求，系统内提供了预置JS引擎。具体可参考NDK开发指导。
...
1. 查询支持的命令参数 命令行执行 cmake --help 可以查看支持的命令参数合集 我们也可以在 [cmake.org](https://cmake.org/cmake/help/v3.28/manual/cmake.1.html) 查看 cmake 官方指导文档，Options 详细描述了支持的命令参数合集及其说明
...
如果so版本冲突，请在工程级或者模块级build-profile.json5文件中buildOption下添加nativeLib/filter/select字段，根据包名、版本、产物名称等，选择打包或排除so文件到HAP产物。具体请参考关于select的使用。
...
2.使用资源拦截的方式加载本地文件，具体参考解决Web组件本地资源跨域问题。
...
详见分布式数据对象开发指南中的在跨端迁移中使用分布式数据对象迁移数据和在多端协同中使用分布式数据对象示例代码。
...
建议声明阶段变更数字字面量的属性名为常量字符串或者标识符。具体说明和修改方式请参考适配指导案例。
...
1. 在@Component修饰的自定义组件中通过@State、@Prop、@Link、@Provide、@Consume、@StorageLink、@StorageProp、LocalStorageLink、@LocalStorageProp修饰并使用@ObservedV2修饰的变量类型时，进行校验并输出错误信息。 简化示例如下：
```ts
@ObservedV2
class TmpA{}
...
```
2. 在@ComponentV2修饰的自定义组件中通过@Param、@Local、@Event、@Provider()、@Consumer()修饰并使用@Observed修饰的类时，进行校验并输出错误信息。 简化示例如下：
```ts
@Observed
class TmpA{}
...
```
3. 以上的变量类型联合使用时，也会进行校验并输出错误信息。 示例如下：
```ts
@ObservedV2
class TmpA{}
...
默认返回值变更，开发者需通过像素单位转换接口，对返回值进行正确处理。
...
新接口使用指南请参考Media Library Kit使用指导。
