...
默认效果变更，需应用适配。横竖屏设置档位规则保持一致，参考detents属性的设置请查阅半模态组件文档进行适配。
...
- 变更后： TimePickerDialog、DatePickerDialog组件12小时制小时默认没有前置零。 如下图所示为变更前后效果对比：
...
![](assets/UX样式或效果的变更/file-20260515134958170-0.png)


API version 12及以后，popupPosition属性设置为undefined时会重置为默认值，提示弹窗位置会发生变化。


![](assets/UX样式或效果的变更/file-20260515134958170-1.png)
...
开发者需要判断变更后popupPosition属性设置undefined时重置为默认值的效果是否符合预期，如不符合可通过改变AlphabetIndexer组件popupPosition属性传入参数以达到预期。
...
![](assets/UX样式或效果的变更/file-20260515134958170-2.png)


API version 12及之后：autoCollapse属性默认值为true，当AlphabetIndexer组件高度不足时，会折叠显示。


![](assets/UX样式或效果的变更/file-20260515134958170-3.png)
