...
新增支持查询、删除应用内快捷方式的能力，支持用户查看、管理应用的桌面快捷方式。（指南、API参考）
...
- 虚拟机维测能力增强：新增支持获取所有虚拟机线程的堆内存信息，包括线程ID、线程名称、堆类型和堆对象大小。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-util#getallvmheapmemoryinfo24)）
- 新增堆内存超过预警阈值的回调函数，在虚拟机主线程完成垃圾回收后如果堆内存仍超过预警阈值则触发回调执行。（[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-util#onvmheapmemorypressure24)）
 ...
 taskpool的execute方法增强，执行任务或任务组可以指定任务超时时长。如果任务或任务组的执行时间超过设置的超时时长，则抛出对应错误信息。（API参考）
...
新增支持后台播放模式的设置，可由应用告知系统是否支持后台播放，系统根据能力决策实况胶囊的显示。（API参考）


### Camera Kit


新增支持创建延迟预览输出对象，在配流时替代普通的预览输出对象加入数据流（API参考）。同时支持配置延迟预览的Surface（API参考）。
...
【新增Kit】Content Embed Kit（内容嵌入服务）提供应用间文档互相嵌入与协同编辑的框架能力，并为开发者封装了客户端与服务端的开发接口，便于快速实现文档跨应用嵌入与协作。（指南、API参考）
...
【新增Kit】Enterprise Threat Protection Kit（企业威胁防护服务）提供了构建安全防护应用的核心能力，专注于文件访问与处置。应用可对文件进行扫描以识别潜在威胁，并对威胁文件执行隔离、恢复或删除操作，从而有效保护企业设备的数据安全。详细信息请参见Enterprise Threat Protection Kit开发指南。
...
在onUpdateForm回调中新增支持卡片更新原因字段。（API参考）


### Image Kit


新增WebP图像元数据类，用于存储图像的元数据。（API参考）


### Input Kit


C API输入事件增强，提供输入事件的压力、相对窗口左上角的XY相对坐标等事件。（API参考）
...
- 在区划查询场景下，新增支持选择区划层级范围。（[指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-scenemap#districtselectoptions)、[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/map-location-division#开发步骤)）
...
新增支持设置数据传输低功耗模式。（API参考）


### Network Kit


TLS支持证书链的验证，可通过传入数组的方式最多支持到1000个证书。（API参考）
...
新增支持请求用户授权的同时配置管控应用在管控生效过程中是否可卸载。（指南、API参考）
...
底部页签新增支持页签栏和迷你栏垂直布局样式。（API参考）


### Vision Kit


卡证识别场景下，新增支持识别港澳居民来往内地通行证、台湾居民来往大陆通行证。（API参考）


### 标准库


新增Seccomp开放系统调用列表。（API参考）
