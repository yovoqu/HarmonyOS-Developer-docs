...
- **FA（Feature Ability）模型：** HarmonyOS API 7开始支持的模型，已经不再主推。FA模型开发可见[FA模型开发概述](https://developer.huawei.com/consumer/cn/doc/lite-wearable-guides/fa-model-development-overview)。**快速入门章节不再详细说明此模型**。
