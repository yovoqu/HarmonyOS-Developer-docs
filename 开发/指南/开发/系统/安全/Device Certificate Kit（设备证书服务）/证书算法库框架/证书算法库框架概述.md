...
由于端侧系统时间不可信，证书链校验不包含对证书有效时间的校验。如果需要检查证书的时间有效性，可使用X.509证书的[checkValidityWithDate()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#checkvaliditywithdate)方法进行检查。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9e/v3/ab3pVo2iTqS9MzwDMKYYrg/zh-cn_image_0000002656348347.png?HW-CC-KV=V1&HW-CC-Date=20260624T020832Z&HW-CC-Expire=86400&HW-CC-Sign=50332291A0C27EC92DC860DB822B73247F41F733BA9AC9CC88DB848692D72985)
...
| 名称 | 类 | 功能 |
| --- | --- | --- |
| X.509证书 | X509Cert | 提供X.509证书的解析、序列化、X.509证书签名验证、证书相关的信息查询等功能。 |
| 证书扩展域段 | CertExtension | 提供X.509证书中扩展域段的获取，如是否CA，CRL分发点等字段。 |
| X.509证书吊销列表 | X509CRL | 提供X.509证书吊销列表的解析、序列化、信息查询等功能。 |
| 证书链校验器 | CertChainValidator | 提供证书链校验（不包括证书有效期的校验）、证书链算法名称查询的功能。 |
| 证书和证书吊销列表集合 | CertCRLCollection | 提供证书和证书吊销列表集合的查询功能。 |
| X.509证书链 | X509CertChain | 提供证书链校验、证书列表获取的功能。 |
