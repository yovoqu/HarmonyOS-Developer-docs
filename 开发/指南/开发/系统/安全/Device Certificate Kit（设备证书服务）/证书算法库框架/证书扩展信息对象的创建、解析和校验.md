...
解析证书扩展域段数据，调用[cert.createCertExtension](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatecertextension10)创建证书扩展域段对象。 调用[CertExtension.getEntry](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#getentry10)获取指定OID证书扩展域段信息。比如，证书扩展域段对象标识符列表，根据对象标识符获取具体数据等。 调用[CertExtension.checkCA](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#checkca10)判断证书是否为CA证书。
```text
import { cert } from '@kit.DeviceCertificateKit';
