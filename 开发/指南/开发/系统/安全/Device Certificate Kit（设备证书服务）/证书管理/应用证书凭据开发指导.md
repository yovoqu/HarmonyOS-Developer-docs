...
安装应用证书凭据。
2. 获取应用证书凭据。
3. 使用应用证书凭据对数据进行签名、验签。
4. 卸载指定的应用证书凭据。
5. 安装应用证书凭据支持的算法类型和签名验签支持的参数组合。

  
证书管理安装凭据及使用凭据中的密钥进行签名、验签，依赖[密钥管理服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/huks-overview)（HUKS）能力。
6. 证书管理支持的算法为HUKS支持的子集，当前仅支持RSA、ECC及SM2算法类型的私有凭据安装及使用。
7. 签名、验签支持的参数组合，详见HUKS声明的[签名/验签介绍及算法规格](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/huks-signing-signature-verification-overview)中RSA、ECC及SM2的描述。
