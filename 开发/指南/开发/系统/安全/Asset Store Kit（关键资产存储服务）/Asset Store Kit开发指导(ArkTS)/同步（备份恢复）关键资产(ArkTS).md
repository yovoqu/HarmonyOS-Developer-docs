...
新增密码demo_pwd（别名demo_alias），附属信息为demo_label，支持同步的关键资产。 引用头文件，定义工具函数。
```text
...
通过API文档查看查询关键资产同步结果接口[asset.querySyncResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-asset#assetquerysyncresult20)。 在查询关键资产同步结果时，关键资产属性的内容（AssetMap）参数如下表所示。
