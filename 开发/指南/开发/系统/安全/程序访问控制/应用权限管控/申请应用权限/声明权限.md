...
> [!NOTE]
> 在多HAP场景下，已在 entry 模块中声明的权限，无需在 feature 模块中重复添加，权限将在整个应用中生效。 同理，在 feature 模块中已声明的权限，在 entry 模块也无需重复添加。
...
**建议句式**：用于做某事。
3. **样例**：以申请相机权限的reason字符串为例。

  正例：用于视频通话。
4. 用途描述的字串建议小于72个字符（即36个中文字符，UI界面显示大约为两行）。不能超过256个字符，以保证多语言适配的体验。
5. 字串不能为空白字符串，即不能不填，也不能只填空格符。
6. 如果应用申请的权限用于多个场景，需要确保字串的完整性，让用户了解应用使用此权限的所有场景；多个HAP包内如果申请同一个权限，各个权限Reason字段需要保持场景的完整性和一致性。
