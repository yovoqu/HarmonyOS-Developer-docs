...
> [!NOTE]
> 如果应用涉及获取受限权限，在应用发布上架时，AGC，AppGallery Connect（AGC）将根据应用的使用场景审核是否可以使用对应的受限权限。如不符合，应用的上架申请将被驳回，审核方式请见 发布HarmonyOS应用 。
...
> [!NOTE]
> 在API 12及以上版本，该权限等级变更为normal，应用可直接 声明使用 ；若需兼容API12之前版本，仍需按 受限权限申请方式 使用该权限。
