...
> [!NOTE]
> ECB、CBC加密模式，明文长度不是16字节整数倍，必须使用填充方法补足。 CCM加密模式，必须指定附加验证数据aad且其长度必须大于等于1字节且小于等于2048字节。
...
从API版本26.0.0开始，支持分组密码算法[RC2](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-sym-key-generation-conversion-spec#rc2)，分组长度为8字节，密钥长度支持8位～1024位，且该值须为8的倍数。
...
从API版本26.0.0开始，支持流密码算法[RC4](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-sym-key-generation-conversion-spec#rc4)，无需分组与填充。密钥长度支持8位～4096位，且该值须为8的倍数。
...
从API版本26.0.0开始，支持密码算法[Blowfish](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-sym-key-generation-conversion-spec#blowfishbf)，是一种分组密码算法，分组长度为8字节，密钥长度支持32位～448位，且该值须为8的倍数。
...
从API版本26.0.0开始，支持分组密码算法[CAST](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-sym-key-generation-conversion-spec#cast)，分组长度为8字节，密钥长度支持40位～128位，且该值须为8的倍数。

[CAST](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-sym-key-generation-conversion-spec#cast)为分组密码算法，分组长度为8字节，密钥长度支持40位～128位，且该值须为8的倍数。
