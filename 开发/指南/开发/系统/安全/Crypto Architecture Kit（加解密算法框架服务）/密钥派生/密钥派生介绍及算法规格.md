...
HKDF（HMAC-based Extract-and-Expand Key Derivation Function）是基于[HMAC](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-compute-hmac)消息认证码的简单密钥派生算法。
 
输入原始的密钥材料和盐值来提取、输入原始的密钥材料和扩展信息来扩展。它是一种密钥派生函数，用于从较短的输入密钥中派生出更长的输出密钥。
 
HKDF包含三个模式，提取（EXTRACT_ONLY）、扩展（EXPAND_ONLY）、提取和扩展（EXTRACT_AND_EXPAND）。
