...
| openFileWrite(path: string, callback: AsyncCallback&lt;number&gt;): void | 在只写模式下，通过Callback方式获取用户个人数据目录下文件的文件描述符（fd）。 |
| openFileWrite(path: string): Promise&lt;number&gt; | 在只写模式下，使用Promise方式获取用户个人数据目录下文件的文件描述符（fd）。 |
