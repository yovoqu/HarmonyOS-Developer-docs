...
| 接口名 | 描述 |
| --- | --- |
| [verifyUserIdentityEnterprise](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataguard-recoverykey#verifyuseridentityenterprise)(userId: number, userType: number, pinCode: string): Promise | 使用Promise方式验证用户锁屏密码。 |
| [verifyUserByDialog](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataguard-recoverykey#verifyuserbydialog)(userId: number): Promise | 通过Dialog弹框验证用户锁屏密码。 |
| [getEnterpriseRecoveryKeyForResettingPin](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataguard-recoverykey#verifyuserbydialog)(userId: number, userType: number): Promise | 使用Promise方式获取用于重置锁屏密码的企业恢复密钥。 |
