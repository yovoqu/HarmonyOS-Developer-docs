...
> [!NOTE]
> 1、创建密钥时指定密钥属性集合中身份认证类型tag: huks.HuksTag.HUKS_TAG_USER_AUTH_TYPE时，必须要包括huks.HuksUserAuthType.HUKS_USER_AUTH_TYPE_TUI_PIN认证方式，其余认证类型可以根据业务需要进行定制配置。 2、开发者如果需要支持密钥加密导出导入功能，初始化生成密钥属性集时，需要设置HUKS_TAG_IS_ALLOWED_WRAP，指定密钥允许导出。
