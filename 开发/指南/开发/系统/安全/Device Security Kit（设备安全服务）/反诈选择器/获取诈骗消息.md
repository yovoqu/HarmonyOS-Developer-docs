...
![](assets/获取诈骗消息/file-20260514131140319-0.png)
**流程说明：** 用户在开发者应用上选择举报诈骗消息功能。 开发者应用调用Device Security Kit的接口拉起诈骗消息选择器。 用户在诈骗消息选择器中选择诈骗消息。 Device Security Kit调用回调函数通知开发者应用，开发者应用根据诈骗消息进行业务处理。
...
| 接口名 | 描述 |
| --- | --- |
| selectFraudMessage(context: common.Context, options?: AntifraudMessageOptions): Promise | 获取诈骗消息信息。 |
...
> [!NOTE]
> 在开发准备过程中，需要申请权限：ohos.permission.USE_FRAUD_MESSAGES_PICKER。 只允许清单内的应用申请该权限，申请方式请参考：申请使用受限权限 开发者需向用户说明数据使用的目的、方式和范围。
