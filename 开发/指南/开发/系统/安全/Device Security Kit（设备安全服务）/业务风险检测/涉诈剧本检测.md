...
4. 从header中获取证书链，使用[Huawei CBG Root CA](https://pki.consumer.huawei.com/ca/cer/RootCaG2Ecdsa.cer)证书对其进行验证。

5. 校验证书链中的叶证书域名，域名：riskopenapi.platform.hicloud.com。

6. 从signature中获取签名，校验其签名。

7. 从payload中获取涉诈剧本检测结果，格式和样例摘录如下：
...
8. 当请求参数FraudDetectionRequest中的version为2，Payload样例：
