...
4. 从header中获取证书链，使用[Huawei CBG Device Attestation Root CA](https://pki.consumer.huawei.com/ca/cer/Huawei_CBG_ECC_Device_Attestation_Root_CA.cer)证书对其进行验证。

5. 校验证书链中x5c[0]证书的Common Name是否为Harmony OS Device Attestation Service。

6. 从signature中获取签名，校验其签名。

7. 从payload中获取模拟点击增强检测结果，格式和样例摘录如下：
