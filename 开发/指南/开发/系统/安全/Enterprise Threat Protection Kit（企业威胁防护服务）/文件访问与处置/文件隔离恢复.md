...
2. 调用接口[restoreIsolatedFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/erprisethreatprotection-virusremediation-interface#restoreisolatedfile)，实现对指定隔离ID的文件恢复并获取恢复路径。id参数为隔离文件的唯一标识符，可通过调用[queryIsolatedFiles](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/erprisethreatprotection-virusremediation-interface#queryisolatedfiles)接口获取。
