...
| 接口 | 描述 |
| --- | --- |
| openFile(path: string): Promise | 获取指定路径文件的文件描述符fd。 |
...
通过调用接口[openFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/erprisethreatprotection-virusremediation-interface#openfile)，获取目标文件的文件描述符（fd）。path参数为目标文件的绝对路径。
```text
