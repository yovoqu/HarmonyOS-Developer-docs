...
| 接口 | 描述 |
| --- | --- |
| queryIsolatedFiles(callback: [QueryCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/erprisethreatprotection-virusremediation-interface#querycallback), batchNum?: number): void | 获取已隔离文件的文件信息。 |
...
通过声明查询结果回调queryCallback，并调用[queryIsolatedFiles](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/erprisethreatprotection-virusremediation-interface#queryisolatedfiles)接口，获取已隔离文件的文件信息。
```text
