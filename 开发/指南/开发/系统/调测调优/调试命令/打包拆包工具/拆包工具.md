...
开发者可以使用拆包工具的jar包对应用进行拆包，通过传入拆包选项、文件路径，将HAP包解压出来。 **示例**
```text
java -jar app_unpacking_tool.jar --mode hap --hap-path  --out-path  [--force true]
```
...
开发者可以使用拆包工具的jar包对应用进行拆包，通过传入拆包选项、文件路径，将App包解压出来。 **示例**
```text
java -jar app_unpacking_tool.jar --mode app --app-path  --out-path  [--force true]
```
...
开发者可以使用拆包工具的jar包对应用进行拆包，通过传入拆包选项、文件路径，获取应用的rpcid。 **示例**
```text
java -jar app_unpacking_tool.jar --mode hap --rpcid true --hap-path  --out-path  [--force true]
```
...
开发者可以使用拆包工具将hap包按照libs包含的架构类型拆包再打包，得到若干仅包含单架构类型库的hap包。 **示例**
```text
java -jar app_unpacking_tool.jar --mode hap --hap-path  --out-path  [--force true] [--libs true] [--cpu-abis option]
```
...
开发者可以使用拆包工具的jar包对应用进行拆包，通过传入拆包选项、文件路径，将HSP包解压出来。 **示例**
```text
java -jar app_unpacking_tool.jar --mode hsp --hsp-path  --out-path  [--force true]
```
...
开发者可以使用拆包工具将hsp包按照libs包含的架构类型拆包再打包，得到若干仅包含单架构类型库的hsp包。 **示例**
```text
java -jar app_unpacking_tool.jar --mode hsp --hsp-path  --out-path  [--force true] [--libs true] [--cpu-abis option]
```
...
开发者可以使用拆包工具的jar包对应用进行拆包，通过传入拆包选项、文件路径，将APPQF包解压出来。 **示例**
```text
java -jar app_unpacking_tool.jar --mode appqf --appqf-path  --out-path  [--force true]
```
...
| packInfos | List | bundle中pack.info文件的packages信息。 | NA |
| profileInfos | List | 应用的配置信息。 | NA |
| profileInfosStr | List | 应用的配置信息。 | NA |
...
| deviceType | List | 表示当前HAP包所支持的设备类型。 | NA |
...
| deviceConfig | Map | 标识设备信息。 | 存储类型为Map，存储设备类型名称及对应设备类型的信息，在stage模型中，这个字段存储在app结构体中。 |
...
| labels | HashMap | 标识多语言应用程序AppJson的标签。 | NA |
| descriptions | HashMap | 标识多语言应用程序AppJson的说明。 | NA |
...
| supportedModes | List | 标识HAP包的支持的模式。 | NA |
| abilities | List | 标识HAP包ability信息。 | NA |
| defPermissions | List | 标识HAP包DefPermission信息。 | NA |
| definePermissions | List | 标识HAP包DefinePermission信息。 | NA |
| defPermissionsGroups | List | 标识HAP包DefPermissionsGroups 信息。 | NA |
| distro | Distro结构体 | 标识HAP包的distro信息。 | NA |
| reqCapabilities | List | 标识HAP包reqCapabilities信息。 | NA |
| deviceType | List | 标识HAP可以运行在哪类设备上。 对应stage模型中的deviceTypes。 | NA |
| metaData | MetaData结构体（见下述metaData） | 标识HAP的自定义元信息。 | NA |
| dependencies | List | 标识HAP包DependencyItem信息。 | NA |
| isJs | boolean | 标识该应用是否是js应用。true表示是js应用，false表示不是js应用。 | FA模型特有。 |
| reqPermissions | list | 标识应用申请的权限的集合。 | 对应stage模型的requestPermissions。 |
| commonEvents | CommonEvent结构体（见下述CommonEvent） | 标识静态事件。 | NA |
| shortcuts | list | 标识应用的shortcuts信息。 | NA |
...
| pages | List | 列举JS Component中每个页面信息。 | stage模型新增。 |
| extensionAbilityInfos | List | 描述extensionAbility的配置信息。 | stage模型新增。 |
| moduleAtomicService | ModuleAtomicService结构体（见下述ModuleAtomicService） | 描述HAP的元服务信息。 | NA |
| formInfos | List | 描述卡片的信息。 | NA |
| descriptions | HashMap | 标识HAP的说明信息。 | NA |
...
| requiredDeviceFeatures | Map> | 标识当前Module运行所需要的特定的设备特性。详细请参见[requiredDeviceFeatures标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#requireddevicefeatures标签) | 从API version 24开始支持。 |
...
| configChanges | List | 标识ability中的configChanges信息。 | NA |
| formInfos | List | 标识ability中的forms信息。 | NA |
| permissions | List | 标识ability中的permissions信息。 | NA |
| skills | List | 标识ability中的skills信息。 | NA |
| backgroundModes | List | 标识ability中的backgroundModes信息。 | NA |
| labels | HashMap | 标识多语言下ability对用户显示的名称。 | NA |
| descriptions | HashMap | 标识多语言下ability的描述。 | NA |
...
| 字段 | 类型 | 描述 | 备注 |
| --- | --- | --- | --- |
| parameters | List | 标识Metadata的参数信息。 | FA模型特有，stage模型废弃。 |
| results | List | 标识Metadata的results信息。 | FA模型特有，stage模型废弃。 |
| customizeDatas | List | 标识Metadata的customizeDatas信息。 | NA |
...
| reasons | HashMap | 当申请的权限为user_grant权限时此字段必填，描述申请权限的原因。 | NA |
...
| 字段 | 类型 | 描述 | 备注 |
| --- | --- | --- | --- |
| ability | List | 标识需要使用到该权限的元能力（ability），该标签值为数组形式。 | NA |
...
| intents | List | 标识快捷方式内定义的目标intent信息集合，每个intent可配置两个子标签，targetClass,targetBundle。 | NA |
| labels | HashMap | 标识多语言下ShortCut对用户显示的名称。 | NA |
...
| permissions | List | 标识被其它应用的ability调用时需要申请的权限的集合。 | stage模型支持。 |
...
| skills | List | 标识extensionAbility能够接收的意图的特征集。 | stage模型支持。 |
| metadataInfos | List | 标识extensionAbility能够接收的元数据信息。 | stage模型支持。 |
...
| descriptions | HashMap | 标识多语言下extensionAbility的描述。 | NA |
| labels | HashMap | 标识多语言下extensionAbility对用户显示的名称。 | NA |
...
| 字段 | 类型 | 描述 | 备注 |
| --- | --- | --- | --- |
| actions | List | 标识能够接收的意图的action值的集合。 | NA |
| entities | List | 标识能够接收的意图的元能力的类别集合。 | NA |
...
| supportDimensions | List | 标识卡片外观规格，取值为”1 * 2“，”2 * 2“，”2 * 4“，”4 * 4“。 | NA |
...
| data | List | 当前静态公共事件需要携带的额外数据数组。 | Stage模型从staticSubscriber类型的Extension中获取。 |
| type | List | 配置当前静态公共事件的类别数组。 | Stage模型从staticSubscriber类型的Extension中获取。 |
| events | List | 标识能够接收的意图的event值的集合。 | Stage模型从staticSubscriber类型的Extension中获取。 |
...
| availableScope | List | 标识DefPermission的可用范围。 | NA |
| labels | HashMap | 标识多语言应用程序DefPermission的标签。 | NA |
| descriptions | HashMap | 标识多语言应用程序DefPermission的说明。 | NA |
...
| descriptions | HashMap | 标识多语言应用程序DefinePermission的说明。 | NA |
| labels | HashMap | 标识多语言应用程序DefinePermission的标签。 | NA |
...
| 字段 | 类型 | 描述 | 备注 |
| --- | --- | --- | --- |
| formEntity | List | 标识formInfo的formEntity | NA |
