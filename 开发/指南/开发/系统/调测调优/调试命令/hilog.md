...
> [!NOTE]
> 日志级别：I表示Info级别，其余级别参考 日志等级 首字母。 domainID：A03200中A表示应用日志（LOG_APP），3200表示domainID为0x3200，domainID具体定义详见接口指导。 进程名：最大长度为30字节，超出后会从开头开始截断，保留尾部完整内容。 日志tag：最大长度为31字节，超出后会从末尾开始截断，保留前部完整内容，不建议使用中文字符，可能出现乱码或者对齐问题。
