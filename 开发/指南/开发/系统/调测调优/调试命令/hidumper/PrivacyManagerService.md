...
Usage:
       -h: command help
       -t : according to specific token id dump permission used records
```
...
```text
hidumper -s PrivacyManagerService -a '-t '
```

 命令所需的tokenId可以通过[atm-tool](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/atm-tool#查询命令)进行查询。  **使用样例：**
```text
