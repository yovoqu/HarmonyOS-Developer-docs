...
> [!NOTE]
> 抓取采样栈有内存开销，影响功耗性能，只建议开发者在本地调试时使用自定义参数功能。 此外，Debug版本应用，采样栈每日采集次数阈值范围为：[-1, 100]； Release版本应用，采样栈每日采集次数阈值范围为阈值范围：[0, 20]，详见 CpuUsageHighPolicy 。
...
> [!NOTE]
> 默认状态下，CPU高负载事件 抓取次数存在限制 。 开发者如果未抓取到CPU高负载事件external_log日志，可以尝试重启测试设备重新抓取或自定义相关配置策略。
