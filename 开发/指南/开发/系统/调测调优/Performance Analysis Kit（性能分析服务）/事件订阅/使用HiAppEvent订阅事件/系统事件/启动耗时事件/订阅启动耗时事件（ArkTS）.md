...
2. **不建议在onReceive中执行任何关键业务逻辑**（例如移除事件观察者、资源释放等操作），否则可能导致业务代码重复执行、资源被多次释放，从而引发程序错误甚至应用崩溃。
3. 建议开发者将 onReceive 仅用于事件的接收和简单记录。
4. 点击DevEco Studio界面中的运行按钮，运行应用工程，添加系统事件订阅者，退出应用，再次点击桌面应用图标，触发一次启动耗时事件。
5. 应用工程再次启动可以在Log窗口看到对系统事件数据的处理日志：
