...
1. Native代码未处理崩溃信号时，会生成NativeCrash类型崩溃事件。
...
3. ProcessDump进程收集完崩溃信息后，上报给Hiview进程。Hiview进程将事件信息存储到应用沙箱目录。
...
#### 参数设置示例
OH_HiAppEvent_SetEventConfig配置参考[订阅崩溃事件（C/C++）开发步骤](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-crash-events-ndk#开发步骤)完成崩溃事件订阅和日志配置参数设置，然后通过[external_log](#params字段说明)字段获取NativeCrash类型崩溃拼接应用日志内容。

> [!CAUTION] 说明
> 

![](assets/崩溃事件介绍/file-20260525091637608-001.png)
> 沙箱路径下必须有应用生成的拼接日志。
