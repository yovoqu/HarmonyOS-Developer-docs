...
> [!NOTE]
> 启动耗时事件支持在 元服务场景 下使用HiAppEvent进行订阅。不支持在 应用分身场景 或 输入法应用场景 下使用HiAppEvent进行订阅。
...
| abilityonforeg_startwindow_dur | number | 系统完成创建窗口管理器WindowStage（系统触发onWindowStageCreate()回调）到UIAbility切换到前台的耗时，单位为ms。 仅在冷启动和预加载启动中存在该值，若依赖的系统内部事件打点次数超限或在热启动类型中，该值为0。从API version 22开始提供。 |
