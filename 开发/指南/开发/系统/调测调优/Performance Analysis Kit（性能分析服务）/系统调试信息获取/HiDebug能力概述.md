...
| 接口名 | 描述 |
| --- | --- |
| hidebug.setProcDumpInSharedOOM | 当发生JS OOM的内存类型为SharedHeap，如果应用已经调用过该接口，且传参为true，那么转储的堆快照将会由线程级别变成进程级别。通过订阅资源泄漏事件（ArkTS）获取对应日志。 该接口仅影响SharedHeap发生OOM时转储的堆快照，不影响其他情况下转储的堆快照。 当应用需要定位JS泄漏问题时，建议总是调用该接口并传参为true。 说明：从API version 24开始，支持该接口。 |
