...
下文展示如何在应用内使用HiDebug NDK接口以进行线程栈回溯，且获取进程内线程的CPU使用率： 步骤一：创建项目 使用DevEco Studio新建一个Native C++工程，并新增文件“test_backtrace.cpp”与“test_backtrace.h”，目录结构如下：
```text
entry:
  src:
    main:
...
{
    if (size (frame.frame.js.relativePc),
            frame.frame.js.line,
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "testTag",
            ...
            "reserved: %{public}s.",
            pc,
            reinterpret_cast(frame.frame.native.relativePc),
            reinterpret_cast(frame.frame.native.funcOffset),
...
void BackTraceObject::SymbolicAddress(int index)  // 栈解析接口。
{
    ...
    OH_HiDebug_SymbolicAddress(backtraceObject_, pcs_[index], this,
        [] (void* pc, void* arg, const HiDebug_StackFrame* frame) {
            reinterpret_cast(arg)->PrintStackFrame(pc, *frame);
...
void BacktraceCurrentThread() // 该接口非线程安全，同一时刻只能由一个线程使用。
{
    ...
    for (int i = 0; i 编辑“CMakeLists.txt”文件，添加库依赖：
```text
...
```
  ...
  编辑“Index.ets”文件，添加触发接口调用的按钮，示例代码如下：  导入依赖：
```text
...
```
   ...
   步骤二：运行工程 点击DevEco Studio界面中的运行按钮，然后分别单击应用界面上的“testGetThreadCpuUsage”和“testHiDebugBackTrace”按钮。  在DevEco Studio底部切换到“Log”窗口，设置日志过滤条件为“testTag”，即可查看相关日志：
```text
...
