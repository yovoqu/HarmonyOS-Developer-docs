...
8. 通过[Smartperf_Host](https://gitcode.com/openharmony/developtools_smartperf_host/blob/master/smartperf_host/README_zh.md)工具进行分析。
...
> [!NOTE]
> 对于API version 19及之后接口，支持自定义输出级别；对于API version 18及之前接口，trace打点默认为COMMERCIAL级别打点。 系统侧存在一条用户态trace隶属于多个Tag的情况，ArkTS接口 @ohos.hiTraceMeter (性能打点) 和C++接口 trace.h 提供的应用侧用户态trace打点功能无法指定所属Tag集合，对应Tag分类固定为app，字段值为62。
