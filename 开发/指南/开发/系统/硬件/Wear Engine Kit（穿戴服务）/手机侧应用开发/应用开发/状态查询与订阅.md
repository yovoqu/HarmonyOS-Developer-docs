...
> [!NOTE]
> 该接口的调用需要在开发者联盟申请设备基础信息权限与穿戴用户状态权限（请参考 申请接入Wear Engine服务 ），穿戴用户状态权限还需获得用户授权。
...
> [!NOTE]
> 穿戴设备侧无对应的应用，手机侧应用也可以使用该能力获取穿戴设备状态。 在查询或订阅穿戴设备电量、充电、佩戴、心率告警状态时，请确保穿戴设备和华为运动健康App处于连接状态。用户可进入App“设备”界面查看设备是否在线。开发者可调用 getConnectedDevices 或根据返回错误码了解设备是否已连接手机，如果设备未连接则提醒用户重新连接设备。 查询和订阅佩戴状态、心率告警时，需要申请穿戴用户状态 USER_STATUS 权限。
...
5. “充电状态”字段：CHARGE_STATUS。
6. “佩戴状态”字段：WEAR_STATUS。
7. “设备模式”字段：POWER_MODE。
8. “可用内存”字段：AVAILABLE_STORAGE_SPACE
9. 通过[MonitorData](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#monitordata)对象，返回指定指标状态的查询结果。         
```text
...
6. “电量降低状态”字段：EVENT_BATTERY_LEVEL_DROPPED。
7. “充电状态”字段： EVENT_CHARGE_STATUS_CHANGED。
8. “佩戴状态”字段：EVENT_WEAR_STATUS_CHANGED。
9. “心率告警”字段：EVENT_HEART_RATE_ALARM。
10. “设备模式”字段：EVENT_POWER_MODE_CHANGED。
11. 通过[Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)<[MonitorEventData](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#monitoreventdata)>对象，返回设备状态的订阅结果。
12. 调用[unsubscribeEvent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#unsubscribeevent)方法，解除穿戴设备状态变化的订阅。（需要传入订阅时使用的回调函数对象）
