...
3. 配置srcEntry为HiWearMainAbility.ets文件的路径。
4. 配置startWindowIcon为标识当前UIAbility组件启动页面图标资源文件的索引。
5. 配置startWindowBackground为标识当前UIAbility组件启动页面背景颜色资源文件的索引。         
```text
"module": {
  "name": "xxxx",
  "type": "entry",
...
6. 在module.json5中的metadata中配置对端应用包名、待拉起的Ability名称，以及是否要等待穿戴侧完成订阅消息接收或者文件接收。

  
对端应用包名配置时，name为wearEngineRemoteAppNameList，value为具体的对端应用包名，如果存在多个应用包名，使用英文逗号隔开，value值最长不超过4KB；
7. Ability名称配置时，name为wearEngineUIAbilityName，value为指定要拉起的UIAbility名称，不配置默认拉起HiWearMainAbility；
8. AwaitRegisterReceiver配置时，name为wearEngineAwaitRegisterReceiver，value取值为true、false。true表示要等待穿戴侧完成订阅消息接收或者文件接收，false则反之。         
```text
"module": {
  "name": "xxxx",
  "type": "entry",
...
> [!NOTE]
> 该接口的调用需要在开发者联盟申请拉起已配对设备的指定应用权限（请参考 申请接入Wear Engine服务 ）。
