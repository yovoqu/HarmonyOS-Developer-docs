...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-tracking-guide
...
| 接口名 | 描述 |
| --- | --- |
| on(type: 'attachStateChange', callback: Callback): void | 注册attachStateChange事件的回调监听，等待连接状态变化。          说明：从API version 20开始支持。 |
| off(type: 'attachStateChange', callback?: Callback): void | 取消注册attachStateChange事件的回调监听。          说明：从API version 20开始支持。 |
...
| on(type: 'trackingStateChange', callback: Callback): void | 注册trackingStateChange事件的回调监听。          说明：从API version 20开始支持。 |
| off(type: 'trackingStateChange', callback?: Callback): void | 取消注册trackingStateChange事件的回调监听。          说明：从API version 20开始支持。 |
...
| on(type: 'rotationAxesStatusChange', callback: Callback): void | 注册rotationAxesStatusChange事件的回调监听。          说明：从API version 20开始支持。 |
| off(type: 'rotationAxesStatusChange', callback?: Callback): void | 取消注册rotationAxesStatusChange事件的回调监听。          说明：从API version 20开始支持。 |
...
支持Mechanic Kit协议的机械体设备。 若要验证目标跟踪功能，主设备的相机驱动必须支持人脸检测。 请将SDK更新到API 20或以上版本，具体操作参见[更新指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-software-install)。 请确保机械体设备已通过蓝牙与主设备连接。

## 管理设备连接状态

确保机械体设备连接或断开时，应用能及时响应，支持设备连接状态的动态管理。 导入机械体设备管理模块。
```text
...
启用目标拍摄功能后，设备将自动识别人脸并进行跟踪拍摄。 启用摄像头的目标拍摄功能。
```text
try {
...
请按照以下步骤调试验证，确保机械体设备管理功能正常： **建立连接** 确保机械体与开发设备已通过蓝牙配对并连接。 将开发设备放置在机械体设备上。 **功能验证步骤** **设备列表查询**：调用 getAttachedMechDevices 接口查询当前已连接的机械体设备列表，验证设备是否正确识别。 **目标拍摄跟踪**：调用 setCameraTrackingEnabled 启用摄像头目标跟踪功能，使用 getCameraTrackingEnabled 验证状态，测试设备是否能跟随目标自动旋转。 **验证结果说明** 如果 getAttachedMechDevices 返回设备列表，表示设备识别成功。 如果 getCameraTrackingEnabled 返回真，目标拍摄跟踪启用成功。应用打开相机后，画面中出现人脸时，设备会跟随人脸转动。
