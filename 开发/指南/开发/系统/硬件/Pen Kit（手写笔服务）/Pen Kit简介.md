...
- 预置图形工具（26.0.0新增）：支持展开二级菜单，调整图形的类型、线条粗细，支持通过调色板调整图形线条颜色，图形参数信息支持记录保持。图形样式包含矩形、圆形、箭头、直线，图形线条粗细可选1~10。默认宽度为4。图形绘制效果默认使用圆珠笔笔刷。
- 手写波轮（26.0.0新增）：环形工具栏，支持笔刷、笔宽、颜色、透明度等属性的快速调节。通过轻捏手写笔笔身，隐藏原本工具栏，展开环形工具栏，再次轻捏笔身，切换回原本的工具栏。
- 其他功能：撤销、重做、禁止手指书写。
