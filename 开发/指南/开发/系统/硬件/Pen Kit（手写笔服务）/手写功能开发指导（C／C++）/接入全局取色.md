...
在应用中拉起全局取色，效果如下：
![](assets/接入全局取色/file-20260514131341483-0.png)
...
native_gcp_api.h提供HMS_GCP_StartColorPicker()和HMS_GCP_StartColorPickerWithColorValue()两种方式启动全局取色功能。 通过调用HMS_GCP_StartColorPicker()，实现启动取色器。
![](assets/接入全局取色/file-20260514131341483-1.png)
该启动方法不支持实时显示色值。
```text
...
![](assets/接入全局取色/file-20260514131341483-2.png)
该启动方法支持实时显示色值。
```text
void onSuccessCallback(void *userData, HMS_GCP_PickedColorInfo pickedColorInfo, const int32_t code) {
    OH_LOG_Print(LOG_APP, LOG_INFO, 0x0000, "penkit", "picked code is: %{public}d", code);
