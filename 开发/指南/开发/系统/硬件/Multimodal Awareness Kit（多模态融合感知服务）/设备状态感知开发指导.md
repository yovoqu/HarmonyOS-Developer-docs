...
在进行设备状态感知模块的使用前，开发者应了解以下基本概念： 支架态 设备进入支架态指设备静止，且屏幕与水平面角度处于45度-135度。折叠屏手机需处于折叠状态或者完全展开状态。
...
开发过程中，需要订阅设备静止姿态（支架态）感知，并且通过订阅时传入的回调函数来获取到当前的状态值。 从API version 18开始，支持获取设备静止姿态（支架态）。
...
| 接口名 | 描述 |
| --- | --- |
| on(type: 'steadyStandingDetect', callback: Callback): void; | 订阅设备静止姿态（支架态）感知，结果通过callback返回。 |
| off(type: 'steadyStandingDetect', callback?: Callback): void; | 取消订阅设备静止姿态（支架态）感知。 |
...
```text
// 定义callback变量
let callback : Callback = (data : deviceStatus.SteadyStandingStatus) => {
   console.info('succeed to get status, now status = ' + data);
};
