...
调用HMS_FAST_RectPartition_CreateConfig生成矩形划分求解器配置实例（FAST_RectPartitionConfig）。 调用HMS_FAST_RectPartition_SetAlgo设置求解算法为“SweepLineAlgo”（扫描线算法）。 调用HMS_FAST_RectPartition_Solve计算矩形划分方案。 调用HMS_FAST_RectPartition_DestroyConfig销毁矩形划分求解器配置实例。
```text
#include
#include
