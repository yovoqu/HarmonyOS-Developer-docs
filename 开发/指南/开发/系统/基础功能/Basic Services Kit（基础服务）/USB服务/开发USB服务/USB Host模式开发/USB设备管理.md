...
USB设备管理主要提供的功能有：查询USB设备列表、USB设备权限控制、设置USB设备配置等。 USB类开放能力如下，具体请查阅[@ohos.usbManager](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-usbmanager)。 **表1** USB类的开放能力接口
| 接口名 | 描述 |
| --- | --- |
...
| getDevices(): Array> | 获取接入主设备的USB设备列表。如果没有设备接入，那么将会返回一个空的列表。 |
...
USB设备可作为Host连接Device进行设备管理，开发示例如下： 导入模块。
```text
// 导入usbManager模块
