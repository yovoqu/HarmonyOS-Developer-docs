...
需同时满足以下条件，才能使用该功能： **设备限制**  本端设备：HarmonyOS版本为HarmonyOS NEXT及以上的Tablet或PC/2in1设备。远端设备：HarmonyOS版本为HarmonyOS NEXT及以上、具有相机能力的Phone或Tablet设备。  **使用限制**  双端设备需要登录同一华为账号。跨设备互通API支持根据特定调用策略调用设备。调用策略：PC/2in1设备可以调用Tablet和Phone，Tablet可以调用Phone，同类型设备不可调用。双端设备需要打开WLAN和蓝牙开关。条件允许时，建议双端设备接入同一个局域网，可提升唤醒相机的速度。
