...
#### 开发步骤
1. 导入相关模块。 import { advertising } from '@kit.NearLinkKit';
import { BusinessError } from '@kit.BasicServicesKit';
2. 订阅星闪广播状态变化事件。 let onReceiveEvent:(data: advertising.AdvertisingStateChangeInfo) => void =
...
3. 构造用户需要的广播参数及数据。 let manufactureValueBuffer = new Uint8Array(4);
...
4. 开启星闪广播，返回advertisingId表示当前广播索引。 let advId = -1;
try {
...
5. 停止星闪广播，其中advId是步骤4开启广播后返回的advertisingId。 try {
