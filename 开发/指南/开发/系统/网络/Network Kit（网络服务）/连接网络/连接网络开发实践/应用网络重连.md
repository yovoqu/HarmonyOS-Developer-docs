...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-network-reconnection
...
- [网络超时重连](#网络超时重连)：客户端向服务器发送请求后，如果发生网络超时，那么客户端将自动尝试与服务器重新建立连接。
- [网络切换重连](#网络切换重连)：当网络连接发生变化后，客户端应用可能会出现网络异常。应用需要检测网络状态变化，并根据网络状态进行连接。
- [应用前后台切换后重连](#应用前后台切换后重连)：应用切换到后台一段时间后，网络资源会被冻结释放，需要客户端重新建立连接。
...
```text
async getHttpRequest(url: string, retry: number): Promise {
  try {
...
async getRcpRequest(url: string, retry: number): Promise {
  try {
