...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/network-connection-security-configuration
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9d/v3/ZlvgkLtQQe2NOZzifjN05A/zh-cn_image_0000002659100385.png?HW-CC-KV=V1&HW-CC-Date=20260701T025435Z&HW-CC-Expire=86400&HW-CC-Sign=2D323EFEBCE6CEA31B6EBC5267628824FB341613351022AFA2E4296C916CEA56)
...
- 面向互联网用户的应用，建议只[配置信任系统预置的CA证书](#配置信任系统预置的ca证书)。
- 只访问企业内部服务器的应用，建议只[配置信任应用管理的CA证书](#配置信任应用管理的ca证书)。
- 同时访问企业内部服务器和互联网服务的应用，建议根据访问的服务器域名分别配置[信任系统预置的CA证书](#配置信任系统预置的ca证书)或[应用管理的CA证书](#配置信任应用管理的ca证书)。
- 应用开发和调测过程中需要进行网络抓包以定位问题和测试，建议仅在调测版本[配置信任用户安装的CA证书](#配置信任用户安装的ca证书)。
- 应用需支持通过企业代理服务器访问应用服务器，可信任企业MDM系统或设备管理员用户手动安装[CA证书](#配置信任用户安装的ca证书)。然而，设备管理员用户可能通过代理工具抓取应用网络数据，建议在应用层实施安全防护，如对敏感数据进行二次加密或签名。
- 对于需要较高网络安全的应用（如金融支付、银行类应用），配置CA证书后，通过[SSL Pinning方式](#配置ssl-pinning证书锁定)绑定服务器证书的公钥，以增强安全性。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fe/v3/q4WJqHxxSzChfonNX7hvxg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025435Z&HW-CC-Expire=86400&HW-CC-Sign=F7281E9368234A76085EA7119698E54338AE51305862E2959D5213EF1F659530)
 
 
系统Network Kit和Remote Communication Kit的HTTPS连接默认信任系统预置的CA证书和用户安装的CA证书，可[配置不信任用户安装的CA证书](#配置不信任用户安装的ca证书)提升安全性。
...
- Network Kit和Remote Communication Kit可以通过src/main/resources/base/profile/network_config.json文件进行配置。例如，可以将应用级信任的CA证书预置到/data/storage/el1/bundle/entry/resources/resfile/appCaCert目录，将特定域名信任的CA证书预置到/data/storage/el1/bundle/entry/resources/resfile/domainCaCert目录。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/09/v3/wOlEfp4DTf2A6CIMSoI08A/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025435Z&HW-CC-Expire=86400&HW-CC-Sign=E85F27609D074B40A0519021C72178678D5CDDA4D22595F28905F538B60CBFD3)
 
 
系统Network Kit和Remote Communication Kit在完成上述配置后，HTTPS连接仍信任系统预置的CA证书和用户安装的CA证书。如需提升安全性，可[配置不信任用户安装的CA证书](#配置不信任用户安装的ca证书)。
...
- 应用开发和调测过程中需要进行网络抓包以定位问题和测试。用户通过系统的设置界面安装的CA证书，保存在目录：/data/certificates/user\_cacerts/\{userid\}，其中userid从100开始。
  

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/92/v3/PI4YHY6OTtSDK4KLRZuf1g/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025435Z&HW-CC-Expire=86400&HW-CC-Sign=8C47621D086B2BAD480B2A7A0C671C406379C83020CF1B898B0A263AE6D2DE92)
 
 正式发布的应用版本应默认不信任用户安装的CA证书。
- 面向2B企业应用的场景，应用需支持通过企业代理服务器访问应用服务器。设备需通过企业的MDM系统或设备管理员用户手工安装CA证书，并保存在目录：/data/certificates/user\_cacerts/0。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/da/v3/cnmUDFpVSBitVIn7o5znxg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025435Z&HW-CC-Expire=86400&HW-CC-Sign=47369422D0715FE373660E993382A146598CD9CCF10CCC8DE8ECB5E816DBC966)
