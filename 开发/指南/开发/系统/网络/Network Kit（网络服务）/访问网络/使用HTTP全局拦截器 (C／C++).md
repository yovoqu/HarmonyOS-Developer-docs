...
#### 构建工程
1. 在源文件中编写调用该API的代码，实现HTTP全局拦截器的处理函数和相关操作。 #include "napi/native_api.h"
...
2. 初始化并导出通过N-API封装的napi_value类型对象，通过外部函数接口将函数提供给JavaScript调用。 EXTERN_C_START
...
EXTERN_C_END
3. 将上一步中初始化成功的对象通过RegisterEntryModule函数，使用napi_module_register函数将模块注册到Node.js中。 static napi_module demoModule = {
 .nm_version = 1,
 .nm_flags = 0,
 .nm_filename = nullptr,
...
4. 在工程的Index.d.ts文件中定义函数的类型。 export const AddResponseInterceptor: () => number;
...
5. 在Index.ets文件中对上述封装好的接口进行调用。 import { hilog } from '@kit.PerformanceAnalysisKit';
...
6. 配置CMakeLists.txt，本模块需要用到的共享库是libhttp_interceptor.so，在工程自动生成的CMakeLists.txt中的target_link_libraries中添加此共享库。 注意：如图所示，在add_library中的entry是工程自动生成的module name，若要做修改，需和步骤 3 中.nm_modname保持一致。
