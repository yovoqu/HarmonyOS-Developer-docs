...
应用通过Socket进行数据传输，支持TCP/UDP/Multicast/TLS协议。主要场景有： 在TCP/UDP传输的客户端（UDP本身并没有客户端和服务器端的明确区分，此处描述UDP传输的客户端是指主动向服务器发送数据的一方），应用通过TCP/UDP Socket进行数据传输 在TCP传输的服务器端，应用通过TCP Socket Server进行数据传输 多播通信场景，应用通过Multicast Socket进行数据传输 同一台主机上不同进程之间传输的客户端，应用通过Local Socket进行数据传输 同一台主机上不同进程之间传输的服务器端，应用通过Local Socket Server进行数据传输 数据加密传输时，客户端侧通过TLS Socket进行加密数据传输 数据加密传输时，服务器侧通过TLS Socket Server进行加密数据传输
> [!NOTE]
...
UDP与TCP流程大体类似，下面以TCP为例： 导入所需的socket模块。
```text
...
multicast.on('message', (data: SocketInfo) => {
  ...
  let str = ''
  for (let i = 0; i              发送数据，数据以广播的形式传输，同一多播组中已经开启消息message监听的多播对象都会接收到数据。
```text
// 发送一条多播消息
...
## 应用通过Local Socket Server进行数据传输
     ...
     服务端LocalSocket Server的主要流程包括：                  导入所需的socket模块。
```text
import { socket } from '@kit.NetworkKit';
import { common } from '@kit.AbilityKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
## 应用通过TLS Socket进行加密数据传输
     ...
     客户端TLS Socket流程（双向认证）包括：                  导入所需的socket模块。
```text
import { socket } from '@kit.NetworkKit';
import { BusinessError } from '@kit.BasicServicesKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
```
          ...
          客户端TLS Socket流程（单向认证）包括：                  导入所需的socket模块。
```text
import { socket } from '@kit.NetworkKit';
import { BusinessError } from '@kit.BasicServicesKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
## 应用通过将TCP Socket升级为TLS Socket进行加密数据传输
     ...
     客户端TCP Socket升级为TLS Socket流程，以TLS Socket双向认证为例：                  导入所需的socket模块。
```text
import { socket } from '@kit.NetworkKit';
import { BusinessError } from '@kit.BasicServicesKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
## 应用通过TLS Socket Server进行加密数据传输
     ...
     服务端TLS Socket Server流程：                  导入所需的socket模块。
```text
import { socket } from '@kit.NetworkKit';
import { BusinessError } from '@kit.BasicServicesKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
