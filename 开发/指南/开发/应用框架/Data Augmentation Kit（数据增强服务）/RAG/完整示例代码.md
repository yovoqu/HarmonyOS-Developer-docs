...
export default class EntryAbility extends UIAbility {
  ...
  onWindowStageCreate(windowStage: window.WindowStage): void {
    ...
    setUp.initTable().then(() => {
      setUp.insertData();
      AppStorage.setOrCreate('SetUpObject', setUp);
    });
  ...
  onWindowStageDestroy(): void {
    ...
    const session = AppStorage.get('RagSessionObject') as rag.RagSession;
    session?.close().catch(() => {
      hilog.error(DOMAIN, 'testTag', 'close rag session failed');
    });
    const setup = AppStorage.get('SetUpObject') as SetUp;
    setup?.closeStore();
  }
...
export default class SetUp extends UIAbility {
  ...
  async getStore() {
    try {
      if (!this.store) {
        let context = AppStorage.get('Context') as common.UIAbilityContext; // 获取全局context
    ...
    try {
      ...
      for (let file of fileList) {  // 从json文件中解析数据到数据库中
        ...
        try {
          ...
          const resultObjArr = JSON.parse(fileData) as Array;
...
let chunk = '';
let isFinished: boolean = (str.length ): Promise {
