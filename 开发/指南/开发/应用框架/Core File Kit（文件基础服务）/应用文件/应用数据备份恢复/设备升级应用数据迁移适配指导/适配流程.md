...
应用的“BackupExtensionAbility”执行完后，“备份恢复框架”会清空备份恢复目录，开发者请在应用的“BackupExtensionAbility”执行结束前，完成所有所需数据的转换和迁移。   后续HarmonyOS应用通过访问HarmonyOS应用沙箱获取应用的数据。
...
开发者完成应用适配后，需要完成自验证HarmonyOS应用数据迁移适配结果。更多自验证指导可参考：[开发者自验证](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/self-verification)。 其中自验证所需“迁移调试”工具获取方式如下： HarmonyOS NEXT Developer Beta1及之后版本，厂商合作伙伴可通过[IssueReporter平台](https://issuereporter.developer.huawei.com/my-created)提交工单，向华为方技术支持人员申请“迁移调试”工具，模拟进行数据迁移验证。公共开发者请通过华为开发者联盟官网->支持，[在线提单](https://developer.huawei.com/consumer/cn/support/)方式获取。  HarmonyOS NEXT Developer Beta1之前版本，开发者可以通过系统内置“迁移调试”工具，模拟进行数据迁移验证。
