...
API的详细介绍请参见[ohos.file.statvfs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-statvfs)、[ohos.file.storageStatistics](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-storage-statistics)。 **表1** 文件系统空间和应用空间统计
| 模块 | 接口名 | 功能 |
| --- | --- | --- |
...
**表2** 应用空间统计
![](assets/应用及文件系统空间统计/file-20260514130905484-0.png)
表格中统计路径列涉及的目录均指应用的沙箱路径，查看路径前需要先进入对应的应用沙箱空间。进入沙箱空间需要执行以下命令： hdc shell。 nsenter -t {pid} -m sh。
