...
从API版本23开始，系统支持配置共享目录。在[应用文件分享](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-app-file)场景中，开发者可[配置共享目录](#配置共享目录)，配置后仅共享目录中的文件可供用户分享给其他应用查阅，以防止应用敏感数据泄露。配置仅在手机（Phone）、平板（Tablet）上生效。
...
| 属性名称 | 含义 | 数据类型 | 必填 |
| --- | --- | --- | --- |
| path | 共享路径配置，路径第一级支持el1~el5级别加密目录，第二级仅支持下列目录： - base - distributedfiles - cloud 详细限制见路径限制说明。 起始版本： 26.0.0 | string | scopes存在时必填 |
