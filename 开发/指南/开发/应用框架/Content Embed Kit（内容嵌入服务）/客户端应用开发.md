...
OE客户端应用指嵌入其他文档的应用，通过调用OE框架层[content_embed_proxy.h](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-content-embed-proxy-h)提供的接口实现嵌入外部文档、展示文档快照，以及按需启动OE服务端应用编辑文档的功能。
...
在使用接口前，需先确认设备具备SystemCapability.ContentEmbed.ObjectEditor系统能力，判断方式请参阅[canIUse()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/init#caniuse)接口查询指定的系统能力是否被支持。并申请ohos.permission.CONNECT_OBJECTEDITOR_EXTENSION权限，配置方式请参阅[声明权限](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/declare-permissions)。
