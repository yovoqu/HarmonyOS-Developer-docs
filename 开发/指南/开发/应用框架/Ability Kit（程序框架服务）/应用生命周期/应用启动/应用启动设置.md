...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-startup-options
...
一个应用可以运行在一个或多个进程中，一个进程中也可以运行单个或多个[UIAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-overview)组件实例。三方应用开发者开发的应用，必须包含至少一个[UIAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-overview)组件，否则没有界面对用户展示。
     
对于用户而言，只有[UIAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-overview)组件是可见的，启动应用内的第一个UIAbility组件就代表启动应用。后面的章节中，启动应用就是指启动UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的windowMode，指定窗口模式。windowMode的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。如果未指定windowMode，UIAbility将以系统默认的窗口展示形态启动。
 - 调用startAbility接口，启动目标UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的supportWindowModes，设置窗口显示模式。supportWindowModes的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的displayId，指定要显示的屏幕。displayId的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的withAnimation，控制是否有启动动效。withAnimation的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的windowLeft、windowTop、windowWidth、windowHeight属性，设置窗口位置和尺寸。相关属性的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。建议同时配置windowLeft和windowTop字段。
 - 调用startAbility接口，启动目标UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的processMode和startupVisibility，设置进程模式和可见性。相关属性的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
     
     
      
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e4/v3/ublyzQeOTJ2XforwL-9WAA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025425Z&HW-CC-Expire=86400&HW-CC-Sign=237747DA9DFE9B3771C2EBA3828B4D1D66047FC5951C2A6CC93B2ADEF23DFF4E)
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的startWindowIcon和startWindowBackgroundColor，设置启动页图标和背景色。相关属性的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
     
     
      
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/10/v3/AjtPzcAJStmoTAS9FSzpaA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025425Z&HW-CC-Expire=86400&HW-CC-Sign=9E7A9D99C879B6AE2597BC79361FD4E062532DE3F8CED03FF1FDFFF5F91BCC82)
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的completionHandler，设置启动结果回调。completionHandler的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的hideStartWindow，设置隐藏启动页。属性的说明参见[StartOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-startoptions)。
 - 调用startAbility接口，启动目标UIAbility。
     
     
      
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/38/v3/NDDmHU5FTlaHyjWxi5evDQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025425Z&HW-CC-Expire=86400&HW-CC-Sign=71DA57D7F6A445E488FFECB50F8683D320C5806812E26F301EBEF17FEA5B645C)
...
**开发步骤如下：**
 ...
 - 在启动UIAbility之前，需要先导入相关的模块。
 - 创建Want对象，指定要启动的UIAbility信息。UIAbilityContext的获取方式参见[获取UIAbility的上下文信息](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uiability-usage#获取uiability的上下文信息)。
 - 配置StartOptions的windowCreateParams，设置窗口参数。窗口参数配置详见[WindowCreateParams](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-i#windowcreateparams20)。
 - 调用startAbility接口，启动目标UIAbility。
