...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hyperstartup-application
...
##### 约束限制
 ...
 - 系统侧对快启启动功能做了权限保护处理，线上使能特性需要开发者先提交“快启启动”申请。       
        登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)，选择“开发与服务”。
 - 在项目列表选择项目，并在应用列表下选择需要申请“应用快启”功能的应用。
 - 进入“项目设置 > 开放能力管理”页面，点击“应用快启”对应的“申请”。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4c/v3/qlpTJHvLTl-i_JuxB82hLw/zh-cn_image_0000002659219273.png?HW-CC-KV=V1&HW-CC-Date=20260701T025425Z&HW-CC-Expire=86400&HW-CC-Sign=460C471B6C91493BE0CE6E5EF59226547C63D2CF28633A7A50BA2D66F5419844)

     
**包含在快启点内的流程有**：AbilityStage模块加载、[AbilityStage.onCreate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#oncreate)和UIAbility模块加载。其中，在[模块加载](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-module-side-effects)过程中将执行部分代码，包括顶层代码（top level）、so的constructor和类静态变量初始化等。
...
系统为开发者提供了新的回调接口[onAboutToCreateAbility()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onabouttocreateability24)。该接口在AbilityStage.onCreate()执行完毕后触发，但不参与快启初始化过程。无论是否开启应用快启功能，该回调均会被调用。
...
建议：
 ...
 - 将快启风险操作后置到AbilityStage.onAboutToCreateAbility。
...
```
 ...
 - 调整业务逻辑时，需要考虑时序依赖，保障业务逻辑正确性。
 - AbilityStage.onCreate()是首个包含较多业务逻辑的生命周期回调。若该回调中的业务不易拆分，建议开发者在首次适配时，将AbilityStage.onCreate()的内容整体移至[onAboutToCreateAbility()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onabouttocreateability24)。后续可通过业务前移扩大收益。
...
建议：
 ...
 - 顶层代码会随import该模块而执行，所以在时序上顶层代码与首个调用者绑定，属于隐式且不可控的调用，修改时容易造成调用时序混乱，所以建议在非顶层代码中实现业务逻辑。
 - 具体到快启启动特性，系统要求应用模块不可以在顶层代码执行快启风险操作，而是要根据业务逻辑调整到生命周期回调中执行。
...
建议：
 ...
 - 逐一检查启动过程中加载的so。constructor中不存在快启风险操作的so在AbilityStage.onCreate中进行dlopen，以扩大快启收益。
 - 存在风险操作constructor的so，需要放到UIAbility等快启点之外的生命周期中调用。
...
考虑到实际调整业务逻辑的困难，部分对外数据交互难以剥离。快启机制为开发者提供新的回调接口[onLaunchFromHyperSnap()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onlaunchfromhypersnap24)，该回调接口仅在快启启动流程（而不是快启初始化流程）中执行，正常冷启动不执行该回调。开发者可以在该回调接口中重新请求外部数据进行同步，保证数据一致性。
...
借助[setHyperSnapEnabled()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-hypersnapmanager#hypersnapmanagersethypersnapenabled)，应用可以根据需要关闭快启启动功能，直至云推开关重新开启。
...
系统对外提供了[requestRebuildHyperSnap()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-hypersnapmanager#hypersnapmanagerrequestrebuildhypersnap)接口，该接口支持应用在自己的业务逻辑中主动调用，清理当前快启初始化结果，在满足条件时重新进行快启初始化。
