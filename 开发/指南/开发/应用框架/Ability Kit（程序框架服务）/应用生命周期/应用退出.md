...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-stop
...
**场景示例**：在手机类设备上，用户按下返回键时，系统回调UIAbility的[onBackPressed()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#onbackpressed10)回调，该回调默认返回true，系统将UIAbility置于后台而不是销毁。
     
**开发实现**：开发者可以通过重写[onBackPressed()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#onbackpressed10)方法返回true来改变返回键的默认行为，例如在onBackPressed()中调用terminateSelf()，实现按下返回键时直接退出UIAbility。
...
**开发实现**：开发者可以在业务逻辑执行完毕后，通过调用[terminateSelf()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#terminateself-1)接口主动退出当前UIAbility。
     
**开发步骤**：
     
 - 在UIAbility中获取UIAbilityContext实例。
 - 调用terminateSelf()接口退出UIAbility。
