...
1. 定义数据库中的数据格式，可以使用Sendable，以减少跨线程操作的耗时。 export interface IValueBucket {
  id: number;
  name: string;
  age: number;
  salary: number;
...
2. UI主线程发起数据库操作请求，在子线程完成数据的增删改查等操作。 import { relationalStore, ValuesBucket } from '@kit.ArkData';
...
1. 定义数据库中的数据格式，采用Sendable，减少跨线程耗时。 export interface IValueBucket {
  id: number;
  name: string;
  age: number;
  salary: number;
...
2. 定义普通类实例对象，持有Sendable类实例对象。 import { SharedValuesBucket } from './SharedValuesBucket';
...
3. UI主线程发起数据库操作请求，在子线程进行数据的增删改查等操作。 import { relationalStore, ValuesBucket } from '@kit.ArkData';
import { collections, taskpool } from '@kit.ArkTS';
import { IValueBucket, SharedValuesBucket } from './SharedValuesBucket';
