...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/sendable-trans-guide
...
##### [h2]使用TurboTransJSON序列化/反序列化并生成Sendable对象
...
export class LayoutS {
  public type: string = '';
  public arr: collections.Array = new collections.Array();
}
```
...
export function testJSON1(): LayoutS {
  ...
  let layoutNormal = TJSON.fromString(str, Layout);
...
export function testJSON2(): LayoutS {
  ...
  let layoutNormal = TJSON.fromString(layoutStr, Layout);
...
##### [h2]使用TurboTransProtobuf生成Sendable对象并编解码
...
使用[UIUtils.makeObserved()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-statemanagement#makeobserved)方法可以将Sendable对象转换为可观察对象。
...
##### [h2]通过taskpool执行异步任务并将Sendable对象转换为可观察对象
...
##### [h2]创建UI组件实时观察Sendable对象属性变化
     
Index.ets中通过@Local状态保存任务返回值，再由Text组件绑定pb.value_string和layout.type。其中，pb存储的是observeProtobuf返回的Sendable对象（test_pb类型），layout存储的是observeJSON1返回的Sendable对象（LayoutS类型）。
