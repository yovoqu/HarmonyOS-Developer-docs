...
从6.0.0(20)版本开始，新增支持[过渡模糊](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ui-design-hdsnavigation#scrolleffecttype)（适用于沉浸式图文类场景）与[渐变模糊](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ui-design-hdsnavigation#scrolleffecttype)（适用于增强页面沉浸感场景）样式。

当开发者需要使用标题栏样式随内容区滚动而动态改变样式的导航组件时，可以通过设置titleBar属性中的[style](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ui-design-hdsnavigation#hdsnavigationtitlebaroptions)，自定义标题栏样式根据滚动距离线性变化。通常需配合滚动容器组件使用，推荐使用bindToScrollable或bindToNestedScrollable属性绑定导航组件和可滚动容器组件。
