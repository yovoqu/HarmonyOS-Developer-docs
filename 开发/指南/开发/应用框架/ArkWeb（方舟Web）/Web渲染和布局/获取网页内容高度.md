...
动态网页可以通过JSBridge传递特定回调，通知到应用侧调用。 应用侧代码
```text
// xxx.ets
import { webview } from '@kit.ArkWeb';
...
普通网页可以通过load事件，在网页的所有资源都完全加载完成后触发。 前端代码
```text
...
当网页含有大图片时，可使用图片加载完成回调触发。 在前端代码中，请将示例图片路径替换为实际使用的图片资源。
```text
...
针对图片密集网页，在所有图片加载完成后触发。 在前端代码中，请替换图片为真实图片。
```text
...
在无法使用JSBridge的场景下，可以通过添加setTimeout等函数来延迟获取当前页面的高度。具体的延迟时间可以根据网页的复杂度来确定。 应用侧代码
```text
// xxx.ets
import { webview } from '@kit.ArkWeb';
