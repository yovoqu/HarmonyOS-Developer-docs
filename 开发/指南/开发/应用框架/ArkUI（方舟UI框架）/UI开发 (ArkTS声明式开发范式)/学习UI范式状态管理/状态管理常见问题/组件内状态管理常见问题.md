...
创建Column组件。
3. 创建Text组件。创建Text组件的过程中，触发this.count++。
4. count的改变再次触发Text组件的刷新。
5. 刷新过程中组件不会再标脏自己。
6. Text最终显示为2。
...
上面示例的渲染过程为：
1. 创建第一个Text组件，触发this.message改变，[标脏](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management-glossary#mark-dirty标脏)第一个Text组件。
