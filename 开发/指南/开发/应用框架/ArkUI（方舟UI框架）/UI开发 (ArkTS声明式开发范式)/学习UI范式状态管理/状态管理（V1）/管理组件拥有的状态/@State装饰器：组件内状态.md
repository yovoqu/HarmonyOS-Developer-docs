...
@State装饰的变量与声明式范式中的其他被装饰变量一样，是私有的，只能从组件内部访问，在声明时必须指定其类型并完成本地初始化；若需从父组件初始化，也可选择使用命名参数机制完成赋值。 @State装饰的变量拥有以下特性： @State装饰的变量生命周期与其所属自定义组件的生命周期相同。
...
**图1** 初始化规则图示
![](assets/@State装饰器：组件内状态/file-20260514130516415-0.png)
...
当装饰的数据类型为class或Object时，可以观察到自身的赋值和属性赋值的变化，即Object.keys(observedObject)返回的所有属性。示例如下： 声明Person和Model类。
```text
// 声明Person类
class Person {
  public value: string;
...
当装饰的对象是Array时，可以观察到Array整体的赋值及数组元素的赋值，同时可以通过调用Array的接口push, pop, shift, unshift, splice, copyWithin, fill, reverse, sort更新Array中的数据。数组项中嵌套的属性赋值无法观察。详见[装饰Array类型变量](#装饰array类型变量)。 当装饰的对象是Date时，可以观察到Date的赋值，以及通过调用Date的接口setFullYear, setMonth, setDate, setHours, setMinutes, setSeconds, setMilliseconds, setTime, setUTCFullYear, setUTCMonth, setUTCDate, setUTCHours, setUTCMinutes, setUTCSeconds, setUTCMilliseconds更新Date的属性，详见[装饰Date类型变量](#装饰date类型变量)。 当装饰的变量是Map时，可以观察到Map整体的赋值，以及通过调用Map的接口set, clear, delete更新Map的值。详见[装饰Map类型变量](#装饰map类型变量)。 当装饰的变量是Set时，可以观察到Set整体的赋值，以及通过调用Set的接口add, clear, delete更新Set的值。详见[装饰Set类型变量](#装饰set类型变量)。

## 框架行为

当状态变量改变时，查询依赖该状态变量的组件。 执行依赖该状态变量的组件更新方法，实现组件更新渲染。
...
@State不支持装饰Function类型的变量，API version 23之前，框架会抛出运行时错误。 从API version 23开始，添加对@State装饰Function类型变量的校验，编译期会报错。 父组件传入undefined时，@State装饰的变量仍使用本地默认值进行初始化。
```text
@Entry
@Component
struct Parent {
...
以下示例为@State装饰的简单类型，count被@State装饰成为状态变量，count的改变引起Button组件的刷新： 当状态变量count改变时，只能查询到Button组件与之关联。 执行Button组件的更新方法，实现按需刷新。
```text
@Entry
@Component
struct MyComponent {
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-1.gif)
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-2.gif)
从上述示例中，我们可以了解到@State变量的初始化机制： 上述示例中，在没有外部传入的情况下，使用默认的值进行本地初始化：
```text
...
在下面的示例中，@State装饰的变量fruits的类型为Array，点击Button改变fruits的值，视图会随之刷新。
```text
class Fruit {
  public name: string;
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-3.gif)
...
在下面的示例中，@State装饰的变量fruits的类型为Map，点击Button改变fruits的值，视图会随之刷新。
```text
@Entry
@Component
struct MapSample {
  @State fruits: Map = new Map([['apple', 1], ['banana', 2]]); // 使用@State装饰Map类型变量
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-4.gif)
...
在下面的示例中，@State装饰的变量fruits的类型为Set，点击Button改变fruits的值，视图会随之刷新。
```text
@Entry
@Component
struct SetSample {
  @State fruits: Set = new Set(['apple', 'banana']); // 使用@State装饰Set类型变量
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-5.gif)
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-6.gif)
...
![](assets/@State装饰器：组件内状态/file-20260514130516415-7.gif)
