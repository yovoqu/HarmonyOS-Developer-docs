...
> [!NOTE]
> 配置此metadata时，必须确保在属性设置中，没有通过函数返回值实现深浅色切换。 HdsNavigation 、 HdsNavDestination 、 HdsTabs 、 HdsListItemCard 四个高级组件暂未适配，这些高级组件的颜色相关属性均需使用 AbilityStage的监听回调 或 Ability的监听回调 方式来处理。
