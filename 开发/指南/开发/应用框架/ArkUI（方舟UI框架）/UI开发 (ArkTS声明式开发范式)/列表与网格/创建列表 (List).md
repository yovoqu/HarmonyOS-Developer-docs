...
1. 首先，需要创建一个Scroller类型的对象listScroller。 private listScroller: Scroller = new Scroller();
2. 然后，列表通过scroller参数绑定滚动控制器。 // listScroller初始化List组件的scroller参数，绑定listScroller与列表。
List({ scroller: this.listScroller }) {
// ···
}
3. 最后，滚动条通过scroller参数绑定滚动控制器。 // listScroller初始化ScrollBar组件的scroller参数，绑定listScroller与列表。
...
2. 绑定swipeAction属性到可左滑的ListItem上。 // 构建List时，通过ForEach基于数据源this.messages循环渲染ListItem。
ListItem() {
  // ···
}.swipeAction({
  end: {
...
![](assets/创建列表%20(List)
添加列表项功能实现主要流程如下：
1. 定义列表项数据结构，以待办事项管理为例，首先定义待办数据结构。 // ToDo.ets
import { util } from '@kit.ArkTS';
...
2. 构建列表整体布局和列表项。 import { ToDo } from './ToDo';
...
3. 初始化待办列表数据和可选事项，最后，构建列表布局和列表项。 import { ToDo } from './ToDo';
...
![](assets/创建列表%20(List)
删除列表项功能实现主要流程如下：
1. 列表的删除功能一般进入编辑模式后才可使用，所以需要提供编辑模式的入口。 以待办列表为例，通过监听列表项的长按事件，当用户长按列表项时，进入编辑模式。 import { util } from '@kit.ArkTS';
...
} 实现参考： Flex({ justifyContent: FlexAlign.SpaceBetween, alignItems: ItemAlign.Center }) {
  // ...
}
// ...
.gesture(
...
2. 需要响应用户的选择交互，记录要删除的列表项数据。 在待办列表中，通过勾选框的勾选或取消勾选，响应用户勾选列表项变化，记录所有选择的列表项。 import { util } from '@kit.ArkTS';
...
3. 需要响应用户点击删除按钮事件，删除列表中对应的选项。 import { util } from '@kit.ArkTS';
...
} 实现参考： // app.string.delete 资源文件中的value值为'删除'
...
![](assets/创建列表%20(List)
列表项折叠与展开效果实现主要流程如下：
1. 定义列表项数据结构。 import { curves } from '@kit.ArkUI';
...
2. 构造列表结构。   @State routes: ItemGroupInfo[] = [
 {
 index: 0,
 name: 'basicInfo',
...
3. 通过改变ListItem的状态，来控制每个列表项是否展开，并通过animation和animateTo来实现展开与折叠过程中的动效效果。 @Builder
...
![](assets/创建列表%20(List)
1. 定义列表项数据结构。 interface Message {
  id: number
  content: ResourceStr
  sender: ResourceStr
}
2. 构造列表结构，同时把stackFromEnd接口参数值设置为true，即可实现List列表在底部插入数据时，内容向上滚动。 @Builder
...
2. 构造新闻条目结构，通过type属性来区分长新闻，短新闻。 @State newsData: Array&lt;News&gt; = [
...
3. 滑动离手事件onWillStopDragging及新闻处理逻辑：   上报离手瞬间滑动速度，支持正负方向速度检测，向上滑动为正，向下滑动为负。 .onWillStopDragging((velocity: number) => {
  if (velocity < 0) {
 // 向下滑动处理
  } else {
 // 向上滑动处理
  ...
  // ...
})  通过getItemRect接口方法获取当前项位置信息。 let rect = this.scrollerForList.getItemRect(this.currentIndex);  处理短新闻：直接跳转相邻项。 if (velocity > 10) {
...
}  处理长新闻：计算剩余显示范围决定滚动终点。 let rect = this.scrollerForList.getItemRect(this.currentIndex);
if (velocity < -30) {
  if (rect) {
 // 当前节点在页面内的剩余显示范围
