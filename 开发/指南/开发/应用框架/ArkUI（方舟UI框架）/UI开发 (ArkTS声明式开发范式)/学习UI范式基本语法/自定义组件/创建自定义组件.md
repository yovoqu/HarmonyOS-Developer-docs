...
> [!NOTE]
> 从API version 24开始，可通过在应用工程的 module.json5配置文件 中配置 metadata标签 来使能自定义组件支持跨 Ability 迁移。具体配置方式为：新增 name 为"enableCustomComponentCrossAbility"， value 为"true"。因为自定义组件提供的是UI能力，所以这里的Ability也特指 UIAbility 。具体示例参考 自定义组件支持跨Ability迁移 。
...
![](assets/创建自定义组件/file-20260514130501367-1.png)
...
![](assets/创建自定义组件/file-20260708103928b4ea4ecf.png)
...
[@Entry](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-entry#entry)装饰的自定义组件将作为[UI页面](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-router-to-navigation#页面结构)的入口。在单个UI页面中，仅允许存在一个由@Entry装饰的自定义组件作为页面的入口。

> [!NOTE]
> 从API version 9开始，该装饰器支持在ArkTS卡片中使用。 从API version 10开始，@Entry可以接受一个可选的 LocalStorage 参数或者一个可选的EntryOptions 10+ 参数。 从API version 11开始，该装饰器支持在元服务中使用。
...
[@Component](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-custom-component-decorator-component#component)装饰的struct为V1自定义组件，可以使用[状态管理V1版本](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management-overview#状态管理v1)装饰器的能力。

> [!NOTE]
> 从API version 9开始，该装饰器支持在ArkTS卡片中使用。 从API version 11开始，@Component可以接受一个 ComponentOptions参数 。 从API version 11开始，该装饰器支持在元服务中使用。
...
[@ComponentV2](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-custom-component-decorator-componentv2#componentv2)装饰的struct为V2自定义组件，可以使用[状态管理V2版本](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management-overview#状态管理v2)装饰器的能力。

> [!NOTE]
> @ComponentV2装饰器从API version 12开始支持。 从API version 12开始，该装饰器支持在元服务中使用。 从API version 23开始，该装饰器支持在ArkTS卡片中使用。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/15/v3/w21iKhz3QkGtiwixgsioaw/zh-cn_image_0000002686085277.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071833Z&HW-CC-Expire=86400&HW-CC-Sign=1D7BF6600DFB724DB10A7CF1476F61C7D01BEE525FFCFD9FB89B6FE7515BDEFF)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/85/v3/vgneQA6eSbSm3tz2oGflyw/zh-cn_image_0000002685925449.png?HW-CC-KV=V1&HW-CC-Date=20260730T071833Z&HW-CC-Expire=86400&HW-CC-Sign=700E2CD66A7CC7AF3C51E1C296D9D47A462F2BA28B6325FDE4B56501F2440BA9)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ce/v3/mXAFsgHERfuvsGNjgpdj8Q/zh-cn_image_0000002656005770.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071833Z&HW-CC-Expire=86400&HW-CC-Sign=D70E6EBA32B989A7182A31ED87653B3FCBE9CC147A1731D827632EEF906B5930)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/bf/v3/OM3H9xV5RxaUGbXya66-5A/zh-cn_image_0000002655845850.png?HW-CC-KV=V1&HW-CC-Date=20260730T071833Z&HW-CC-Expire=86400&HW-CC-Sign=C5D8C448E1AF3D8D1E12D7A053B302B81EE128413E9A78E0D0BD52694830EE92)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/29/v3/YNAY2kOwRGaYU_PEUa-uLg/zh-cn_image_0000002686085279.png?HW-CC-KV=V1&HW-CC-Date=20260730T071833Z&HW-CC-Expire=86400&HW-CC-Sign=73FD587F4C3A8ECDF31E6272C4658A2B2531356AAB0669683A7A33758E9FB6EE)
...
2. 仅支持组件树上的自定义组件迁移。对于未挂载在组件树上的自定义组件将不支持迁移。例如使用[OH_ArkUI_GetNodeHandleFromNapiValue](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-napi-h#oh_arkui_getnodehandlefromnapivalue)获取[ArkUI_NodeHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-node8h)场景中，如果OH_ArkUI_GetNodeHandleFromNapiValue接收的参数为[ComponentContent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-componentcontent)，获取到的ArkUI_NodeHandle为ComponentContent下子树的第一个[FrameNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-framenode)节点，中间跳过的自定义组件将不会在组件树上，不支持迁移。
...
5. 点击自定义组件ComponentUnderBuilderNode内Button('change message')，改变状态变量message的值，触发@Watch('messageUpdate') 回调和UI刷新。

  下面的示例包含了创建新的Ability流程，具体示例可参考[startAbility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext#startability)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f1/v3/zDh-05-qQbuC_tVOPktsug/zh-cn_image_0000002685925451.gif?HW-CC-KV=V1&HW-CC-Date=20260730T071833Z&HW-CC-Expire=86400&HW-CC-Sign=733DD281E30D1F82BB9F2CD7262EC2EB9FFA5D6FB87E0124A8296737831A7DD1)
...
当将@Reusable或@ReusableV2装饰的复用组件与其他自定义组件混合使用时，可参考[使用限制](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-reusablev2#使用限制)。
