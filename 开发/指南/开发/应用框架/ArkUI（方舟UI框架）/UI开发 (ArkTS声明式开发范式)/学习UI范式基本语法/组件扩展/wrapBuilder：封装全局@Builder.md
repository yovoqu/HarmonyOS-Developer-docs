...
> [!NOTE]
> 从API version 11开始支持。 从API version 12开始，wrapBuilder支持在元服务中使用。 从API version 22开始，推荐开发者使用mutableBuilder，支持二次赋值后刷新UI。
...
wrapBuilder方法只能传入[全局@Builder](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-builder#全局自定义构建函数)方法。 WrappedBuilder对象的builder属性方法仅限在struct内部使用。
...
const builderArr: WrappedBuilder[] = [wrapBuilder(myBuilder0), wrapBuilder(yourBuilder)];
...
interface ChildOptions {
  wrappedBuilder: WrappedBuilder; // 类型为WrappedBuilder的属性可以传递@Builder函数
}
...
const wBuilder: WrappedBuilder = wrapBuilder(overBuilder);
