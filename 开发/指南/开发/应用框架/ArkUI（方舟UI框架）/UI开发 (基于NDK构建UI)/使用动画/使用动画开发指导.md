...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ndk-use-animation-scene
...
该示例主要演示了如何通过全局[animateTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-nativeanimateapi-1#animateto)接口添加属性动画。NDK接口开发的UI界面挂载到ArkTS主页面的完整流程可参考[接入ArkTS页面](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ndk-access-the-arkts-page)。
     
      
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1b/v3/e8y88lmbRh61yCWsDYQGig/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025428Z&HW-CC-Expire=86400&HW-CC-Sign=3D1F54443B394A40E7164F5018CE77083C09E6E87F6C77D0B6212F0EFD849D47)
...
```
 ...
 - 解析NodeContent，转换为C中的[ArkUI_NodeContentHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-nodecontent8h)对象。
...
completeCallback->userData = reinterpret_cast(data);
completeCallback->callback = [](void *userData) {
    AnimateData* data = reinterpret_cast(userData);
    if (data) {
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c8/v3/dko-4CyRQWiP9Tt2UR8h9A/zh-cn_image_0000002659219935.gif?HW-CC-KV=V1&HW-CC-Date=20260701T025428Z&HW-CC-Expire=86400&HW-CC-Sign=C9F08D794D04D21FC614871471E2FBF259149AE73A5D424E9BFA4C1C25979D60)
...
该示例主要演示了如何通过[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_ROTATE_TRANSITION，NODE_SCALE_TRANSITION，NODE_TRANSLATE_TRANSITION属性配置转场参数，并通过[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_TRANSFORM_CENTER属性设置NODE_SCALE_TRANSITION和NODE_ROTATE_TRANSITION动效的中心点坐标，实现组件的插入和删除时显示过渡动效。
     
 - 创建可交互界面，界面中包含一个Button组件，点击可以控制转场节点的添加和移除。其中[ArkUI_NodeContentHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-nodecontent8h)类型节点的获取与使用可参考[接入ArkTS页面](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ndk-access-the-arkts-page)。
...
void mainViewMethod(ArkUI_NodeContentHandle handle)
{
    ...
    ArkUI_NumberValue buttonOpenTypeValue[] = {{.i32 = static_cast(ARKUI_BUTTON_TYPE_NORMAL)}};
...
```
 ...
 - 创建一个设置了[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_ROTATE_TRANSITION，NODE_SCALE_TRANSITION属性的节点，当目标节点上下树时会播放转场动画。
...
ArkUI_NodeHandle CreateChildNode()
{
    ...
    ArkUI_NumberValue rotateAnimationValue[] = {
        0.0f, 0.0f, 1.0f, 360.0f, 0.0f, {.i32 = 500}, {.i32 = static_cast(ARKUI_CURVE_SHARP)}};
    ...
    ArkUI_NumberValue scaleAnimationValue[] = {
        0.0f, 0.0f, 0.0f, {.i32 = 500}, {.i32 = static_cast(ARKUI_CURVE_SHARP)}};
    ...
    ArkUI_NumberValue translateAnimationValue[] = {
        200, 200, 0.0f, {.i32 = 500}, {.i32 = static_cast(ARKUI_CURVE_SHARP)}};
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/43/v3/tNRESePPQqO4Ydv9KewdWA/zh-cn_image_0000002628700740.gif?HW-CC-KV=V1&HW-CC-Date=20260701T025428Z&HW-CC-Expire=86400&HW-CC-Sign=BD57E40540D0CC7AC3B331A31F9032EF35879C572C73CE87E86B8E4A6B774D0B)
...
该示例主要演示如何通过[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_GEOMETRY_TRANSITION属性来实现一镜到底转场，NDK接口开发的UI界面挂载到ArkTS主页面的完整流程可参考[接入ArkTS页面](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ndk-access-the-arkts-page)。
     
 - 创建一个设置了[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_GEOMETRY_TRANSITION属性的image节点，同时设置该节点的宽度、高度和位置，用于区别于另一个image节点，然后将当前image节点挂载到一个父节点，当前示例中父节点为stack。
...
```
 ...
 - 再创建一个设置了[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_GEOMETRY_TRANSITION属性的image节点，属性值与第一个image节点相同，同时为该节点设置不同的宽度、高度和位置。
...
```
 ...
 - 在stack的OnImageTransitionClicked监听回调函数中添加两个节点的上下树逻辑，控制节点的上下树，确保同时只有一个节点存在，同时，当其中一个节点挂载到父节点时需要重置[ArkUI_NodeAttributeType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-node-h#arkui_nodeattributetype)中的NODE_GEOMETRY_TRANSITION属性的值，并重新设置为原来的值。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/31/v3/aHHRCI9XSAy_Is2sxpAhZw/zh-cn_image_0000002659099969.gif?HW-CC-KV=V1&HW-CC-Date=20260701T025428Z&HW-CC-Expire=86400&HW-CC-Sign=5F4C2BA870CABC5E57B2C02908CF296E686E5AFCB221FD079306CCC432EE6F21)
...
g_keyframe_text = std::make_shared();
...
auto onTouch = [](ArkUI_NodeEvent *event) {
    ...
    if (OH_ArkUI_NodeEvent_GetTargetId(event) == NUM_1) {
        ...
        // std::shared_ptr g_keyframe_button存储Button的全局变量，在onTouch注册时需要使用
        ...
        OH_ArkUI_KeyframeAnimateOption_RegisterOnFinishCallback(option, nullptr, [](void *user) {
            KeyFrameAnimateToData* data = reinterpret_cast(user);
            if (data) {
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f1/v3/Y1FfvDFBThSZ8LcLybO59A/zh-cn_image_0000002628860620.gif?HW-CC-KV=V1&HW-CC-Date=20260701T025428Z&HW-CC-Expire=86400&HW-CC-Sign=84F76EE7B9E0F53A0F324E3E1CAD83EAC14C0FCE508DEC816F6307338231810D)
...
该示例主要演示如何通过[createAnimator](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-nativeanimateapi-1#createanimator)来设置帧动画。
     
```text
std::shared_ptr CreateAnimator()
{
    auto column = std::make_shared();
    ...
    auto createButton = std::make_shared();
    ...
    g_animator_text = std::make_shared();
    ...
    // 创建容器，用于存放Button按键
    auto buttoColumn = std::make_shared();
    ...
    auto playButtonColumn = std::make_shared();
    ...
    // 设置animator结束按钮
    auto finishButton = std::make_shared();
    ...
    // 设置animator暂停按钮
    auto pauseButton = std::make_shared();
    ...
    auto reverseButton = std::make_shared();
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/81/v3/2cHRCZgfRvSUTAfps4eTdg/zh-cn_image_0000002659219937.gif?HW-CC-KV=V1&HW-CC-Date=20260701T025428Z&HW-CC-Expire=86400&HW-CC-Sign=D3A0B149F0371D8EE2A3EB03A79249FA3C20932B97661FE1F6130F6ACDDD1C32)
