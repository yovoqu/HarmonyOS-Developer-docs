...
1. 注册NODE_ON_GESTURE_COLLECT_INTERCEPT节点事件并绑定回调处理函数。在回调中使用[OH_ArkUI_GestureCollectInterceptInfo_GetResponseRecognizers](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_gesturecollectinterceptinfo_getresponserecognizers)获取手势识别器、[OH_ArkUI_GestureCollectInterceptInfo_GetTouchRecognizers](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_gesturecollectinterceptinfo_gettouchrecognizers)获取触摸识别器，并结合处理函数完成干预。
...
通过[OH_ArkUI_GetGestureBindNodeUniqueId](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_getgesturebindnodeuniqueid)接口获取绑定节点ID。
3. 通过[OH_ArkUI_GestureRecognizer_IsHostBelongsTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_gesturerecognizer_ishostbelongsto)接口判断手势识别器是否为指定节点的后代。
4. 通过[OH_ArkUI_TouchRecognizer_IsHostBelongsTo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_touchrecognizer_ishostbelongsto)接口判断触摸识别器是否为指定节点的后代。
5. 通过[OH_ArkUI_GestureCollectInterceptInfo_SetGestureCollectIntervention](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-gesture-h#oh_arkui_gesturecollectinterceptinfo_setgesturecollectintervention)接口设置收集干预策略。
