...
public:
    FlowItemAdapter()
    {
        ...
        // 初始化懒加载数据。
        for (int32_t i = 0; i (OH_ArkUI_NodeAdapterEvent_GetUserData(event));
...
void SetSection(const std::shared_ptr &section)
{
...
std::shared_ptr GetWaterflowSection() { return section_; }
...
std::shared_ptr section_ = nullptr;
...
inline void SetupSections(std::shared_ptr sections)
{
...
for (int i = 0; i SetSection(sections->GetSectionOptions(), i,
i % SECTION_2_ID ? MARGIN_GAP_SECTION_1 : MARGIN_GAP_SECTION_2);
}
}

inline std::shared_ptr CreateWaterflowExample(napi_env env)
{
auto waterflow = std::make_shared();
waterflow->SetHeight(UI_HEIGHT);
waterflow->SetWidth(UI_WIDTH);
waterflow->SetLazyAdapter(std::make_shared());
auto sections = std::make_shared();
...
```
     ...
     ![](assets/使用瀑布流/file-20260514130730289-0.gif)
