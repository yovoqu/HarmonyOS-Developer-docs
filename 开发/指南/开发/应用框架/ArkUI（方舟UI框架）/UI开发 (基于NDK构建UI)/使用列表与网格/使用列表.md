...
#### 在列表中应用懒加载适配器
1. 在ArkUIListNode中添加SetLazyAdapter函数，给列表节点设置[NODE_LIST_NODE_ADAPTER](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ative-node-h-nodeattributetype-scrollablecontainer#node_list_node_adapter)属性，并将NodeAdapter作为属性入参传入。
...
#### ListItem横划删除
1. [ListItem](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-listitem)设置[NODE_LIST_ITEM_SWIPE_ACTION](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ative-node-h-nodeattributetype-scrollablecontainer#node_list_item_swipe_action)属性，将[ArkUI_ListItemSwipeActionOption](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkui-nativemodule-arkui-listitemswipeactionoption)对象作为属性参数传入。
