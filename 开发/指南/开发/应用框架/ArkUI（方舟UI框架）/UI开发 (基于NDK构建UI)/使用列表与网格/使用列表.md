...
#### 在列表中应用懒加载适配器
1. 在ArkUIListNode中添加SetLazyAdapter函数，给列表节点设置NODE_LIST_NODE_ADAPTER属性，并将NodeAdapter作为属性入参传入。 // ArkUIListNode.h
// 提供列表组件的封装。
...
2. 创建List使用懒加载的示例代码，调用List节点的SetLazyAdapter接口设置懒加载适配器。 // LazyTextListExample
// 懒加载列表示例代码。
...
3. 在NativeEntry.cpp中调用List使用懒加载的示例代码。 // NDK接口入口挂载文件。
...
#### 控制列表滚动位置
1. 控制列表滚动到指定偏移量位置。 // ArkUIListNode.h
// 提供列表组件的封装。
// ...
class ArkUIListNode : public ArkUINode {
public:
...
#### ListItem横划删除
1. ListItem设置NODE_LIST_ITEM_SWIPE_ACTION属性，将ArkUI_ListItemSwipeActionOption对象作为属性参数传入。 // ArkUIListItemNode.h
// 提供列表项的封装类。
...
2. 设置创建ListItem和复用ListItem的回调函数。当创建ListItem时，创建ListItem的划出组件，并绑定点击事件，在点击事件中执行删除数据源操作。ListItem复用时，更新划出组件的绑定事件。 // LazyTextListExample
// 懒加载列表示例代码。
...
#endif // MYAPPLICATION_LAZYTEXTLISTEXAMPLE1_H
3. 添加新的ListItem时，优先复用已缓存的ListItem实例，并更新其内容；若无可用缓存，则创建新的ListItem。当回调返回空时，创建一个默认的ListItem作为兜底方案。最后，将生成的节点句柄回填至OH_ArkUI_NodeAdapterEvent_SetItem事件中，完成绑定。 // ArkUIListItemAdapter.h
// ...
template <typename ItemType> class ArkUIListItemAdapterT : public IArkUIListItemAdapter {
 // ...
 // 需要新的Item显示在可见区域。
...
4. ArkUIListItemAdapter中新增RemoveItem，用于删除数据源并且调用OH_ArkUI_NodeAdapter_RemoveItem接口通知框架刷新UI。 // ArkUIListItemAdapter.h
// ...
template <typename ItemType> class ArkUIListItemAdapterT : public IArkUIListItemAdapter {
 // ...
 void RemoveItem(size_t index)
...
2. [List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)组件设置吸顶。 // ArkUIListNode.h
// 提供列表组件的封装。
// ...
class ArkUIListNode : public ArkUINode {
 // ...
...
3. [List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)组件下使用[ListItemGroup](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-listitemgroup)实现分组列表界面。 // LazyTextListExample.h
// 懒加载列表示例代码。
