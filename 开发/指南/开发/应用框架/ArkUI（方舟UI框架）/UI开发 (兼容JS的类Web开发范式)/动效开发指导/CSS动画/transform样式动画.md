...
设置scale样式属性实现涟漪动画，先使用定位确定元素的位置，确定坐标后创建多个组件实现重合效果，再设置opacity属性改变组件不透明度实现组件隐藏与显示，同时设置scale值使组件可以一边放大一边隐藏，最后设置两个组件不同的动画执行时间，实现扩散的效果。 设置scale3d中X轴、Y轴、Z轴的缩放参数实现动画。
```text
...
```
 ...
 ![](assets/transform样式动画/file-20260514130802251-3.gif)
> [!NOTE]
...
```
 ...
 ![](assets/transform样式动画/file-20260514130802251-5.gif)
> [!NOTE]
