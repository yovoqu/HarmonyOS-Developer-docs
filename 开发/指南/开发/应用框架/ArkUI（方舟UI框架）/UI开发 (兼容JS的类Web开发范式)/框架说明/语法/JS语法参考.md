...
| props | Array/Object | props用于组件之间的通信，可以通过方式传递给组件；props名称必须用小写，不能以\$或_开头，不要使用保留字for, if, show, tid。目前props的数据类型不支持Function。示例见[Props](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-components-custom-props#props)。 |
...
| 方法 | 参数 | 描述 |
| --- | --- | --- |
| \$element | id: string | 获得指定id的组件对象，如果无指定id，则返回根组件对象。示例见[获取DOM元素](#获取dom元素)。            用法：                        - this.\$element('xxx')：获得id为xxx的组件对象。            - this.\$element()：获得根组件对象。 |
...
![](assets/JS语法参考/file-20260514130738182-0.gif)
...
![](assets/JS语法参考/file-20260514130738182-1.png)
自定义parent组件：
```text
...
![](assets/JS语法参考/file-20260514130738182-2.gif)
