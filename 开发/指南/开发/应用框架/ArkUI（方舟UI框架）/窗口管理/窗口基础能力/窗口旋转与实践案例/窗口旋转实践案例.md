...
struct Index {
  ...
  aboutToAppear() {
    ...
    if (ret) {
      let callback: Callback = (data: display.FoldDisplayMode) => {
...
若开发者想准确知道当前窗口方向从而选择旋转策略（比如视频播放页面锁定当前方向），推荐获取到[display.rotation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-display#属性)或[display.orientation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-display#属性)后，再使用[convertOrientationAndRotation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#convertorientationandrotation23)将屏幕方向转化为窗口方向，具体示例如下：
 ...
 - 获取目标屏幕方向。调用[getDefaultDisplaySync()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-display#displaygetdefaultdisplaysync9)获取屏幕方向。
 - 将屏幕方向转换为窗口方向。调用[convertOrientationAndRotation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#convertorientationandrotation23)可以把屏幕方向[display.orientation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-display#属性)转换为窗口方向[orientation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-i#rotationchangeinfo19)。
 - 将窗口方向转换为旋转策略。窗口方向还需要进一步转换为系统可识别的旋转策略窗口[Orientation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-e#orientation9)，才能传给setPreferredOrientation()。
 - 调用[setPreferredOrientation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setpreferredorientation9-1)接口设置旋转策略锁定显示方向。
