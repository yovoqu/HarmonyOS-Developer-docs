...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/6xPUVX0UTwuhwaZ4QRuDxg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025429Z&HW-CC-Expire=86400&HW-CC-Sign=780458DC0929DDF8D568F3A7DA9DE2984EF3F35C517AD39DDF743C8FBD4202EA)
...
- 使用闪控窗前请先参考[前提条件](#前提条件)申请ohos.permission.FLOAT_VIEW权限。
...
- 使用[create()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#floatviewcreate)接口创建闪控窗控制器实例后注册状态变化事件回调。
- 使用[setUIContext()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#setuicontext)设置闪控窗页面内容。
- 通过[getFloatViewLimits()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#floatviewgetfloatviewlimits)获取窗口尺寸限制，通过[setWindowSize()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#setwindowsize)设置合适的窗口大小。
- 通过[start()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#start)接口启动闪控窗。
    
```ArkTS
// 启动闪控窗
async createWindowPlain(): Promise {
...
- （可选）通过[onLimitsChange()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#onlimitschange)监听闪控窗尺寸限制变化，动态调整窗口尺寸, 通过[onStateChange()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#onstatechange)监听闪控窗状态变化，绑定状态变化回调。
...
- 在[onStateChange()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#onstatechange)监听[start()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#start)完成后，通过[stop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#stop)停止闪控窗。
...
- 使用闪控窗和闪控球前请先参考[前提条件](#前提条件)申请ohos.permission.FLOAT_VIEW和ohos.permission.USE_FLOAT_BALL权限。
...
- 使用[floatView.create()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#floatviewcreate)接口创建闪控窗控制器实例，使用[floatingBall.create()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatingball#floatingballcreate)接口创建闪控球控制器实例。
    
```ArkTS
async createWindowAdvanced(): Promise {
...
async bindFloatViewAndBall(): Promise {
...
- 使用[floatView.bind()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#floatviewbind)接口将闪控窗控制器与闪控球控制器绑定，绑定时需要配置闪控球参数。
- 使用[setUIContext()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#setuicontext)设置闪控窗页面内容。
- 通过[getFloatViewLimits()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#floatviewgetfloatviewlimits)获取窗口尺寸限制，通过[setWindowSize()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#setwindowsize)设置合适的窗口大小。
- 通过[start()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#start)接口启动闪控窗，绑定状态下会同时启动闪控球, 开发者可收到闪控球创建回调。
    
```ArkTS
private async bindControllersAndStart(): Promise {
...
- 通过[stop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#stop)停止闪控窗，绑定状态下会同时停止闪控球。
...
- （可选）通过[floatView.unbind()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-floatview#floatviewunbind)解绑闪控窗与闪控球。
