...
4. 场景2：使用OH_Rdb_SetCryptoParam接口配置加密参数，此时会使用开发者自定义的密钥和算法参数进行数据库的加密/解密。
...
5. 从API version 22开始，支持更换数据库密钥和加密参数，如果开发者需要更换已创建的加密数据库的密钥或者加密参数，可以使用OH_Rdb_RekeyEx进行更换。
...
6. 场景2：原数据库为自定义参数加密数据库，更换自定义密钥和加密参数。
...
7. 场景3：原数据库为默认参数加密库，更换自定义密钥和加密参数。
...
8. 场景4：原数据库为自定义参数加密数据库，更换数据库生成密钥和自定义加密参数。
