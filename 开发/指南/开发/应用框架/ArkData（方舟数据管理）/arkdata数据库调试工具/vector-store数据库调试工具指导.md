...
3. 创建新的数据库时，系统会自动生成以下类型的文件， 标签debug_hap_data_file代表属于调试应用。
...
2. 通过".mode table"和"select * from 表名;"命令查询指定表所有内容，显示结果如下：
...
3. 在vector>>>提示符下，通过"select * from 表名 where 筛选条件"命令查询指定key的键值对，显示结果如下：
...
4. 在vector>>>提示符下，通过.count命令列出所有表的记录总数，显示结果如下：
