...
在使用vector-store调试工具之前，必须要先切换至目标调试应用路径下，再使用arkdata命令进入到vector-store调试工具（需要使用arkdata配置相关参数打开vector-store数据库，开库成功后才能使用vector-store工具进行数据的增删改查）。  arkdata命令支持的参数如下表所示：
| 参数 | 参数值类型 | 描述 |
| --- | --- | --- |
...
vector-store数据库调试工具，命令语句最大的长度为1024*1024-1。  由于hdc使用中文会显示乱码，因此数据库调试工具不支持中文。  支持的设备：Phone、PC/2in1。
...
执行hdc shell命令进入shell交互模式。  必须要先切换至目标调试应用路径下, 再进入已有db文件层级，执行"arkdata  -t vector"新建一个数据库。
```text
C:\Users\*****>hdc shell
...
全表查询。  在vector>>>提示符下，通过".mode print"和"select * from 表名;"命令查询指定表所有内容，显示结果如下：
```text
vector>>> .mode print
vector>>> select * from t1;
[row-0]
