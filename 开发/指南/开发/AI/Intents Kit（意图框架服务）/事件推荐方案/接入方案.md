...
开发者需要自己实现InsightIntentExecutor，并在对应回调实现打开落地页（点击推荐卡片跳转的界面）的能力，ViewRepayment的意图调用字段定义见对应[垂域意图Schema](https://developer.huawei.com/consumer/cn/doc/service/intents-schema-0000001901962713)定义表。 步骤如下： 继承InsightIntentExecutor。 重写对应方法，例如目标拉起前台页面，则可重写onExecuteInUIAbilityForegroundMode方法。 通过意图名称，识别查看还款意图（ViewRepayment）。 在对应的方法中传递意图参数（param），并拉起对应落地页（如还款页面）。
```text
...
export default class InsightIntentExecutorImpl extends InsightIntentExecutor {
   ...
   * @param pageLoader 窗口
   */
  private viewRepayment(param: Record, pageLoader: window.WindowStage): Promise {
