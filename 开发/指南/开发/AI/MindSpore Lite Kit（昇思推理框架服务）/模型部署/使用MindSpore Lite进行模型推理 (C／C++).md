...
![](assets/使用MindSpore%20Lite进行模型推理%20(C／C++)/file-20260514132649567-0.png)
...
2. 如果是第三方框架的模型，比如 TensorFlow、TensorFlow Lite、Caffe、ONNX等，可以使用[模型转换工具](https://www.mindspore.cn/lite/docs/zh-CN/master/use/downloads.html#2-3-0)转换为.ms格式的模型文件。
3. 创建上下文，设置线程数、设备类型等参数。
...
2. 工具链默认编译64位的程序，如果要编译32位，需要添加：-DOHOS_ARCH="armeabi-v7a"。
3. 运行。

  
使用hdc_std连接设备，并将demo和mobilenetv2.ms推送到设备中的相同目录。
4. 使用hdc_std shell进入设备，并进入demo所在的目录执行如下命令，即可得到结果。
