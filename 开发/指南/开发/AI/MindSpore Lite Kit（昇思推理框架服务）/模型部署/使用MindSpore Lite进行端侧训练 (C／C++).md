...
![](assets/使用MindSpore%20Lite进行端侧训练%20(C／C++)/file-20260514132649936-0.png)
...
2. 然后将.mindir模型文件转换成.ms文件，转换操作步骤可以参考[训练模型转换](https://www.mindspore.cn/lite/docs/zh-CN/r2.1/use/converter_train.html)，.ms文件可以导入端侧设备并基于MindSpore Lite端侧框架进行训练。
3. 创建上下文，设置设备类型、训练配置等参数。
...
2. 编译命令如下，其中OHOS_NDK需要设置为native工具链路径：
...
4. 使用hdc shell进入设备，并进入train_demo所在的目录执行如下命令，即可得到结果。
