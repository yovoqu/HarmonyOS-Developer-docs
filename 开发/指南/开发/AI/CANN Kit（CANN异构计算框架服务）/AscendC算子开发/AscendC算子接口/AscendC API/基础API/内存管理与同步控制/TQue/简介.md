...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tque-overview
...
AscendC::TQue que0;
pipe.InitBuffer(que0, 1, len);
AscendC::LocalTensor tensor1 = que0.AllocTensor();
que0.EnQue(tensor1);
tensor1 = que0.DeQue(); // 将tensor从VECOUT的Queue中搬出
que0.FreeTensor(tensor1);
...
template  class KernelAscendQuant {
public:
    __aicore__ inline KernelAscendQuant() {}
    __aicore__ inline void Init(GM_ADDR src_gm, GM_ADDR dst_gm, uint32_t inputSize)
    {
        dataSize = inputSize;
        src_global.SetGlobalBuffer(reinterpret_cast(src_gm), dataSize);
        dst_global.SetGlobalBuffer(reinterpret_cast(dst_gm), dataSize);
...
template  __aicore__ void kernel_ascend_quant_operator(GM_ADDR src_gm, GM_ADDR dst_gm, uint32_t dataSize)
{
