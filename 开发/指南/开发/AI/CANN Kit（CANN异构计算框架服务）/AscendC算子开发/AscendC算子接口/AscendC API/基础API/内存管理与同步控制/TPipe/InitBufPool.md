...
初始化TBufPool内存资源池。本接口适用于内存资源有限时，希望手动指定UB/L1内存资源复用的场景。本接口初始化后在整体内存资源中划分出一块子资源池。划分出的子资源池TBufPool，提供了如下方式进行资源管理： TPipe::InitBufPool的重载接口指定与其他TBufPool子资源池复用。  TBufPool::InitBufPool接口对子资源池继续划分。  TBufPool::InitBuffer接口分配Buffer。   关于TBufPool的具体介绍及资源划分图示请参考[TBufPool](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tbufpool-overview)。
...
新划分的资源池与被复用资源池的硬件属性需要一致，两者共享起始地址及长度。  输入长度需要小于等于被复用资源池长度。  其他泛用约束参考[TBufPool](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tbufpool-overview)。
...
private:
    ...
    {
        AscendC::LocalTensor src0Local = queSrc0.AllocTensor();
        AscendC::LocalTensor src1Local = queSrc1.AllocTensor();
    ...
    {
        AscendC::LocalTensor src0Local = queSrc0.DeQue();
        AscendC::LocalTensor src1Local = queSrc1.DeQue();
        AscendC::LocalTensor dstLocal = queDst0.AllocTensor();
        AscendC::Add(dstLocal, src0Local, src1Local, 512);
        queDst0.EnQue(dstLocal);
        queSrc0.FreeTensor(src0Local);
        queSrc1.FreeTensor(src1Local);
    }
    __aicore__ inline void CopyOut()
    {
        AscendC::LocalTensor dstLocal = queDst0.DeQue();
        AscendC::DataCopy(dstGlobal, dstLocal, 512);
        queDst0.FreeTensor(dstLocal);
    }
    __aicore__ inline void CopyIn1()
    {
        AscendC::LocalTensor src0Local = queSrc2.AllocTensor();
        AscendC::LocalTensor src1Local = queSrc3.AllocTensor();
        AscendC::DataCopy(src0Local, src0Global, 512);
        AscendC::DataCopy(src1Local, src1Global, 512);
    ...
    {
        AscendC::LocalTensor src0Local = queSrc2.DeQue();
        AscendC::LocalTensor src1Local = queSrc3.DeQue();
        AscendC::LocalTensor dstLocal = queDst1.AllocTensor();
        AscendC::Add(dstLocal, src0Local, src1Local, 512);
        queDst1.EnQue(dstLocal);
        queSrc2.FreeTensor(src0Local);
        queSrc3.FreeTensor(src1Local);
    }
    __aicore__ inline void CopyOut1()
    {
        AscendC::LocalTensor dstLocal = queDst1.DeQue();
        AscendC::DataCopy(dstGlobal, dstLocal, 512);
        queDst1.FreeTensor(dstLocal);
    }
private:
    AscendC::TPipe pipe;
    AscendC::TBufPool tbufPool1, tbufPool2;
    AscendC::TQue queSrc0, queSrc1, queSrc2, queSrc3;
    AscendC::TQue queDst0, queDst1;
