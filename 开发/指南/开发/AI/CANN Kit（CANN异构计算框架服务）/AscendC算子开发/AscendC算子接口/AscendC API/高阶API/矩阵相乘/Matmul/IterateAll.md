...
```text
template  __aicore__ inline void IterateAll(const GlobalTensor& gm, uint8_t enAtomic = 0, bool enSequentialWrite = false, bool waitIterateAll = false, bool fakeMsg = false)
template  __aicore__ inline void IterateAll(const LocalTensor& ubCmatrix, uint8_t enAtomic = 0)
```
