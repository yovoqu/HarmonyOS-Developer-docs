...
Scalar流水之间的同步由硬件自动保证，调用PipeBarrier()会引发硬件错误。

## 调用示例

如下示例，Mul指令的输入dst0Local是Add指令的输出，两个矢量运算指令产生依赖，需要插入PipeBarrier保证两条指令的执行顺序。 注：仅作为示例参考，开启自动同步的情况下，编译器自动插入PIPE_V同步，无需开发者手动插入。 **图1** Mul指令和Add指令是串行关系，必须等待Add指令执行完成后，才能执行Mul指令。
![](assets/PipeBarrier(ISASI)
/file-20260514132332658-0.png)
```text
AscendC::LocalTensor src0Local;
AscendC::LocalTensor src1Local;
AscendC::LocalTensor src2Local;
AscendC::LocalTensor dst0Local;
AscendC::LocalTensor dst1Local;
