...
```text
template  class GlobalTensor : public BaseGlobalTensor {
public:
...
类型T支持基础数据类型，但需要遵循使用此GlobalTensor的指令的数据类型支持情况。 **表1** 函数说明
| 函数名称 | 入参说明 | 含义 |
| --- | --- | --- |
...
void Init(__gm__ uint8_t *src_gm, __gm__ uint8_t *dst_gm)
{
    ...
    AscendC::GlobalTensor inputGlobal; // 类型为int32_t
    inputGlobal.SetGlobalBuffer(reinterpret_cast(src_gm), dataSize); // 设置源操作数在Global Memory上的起始地址为src_gm，所占外部存储的大小为256个int32_t

    AscendC::LocalTensor inputLocal = inQueueX.AllocTensor();
