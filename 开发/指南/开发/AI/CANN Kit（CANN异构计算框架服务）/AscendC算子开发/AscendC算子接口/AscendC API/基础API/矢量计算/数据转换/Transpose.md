...
可实现16*16的二维矩阵数据块的转置。  可实现[N, C, H, W]与[N, H, W, C]互相转换。
...
```text
template
void Transpose(const LocalTensor& dstLocal, const LocalTensor& srcLocal)
```

增强转置，支持16*16的二维矩阵数据块转置，支持[N, C, H, W]与[N, H, W, C]互相转换
```text
template
void Transpose(const LocalTensor &dstLocal, const LocalTensor &srcLocal, const LocalTensor &sharedTmpBuffer, const TransposeParamsExt &transposeParams)
```
...
操作数地址偏移对齐要求请参见[通用约束](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-general-constraints)。  该指令不可迭代（即不能通过repeatTimes重复）。  [N, C, H, W]与[N, H, W, C]互相转换，H * W需要32B对齐。  普通转置接口支持srcLocal和dstLocal复用。  增强转置接口，transposeType为TRANSPOSE_ND2ND_B16时支持srcLocal和dstLocal复用，transposeType为TRANSPOSE_NCHW2NHWC、TRANSPOSE_NHWC2NCHW时不支持srcLocal和dstLocal复用。
...
#include "kernel_operator.h"
 ...
 private:
     ...
     __aicore__ inline void Compute()
     {
         ...
         AscendC::Transpose(dstLocal, srcLocal);
...
#include "kernel_operator.h"
 ...
 private:
     __aicore__ inline void CopyIn()
     {
         AscendC::LocalTensor srcLocal = inQueueSrcVecIn.AllocTensor();
         AscendC::DataCopy(srcLocal, srcGlobal, inputSize);
         inQueueSrcVecIn.EnQue(srcLocal);
     }
     __aicore__ inline void Compute()
     {
         AscendC::LocalTensor srcLocal = inQueueSrcVecIn.DeQue();
         AscendC::LocalTensor dstLocal = inQueueSrcVecOut.AllocTensor();
         AscendC::LocalTensor stackBuffer = tmpQueue.AllocTensor();
     ...
     __aicore__ inline void CopyOut()
     {
         AscendC::LocalTensor dstLocal = inQueueSrcVecOut.DeQue();
         AscendC::DataCopy(dstGlobal, dstLocal, inputSize);
         inQueueSrcVecOut.FreeTensor(dstLocal);
     }
 private:
     AscendC::TPipe pipe;
     AscendC::TQue inQueueSrcVecIn;
     AscendC::TQue inQueueSrcVecOut;
     AscendC::TQue tmpQueue;
