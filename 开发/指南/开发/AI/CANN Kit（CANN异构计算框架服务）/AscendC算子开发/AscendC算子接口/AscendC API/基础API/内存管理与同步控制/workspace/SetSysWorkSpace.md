...
框架需要使用的workspace称之为系统workspace。[Matmul](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-matmul-usage-description)等高阶API需要系统workspace，所以在使用该类API时，需要调用该接口，设置系统workspace的指针。 在kernel侧调用该接口前，需要在host侧调用GetLibApiWorkSpaceSize获取系统workspace的大小，并在host侧设置workspacesize大小。样例如下。
```text
// 开发者自定义的tiling函数
...
```text
template
__aicore__ inline void MatmulLeakyKernel::Init(
