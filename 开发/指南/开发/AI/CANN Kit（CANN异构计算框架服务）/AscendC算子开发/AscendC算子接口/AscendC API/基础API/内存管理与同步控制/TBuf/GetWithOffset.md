...
```text
LocalTensor GetWithOffset(uint32_t size, uint32_t bufOffset)
```
...
size的数值是Tensor中元素的个数，size*sizeof(T) + bufOffset不能超过TBuf初始化时的长度。  bufOffset需满足32B对齐的要求。
