...
```text
graphStatus SetData(std::vector &&data);
graphStatus SetData(const std::vector &data);
...
graphStatus SetData(const std::vector &data);
graphStatus SetData(const std::vector &datas);
...
| size | 输入 | 数据的长度，单位为字节。 |
| deleter_func | 输入 | 用于释放data数据。 using DeleteFunc = std::function; |
