...
```text
static std::unique_ptr CreateFollowing(const int64_t shape_size, const ge::DataType dt, size_t &total_size)
```

传入数据类型和Tensor长度，创建Tensor
```text
static std::unique_ptr CreateFollowing(const ge::DataType dt, const size_t tensor_size, size_t &total_size)
```
