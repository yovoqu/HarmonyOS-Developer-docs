...
```text
template::value, int>::type = 0>  ge::graphStatus Append(const T &data);
template::value, int>::type = 0>  ge::graphStatus Append(const T *data, size_t append_num);
```
...
成功返回ge::GRAPH_SUCCESS。  失败返回ge::GRAPH_FAILED。
...
AppendData ad;
auto ret = td->Append(ad); // ge::GRAPH_SUCCESS

// 2
size_t append_num = 10;
int32_t *td = new int32_t[append_num];
auto ret = td->Append(td, append_num); // ge::GRAPH_SUCCESS

// 3
size_t append_num = 50;
int32_t *td = new int32_t[append_num];
auto ret = td->Append(td, append_num); // ge::GRAPH_FAILED
```
