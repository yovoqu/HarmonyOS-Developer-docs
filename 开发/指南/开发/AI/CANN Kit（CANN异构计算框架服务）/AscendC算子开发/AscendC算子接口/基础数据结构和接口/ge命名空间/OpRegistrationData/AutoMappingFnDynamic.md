...
```text
Status AutoMappingFnDynamic(const google::protobuf::Message *op_src, ge::Operator &op, std::map> dynamic_name_attr_value, int32_t in_pos = -1, int32_t out_pos = -1)
```
...
Status MapStageMapping(const google::protobuf::Message* op_src, ge::Operator& op) {
  map> value;
  value["in"] = pair("values", "fake_dtypes");
