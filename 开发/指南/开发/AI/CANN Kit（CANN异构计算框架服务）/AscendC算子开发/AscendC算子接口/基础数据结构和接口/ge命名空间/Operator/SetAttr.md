...
设置算子属性的属性值。 算子可以包括多个属性，初次设置值后，算子属性值的类型固定，算子属性值的类型包括： 整型：接受int64_t、uint32_t、int32_t类型的整型值  使用SetAttr(const string& name, int64_t attrValue)设置属性值，以GetAttr(const string& name, int32_t& attrValue) 、GetAttr(const string& name, uint32_t& attrValue) 取值时，开发者需保证整型数据没有截断，同理针对int32_t和uint32_t混用时需要保证不被截断。  整型列表：接受std::vector、std::vector、std::vector、std::initializer_list&&表示的整型列表数据  浮点数：float  浮点数列表：std::vector  字符串：string  字符串列表：std::vector  布尔：bool  布尔列表：std::vector  Tensor：Tensor  Tensor列表：std::vector  Bytes：字节数组，SetAttr接受通过OpBytes（即vector），和（const uint8_t* data, size_t size）表示的字节数组  AttrValue类型  整型二维列表类型：std::vector>  DataType列表类型：std::vector  DataType类型：ge::DataType  NamedAttrs类型： ge::NamedAttrs  NamedAttrs列表类型：std::vector
...
Operator &SetAttr(const std::string &name, const std::vector &attr_value);
Operator &SetAttr(const std::string &name, const std::vector &attr_value);
Operator &SetAttr(const std::string &name, const std::vector &attr_value);
Operator &SetAttr(const std::string &name, std::initializer_list &&attr_value);
...
Operator &SetAttr(const char_t *name, const std::vector &attr_value);
Operator &SetAttr(const char_t *name, const std::vector &attr_value);
Operator &SetAttr(const char_t *name, const std::vector &attr_value);
Operator &SetAttr(const char_t *name, std::initializer_list &&attr_value);
...
Operator &SetAttr(const char_t *name, const std::vector &attr_values);
...
| attr_value | 输入 | 需设置的vector表示的整型列表类型属性值。 |
| attr_value | 输入 | 需设置的vector表示的整型列表类型属性值。 |
| attr_value | 输入 | 需设置的std::initializer_list&&表示的整型列表类型属性值。 |
...
| attr_value | 输入 | 需设置的Bytes，即字节数组类型的属性值，OpBytes即vector。 |
| data | 输入 | 需设置的Bytes，即字节数组类型的属性值，指定了字节流的首地址。 |
| size | 输入 | 需设置的Bytes，即字节数组类型的属性值，指定了字节流的长度。 |
| attr_value | 输入 | 需设置的量化数据的属性值。 |
| attr_value | 输入 | 需设置的vector>表示的整型二维列表类型属性值。 |
| attr_value | 输入 | 需设置的vector表示的DataType列表类型属性值。 |
| attr_value | 输入 | 需设置的DataType类型的属性值。 |
| attr_value | 输入 | 需设置的NamedAttrs类型的属性值。 |
| attr_value | 输入 | 需设置的vector表示的NamedAttrs列表类型的属性值。 |
