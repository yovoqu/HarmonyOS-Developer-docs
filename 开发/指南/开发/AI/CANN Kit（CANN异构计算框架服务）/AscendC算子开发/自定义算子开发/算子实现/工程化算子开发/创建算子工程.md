...
3. -c：ai_core-&lt;soc_version&gt;代表算子在AI Core上执行，&lt;soc_version&gt;为AI处理器的型号，当前支持ai_core-kirin9020、ai_core-kirinX90。
4. -f：第三方算子平台，目前支持TF、ONNX。
5. -out：生成文件所在路径，可配置为绝对路径或者相对路径，并且工具执行开发者对路径具有可读写权限。若不配置，则默认生成在执行命令的当前路径。
6. 命令执行完后，会在-out指定目录或者默认路径下生成算子工程目录，工程中包含算子实现的模板文件，编译脚本等，以AddCustom算子为例，目录结构如下所示：
