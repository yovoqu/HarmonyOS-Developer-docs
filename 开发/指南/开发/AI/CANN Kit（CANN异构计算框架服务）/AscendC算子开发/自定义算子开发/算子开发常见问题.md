...
6. 查看具体层的堆栈信息，打印具体变量的值。本示例中，打印了tileLength为1024，该程序中表示需要处理1024个half类型的数，大小为1024*sizeof(half)=2048字节；输入Tensor xLocal的值，其中dataLen表示LocalTensor的size大小为1024字节，只能计算1024字节的数据。可以看出两者的长度不匹配，由此可以定位问题。
```cpp
(gdb) f 5
...
![](assets/算子开发常见问题/file-20260514132300839-1.png)
...
4. 手动拷贝opc npu compile start后的命令，如图2所示，并在终端窗口执行，通过打屏或者落盘的日志文件进一步分析问题。

  **图2** NPU编译命令

  
![](assets/算子开发常见问题/file-20260514132300839-2.png)
...
![](assets/算子开发常见问题/file-20260514132300839-3.png)
...
4. 手动拷贝关键字后的所有命令，在终端窗口分别执行，通过打屏或者落盘的日志文件信息进一步分析问题。
...
![](assets/算子开发常见问题/file-20260514132300839-4.png)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2d/v3/mr6hgni1QMmPutA6R_0aQg/zh-cn_image_0000002611755293.png?HW-CC-KV=V1&HW-CC-Date=20260528T014258Z&HW-CC-Expire=86400&HW-CC-Sign=BA94781FD1755E4D41D7999DD14933B7515316B25FDD825BC7F5DEE85232FD34)
...
3. 手动拷贝gen_tiling_data_cmd后的所有命令，在终端窗口执行，通过打屏或者落盘的日志文件进一步分析问题。

  
> [!NOTE]
> 执行命令之前，请确保当前终端 环境准备 设置并生效。
