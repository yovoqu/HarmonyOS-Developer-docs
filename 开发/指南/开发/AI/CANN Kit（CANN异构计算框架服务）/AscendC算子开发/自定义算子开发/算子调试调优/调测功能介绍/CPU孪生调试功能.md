...
> [!NOTE]
> 当开发者需要快速进行代码逻辑调试，可优先选择算子CPU调测。使用通用的打印、gdb调测手段，快速定位代码问题。  在NPU板端上板运行之前，可优先选择算子CPU调测初步定位算子精度问题，提高算子NPU上板成功率。

 CPU孪生调试主要基于开发者输入生成编译所需的二进制bin文件，然后自动执行算子编译和运行，该阶段支持的调测项如表1所示。 **表1** CPU调测功能列表
| 功能名称 | 功能说明 |
| --- | --- |
...
通过命令行进行CPU调测的关键步骤如下。 完成环境搭建，并准备好输入/标杆数据文件。  执行如下命令进行CPU调测，这里仅提供关键参数项示例，其他参数请参考[CPU调测参数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-cli-parameters#cpu调测参数)按需设置。
```text
...
CPU调测涉及的所有参数可通过**ascendebug kernel -h**或**ascendebug kernel --help**查看。  查看结果文件，详细说明参见[产物说明](#产物说明)。
