...
以下各步骤示例为片段代码，可通过示例代码右下方链接获取[完整示例](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Media/Audio/AudioCaptureSampleJS)。 查询返听能力并创建AudioLoopback实例，音频返听模式可以查看[AudioLoopbackMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#audioloopbackmode20)。
> [!NOTE]
...
![](assets/实现音频低时延耳返/file-20260514131444974-1.png)
...
在启用返听前设置音量，音量将在启用返听成功后生效。 在启用返听后设置音量，音量将立即生效。 启用返听前未设置音量，启用返听时将采用默认音量0.5。
```text
import { BusinessError } from '@kit.BasicServicesKit'; // 导入BusinessError。
// ...
    try {
...
在启用返听前设置混响模式，混响模式将在启用返听成功后生效。 在启用返听后设置混响模式，混响模式将立即生效。 启用返听前未设置混响模式，启用返听时将采用默认混响模式[THEATER](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#audioloopbackreverbpreset21)。
```text
import { BusinessError } from '@kit.BasicServicesKit'; // 导入BusinessError。
// ...
    try {
...
在启用返听前设置均衡器类型，均衡器类型将在启用返听成功后生效。 在启用返听后设置均衡器类型，均衡器类型将立即生效。 启用返听前未设置均衡器类型，启用返听时将采用默认均衡器类型[FULL](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#audioloopbackequalizerpreset21)。
```text
...
// ...
// 设置监听事件，启用音频返听。
async function enable(updateCallback?: (msg: string, isError: boolean) => void): Promise {
...
// 禁用音频返听，关闭监听事件。
async function disable(updateCallback?: (msg: string, isError: boolean) => void): Promise {
  if (audioLoopback !== undefined) {
    try {
      let status = await audioLoopback.getStatus();
...
// 设置音频返听音量。
async function setVolume(volume: number, updateCallback?: (msg: string, isError: boolean) => void): Promise {
  if (audioLoopback !== undefined) {
    try {
      await audioLoopback.setVolume(volume);
      console.info(`Invoke setVolume ${volume} succeeded.`);
...
async function setEqualizerPreset(preset: audio.AudioLoopbackEqualizerPreset, updateCallback?:
  (msg: string, isError: boolean) => void): Promise {
  if (audioLoopback !== undefined) {
    try {
