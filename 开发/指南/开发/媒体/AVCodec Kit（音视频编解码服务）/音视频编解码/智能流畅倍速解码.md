...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-smart-fluency-decoding
...
在传统多媒体架构中，开启高倍速播放会导致视频理论帧率成倍增加（如60fps视频在3.0x下需每秒处理180帧），并带来以下问题。
 ...
 - 移动端设备的视频送显帧率通常受限于整机功耗管控（常为60fps），远低于高倍速下的理论帧率。此外，在高速动态画面下，受限于人眼的视觉暂留效应与动态视觉敏感度下降，强行解码并渲染超出分辨能力的微小运动帧，会产生无效的算力消耗。
 - 传统架构中的倍速丢帧通常在应用层的渲染端被动触发，此时多余帧的解码算力已经消耗。若应用层试图在解码前主动进行内容感知，则需引入高耗能的像素级计算（如光流分析），导致整体功耗反而增加。
...
本能力需主动下发配置才可以生效。通过在[OH_AVFormat](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-core-oh-avformat)中配置以下键值对来精确控制行为。
     
 - **OH_MD_KEY_VIDEO_DECODER_FRAME_RETENTION_MODE（模式配置）**
       
        类型：int32_t
 - 取值范围：参考[OH_FrameRetentionMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-base-h#oh_frameretentionmode)。
 - 描述：能力的总开关与模式选择，建议在初始化阶段配置。
       
 - **OH_MD_KEY_VIDEO_DECODER_SPEED（解码倍速）**
       
        类型：double
 - 取值范围：严格大于0.0。
 - 描述：**在ADAPTIVE模式下必须同步配置此参数**，作为系统动态计算的依据。
       
 - **OH_MD_KEY_VIDEO_DECODER_FRAME_RETENTION_RATIO（保留比例）**
...
系统支持在运行态下动态切换帧保留模式。开发者可通过调用OH_VideoDecoder_SetParameter实时下发配置，参数的生效时机与该接口的标准行为一致。
