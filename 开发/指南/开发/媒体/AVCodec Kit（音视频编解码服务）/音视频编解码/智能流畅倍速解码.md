...
2. 取值范围：参考[OH_FrameRetentionMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-base-h#oh_frameretentionmode)。

3. 描述：能力的总开关与模式选择，建议在初始化阶段配置。

4. **OH_MD_KEY_VIDEO_DECODER_SPEED（解码倍速）**
...
6. 描述：**在ADAPTIVE模式下必须同步配置此参数**，作为系统动态计算的依据。

7. **OH_MD_KEY_VIDEO_DECODER_FRAME_RETENTION_RATIO（保留比例）**

  
类型：double

8. 取值范围：[0.01, 1.0]（例如0.5表示保留50%的解码帧）。

9. 描述：仅在UNIFORM模式下生效。当配置越界或异常时，系统默认以30fps为目标帧率进行兜底。
