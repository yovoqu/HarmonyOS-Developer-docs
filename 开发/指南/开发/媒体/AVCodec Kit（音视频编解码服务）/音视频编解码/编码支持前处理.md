...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding-preproc
...
编码前处理是视频编码器在输入帧进入编码管线之前执行的预处理能力，通过[OH_VideoEncoder_CreatePrimaryWithPreproc](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-videoencoder-h#oh_videoencoder_createprimarywithpreproc)创建的主编码器支持以下三种前处理功能：
...
支持前处理编码和普通编码器的使用流程一致，主要差异点在于创建方式、支持前处理参数配置以及动态更新。本文主要对差异点进行详细说明。完整编码器开发流程参考视频编码[Surface模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#surface模式)。
...
参考视频编码[Surface模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#surface模式)的“步骤3-调用OH_VideoEncoder_RegisterCallback()设置回调函数”。
    
    
          
##### [h2]配置编码参数与前处理参数
     
编码器参数配置参考视频编码[Surface模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#surface模式)的“步骤5-调用OH_VideoEncoder_Configure()配置编码器”。以下内容重点说明基础参数与前处理参数的配置。
...
**重要规则**：
 ...
 - **仅主编码器**可以调用GetSurface，副编码器调用将返回AV_ERR_OPERATE_NOT_PERMIT。
...
参考视频编码[Surface模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#surface模式)的步骤7、8、10。
...
其他编码器仍可动态配置，参考视频编码[Surface模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#surface模式)的步骤9。
    
    
          
##### [h2]通知编码结束、释放编码帧、销毁编码器
     
和普通编码器一致，参考视频编码[Surface模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/video-encoding#surface模式)的步骤12、13、17。
