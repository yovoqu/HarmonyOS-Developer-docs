...
2. capability：编码器能力查询实例的指针。

3. [OH_AVCODEC_MIMETYPE_VIDEO_AVC](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-base-h#变量)：AVC格式视频编解码器。
...
5. 参数校验规则请参考[OH_VideoEncoder_Configure()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-videoencoder-h#oh_videoencoder_configure)。

6. 参数取值范围可以通过能力查询接口获取，具体示例请参考[获取支持的编解码能力](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/obtain-supported-codecs)。

7. 要使能视频编码同步模式，必须将OH_MD_KEY_ENABLE_SYNC_MODE配置为1。

8. 同步模式在调用OH_VideoEncoder_Configure接口前不能调用OH_VideoEncoder_RegisterCallback或OH_VideoEncoder_RegisterParameterCallback接口，否则为异步模式。

9. 不支持Surface模式的随帧通路的同步模式。
 - 设置surface。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b8/v3/lDv7jcoSSGCT7i9IkSrGsQ/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260528T030337Z&HW-CC-Expire=86400&HW-CC-Sign=68A7DFB9F8BFEA05D3BC291DC9F101E608C9BC00C34700878004CBE8F141A81B)
 

  
 - 要使能视频编码同步模式，必须将OH_MD_KEY_ENABLE_SYNC_MODE配置为1。

3. 同步模式在调用OH_VideoEncoder_Configure接口前不能调用OH_VideoEncoder_RegisterCallback或OH_VideoEncoder_RegisterParameterCallback接口，否则为异步模式。
 - 调用OH_VideoEncoder_Prepare()编码器就绪。
