...
4. OH_AVCodecOnStreamChanged：码流信息变化回调，包括采样率变化、声道数变化、音频采样格式变化，支持检测此变化的解码格式有：Audio Vivid，AAC，FLAC，MP3，VORBIS。(API version 15开始支持)
5. OH_AVCodecOnNeedInputBuffer：运行过程中需要新的输入数据，即解码器已准备好，可以输入数据。
6. OH_AVCodecOnNewOutputBuffer：运行过程中产生了新的输出数据，即解码完成。
7. （可选）OH_AudioCodec_SetDecryptionConfig设置解密配置。
...
8. 调用OH_AudioCodec_Configure()配置解码器。

  配置选项key值说明：

  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c8/v3/UyXdAfBzQ-a77E6znr8wCw/zh-cn_image_0000002587109036.png?HW-CC-KV=V1&HW-CC-Date=20260604T012921Z&HW-CC-Expire=86400&HW-CC-Sign=B02A49C1D4EF7E789B69D70518EAAD1098B41B3C92E4ECCC2F591F0263058526)


  各音频解码类型参数范围说明：

  
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7f/v3/FYgYSldyQXiSxnoJ1wdPIA/zh-cn_image_0000002617668717.png?HW-CC-KV=V1&HW-CC-Date=20260604T012921Z&HW-CC-Expire=86400&HW-CC-Sign=050C3F4560A1FCAA3F602A7C34BBC82EC820475F826E4C4053CD622096395331)


  从API version 20开始，支持通过[OH_AVCapability_GetAudioSupportedSampleRateRanges](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcapability-h#oh_avcapability_getaudiosupportedsamplerateranges)接口进行采样率范围能力查询，以下几种音频解码类型支持对范围内的任意采样率进行解码：
...
9. 调用OH_AudioCodec_Prepare()，解码器就绪。
...
10. 调用OH_AudioCodec_Start()启动解码器，进入运行态。
...
12. 调用OH_AudioCodec_PushInputBuffer()，写入待解码的数据。
...
13. 调用OH_AudioCodec_FreeOutputBuffer()，释放解码后的数据。
...
2. 在执行过程中遇到可继续执行的错误时（即OH_AudioCodec_IsValid()为true）可以调用刷新，然后调用OH_AudioCodec_Start()重新开始解码。
3. （可选）调用OH_AudioCodec_Reset()重置解码器。
...
5. 调用OH_AudioCodec_Destroy()销毁解码器实例，释放资源。
