...
将音视频文件解码为PCM码流，通常需要以下步骤：[媒体数据解封装](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-video-demuxer) -> 音频解码。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c/v3/DldldWpfScKiWadnkiaerA/zh-cn_image_0000002656007148.png?HW-CC-KV=V1&HW-CC-Date=20260730T071938Z&HW-CC-Expire=86400&HW-CC-Sign=8DBB6B3662F6AC73D562D0A0F60E4E809293174D553E9A65DB77DC216B06934B)
...
7. （可选）调用OH_AVCencInfo_SetAVBuffer()，设置cencInfo。
  ...
  如果当前播放的节目是DRM加密节目，并且由上层应用进行[媒体数据解封装](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-video-demuxer)，则需要调用OH_AVCencInfo_SetAVBuffer()将cencInfo设置给AVBuffer，以实现媒体数据的解密。
...
10. 在解码输出buffer属性为AVCODEC_BUFFER_FLAGS_EOS后，若想重新使用相同配置进行解码时，需要调用刷新。
11. 在执行过程中遇到可继续执行的错误时（即OH_AudioCodec_IsValid()为true）可以调用刷新，然后调用OH_AudioCodec_Start()重新开始解码。
