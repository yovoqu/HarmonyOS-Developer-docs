...
参考以下示例代码，完成音视频封装的全流程。以封装mp4格式的音视频文件为例。 不同的封装格式需要配置的key请参考[AVCodec支持的格式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/avcodec-support-formats#媒体数据封装)。 添加头文件。
```text
#include
#include
#include
...
添加文件级数据。 文件级数据已定义的key详见[AVCodec支持的格式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/avcodec-support-formats#媒体数据封装)。 用户自定义的key必须以"com.openharmony."为开头。值类型可以为int32_t、float、string，从API20开始增加支持uint8_t*。
> [!NOTE]
...
添加音频轨。 **方法一：用OH_AVFormat_Create创建format**
```text
int audioTrackId = -1;
...
if (ret != AV_ERR_OK || audioTrackId **方法二：用OH_AVFormat_CreateAudioFormat创建format**
```text
int audioTrackId = -1;
uint8_t *buffer = ...; // 编码config data，如果没有可以不传。
size_t size = ...;  // 编码config data的长度，根据实际情况配置。
...
int ret = OH_AVMuxer_AddTrack(muxer, &audioTrackId, formatAudio);
if (ret != AV_ERR_OK || audioTrackId 添加视频轨。
> [!NOTE]
> 当开发者需要设置色彩信息时，必须保证OH_MD_KEY_COLOR_PRIMARIES、OH_MD_KEY_TRANSFER_CHARACTERISTICS、OH_MD_KEY_MATRIX_COEFFICIENTS三个key设置的值都在其各自的参数范围内，系统才会识别这是有效色彩信息数据。参数设置范围可参考OH_ColorPrimary、OH_TransferCharacteristic和OH_MatrixCoefficient。 当开发者输入的色彩信息与视频码流的参数集中的色彩信息不一致时，系统以视频码流中的色彩信息为准。
...
if (ret != AV_ERR_OK || videoTrackId **方法二：用OH_AVFormat_CreateVideoFormat创建format**
```text
int videoTrackId = -1;
uint8_t *buffer = ...; // 编码config data，如果没有可以不传。
size_t size = ...;  // 编码config data的长度，根据实际情况配置。
...
int ret = OH_AVMuxer_AddTrack(muxer, &videoTrackId, formatVideo);
if (ret != AV_ERR_OK || videoTrackId 添加辅助轨。
> [!NOTE]
> 设置OH_MD_KEY_TRACK_TYPE时，值为MEDIA_TYPE_AUXILIARY代表添加辅助轨。 设置OH_MD_KEY_TRACK_REFERENCE_TYPE时，值必须为"hint"、"cdsc"、"font"、"hind"、"vdep"、"vplx"、"subt"、"thmb"、"auxl"、"cdtg"、"shsc"或"aest"其中一项。 设置OH_MD_KEY_TRACK_DESCRIPTION时，值必须为"com.openharmony."开头且长度不超过256的字符串。 设置OH_MD_KEY_REFERENCE_TRACK_IDS时，track id值必须大于等于0，且必须是已经存在的track id。

**添加音频辅助轨**
```text
int32_t audioAuxlTrackId = -1;
std::vector audioTrackIDsVector = {0}; // 根据实际情况设置被辅助音频轨的编号，可填写多个值。
...
OH_AVFormat_SetIntValue(audioAuxlFormat, OH_MD_KEY_TRACK_TYPE, static_cast(OH_MediaType::MEDIA_TYPE_AUXILIARY)); // 必填。
...
if (ret != AV_ERR_OK || audioAuxlTrackId **添加视频辅助轨**
```text
int32_t videoAuxlTrackId = -1;
std::vector videoTrackIDsVector = {0}; // 根据实际情况设置被辅助视频轨的编号，可填写多个值。
...
OH_AVFormat_SetIntValue(videoAuxlFormat, OH_MD_KEY_TRACK_TYPE, static_cast(OH_MediaType::MEDIA_TYPE_AUXILIARY)); // 必填。
...
if (ret != AV_ERR_OK || videoAuxlTrackId 添加封面轨。 **方法一：用OH_AVFormat_Create创建format**
```text
int coverTrackId = -1;
...
if (ret != AV_ERR_OK || coverTrackId **方法二：用OH_AVFormat_CreateVideoFormat创建format**
```text
int coverTrackId = -1;
OH_AVFormat *formatCover = OH_AVFormat_CreateVideoFormat(OH_AVCODEC_MIMETYPE_IMAGE_JPG, 1280, 720);

int ret = OH_AVMuxer_AddTrack(muxer, &coverTrackId, formatCover);
if (ret != AV_ERR_OK || coverTrackId 调用OH_AVMuxer_Start()开始封装。
```text
...
调用OH_AVMuxer_WriteSampleBuffer()，写入封装数据。 封装数据包括视频、音频、封面等数据。
```text
// start后，才能开始写入数据。
int size = ...;
...
调用OH_AVMuxer_Destroy()销毁实例，释放资源。 注意不能重复销毁，否则将会导致程序崩溃。
```text
