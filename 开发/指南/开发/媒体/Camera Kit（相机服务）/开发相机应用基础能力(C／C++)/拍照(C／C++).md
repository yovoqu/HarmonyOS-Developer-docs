...
![](assets/拍照(C／C++)/file-20260514131526557-0.png)
...
6. 在单段式拍照回调函数中获取图片信息，解析出buffer数据，做自定义业务处理。

7. 将处理完的buffer通过回调传给ArkTS侧，做图片显示或通过安全控件写文件保存图片。
...
9. 创建拍照类型会话，参考[会话管理(C/C++)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/native-camera-session-management)，开启会话，准备拍照。
...
![](assets/拍照(C／C++)/file-20260514131526557-1.png)
