...
5. 在单段式拍照回调函数中获取图片信息，解析出pixelMap数据，做自定义业务处理。

6. 将处理完的pixelMap通过回调回传，做图片显示或通过安全控件写文件保存图片。

7. 使用完后解注册单段式拍照回调函数。

8. **分段式拍照（PhotoAvailable）开发流程**：
...
9. 在分段式拍照回调函数中获取图片信息，解析出pixelMap数据，做自定义业务处理。
