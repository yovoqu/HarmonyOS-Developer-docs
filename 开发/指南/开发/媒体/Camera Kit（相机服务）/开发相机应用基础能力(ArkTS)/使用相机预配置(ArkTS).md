...
![](assets/使用相机预配置(ArkTS)/file-20260514131523949-0.png)
...
![](assets/使用相机预配置(ArkTS)/file-20260514131523949-1.png)
...
> [!NOTE]
> SceneMode需要指定为NORMAL_PHOTO或NORMAL_VIDEO，对应拍照场景 PhotoSession 和录像场景 VideoSession 。
...
> [!NOTE]
> 若当前模式为普通录像模式，且 PreconfigType 使用PRECONFIG_HIGH_QUALITY类型时，为避免录制失败，请务必将 AVRecorderProfile 配置文件中videoCodec参数必须配置为 VIDEO_HEVC ，isHdr参数配置为true。
