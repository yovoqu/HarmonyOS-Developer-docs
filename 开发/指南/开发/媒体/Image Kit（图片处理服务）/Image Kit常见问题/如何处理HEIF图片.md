...
如果应用需要获取HEIF原图，应显式声明应用支持HEIF图片。通过[PhotoViewPicker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-photoviewpicker)获取图片时，可在[PhotoSelectOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-class#photoselectoptions)中配置assetCompatibleCapability，并通过preferredCompatibleMode指定首选兼容模式。
...
获取到[photoAccessHelper.PhotoSelectResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-class#photoselectresult)结果后，应用可使用photoSelectResult.photoUris读取返回URI。

如果应用不通过PhotoViewPicker选择图片，而是通过[PhotoAccessHelper](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-photoaccesshelper)查询、打开或批量处理媒体资产。例如网盘、备份、内容归档等应用场景，可使用[setAssetCompatibleCapability](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-photoaccesshelper#setassetcompatiblecapability24)接口配置[AssetCompatibleCapability](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-i#assetcompatiblecapability24)，声明应用支持image/heic。配置后再通过getAssets等接口访问资产。
...
应用根据URI后缀、文件后缀名或媒体库[PhotoKeys](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-e#photokeys)记录的MEDIA_SUFFIX作为图片文件格式的依据。文件真实格式应以其二进制数据内容为准。

应用根据媒体库[PhotoKeys](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-e#photokeys)记录的SIZE作为文件大小，但最终获取到的图片文件由于经过兼容性转换，导致文件体积发生了变化。如果此时直接按照原图大小使用该图片文件，由于HEIF图片通常比JPG图片体积更小，仅读取JPG文件的一部分就认为文件结束，会导致图片显示不完整。
...
C/C++应用可参考[使用Image_NativeModule完成图片解码](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-source-c)，通过ImageSource读取图片真实格式、宽高等信息。
