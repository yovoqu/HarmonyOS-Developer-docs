...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-common-mistakes
...
#15 pc 00000000000b7d00 /system/lib64/platformsdk/libimage_native.z.so(std::__h::__function::__func, unsigned int (OHOS::ImagePlugin::AbsImageEncoder*)>::operator()(OHOS::ImagePlugin::AbsImageEncoder*&&)+28)(abee48eb37a365d523ba3560f087b63a)
#16 pc 00000000000b58b4 /system/lib64/platformsdk/libimage_native.z.so(OHOS::Media::ImagePacker::DoEncodingFunc(std::__h::function, bool)+272)(abee48eb37a365d523ba3560f087b63a)
...
**崩溃原因：** 编码过程中未使用await等待异步操作执行完毕，致使资源对象在异步操作完成前被提前释放或修改。 **解决措施：** **异步操作的生命周期管理：** 在调用Image Kit的异步接口（如[packToData](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagepacker#packtodata13)、[packToFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagepacker#packtofile11)、[createPixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagesource#createpixelmap7)等）时，必须确保传入的资源对象（如PixelMap、ImageSource）在异步操作完成之前不被释放或修改。 **使用await或Promise.then：** 推荐使用await等待异步操作完成，或者在Promise.then()的回调中释放资源，确保释放时机正确。 **页面生命周期管理：** 如果在页面中使用异步图片操作，需要在页面销毁时确保所有异步操作已经完成或取消，避免页面卸载后异步回调访问已销毁的资源。 **错误代码示例：**
```text
import { image } from '@kit.ImageKit';
import { BusinessError } from '@kit.BasicServicesKit';

async function wrongPackingExample(pixelMap: image.PixelMap, fd: number): Promise {
...
async function correctPackingExample(pixelMap: image.PixelMap, fd: number): Promise {
  let imagePacker = image.ImagePacker | null = null;
...
#00 pc 00000000000b0864 /system/lib64/platformsdk/libimage_napi.z.so(OHOS::Media::CreatePixelMapInner(OHOS::Media::ImageSourceNapi*, std::__h::shared_ptr, unsigned int, OHOS::Media::DecodeOptions, unsigned int8)+116) (3a63d0a0dc3ac58d9e1a58a77ad194f9)
...
**崩溃原因：** 并发操作同一个ImageSource对象，引发资源竞态问题导致崩溃。 **解决措施：** **避免并发访问：** 不要对同一个ImageSource实例并发执行多个异步操作，推荐按顺序执行，确保一个操作完成后再执行下一个操作。 **资源生命周期：** ImageSource的生命周期应该覆盖所有使用它的异步操作，只有在确认所有异步操作都完成后才能释放。 **性能考虑：** 虽然顺序执行可能会降低并发性能，但可以避免竞态条件和崩溃问题。如果确实需要并发处理，应该创建多个独立的ImageSource实例。 **错误代码示例：**
```text
import { image } from '@kit.ImageKit';
import { BusinessError } from '@kit.BasicServicesKit';

async function wrongSharedImageSourceExample(filePath: string, decodingOptions: Array): Promise> {
  // 创建一个ImageSource实例。
  const imageSource = image.createImageSource(filePath);
  const pixelMaps: Array = [];
...
async function correctSharedImageSourceExample(filePath: string): Promise {
  // 创建一个ImageSource实例。
  const imageSource = image.createImageSource(filePath);
