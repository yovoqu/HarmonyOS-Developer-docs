...
调用者可以调用本模块提供的[C API接口](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-imageprocessing)，实现HDR图片动态元数据生成。
 ...
 ![](assets/图片动态元数据生成/file-20260514131540726-0.png)
...
创建图片元数据生成模块。  应用可以通过图片处理引擎模块类型来创建图片元数据生成模块。示例中的变量说明如下： instance：图片处理模块实例。  IMAGE_PROCESSING_TYPE_METADATA_GENERATION：图片元数据生成类型。  预期返回值：IMAGE_PROCESSING_SUCCESS
```text
