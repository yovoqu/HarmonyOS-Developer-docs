...
3. Picture路径：解码为Picture对象，可分别获取SDR主图和GainMap增益图，并按需合成HDR图。

4. 如果使用PixelMap路径，获取PixelMap图像信息，判断解码结果是否为HDR。

5. 如果使用Picture路径，可根据业务场景进行以下处理：      
...
6. 调用[OH_PictureNative_GetGainmapPixelmap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-picture-native-h#oh_picturenative_getgainmappixelmap)获取GainMap增益图。

7. 调用[OH_PictureNative_GetHdrComposedPixelmap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-picture-native-h#oh_picturenative_gethdrcomposedpixelmap)合成HDR图。

8. 使用完成后释放ImageSource、PixelMap、Picture和解码参数等资源。
...
12. 需要用于HDR显示或处理时，可合成HDR PixelMap。
