...
动图解码主要包含以下流程：
 ...
 - 创建ImageSource实例。
 - 调用接口获取图片帧数，判断图片是否为动图。
 - 获取每帧的延迟时间。
 - 根据业务场景选择全量解码所有帧，或按索引解码指定帧。
 - 使用完成后释放ImageSource、PixelMap和解码参数等资源。
...
```
 ...
 - 定义动图解码相关类，用于保存ImageSource实例、帧数和解码得到的PixelMap对象。
...
napi_value GetAnimatedDelayTimeList(napi_env env, napi_callback_info info)
{
    ...
    if (errCode == IMAGE_SUCCESS) {
        for (size_t index = 0; index 全量解码：一次性解码所有帧，适用于帧数较少、需要连续播放所有帧的场景。全量解码会同时持有多帧PixelMap，内存占用与帧数和单帧大小相关。
...
napi_value CreateAnimatedPixelmapByIndex(napi_env env, napi_callback_info info)
{
    ...
    size_t argCount = 1;
    if (napi_get_cb_info(env, info, &argCount, argValue, nullptr, nullptr) != napi_ok ||
        argCount = g_thisAnimated->frameCnt) {
