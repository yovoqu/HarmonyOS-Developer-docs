...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-playback-concurrency-audio-session-overview
...
用户正在进行[VoIP通话](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-call-overview)，此时有系统来电，需要正确处理通话状态切换。

解决方案：系统来电对应音频焦点最高优先级，独占音频焦点，VoIP收到INTERRUPT_HINT_PAUSE后，执行暂停通话逻辑。来电结束后VoIP应用收到INTERRUPT_HINT_RESUME，可以恢复通话。
...
解决方案：后台应用请求音频焦点（通过会话机制或焦点机制），后台播放器收到中断回调，根据[InterruptHint](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-e#interrupthint)判断执行INTERRUPT_HINT_PAUSE或INTERRUPT_HINT_DUCK。当前台应用释放焦点后，后台播放器收到INTERRUPT_HINT_RESUME，继续播放。
...
解决方案：音乐播放器收到来电的INTERRUPT_HINT_PAUSE提示后暂停播放，来电结束时收到INTERRUPT_HINT_RESUME提示，自动恢复播放。

焦点特点：利用音频焦点的自动恢复机制，INTERRUPT_TYPE_END事件携带RESUME提示，应用被动响应恢复播放，适合应用保持播放状态。
...
| 方案六 | 无需适配 | 音频会话并发策略使用CONCURRENCY_DUCK_OTHERS | A音量压低，B停止后A音量自动恢复 | 音量自动恢复，无需额外适配 |
...
同应用内不同音频流默认使用相同的焦点策略，如需对各音频流进行差异化管理，应用需先将[焦点模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-playback-concurrency#焦点模式)设置为独立焦点模式，再分别为每条流设置对应的焦点策略。
