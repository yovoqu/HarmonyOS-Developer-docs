...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/background-recording
...
应用实现录音功能的具体内容请参考[使用AudioCapturer开发音频录制功能(ArkTS)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-audiocapturer-for-recording)、[推荐使用OHAudio开发音频录制功能(C/C++)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-ohaudio-for-recording)或[使用AVRecorder录制音频(ArkTS)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-avrecorder-for-recording)。当应用需要退至后台持续录音时，还需要申请录音类型的[长时任务(ArkTS)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/continuous-task)。
1. 声明后台录音模式。

  在[module.json5配置文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file)中，为需要执行后台录音的UIAbility声明audioRecording后台模式。
...
2. 启动录音任务。
  ...
  录音开始前，应用需要确认用户已经明确触发录音操作，并处理麦克风权限未授权、设备被占用、存储路径不可用等异常场景。录音开发方式请参考[音频录制开发概述](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-recording-overview)。

  针对应用在后台录音被其他音频流打断后无法恢复的场景，推荐使用以下解决方案：

  
应用录音在后台被打断，收到焦点恢复通知时出现start失败，可以弹窗提醒用户再次打开应用手动重新开始录音。
3. 使用音频录制接口[setIndependentAudioSessionStrategy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer#setindependentaudiosessionstrategy24)，AudioSessionBehaviorFlags使用MUTE_WHEN_INTERRUPTED，录音流焦点策略被设置成静音打断模式，被其他应用打断导致停止或暂停录制时会进入静音录制状态，在此状态下录制的音频没有声音，其他应用录制动作完成后恢复录制有声数据。
4. 申请录音类型长时任务。
...
```
长时任务启动失败时，应用需避免继续以后台录音方式运行，应停止录音或引导用户回到前台处理。长时任务的完整申请和取消流程请参考[长时任务(ArkTS)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/continuous-task)。
5. 停止录音并释放资源。

  用户停止录音、录音异常中断或业务结束时，应用需要调用AudioCapturer的[release](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer#release8)接口停止录音、释放音频采集资源，并同步取消录音类型长时任务。
