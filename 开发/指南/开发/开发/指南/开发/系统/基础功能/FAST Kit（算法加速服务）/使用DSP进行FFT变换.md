...
2. 调用HMS_FAST_FFT_CreateConfig（单精度）或HMS_FAST_FFT_CreateConfigD（双精度）创建FFT配置实例（FAST_FFTConfig）。
3. 根据FFT大小分配输入、输出数组。

  
正向变换：输入数组长度为N（实数时域信号），输出实部和虚部数组长度均为N/2+1。
4. 逆变换：输入实部和虚部数组长度均为N/2+1，输出数组长度为N（实数时域信号）。
5. 调用HMS_FAST_FFT_ForwardTransform/HMS_FAST_FFT_ForwardTransformD计算FFT，或调用HMS_FAST_FFT_InverseTransform/HMS_FAST_FFT_InverseTransformD计算IFFT。
6. 调用HMS_FAST_FFT_DestroyConfig销毁FFT配置实例，释放内部资源。
