...
#### 约束限制
 ...
 - 系统对”快启启动”功能进行了权限管控。线上使能特性需要开发者先提交”快启启动”申请。该权限仅面向企业开发者开放。       1. 登录[AppGallery Connect](https://developer.huawei.com/consumer/cn/service/josp/agc/index.html)，选择“开发与服务”。
...
考虑到实际调整业务逻辑的困难，部分对外数据交互难以剥离。快启机制为开发者提供新的回调接口[onLaunchFromHyperSnap()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onlaunchfromhypersnap24)，该回调接口仅在快启流程（而不是快启初始化流程）中执行，正常冷启动不执行该回调。开发者可以在该回调接口中重新请求外部数据进行同步，保证数据一致性。
...
借助[setHyperSnapEnabled()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-hypersnapmanager#hypersnapmanagersethypersnapenabled)，应用可以根据需要关闭快启功能，直至云推开关重新开启。
