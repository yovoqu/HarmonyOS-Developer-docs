...
> [!NOTE]
> 当设置目标UIAbility为不可见时： 目标UIAbility的窗口不会显示在前台。 Dock栏不会有图标。 目标UIAbility的onForeground生命周期不会被调用。
...
> [!NOTE]
> 如果未配置startWindowIcon，则默认采用module.json5文件中startWindowIcon字段的配置。 如果未配置startWindowBackgroundColor，则默认采用module.json5文件中startWindowBackground字段的配置。 背景颜色为ARGB格式，如：#E5FFFFFF（透明度为E5，RGB值为FFFFFF）。
