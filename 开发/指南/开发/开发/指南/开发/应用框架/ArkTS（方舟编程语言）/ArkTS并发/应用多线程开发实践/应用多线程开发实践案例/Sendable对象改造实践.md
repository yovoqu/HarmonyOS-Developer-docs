...
3. @SerialName({ name: 'xxx' })：将类属性与JSON字段名绑定。
...
4. 从JSON字符串反序列化为普通对象（TJSON.fromString）。
...
5. testJSON2方法是将手写的JSON字符串反序列化为普通对象，然后再转换为Sendable对象的过程。
...
6. 将普通对象转换为Sendable对象（toSendable）。
...
4. test_pb.encode(message)：将实例编码为ArrayBuffer。

5. test_pb.decode(buffer)：将二进制数据解码回test_pb。

6. 创建Sendable消息对象并进行编码与解码。
