...
用户进入光线追踪反射适用的游戏场景。游戏应用调用HMS_XEG_CreateRTReflection接口创建光线追踪反射实例。如果光线追踪反射创建失败，直接调用后处理并送显当前帧即可。游戏应用调用HMS_XEG_CmdRenderRTReflection接口计算光线追踪反射命中信息，并返回错误码。如果计算失败，直接调用后处理并送显当前帧即可。游戏应用根据3中计算的反射命中信息，生成反射图像。游戏应用将反射图像和游戏的主场景渲染结果进行融合。游戏应用完成其余后处理，并送显当前帧。用户退出光线追踪反射适用的游戏场景。游戏应用调用HMS_XEG_HMS_XEG_DestroyRTReflection接口销毁光线追踪反射实例。
