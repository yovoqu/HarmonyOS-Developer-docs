...
用户在进入游戏初始化场景时调用HMS_XEG_GetString接口查询XEngine支持的特性，当查询接口返回支持的特性列表中包含自适应VRS时代表可以使用此特性。此时调用HMS_XEG_AdaptiveVRSParameter接口配置超分参数。当游戏运行时，游戏渲染当前帧纹理。在使用自适应VRS特性的阶段前，调用HMS_XEG_DispatchAdaptiveVRS接口计算着色率图。调用HMS_XEG_ApplyAdaptiveVRS将着色率图像应用到渲染目标中。游戏渲染剩下的纹理，如UI等。当前帧已全部渲染完成，进行送显。当游戏退出时，超分资源会自行释放。 下面是基于Vulkan图形API平台集成自适应VRS的主要业务流程
![](assets/自适应VRS/file-20260514131720849-1.jpg)
用户在进入游戏初始化场景时调用HMS_XEG_EnumerateDeviceExtensionProperties接口查询XEngine支持的特性，当查询接口返回支持的特性列表中包含自适应VRS时代表可以使用此特性。此时调用HMS_XEG_CreateAdaptiveVRS接口创建自适应VRS实例。使用自适应VRS特性时，需要创建能够支持VRS的vulkan资源。当游戏运行时，游戏渲染当前帧纹理。调用HMS_XEG_CmdDispatchAdaptiveVRS计算着色率图。将着色率图像应用到渲染目标中。渲染剩下的游戏纹理，如UI等。当前帧已全部渲染完成，进行送显。当游戏退出时，调用HMS_XEG_DestroyAdaptiveVRS接口销毁自适应VRS实例。
...
编译HAP时，Native层so编译需要依赖NDK中的libxengine.so。 头文件引用  按需引用XEngine的头文件，如使用OpenGL ES自适应VRS功能。
```text
#include
#include
#include
...
编写CMakeLists.txt  按需引用XEngine的CMakeLists，如使用OpenGL ES自适应VRS功能，CMakeLists.txt部分示例代码如下，完整示例代码请参见[Demo（GPU加速引擎-GLES）](https://gitcode.com/harmonyos_samples/xengine-samplecode-gles-demo-cpp)。
```text
find_library(
    # 设置路径变量的名称。
    xengine-lib
