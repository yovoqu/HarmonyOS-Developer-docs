...
| getGlyphRuns(): Array | 获取文本行的排版单元数组。 |
| getGlyphs(): Array | 获取该排版单元中每个字符的字形序号。 |
| getFont(): drawing.Font | 获取排版单元的字体属性对象。 |
| getAdvances(range: Range): Array | 获取该排版单元指定范围内每个字形的字形宽度数组。 |
| static makeFromRunBuffer(pos: Array, font: Font, bounds?: common2D.Rect): TextBlob | 基于RunBuffer信息创建TextBlob对象。 |
| static makeFromRunBuffer(pos: Array, font: Font, bounds?: common2D.Rect): TextBlob | 基于RunBuffer信息创建TextBlob对象。 |
...
从API version 18开始，支持获取文字塑形结果能力。从API version 20开始，支持获取文字排版方向和文字字形宽度。关键代码如下： 导入依赖的相关模块。
```text
...
let x: number = 0;
let y: number = 0;
for (let index = 0; index           效果展示：     ![](assets/自定义文本绘制与显示（ArkTS）/file-20260514131645987-0.png)
