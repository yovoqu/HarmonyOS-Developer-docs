...
1. 导入模块。 import { displaySync } from '@kit.ArkGraphics2D';
2. 定义和构建DisplaySync对象。 @Entry
@Component
struct Index {
  // ...
...
3. 定义两个文本组件。 @State drawFirstSize: number = 25;
...
4. 通过DisplaySync实例设置帧率和注册订阅函数。 订阅函数运行于UI主线程，故涉及UI线程的耗时操作不应运行于订阅函数中，以免影响性能。   CreateDisplaySyncSlow() {
