...
#### 开发步骤
1. 定义ArkTS接口文件XComponentContext.ts，用来对接Native层。 export default interface XComponentContext {
  register(): void;
...
2. 定义演示页面，包含两个XComponent组件。 import XComponentContext from '../interface/XComponentContext';
// ...
...
3. 在 Native C++层获取NativeXComponent。建议使用单例模式保存XComponent。此步骤需要在napi_init的过程中处理。 创建一个PluginManager单例类，用于管理NativeXComponent。 class PluginManager {
public:
 ~PluginManager();
...
4. Native层配置帧率和注册回调函数。 定义每帧回调函数内容。 static void TestCallback(long long timestamp, long long targetTimestamp, void *data)
{
 // ...
...
} 使用DisplaySoloist接口配置帧率和注册每帧回调函数。如果使用OH_DisplaySoloist_Create创建DisplaySoloist实例时传入的参数useExclusiveThread为true，则OH_DisplaySoloist_FrameCallback以独占线程方式执行，否则OH_DisplaySoloist_FrameCallback以共享线程方式执行。 实例在调用NapiRegister后，在不需要进行帧率控制时，应进行NapiUnregister操作，避免内存泄漏问题。 在页面跳转时，应进行NapiUnregister和NapiDestroy操作，避免内存泄漏问题。 static std::unordered_map<std::string, OH_DisplaySoloist *> g_displaySync;
...
5. TS层注册和取消注册每帧回调，销毁OH_DisplaySoloist实例。 aboutToDisappear(): void {
  // ...
