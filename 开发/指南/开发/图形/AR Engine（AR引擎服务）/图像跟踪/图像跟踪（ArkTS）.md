...
图像跟踪能力支持部分Phone、部分Tablet设备。请参考[硬件要求](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arengine-preparations#硬件要求)判断设备是否支持运动跟踪及平面识别特性（[ARENGINE_FEATURE_TYPE_IMAGE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arengine-api-arengine#arfeaturetype)）。
...
struct ARImage {
  ...
  // 选择本地图片模式
  private async chooseImageToTrack(): Promise {
    try {
...
加载本地图片模式。 选择本地图片进行图像识别能力所需要导入的模块如下：
```text
// ARImageByAdd.ets
...
struct ARImageByAdd {
  ...
  build(): void {
    NavDestination() {
      RelativeContainer() {
        ...
        .visibility(this.addFailedImageCounts + this.succeedImageCounts  {
  ...
  private async stopARView(): Promise {
    if (!this.arContext) {
      return;
    }
    try {
  ...
  // 异步执行添加图片的任务
  async addImage(dataBase: arEngine.ARAugmentedImageDatabase): Promise {
...
async function saveBufferToLocal(dataBase: arEngine.ARAugmentedImageDatabase, context: Context): Promise {
...
class ARViewCallbackImpl extends arViewController.ARViewCallback {
  ...
  onFrameUpdate(ctx: arViewController.ARViewContext, sysBootTs: number): void {
    ...
    try {
      ...
      for (let i = 0; i  = new collections.Map([
  [0, 'success'],
  [1, 'size not match'],
...
加载本地数据库模式。 选择本地数据库进行图像识别能力所需要导入的模块如下：
```text
