...
![](assets/删除应用内快捷方式/file-20260514131759758-0.png)
用户需要删除桌面快捷方式。 应用调用[removePinShortcut](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-productviewmanager#productviewmanagerremovepinshortcut)接口删除快捷方式。 AppGallery Kit向应用弹出快捷方式删除确认框。 用户确认是否删除快捷方式。
...
| 接口名 | 描述 |
| --- | --- |
| [removePinShortcut](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-productviewmanager#productviewmanagerremovepinshortcut)(context: [common.UIAbilityContext](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-uiabilitycontext), shortcutId: string): Promise | 删除桌面快捷方式。 |
...
当应用已有自己的删除确认弹框并在弹框中提示用户删除桌面快捷方式时，开发者可以申请静默删除权限，实现在不显示系统确认弹框的情况下完成删除操作。 登录AppGallery Connect，选择“开发与服务”。
...
申请审批通过后，互动中心会发送通知给您，同时“申请中”按钮会变为置灰显示的“申请”。 能力申请通过后，勾选删除桌面快捷方式的能力开关，点击右上角“保存”。至此，您的应用已成功接入开放能力。
...
调用[removePinShortcut](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-productviewmanager#productviewmanagerremovepinshortcut)方法删除桌面快捷方式。
```text
