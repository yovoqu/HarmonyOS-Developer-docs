...
#### 开发步骤
#### 预签约（服务器开发）
1. 开发者按照商户模型调用预直连商户预签约或服务商预签约接口获取preSignNo构建签约信息参数contractStr。 为保证支付订单的安全性和可靠性需要对请求body和请求头PayMercAuth对象内的入参排序拼接进行签名。可参考签名规则。 以下为开放API接口请求及contractStr构建示例代码片段： import com.huawei.petalpay.paymentservice.apiservice.client.model.BaseGwRspWithSign;
