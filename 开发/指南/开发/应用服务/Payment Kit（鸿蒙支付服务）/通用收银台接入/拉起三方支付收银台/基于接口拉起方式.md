...
1. 商户客户端根据Payment Kit接口返回的支付信息PayResult(混合支付场景）/PickerResult（纯外部支付场景），按照三方支付平台接入要求构建三方支付信息payInfo调用ThirdPayClient.pay接口拉起三方支付收银台。 拉起三方支付收银台示例代码如下： import { BusinessError } from '@kit.BasicServicesKit';
...
2. 拉起三方支付收银台可同步通过调用ThirdPayClient.handlePayCallback接口（用户支付完成后，会将支付操作结果回调给商户客户端），获取三方支付处理结果，完成支付操作处理。参考示例代码如下： import { hilog } from '@kit.PerformanceAnalysisKit';
