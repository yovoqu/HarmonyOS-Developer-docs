...
| 收银台模式 | 是 | 收银台接入的支付模式（未配置商户三方支付相关配置时，默认为华为支付模式）。 2：混合支付模式（部分交易走华为支付通道，部分走外部三方支付通道） 3：纯外部支付模式（全部交易走外部三方支付通道） |
| 启用状态 | 是 | 是否启用应用申请的三方支付相关配置。 - 启用（默认） - 未启用 |
| 支付方式名称 | 是 | 三方支付方式配置项。 收银台展示的支付方式名称。 |
| 支付方式code | 否 | 三方支付方式配置项。当支付方式非微信支付和支付宝时需要填写该项。 |
| 拉起收银台方式 | 是 | 三方支付方式配置项。是否SDK方式拉起收银台 - 是：基于接口拉起三方支付收银台 - 否：基于URL跳转三方支付收银台。 说明： 不同的配置在拉起三方支付收银台方式上有区别，当前支持基于接口拉起方式拉起三方支付收银台支持的支付方式参见PayMethod。 |
| 支付方式跳转路径 | 否 | 三方支付方式配置项。基于URL跳转三方支付收银台方式时必选。 常见支付方式的跳转链接如下（供参考）。 - 支付宝：https://mclient.alipay.com - 微信支付：https://wx.tenpay.com |
| 支付方式deeplink链接 | 否 | 三方支付方式配置项。基于URL跳转三方支付收银台 方式时必选。 常见支付方式的deeplink链接如下（供参考）。 - 支付宝：alipays://platformapi - 微信支付：weixin://wap/pay |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/f9/v3/PRVy-0KFTviW6SCBeXWbEQ/zh-cn_image_0000002611835017.png?HW-CC-KV=V1&HW-CC-Date=20260528T030110Z&HW-CC-Expire=86400&HW-CC-Sign=0D3A9B6CBC494D4F06CAED3D07230C02D44B26396D22EA2DD591A28B0FD7D751)
