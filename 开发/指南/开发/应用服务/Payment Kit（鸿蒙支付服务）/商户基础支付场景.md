...
![](assets/商户基础支付场景/file-20260525091718698-001.png)

#### 业务流程
开发者通过接入Payment Kit基础支付，可以简便快捷的实现应用的支付能力。具体接入流程如下：

![](assets/商户基础支付场景/file-20260525091718698-002.png)
...
5. 商户客户端调用requestPayment接口拉起Payment Kit支付收银台。
...
2. 构建订单信息参数orderStr。 商户服务器需要将客户端支付接口入参orderStr签名后返回给客户端。 orderStr中sign字段签名规则是将除sign外的参数都做排序拼接后再签名（参见签名规则），签名值赋值给sign字段。   以下为开放API接口请求及orderStr构建示例代码片段： import com.huawei.petalpay.paymentservice.apiservice.client.model.BaseGwRspWithSign;
