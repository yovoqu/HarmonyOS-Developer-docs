...
流程说明： 应用调用拉起许可应用跳转页的接口，拉起健康使用设备查询开发者是否已申请权限，以及用户是否授权。 若状态为未授权，则抛出对应错误码；若状态为已授权，应用将根据传入的应用token信息获取全量应用信息，判断是否展示TrustApp，并拉起应用列表Form。 用户点击跳转页中的应用，跳转到相应的应用。
...
```text
private async jumpTo3rdApp(selectedAppTokens: string[], subtitle: string): Promise {
   try {
