...
#### 场景介绍
 ...
 - **订阅华为账号登录/登出事件**
  ...
  当应用需要跟随华为账号的登录状态进行登录登出时，可以通过订阅华为账号的登录登出事件进行判断，参考[华为账号登录/登出事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-notification-events#华为账号登录登出事件)。
