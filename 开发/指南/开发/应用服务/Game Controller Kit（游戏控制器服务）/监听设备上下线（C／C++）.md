...
玩家启动游戏。 游戏调用OH_GameDevice_RegisterDeviceMonitor接口注册设备状态变化事件监听。 玩家插拔设备。 终端系统将设备状态变化通知Game Controller Kit。 Game Controller Kit向游戏反馈设备状态变化。 游戏调用OH_GameDevice_GetAllDeviceInfos接口向Game Controller Kit查询所有在线的游戏设备信息。 Game Controller Kit从终端系统获取所有在线的游戏设备信息。 如果不再需要订阅，游戏可以调用OH_GameDevice_UnregisterDeviceMonitor接口取消设备状态变化事件监听。
...
GameController_ErrorCode DeviceApi::DoQueryAllDeviceInfos() {
    ...
    for (int idx = 0; idx PrintLog("AllDeviceInfos[" + std::to_string(idx) + "]" + temp);
