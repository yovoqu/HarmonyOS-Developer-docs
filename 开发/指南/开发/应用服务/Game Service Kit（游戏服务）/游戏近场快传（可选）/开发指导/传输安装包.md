...
![](assets/传输安装包/file-20260514131911674-0.png)
发送端设备打开游戏后与接收端设备通过[碰一碰](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/knock-share-between-phones-overview)或[隔空传送](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/gestures-share-overview)触发安装包传输。 发送端调用[create](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransfercreate)创建安装包传输任务。 创建成功后，游戏客户端调[onRemoteInstallationInfoNotify](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferonremoteinstallationinfonotify)注册远程安装信息事件监听。 游戏应用获取到安装游戏所需要的linkingForInstallation地址。 通过linkingForInstallation地址拉起接收端游戏服务。 接收端发送游戏安装包是否安装的信息，发送端接收到[onRemoteInstallationInfoNotify](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferonremoteinstallationinfonotify)远程安装信息事件监听回调。 根据接收端游戏是否已安装，后续分为两种情况： 接收端未安装游戏 发送端传输游戏安装包。 接收端检查游戏中心是否安装，若未安装将重新自动安装游戏中心。 接收端拉起游戏中心客户端，并打开游戏详情页。 接收端完成安装包的接收，安装并打开游戏。 接收端游戏中心客户端向游戏服务返回安装包安装结果。 接收端游戏服务自动关闭。 接收端已安装游戏 发送端调用[destroy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferdestroy)销毁服务，并确认是否进行资源包传输。若确认进行资源包传输，则发送端创建资源包传输任务，详情请参见[传输资源包](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/gameservice-nearbytransfer-resource-package)。接收端打开已安装的游戏。
> [!NOTE]
> destroy接口会清除已接收数据，请确保对已接收数据做好处理或转移后再调用该接口。 发送端或接收端每次调用create接口都会自动清理自身历史数据。
...
| [onRemoteInstallationInfoNotify](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferonremoteinstallationinfonotify)(callback: Callback): void | 订阅远程安装信息事件。 |
| [offRemoteInstallationInfoNotify](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferoffremoteinstallationinfonotify)(callback?: Callback): void | 取消订阅远程安装信息事件。 |
