...
> [!NOTE]
> openType参数填写"functionalButtonComponentManager.OpenType.GET_PHONE_NUMBER"指定Button为快速验证手机号类型。 controller参数必须对应填写"new functionalButtonComponentManager.FunctionalButtonController().onGetPhoneNumber"。 若成功调用，可通过回调函数中的临时登录凭证（Authorization Code）获取真实手机号，临时登录凭证时效5分钟，具体操作可参考 服务端开发 。 可使用自定义Modifier设置按钮样式，参考 示例 。 其他参数请参考： FunctionalButton（Button组件） 。
