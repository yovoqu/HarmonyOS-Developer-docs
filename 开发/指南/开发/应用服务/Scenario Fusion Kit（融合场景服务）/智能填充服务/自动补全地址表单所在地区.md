...
> [!NOTE]
> 需要在module.json5文件中设置模糊位置权限：ohos.permission.APPROXIMATELY_LOCATION。  所在地区地址选择器需要开通地图服务。  需要配置签名和指纹。


## 效果图

地址表单中的所在地区能根据详细地址内容自动补全，当补全内容不符合预期时，可通过地址选择器进行修改。
![](assets/自动补全地址表单所在地区/file-20260514132147939-0.png)
...
const PERMISSIONS: Array = ['ohos.permission.APPROXIMATELY_LOCATION'];
const ADMINISTRATIVE_REGION: Array =
