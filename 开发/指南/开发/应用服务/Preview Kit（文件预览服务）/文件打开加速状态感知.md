...
具体API说明详见[预加载状态感知接口文档](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/preview-arkts-openfileboost-api)。 **表1** 文件预加载状态感知接口介绍（ArkTS API）
| 接口名 | 描述 |
| --- | --- |
| on(type: 'filePreloadStateChanged', callback: Callback): void | 文件预加载状态回调，应用通过注册回调函数获取文件预加载的状态变化。 |
| off(type: 'filePreloadStateChanged', callback?: Callback): void | 文件预加载状态注销回调，通过注销回调函数取消获取文件预加载的状态变化。 |
