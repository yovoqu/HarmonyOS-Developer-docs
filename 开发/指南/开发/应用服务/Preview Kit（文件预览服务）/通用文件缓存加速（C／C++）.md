...
具体API说明详见[API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/openfileboost_preview)。 **表1** 文件缓存接口介绍
| 接口名 | 描述 |
| --- | --- |
...
std::vector keyVector(keyStr.begin(), keyStr.end());
...
uint32_t weight = 100;
// 用 abc 填充
for (size_t i = 0; i              如果开发者需要清除所有缓存，可以调用HMS_FileCacheBoost_ClearAllCache。
```text
