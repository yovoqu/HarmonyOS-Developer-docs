...
const PREFETCH_TASK_REGISTER_INTERVAL = 24 * 60 * 60 * 1000; // 24小时 = PREFETCH_DATA_UPDATE_INTERVAL) {
  ...
  private static async registerPrefetchTaskNotForced(token: string, params: string | object) {
    ...
    if (expireTime && (PrefetchUtil.now  {
...
预加载数据获取成功时，需要增加页面的渲染逻辑。  预加载数据获取失败时，需要做数据降级处理。如下示例代码以cloudFunctionCall接口触发云函数为例获取数据，请根据实际业务实现进行修改。  需要注意以下两点：  使用cloudFunctionCall接口之前，请先[设置云函数配置项](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cloudfoundation-call-function#设置云函数配置项)。  测试周期性预加载时，需要将下文示例代码periodicPrefetch方法中的如下代码块注释。若不注释，则需等待12h才能获取周期性预加载数据。
```text
...
export class PrefetchWrapper {
  ...
  public async doLinkPrefetch(): Promise {
    try {
      let link = await this.popPrefetchLink();
      if (link.length  { // 接口调用成功，处理缓存的应用数据
  ...
  // 获取跳链安装预加载的链接信息
  private async popPrefetchLink(): Promise {
