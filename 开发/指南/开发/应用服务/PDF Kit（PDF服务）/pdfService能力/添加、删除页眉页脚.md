...
**添加页眉页脚：** 调用loadDocument方法，加载PDF文档。 实例化页眉页脚HeaderFooterInfo类，并设置相关属性，包括字体大小、颜色和间距等。 调用addHeaderFooter方法，添加页眉页脚到页面中。 保存PDF文档到应用沙箱。 **删除页眉页脚：** 调用loadDocument方法，加载PDF文档。 调用removeHeaderFooter方法，删除页眉页脚。 保存PDF文档到应用沙箱。
```text
    ...
    Column() {
      Button('addHeaderFooter').onClick(async () => {
        ...
        if (res === pdfService.ParseResult.PARSE_SUCCESS) {
          ...
          hfInfo.headerLeftText = 'left H > >';
          hfInfo.headerCenterText = 'center H > >';
          hfInfo.headerRightText = 'right H >>';
          hfInfo.footerLeftText = 'left F >>';
          hfInfo.footerCenterText = 'center F >>';
          hfInfo.footerRightText = 'right F >>';
