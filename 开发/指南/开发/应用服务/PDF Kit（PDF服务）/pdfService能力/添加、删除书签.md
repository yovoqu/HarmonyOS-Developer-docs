...
**添加书签**： 调用loadDocument方法，加载PDF文档。 调用createBookmark方法，创建书签。 调用setDestInfo方法，设置书签的跳转信息。 调用getBookmarkInfo方法，获取书签信息。 调用setBookmarkInfo方法，设置书签内容及样式。 设置保存文档沙箱路径并保存 **删除书签**： 调用loadDocument方法，加载PDF文档。 调用getRootBookmark方法，获取文档的第一个根书签。 调用removeBookmark方法，删除书签。 设置保存文档沙箱路径并保存
```text
