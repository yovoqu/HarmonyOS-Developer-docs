...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cross-language-complex-param
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a3/v3/ajxFEBx4T_qc8hKteUg6sQ/zh-cn_image_0000002659101449.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=13AEC11C144806B04B36EB85D70934314AB82F3D6CAB3ECD07123707E2CBA621)
...
- 需要先在index.d.ts文件中，声明一个用于数据传递的函数。
  ...
  try {
    this.inputArray.forEach((inputField) => {
      if (inputField.inputStr.length > 0) {
        if (Number(inputField.inputStr)  num_array = {};
...
```text
// entry\src\main\cpp\napi_init.cpp
void Uint8ArrayPassingTs2Napi(napi_env env, napi_callback_info info, vector &input_array) {
    ...
    for (int i = 0; i napi_create_arraybuffer()：负责构建buffer。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4/v3/48vy5bOgTPiZt-h9ux2Neg/zh-cn_image_0000002628862100.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=8DED9A8F585A00506F9C7D6F9C011CD09F0721D10986B204CA66B235A292DDA6)
...
- 函数声明与调用，与[arrayBuffer类型数据交互](#arraybuffer类型数据交互)类同，此处不再赘述。
...
hashMap2Rec(map: HashMap): Record {
  let Rec: Record = {}
  ...
  try {
    map.forEach((value: Object, key: string) => {
      ...
      if (value instanceof HashMap) {
        let vRec: Record = this.hashMap2Rec(value);
        value = vRec;
      }
      Rec[key] = value;
    })
...
```
  ...
  三方库获取：[HarmonyOS / third_party_json · GitCode](https://gitcode.net/HarmonyOS/third_party_json)
    
```text
// entry\src\main\cpp\napi_init.cpp
map HashMapPassingTs2Napi(napi_env env, napi_callback_info info) {
    size_t argc = 1;
    napi_value args;
    // Gets detailed information about the function call, such as input parameters.
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/17/v3/axSmGL9dSOCL1513r1eiiQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=879ECC93D862C6037509C7CD23EBECF811BAA7CA266ADB42AAD26C841D3511BB)
...
```text
// entry\src\main\cpp\napi_init.cpp
napi_value HashMapPassingNapi2Ts(napi_env env, map inputMap) {
    static map points_table;
...
- 需在DevEco Studio[预生成](#概述)的Init()函数中设置写好的数据传递函数。与[arrayBuffer类型数据交互](#arraybuffer类型数据交互)类同，此处不再赘述。
- 将C++侧返回的string重新反序列化为hashMap。
    
```ArkTS
// entry/src/main/ets/pages/HashMapPage.ets
let receiveObj: object = JSON.parse(receiveStr);
let receiveMap: HashMap = new HashMap();
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/48/v3/hy54GywiQRe_Om1PhCZaWQ/zh-cn_image_0000002628702222.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=DFA05001EC23A4B9D50B10E7F9FE132FD86E3EB17A2F025E61B347278EDE5F76)
...
除本文介绍的方法外，也可通过readPixelsToBuffer将其转换为Uint8Array类型，进行处理。此方案可以参考[arrayBuffer类型数据交互](#arraybuffer类型数据交互)，本文不作介绍。
 

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/58/v3/D2zsLAcXRRCbghqGF79zRQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=986EF2B08BE0091EF07C3C66CE67EA4D6B95729C43C7C0EB0978D3F2DA397EAC)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a8/v3/-r5yV0dzQvursLEW3WDW6A/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=21780B6A8B369DCD4BF1E91E7E3A72D8BEFF99E85AF7739CD5780ACBAAA21F54)
 
 输入的TSPixelMap与生成的NativePixelMap共享内存空间，因此直接调用相关方法修改NativePixelMap对象，即可同步影响TSPixelMap对象。所以此处无需回传数据。
- 在Init()函数中的设置流程与[arrayBuffer类型数据交互](#arraybuffer类型数据交互)相同，不再赘述。
- 在ArkTS中令Image组件重新渲染。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/6c/v3/zn_WzqSDSyKRoudswKSOqw/zh-cn_image_0000002659101451.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=2EBBE62E5C545306611FD9B762EF40850E22D0FD56FFD5814E716E3F8BDA9208)
...
```
  ...
  value2String、string2value()：负责napi_value与string类型之间的相互转换，实现方法详见[hashMap类型数据交互](#hashmap类型数据交互)。
- 需在DevEco Studio[预生成](#概述)的Init()函数中设置写好的数据传递函数。与[arrayBuffer类型数据交互](#arraybuffer类型数据交互)类同，此处不再赘述。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9f/v3/uX8ET20gTiWiL4gnkNePAw/zh-cn_image_0000002628862102.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=C5D5078034C2E5679705706B5491B6996466A976F982E92F3C55C49A21BB7140)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/28/v3/T7X8sS0eSKevVUNC18g59w/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=79AFE0FC908989FCA84191B3D6F0BBD887144387E77C448E376DEFFC5322339B)
...
static napi_value GetHintStr(napi_env env, napi_callback_info info) {
    ...
    napi_unwrap(env, ts_class_obj, reinterpret_cast(&c_class_obj));
    if (c_class_obj) {
...
string2value()：负责将obj->hintStr转换为napi_value，作为输出。此函数的实现方法详见[hashMap类型数据交互](#hashmap类型数据交互)。
  
ArkTS侧调用hintStr的值的时候，需要调用此函数，从而获得C++侧的hintStr的值。
  

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7a/v3/_xGKQKiiQES77TLlUJ7PAA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=FBFC5095B187E4B379D642270A6D17969BE9253663E694D077A6E3DFEB0464CE)
...
- TsConstructor()，用于构造ArkTS class。
  napi_get_cb_info()：负责从ArkTS侧获取输入参数。
  value2String()：负责将入参转换为string。此函数的实现方法详见[hashMap类型数据交互](#hashmap类型数据交互)。
...
EXTERN_C_END
```
  需要先在DevEco Studio[预生成](#概述)的Init()函数中，定义一个classProp，并写入class对应的访问器与方法
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/69/v3/sXasmQJ4QU6mieTr18Awew/zh-cn_image_0000002659221413.png?HW-CC-KV=V1&HW-CC-Date=20260701T025443Z&HW-CC-Expire=86400&HW-CC-Sign=89DDC118E3D38CD4DA409CE7D56FE81A758A2211CA283A0AC7C1F8D5F48F4CBD)
