...
使用Node-API操作ArkTS对象时，需要了解一些基本概念。 **ArkTS值到C/C++类型的转换：** 在Node-API模块中，可以使用Node-API函数将ArkTS值转换为C/C++的数据类型，如将ArkTS数值转换为C/C++的整数、将ArkTS字符串转换为C/C++的字符数组等。同样，也可以将C/C++的数据类型转换为ArkTS值，以便将结果返回给ArkTS代码。
...
用于将给定的ArkTS value强转成ArkTS boolean值。 cpp部分代码
```text
// napi_coerce_to_bool
...
```
 ...
 接口声明
```text
export const coerceToBool: (data: T) => boolean; // napi_coerce_to_bool
```
...
let str = testNapi.coerceToBool('111111111');
let obj = new Object();
let res = testNapi.coerceToBool(obj);
let result = testNapi.coerceToBool(null);
// false
...
将给定的ArkTS value强转成ArkTS number。 cpp部分代码
```text
// napi_coerce_to_number
...
```
 ...
 接口声明
```text
export const coerceToNumber: (data: T) => number; // napi_coerce_to_number
```

 ArkTS侧示例代码
```text
// napi_coerce_to_number
let value = testNapi.coerceToNumber('2556');
let str = testNapi.coerceToNumber('sssss');
let bool = testNapi.coerceToNumber(true);
...
用于将给定的ArkTS value强转成ArkTS Object。 cpp部分代码：
```text
// napi_coerce_to_object
...
```
 ...
 接口声明
```text
export const coerceToObject: (data: T) => Object; // napi_coerce_to_object
```

 ArkTS侧示例代码
```text
// napi_coerce_to_object
let value = testNapi.coerceToObject('222222');
let result = testNapi.coerceToObject(111);
...
用于将给定的ArkTS value强转成ArkTS string。 cpp部分代码
```text
// napi_coerce_to_string
...
```
 ...
 接口声明
```text
export const coerceToString: (data: T) => string; // napi_coerce_to_string
```

 ArkTS侧示例代码
```text
// napi_coerce_to_string
let value = testNapi.coerceToString(212);
let obj = new Object();
let res = testNapi.coerceToString(obj);
let bool = testNapi.coerceToString(false);
...
根据给定的C boolean值，获取等价的ArkTS boolean值。 cpp部分代码
```text
// napi_get_boolean
...
```
 ...
 接口声明
```text
export const getBoolean: (data: T, value: String) => boolean; // napi_get_boolean
```

 ArkTS侧示例代码
```text
// napi_get_boolean
let value = testNapi.getBoolean(1, '1');
let data = testNapi.getBoolean('sss', '1');
...
使用此函数将ArkTS中的布尔值转换为等价的C布尔值。 cpp部分代码
```text
// napi_get_value_bool
...
获取全局ArkTS对象。此函数用于获取表示ArkTS全局对象的napi_value，使C/C++模块能与ArkTS运行时的全局对象交互。 cpp部分代码
```text
// napi_get_global
