...
```
 ...
 接口声明
```text
export const getCbArgs: (arg: T) => T; // napi_get_cb_info
...
function summation(arr: Array) {
  let sum: number = 0;
  for (let i = 0; i

## napi_call_function

在C/C++侧对ArkTS函数进行调用。 注意事项：napi_call_function传入的argv的长度必须大于等于argc声明的数量，并且每个元素都应初始化为nullptr。 cpp部分代码
```text
// napi_call_function
...
将一个C/C++函数包装为可在ArkTS中调用的函数，并返回一个表示该函数的napi_value。 cpp部分代码
```text
// napi_create_function
