...
**ArrayBuffer**：ArrayBuffer 对象用来表示一个通用的、固定长度的原始二进制数据缓冲区。不能直接操作 ArrayBuffer 的内容，而是需要通过包装成 TypedArray 对象或 DataView 对象来读写。ArrayBuffer 常用于处理固定长度的原始二进制数据，如文件、网络数据包等。**生命周期和内存管理**：在使用 JSVM 处理 ArrayBuffer 时，需要特别注意对象的生命周期管理，确保及时释放内存。
...
检索 ArrayBuffer 的底层数据缓冲区及其长度。 cpp部分代码：
```text
// hello.cpp
...
判断一个 JavaScript 对象是否为 ArrayBuffer 类型对象。 cpp部分代码：
```text
// hello.cpp
#include "napi/native_api.h"
#include "ark_runtime/jsvm.h"
...
检查给定的 ArrayBuffer 是否已被分离。 cpp部分代码：
```text
// hello.cpp
#include "napi/native_api.h"
#include "ark_runtime/jsvm.h"
...
创建一个指定大小的 ArrayBuffer 对象。 cpp部分代码：
```text
// hello.cpp
#include "napi/native_api.h"
#include "ark_runtime/jsvm.h"
