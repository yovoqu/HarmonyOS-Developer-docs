...
| 共包 | 动态卡片（Dynamic Widget） |
| 独立包 | 独立静态卡片（Static Widget(Standalone)） |
| 独立包 | 独立动态卡片（Dynamic Widget(Standalone)） |
...
2. 选择模块（如entry模块）下的任意文件，单击**右键 > New > Service Widget**，[按需选择卡片](#section6353219163116)。
3. 在**Choose a Template for Your Service Widget**界面中，选择卡片模板，单击**Next**。

  
![](assets/创建服务卡片/file-20260514134345622-1.png)

4. 在**Configure Your Service Widget**界面中，配置卡片的基本信息，包括：

  
**Service widget name**：卡片的名称，在同一个应用/元服务中，卡片名称不能重复，且只能包含大小写字母、数字和下划线。
5. **Display name**：卡片预览面板上显示的卡片名称。仅API 11 及以上Stage工程支持配置该字段。
6. **Description**：卡片的描述信息。
7. **Language：**界面开发语言，可选择创建ArkTS/JS卡片。
8. **Support dimension**：选择卡片的规格。部分卡片支持同时设置多种规格。首次创建卡片时，将默认生成一个EntryCard目录，用于存放卡片快照。
9. **Default dimension**：在下拉框中可选择默认的卡片。
10. **Ability name：**选择一个挂靠卡片的Form Ability，或者创建一个新的Form Ability。
11. **Module name：**卡片所属的模块。
12. 单击**Finish**完成卡片的创建。创建完成后，工具会自动创建出卡片的布局文件，并在form_config.json文件中写入服务卡片的属性字段，关于各字段的说明请参考[配置文件说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget-configuration)。
13. 卡片创建完成后，请根据开发指导，完成卡片的开发，详情请参考[服务卡片开发指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-ui-widget)。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/db/v3/tBVTfQaVQaKrAzycZ-CEWw/zh-cn_image_0000002602065989.png?HW-CC-KV=V1&HW-CC-Date=20260528T030645Z&HW-CC-Expire=86400&HW-CC-Sign=609FCB23A492CC873E11BCBC5F05873311017B47FFDBA68BAA314F2840CC4802)
