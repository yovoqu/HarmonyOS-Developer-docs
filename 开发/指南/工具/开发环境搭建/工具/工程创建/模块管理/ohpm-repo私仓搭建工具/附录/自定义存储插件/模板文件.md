...
来源：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-ohpm-repo-template-file
...
export class CustomStorage implements StoragePlugin {
   ...
   * <span style="color: rgb(134,138,153);">@param</span> packageInfo: 可选参数，待上传包的详细信息，包含包名（含组织名）和包版本号两部分，包名：packageInfo.packageName，包版本：packageInfo.version.
   * @returns 上传文件 save 后的返回信息： 能够标识文件，方便文件删除和读取
   */
  async save(srcPath: string, <span style="color: rgb(152,104,1);">packageInfo</span>: <span style="color: rgb(193,132,1);">any</span>): Promise<string>{
