...
开始时间：以APP_LIST_FLING滑动泳道为例，泳道的起点（如图标记1）。结束时间：以APP_LIST_FLING滑动泳道为例，泳道的终点（如图标记2）。其他滑动泳道标记如下： H:APP_SWIPER_SCROLL H:WEB_LIST_FLING
![](assets/滑动过程流畅/file-20260514133049187-0.png)
查找滑动泳道：H:APP_LIST_FLING，如果是web页面，找H:WEB_LIST_FLING。刷新率：查找关键词H:RSHardwareThread::CommitAndReleaseLayers rate，如下图：
