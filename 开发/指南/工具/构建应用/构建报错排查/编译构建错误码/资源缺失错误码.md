...
确保path配置的文件存在，且存放在本模块的libs/{abi}/目录下，其中{abi}为设备CPU架构类型（如arm64-v8a、x86_64、armeabi-v7a）。
...
**可能原因**
 1. hvigorfile.ts或hvigorconfig.ts中，在[taskGraphResolved](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-life-cycle#section746253616316)之前的阶段调用了getOhpmDependencyInfo或getOhpmRemoteHspDependencyInfo。
2. hvigorfile.ts或hvigorconfig.ts中，直接或间接依赖的插件在[taskGraphResolved](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-life-cycle#section746253616316)之前的阶段调用了getOhpmDependencyInfo或getOhpmRemoteHspDependencyInfo。
 
**处理步骤**
 
使用[getOhpmDependencyInfoV2](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-build-expanding-context#section1897251184819)、[getOhpmRemoteHspDependencyInfoV2](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-build-expanding-context#section1697717782918)替换getOhpmDependencyInfo、getOhpmRemoteHspDependencyInfo。
