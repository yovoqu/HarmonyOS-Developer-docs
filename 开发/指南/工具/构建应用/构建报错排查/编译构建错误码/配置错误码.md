...
支持的SDK版本请查看[所有HarmonyOS开发套件版本](https://developer.huawei.com/consumer/cn/doc/harmonyos-releases/overview-allversion)。
...
```text
registry=<span style="color: rgb(62,159,204);">https:</span>/<span style="color: rgb(104,153,50);">/repo.huaweicloud.com/repository</span><span style="color: rgb(104,153,50);">/npm/</span>
<span style="color: rgb(73,179,172);">@ohos</span><span style="color: rgb(62,159,204);">:registry=https</span><span style="color: rgb(62,159,204);">://repo</span>.harmonyos.com/npm/
```
...
**处理步骤**
1. 将文件移动到libs/{abi}目录下，其中{abi}为设备CPU架构类型（如arm64-v8a、x86_64、armeabi-v7a）。
2. 检查文件路径是否正确。
...
**处理步骤**
1. 检查模块WWW的target YYY是否配置了自定义资源路径，具体配置方式请参考[定义产物的资源](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-customized-multi-targets-and-products-guides#section1853511169480)。
...
skillProfiles的abilityName仅支持[abilities标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#abilities标签)下的UIAbility或[extensionAbilities标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#extensionabilities标签)下type为service的ExtensionAbility。
...
2. 系统中配置了环境变量DEVECO_SDK_HOME，但是路径下实际没有SDK，或与当前DevEco Studio版本不配套。
...
2. 确保DEVECO_SDK_HOME环境变量指向的SDK路径正确，并且与DevEco Studio内置的SDK版本一致。
...
**处理步骤**
1. 在DevEco Studio中，在Terminal执行java -version，如果当前JDK版本是25，需要关闭DevEco Studio内置的JDK环境变量，点击**File > Settings**（macOS为**DevEco Studio > Preferences/Settings **）** > Tools > Terminal > Application Settings**，将**Add deveco's embedded java path to %PATH%**选项去勾选，并在系统中配置环境变量JAVA_HOME为24或更低的版本。
