...
拥有publish命令的所有内容校验规则，可以在发布前检测待发布的三方库能否通过ohpm客户端校验。只校验待发布三方库内容，不对publish_registry、publish_id、key_path等做校验。包的格式、结构及具体校验规则可参考[publish命令说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-ohpm-publish)。
...
默认值：无类型： String 从ohpm 6.0.2.636版本开始，可以在 prepublish 命令后配置--log_level 参数，指定执行当前命令的日志级别（info、debug、warn、error），如果未指定该值则日志级别为.ohpmrc中配置的log_level的级别。
