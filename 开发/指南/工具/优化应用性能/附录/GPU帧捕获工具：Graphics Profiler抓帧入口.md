...
4. Verify Buffer Access：默认关闭，设置校验Buffer是否可以访问。
5. 在帧捕获界面拉起应用，成功建立连接后，Capture按钮点亮。设置抓帧数量，点击Capture按钮，等待帧捕获完成。

  
Scope：不可修改，默认为Frame。
6. Count：抓帧数量设置，范围为1-10帧。
7. 当抓帧完成，在下方显示界面中选择一条捕获帧，点击
