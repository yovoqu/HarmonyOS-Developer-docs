...
let listener: speechRecognizer.RecognitionListener = {
  ...
  onComplete(sessionId: string, eventMessage: string) {
    asrEngine.shutdown(); <em>// </em><em>识别完成时，及时释放资源，保证下一次能再正常创建引擎实例</em>
  ...
  speechRecognizer.createEngine(initParamsInfo)
    ...
    <em>  // 开始监听</em>
      asrEngine.startListening({
        sessionId: 'xxx', <em>// sessionId</em><em>字符长度需要小于64，而且不能包含特殊字符，如'.'</em>
        audioInfo: {
          audioType: 'pcm',
          sampleRate: 16000,
          soundChannel: 1,
