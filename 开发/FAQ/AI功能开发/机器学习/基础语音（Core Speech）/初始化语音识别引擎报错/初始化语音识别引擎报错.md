...
let listener: speechRecognizer.RecognitionListener = {
  ...
  onComplete(sessionId: string, eventMessage: string) {
    asrEngine.shutdown(); <em>// 识别完成时，及时释放资源，保证下一次能再正常创建引擎实例</em>
...
function CreateEngine() {
  ...
  <em>    // 开始监听</em>
      asrEngine.startListening({
        sessionId: 'xxx', <em>// sessionId字符长度需要小于64，而且不能包含特殊字符，如'.'</em>
        audioInfo: {
          audioType: 'pcm',
          sampleRate: 16000,
          soundChannel: 1,
...
A：不支持。Core Speech Kit当前支持的设备类型为Phone、Tablet、PC/2in1，不支持穿戴设备，详情可参考[Core Speech Kit支持的设备](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/core-speech-introduction#支持的设备)。
