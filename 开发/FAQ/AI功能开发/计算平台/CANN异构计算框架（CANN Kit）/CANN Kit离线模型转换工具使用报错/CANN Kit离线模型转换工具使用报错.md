...
Q：调用OH_AI_ContextSetThreadNum设置线程数为2，同时调用OH_AI_ContextSetThreadAffinityMode设置绑大核（值为1）后，在某些设备（如畅享70x、畅享90标准版）上模型推理耗时从几十毫秒增加到好几秒，如何解决？
 
A：出现该问题时，可以先验证仅调用OH_AI_ContextSetThreadNum设置线程数2，不调用OH_AI_ContextSetThreadAffinityMode时的耗时波动情况。如果耗时仍然异常，可以尝试将线程数修改为1，或者将亲核性设置为0（默认不绑核），耗时即可恢复正常的几十毫秒。
