...
- 应用启动流程，应用启动大致分为6个阶段：
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/0c/v3/A7DtsEkEScu8H7lSDSsASg/zh-cn_image_0000002628476146.png?HW-CC-KV=V1&HW-CC-Date=20260701T025505Z&HW-CC-Expire=86400&HW-CC-Sign=A02A218BC7859E012860D5B4CA3CF57DE31BC178FBF6E980C10581C54CE4B3C8)

 
AbilityManagerService请求AppSpawn创建应用进程。
- AppManagerService触发应用启动流程、应用进程加载应用包—handleLaunchApplication。
- AppManagerService触发Ability启动流程、应用进程加载Ability资源、根据应用生命周期定义，触发生命周期回调—HandleLaunchAbility。
- 创建UI Ability持有的Window对象。
- 绘制UI界面首帧。
- 首页加载绘制完成。
...
- 问题复现：在新版本和老版本系统分别测试，初步判断卡死问题是否和系统变更有关。本案例中，5.0系统上应用启动正常，5.1(API18)上问题稳定复现，应用卡在启动页，未弹出隐私弹框。
- 问题分析：应用卡在启动页，从应用启动流程开始分析。
