...
- 使用hdc file recv /data/log/hilog获取hilog和hilog_kmsg日志，hilog日志无法直接解压查看，需要使用hilogtool解析，可以参考[常用解析命令示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hilog-tool#常用解析命令示例)，hilog_kmsg日志可以直接解压查看。
- 在hilog日志中以正则表达式搜索Kill Reason|exit with，查看是否有相关日志。
