...
- HTTP协议为表单数据传输主要提供了两种表单传输场景：简单字符串键值对表单数据传输和多表单混合数据传输。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/34/v3/9XKEUTQeQ6i6VSNfq150Ag/zh-cn_image_0000002658851751.png?HW-CC-KV=V1&HW-CC-Date=20260701T025800Z&HW-CC-Expire=86400&HW-CC-Sign=44AB87E539EB2463ABBFA14E3BC87631B09BD60B56D559A6D15BECCF3F682B4F)
...
- 指定表单数据的传输顺序，部分服务端校验传输参数的顺序，顺序不对可能导致传输失败。
```text
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8f/v3/Id86sy_MTH2RR0n0vjLFqQ/zh-cn_image_0000002628772386.png?HW-CC-KV=V1&HW-CC-Date=20260701T025800Z&HW-CC-Expire=86400&HW-CC-Sign=E41B43C1813D46D362AFE93B6EF8BC91556975B4F82696B397D42E5038345F4B)

 
 
**场景二：复杂混合表单数据传输**
 
- 使用picker组件选择图片文件并将文件复制到沙箱路径，rcp模块的MultipartForm传输只支持沙箱路径下的文件路径、读取的文件buffer数据和读取文件数据的回调函数作为入参，不支持文件流和文件操作符作为入参。
```text
let sandBoxFileName = await copyFileToSandBox(sandboxDir);
```

- 构建传输文件的表单数据。详细说明见[rcp如何实现文件上传](https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs/faqs-remote-communication-15)。
```text
let multiFormFieldValue = this.buildMultipartFormFieldValue(remoteFileName, sandBoxFileName, 'image/jpeg');
```

- 将字符串参数与文件数据合并为多部分表单数据。
```text
...
- 设置keys顺序用于指定多部分表单各部分传输顺序，不设置系统会随机生成传输顺序。
```text
multiForm.keys = ['key3', 'key1', 'key2']; // 开发者自行设置参数顺序
```

- 自定义表单分隔符，如果不设置，系统使用随机字符串生成分隔符。
```text
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/a2/v3/aloi8zGFSPO2-KsffbIYHg/zh-cn_image_0000002658971707.png?HW-CC-KV=V1&HW-CC-Date=20260701T025800Z&HW-CC-Expire=86400&HW-CC-Sign=229AF7E7395AC6A1BE81412159DE4052710ED0D35075062E51CD6CD9490BD1E3)
...
struct FormDataTransfer {
  ...
  @State tabOptions: SegmentButtonOptions = SegmentButtonOptions.tab({
    buttons: [{ text: '简单表单传输' }, { text: '多部分表单传输' },] as ItemRestriction,
