...
- 使用rcp.createSession与http.createhttp()的区别：1. rcp库请求发送能力与HTTP一致，两者使用底层库能力不同（HTTP底层使用为libcurl，rcp库底层由内部开发封装），两类型库相比较rcp上具备较好的演进性，可实现自定义证书校验，场景化传输API、服务器身份校验、NTLM校验、数字签名校验等场景能力。
