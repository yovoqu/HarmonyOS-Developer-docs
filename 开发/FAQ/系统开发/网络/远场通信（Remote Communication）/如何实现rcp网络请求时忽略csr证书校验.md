...
- 自定义配置 [ValidationCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/remote-communication-rcp#validationcallback)：通过重写验证回调，手动控制校验逻辑。
- 直接显式跳过（Skip）：将[SecurityConfiguration](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/remote-communication-rcp#securityconfiguration)中的安全配置项remoteValidation设为 'skip'，由底座直接放行。
