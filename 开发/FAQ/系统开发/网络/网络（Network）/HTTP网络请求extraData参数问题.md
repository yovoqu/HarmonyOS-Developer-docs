...
- **问题二**：1. 检查服务端配置：确保服务端能够正确处理URL编码字符。如果服务端不支持直接传递编码后的字符串，需要在客户端解码后再发送。
...
6. 当HTTP请求为GET、OPTIONS、DELETE、TRACE、CONNECT等方法时，此字段为HTTP请求参数的补充。开发者需传入Encode编码后的string类型参数，Object类型的参数无需预编码，参数内容会拼接到URL中进行发送。ArrayBuffer类型的参数不会做拼接处理。
7. 发起HTTP请求时，extraData为可选配置参数，发送请求的额外数据，默认无此字段。没有额外数据时，避免添加该参数；若必须添加，请填写undefined或者null，避免直接传入空字符串。
