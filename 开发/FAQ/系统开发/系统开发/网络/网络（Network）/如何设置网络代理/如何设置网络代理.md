...
struct ProxyCheckPage {
  @State proxyEnabled: boolean = false; <em>// 是否启用代理</em>
  url: string = 'XXX.XXX.XXX'; <em>// 需要替换为开发者需要的请求地址</em>
  proxyUrl: string = 'XXX.XXX.XXX.XXX'; <em>// 需要替换为开发者需要的代理地址</em>
  proxyPort: number = 8888; <em>// 需要替换为开发者需要的代理端口</em>
  @State proxyInfo: string = '未检测'; <em>// 代理信息文本</em>
  @State testResult: string = '未测试';<em> // 测试请求结果</em>
  @State isLoading: boolean = false; <em>// 加载状态</em>
