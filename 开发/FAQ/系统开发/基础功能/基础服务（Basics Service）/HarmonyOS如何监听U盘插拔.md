...
- 步骤：
模块导入：使用[@ohos.commonEventManager](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-commoneventmanager)模块来管理公共事件。导入[BusinessError](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#businesserror)用于错误处理。
- 订阅U盘插拔事件：使用commonEventManager.createSubscriber创建订阅者。
 订阅usual.event.usb.action.USB_STATE事件，监听U盘的插拔状态。
- 事件处理：在subscribe回调中解析事件参数，判断U盘的状态（connected或disconnected）。
 根据状态调用handleUsbAttached或handleUsbDetached方法。
- 取消订阅：在不需要监听时，调用commonEventManager.unsubscribe取消订阅。
