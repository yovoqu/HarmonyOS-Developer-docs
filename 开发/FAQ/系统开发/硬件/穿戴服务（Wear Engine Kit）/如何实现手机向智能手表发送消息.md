...
- 手机端应用使用WearEngine的[sendMessage](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#section10826949193413)接口向智能手表端应用发送消息通知，需配置智能手表端应用的包名与指纹，代码示例如下：
```text
...
- 智能手表端应用通过[registerMessageReceiver](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#section893873771918)接口订阅接收手机端应用发来的消息，需配置手机端应用的包名与指纹，并在aboutToDisappear中调用[unregisterMessageReceiver](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#section13676139154415)方法，取消接收手机端应用发过来的消息，需要传入订阅监听时的同一个回调函数对象。代码示例如下：
```text
