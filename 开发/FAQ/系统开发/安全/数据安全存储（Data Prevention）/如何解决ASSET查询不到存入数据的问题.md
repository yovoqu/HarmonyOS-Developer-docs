...
try {
  let res: Array = asset.querySync(query);
  for (let i = 0; i 
##### 背景知识
...
- 使用相同资产别名进行重复插入，提示关键资产已存在。
- 检查报错信息，由于未报错Failed to query Asset，可以确认[asset.querySync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-asset#assetquerysync12)方法执行成功。
- 根据执行代码确认方法为根据别名查询单条关键资产，检查关键资产查询对象query是否正确设置属性参数，可以确认query对象只设置了别名ALIAS属性，未设置返回的结果类型ReturnType属性。
