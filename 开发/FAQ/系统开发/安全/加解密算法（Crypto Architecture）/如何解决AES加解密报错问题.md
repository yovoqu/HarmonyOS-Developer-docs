...
将明文数据转换为Uint8Array格式，检查长度是否为128的整数倍。
- 检查入参是否格式正确，例如AES|CFB|NoPadding是否输入为AES/CFB/NoPadding。
- 检查算法入参是否与密钥长度匹配，例如AES128对应的密钥长度也需要为128bit。
- 检查明文或密文转换编码格式方法是否正确，入参为Uint8Array格式，实际应用中需要将Base64等数据编码格式转换，检查转码方法是否正确。
- 检查加密参数与解密参数是否一致，包含加密算法密钥长度、填充模式、密钥、CBC模式对应的解密参数[IvParamsSpec](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cryptoframework#ivparamsspec)等。
...
- 17630001等报错问题：
采用ECB、CBC加密模式，明文长度不是128位整数倍，必须使用填充方法补足，不可选择NoPadding。
...
- 加密方法的入参未与解密方法入参保持一致，如AES采用CBC模式下加密与解密方法的入参IvParamsSpec各自生成一份。
