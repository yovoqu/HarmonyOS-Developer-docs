...
- 在[并发函数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/taskpool-introduction#并发函数中使用自定义类或函数)中使用自定义类或函数时，需将其定义在单独的文件中，否则可能被视为闭包。加解密逻辑拆分为独立文件，确保所有对象可序列化。
```ArkTS
// utils/Func.ets
