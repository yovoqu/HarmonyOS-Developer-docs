...
- 自定义持续扫码时序图如下：
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/88/v3/PrcbgVcmS7SoB_git-tJGg/zh-cn_image_0000002658911829.png?HW-CC-KV=V1&HW-CC-Date=20260701T025822Z&HW-CC-Expire=86400&HW-CC-Sign=5C56F8BF3458E29F5A1E0F6ECF78EC2F30C5C607AD41BA59AD478319C814C901)
...
- 从问题代码可以了解到使用的是自定义扫码功能，调用customScan.start()接口后通过Callback回调函数得到扫码结果，但是start接口只执行一次扫码，返回结果后扫码识别功能结束。
- 在Callback回调函数得到扫码结果后只对结果进行了处理，没有再进行其他操作。当前通过start接口开启的扫码识别功能已经结束，导致后续再进行扫码也无法触发Callback回调。
...
- 在start接口的异步回调中通过变量控制显示扫描结果和重新扫描按钮，点击重新扫描按钮即可调用rescan接口再次触发重新扫码功能。注意：本案例进入页面之后需要扫码才能出现重新扫码页面，重新扫码响应快速，因此点击重新扫码之前建议将摄像头远离码图。
 示例代码如下：
...
struct CustomScanPage {
  ...
  private callback: AsyncCallback> =
    async (error: BusinessError, result: Array) => {
      if (error) {
  ...
  async requestCameraPermission() {
    ...
    for (let i = 0; i  // 竖屏时获取屏幕尺寸，设置预览流全屏示例
  async setDisplay() {
    try {
      // 默认竖屏
