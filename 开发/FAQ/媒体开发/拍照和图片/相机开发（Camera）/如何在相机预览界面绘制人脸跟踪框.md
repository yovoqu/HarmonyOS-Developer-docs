...
cameraOutputCapability: camera.CameraOutputCapability) {
  let metadataObjectTypes: Arraycamera.MetadataObjectType> = cameraOutputCapability.supportedMetadataObjectTypes;
  try {
...
onMetadataObjectsAvailable(metadataOutput: camera.MetadataOutput): void {
  ...
  (err: BusinessError, metadataObjectArr: Arraycamera.MetadataObject>) => {
...
struct Index {
  ...
  async prepareCamera() {
    ...
    // 获取支持指定的相机设备对象
    let cameraDevices: Arraycamera.CameraDevice> = [];
    ...
    if (targetProfile) {
      ...
      // 创建相机会话
      try {
        this.cameraSession = this.cameraManager.createSessioncamera.PhotoSession>(camera.SceneMode.NORMAL_PHOTO);
      } catch (error) {
        let err = error as BusinessError;
        console.error(`CameraDemo createCaptureSession error. error: ${JSON.stringify(err)}`);
        return;
