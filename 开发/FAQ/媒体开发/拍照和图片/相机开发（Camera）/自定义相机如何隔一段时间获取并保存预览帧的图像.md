...
- 首先使用[ImageReceiver](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagereceiver)接收预览帧图片，具体方法可参考[双路预览](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-dual-channel-preview)。
- 将获取到的预览帧图片编码成图片保存至应用沙箱，参考代码如下：
```text
/**
...
- 使用[setInterval](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-timer#setinterval)定时器，设置时间定时执行步骤2：
```text
onPageShow(): void {
        ...
        nextImage.getComponent(image.ComponentType.JPEG, async (err: BusinessError, imgComponent: image.Component) => {
          ...
          if (imgComponent.byteBuffer) {
            ...
            if (stride === width) {
              ...
              for (let j = 0; j // 确保当前buffer没有在使用的情况下，可进行资源释放
