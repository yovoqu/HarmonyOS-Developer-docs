...
export async function cameraShooting(cameraPosition: number, surfaceId: string, context: Context): Promise<void> {
  ...
  try {
   ...
   <em> // 获取预览流的Profile列表</em>
    ...
    <em>// </em><em>选择合适的预览流分辨率，可以手动选取或通过代码选取</em>
...
<em>// </em><em>需要连续拍摄的照片个数</em>
let picturesTaken: number = 3;
<em>// </em><em>设备显示屏的宽度</em>
     ...
     <em> // 拍摄完后显示缩略图</em>
      Row() {
        ...
        Image($r('app.media.capture'))
          .height(50)
          .onClick(() => {
            <em>// isFront</em><em>为是否摄像头是否前置，picturesTaken为连续调用capture的次数</em>
