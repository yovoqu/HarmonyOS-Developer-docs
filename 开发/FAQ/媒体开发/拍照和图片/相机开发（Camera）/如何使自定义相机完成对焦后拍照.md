...
async function cameraShootingCase(context: Context, surfaceId: string): Promise {
  try {
    ...
    if (!cameraArray || cameraArray.length // 获取相机ID。
    ...
    if (!photoProfilesArray || photoProfilesArray.length   // 创建预览输出流，其中参数surfaceId参考上文XComponent组件，预览流为XComponent组件提供的surface。
