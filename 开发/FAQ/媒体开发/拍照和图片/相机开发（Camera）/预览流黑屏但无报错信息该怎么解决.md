...
- 配置的预览流尺寸不被支持。
**解决措施**
- 确保在初始化相机和获取相机输入流之前，先正确获取相机权限。首先在module.json5文件中申请需要的权限（地理位置权限可以根据需要申请，非必选）；"requestPermissions": [
  {
...
], 接着在EntryAbility.ets中onWindowStageCreate中使用requestPermissionsFromUser方法获取权限。onWindowStageCreate(windowStage: window.WindowStage): void {
...
- 当应用被切换到后台后，相机资源会被全部回收，所以为了避免出现前后台切换时预览流黑屏的问题，需在onPageShow中进行重新创建会话、配置会话、启动等操作，并在onPageHide中对相机资源进行释放。async onPageShow() {
...
- 获取相机设备支持的输出流能力，确定支持的预览尺寸。// Obtain the output stream capability supported by the camera device
