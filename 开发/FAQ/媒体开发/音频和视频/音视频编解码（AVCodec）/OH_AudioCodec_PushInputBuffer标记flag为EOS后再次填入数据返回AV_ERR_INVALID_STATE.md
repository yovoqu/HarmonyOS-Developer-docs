...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/97/v3/AC41kefVRpWOcq72ZNyyfA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025832Z&HW-CC-Expire=86400&HW-CC-Sign=20444860BCCFEC4E4CE234B53224662F7004E2D329D9793B12A0C426D3887E0C)
 
音频文件结束后，需要将[OH_AVCodecBufferAttr](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/_o_h___a_v_codec_buffer_attr)的flags标识成AVCODEC_BUFFER_FLAGS_EOS。
...
- 收到输入缓存区回调->使用解码器读取buffer（实际上读取到的数据是0，读取的buffer属性中flags为AVCODEC_BUFFER_FLAGS_EOS）->设置buffer到缓存区（此时正常）。
- 再次收到输入缓存区回调->使用解码器读取buffer（实际上读取到的数据是0，读取的buffer属性中flags为AVCODEC_BUFFER_FLAGS_EOS）->设置buffer到缓存区（此时OH_AudioCodec_PushInputBuffer返回值为8，并且不再收到OnOutputBufferAvailable回调）。
