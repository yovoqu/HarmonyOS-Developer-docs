...
struct ChangeStatus {
  ...
  fileName: string = 'test_01.mp3'; <em>// 'test_01.mp3'需要在src/main/resources/rawfile中替换为开发者需要的音频文件</em>
  isSwiping: boolean = false; <em>// 用户滑动过程中</em>
  @State isPlay: boolean = false; <em>// 暂停播放</em>
  @State xComponentFlag: boolean = false;
  @State speedSelect: number = 0; <em>// 倍速选择</em>
 ...
 <em> // 注册avplayer回调函数</em>
    ...
    this.avPlayer.on('stateChange', async (state) => {
      ...
      switch (state) {
        ...
        case 'completed': <em>// 播放结束后触发该状态机上报</em>
          ...
          this.isPlay = false; <em>// 改变播放状态</em>
  ...
  onPageShow() {
    if (this.surfaceId.length !== 0) {
      this.avSetupAudio(); <em>// 设置AVPlayer组件处于工作状态</em>
   <em>   // 恢复到上次播放的位置</em>
      setTimeout(() => {
      ...
      Stack() {
        ...
        Column() {
          ...
          Column() {
            ...
            Row() {
              Row() {
               <em> // $r("app.media.ic_pause")和$r("app.media.ic_play"))需要替换为开发者需要的图片资源文件</em>
                Image(this.isPlay ? $r('app.media.ic_pause') : $r('app.media.ic_play')) <em>// 暂停/播放</em>
                  .width(40)
                  .height(40)
                  .onClick(() => {
                    if (this.isPlay) {
