...
- 将应用从前台切换到后台，输出日志：“AVPlayer state released called.”，说明AVPlayer组件被销毁，状态变更为released。查看切换到后台时的设置，退到后台后销毁AVPlayer组件，且返回到前台时未重新使AVPlayer组件处于工作状态，导致播放失败。
```text
onPageHide() {
  // 应用退到后台后销毁该组件
