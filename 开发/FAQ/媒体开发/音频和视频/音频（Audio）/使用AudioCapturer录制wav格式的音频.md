...
- 使用AudioCapturer录制音频：开发步骤及完整示例可以参考[使用AudioCapturer开发音频录制功能](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-audiocapturer-for-recording)。
- 将录制的pcm文件转换成wav格式。通过对pcm文件添加文件头，封装成wav格式的音频文件，具体步骤可查看pcmToWav类。
...
let file: fs.File;
let readDataCallback: CallbackArrayBuffer>;
...
async function requestPermissions(permissions: Permissions[], context: common.UIAbilityContext): Promiseboolean> {
