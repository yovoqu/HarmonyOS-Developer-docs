...
- 当AVPlayer播放音频和低分辨率视频时，会占用进程的音频流资源，当进程的音频流资源被耗尽后，AVPlayer再次调用prepare进入工作状态时会报错。
- 当AVPlayer播放高分辨率视频时，会占用系统硬件解码器资源，当硬件解码器资源被耗尽后，AVPlayer再次调用prepare进入工作状态时会报错。
