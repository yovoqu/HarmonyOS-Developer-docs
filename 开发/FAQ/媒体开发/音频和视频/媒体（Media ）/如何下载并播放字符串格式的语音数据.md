...
- 使用[HttpRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-http#httprequest)的[request方法](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-http#request)，传入请求的url地址和可选的参数，发起HTTP数据请求，并在回调中根据实际的业务需求，解析服务端响应的内容（需根据不同的服务端返回内容格式，重写解析方法）。
```text
...
- 将服务端响应的字符串格式的数据转换成mp3格式的数据，并存储在本地应用沙箱目录下。
```text
save(music: string) {
...
- 使用AVPlayer播放存储在本地应用沙箱目录下的mp3音频文件。
```text
