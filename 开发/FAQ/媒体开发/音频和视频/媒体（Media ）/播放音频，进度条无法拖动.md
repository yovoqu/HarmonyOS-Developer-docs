...
- [使用AVPlayer播放音频(ArkTS)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-avplayer-for-playback)：使用AVPlayer可以实现端到端播放原始媒体资源，在应用开发的过程中，开发者可以通过AVPlayer的state属性主动获取当前状态，或使用on('stateChange')方法监听状态变化。
...
- 未进行相关事件监听。
- 异步操作时序混乱。
- Slider组件未绑定AVPlayer实例调用seek方法控制滑动条跳转。
...
- 确保在seek操作完成后更新进度。
- Slider组件在onChange时调用AVPlayer的seek方法同步状态。
