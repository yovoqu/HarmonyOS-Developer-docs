...
- 创建AVScreenCapture实例，配置音频采集参数为同时采集内部和麦克风音频数据。
```text
...
- 创建并配置内录与外录音频编码器。创建媒体文件封装器，并添加内录与外录的音频轨道。
```text
void CAVScreenCaptureToStream::InitMuxerAndEncoder(int32_t outputFd)
{
    ...
    // 音频编码器
    audioEncoderInner_ = std::make_unique(); // 内录音频编码器
    audioEncoderMic_ = std::make_unique();   // 外录音频编码器
    // 媒体文件封装器
...
- 监听录屏数据获取回调[OH_AVScreenCapture_OnBufferAvailable](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-base-h#oh_avscreencapture_onbufferavailable)获取设备内部与麦克风采集的原始音频数据。
```text
...
- 将内录与外录的原始音频数据通过内录与外录音频编码器分别编码，并通过媒体文件封装其写入对应的音频轨道。
```text
void CAVScreenCaptureToStream::AudioEncInputThread()
{
    while (g_isRunning) {
        ...
        std::unique_lock lock(audioEncContext_->inputMutex);
        ...
        if (bufferInfo.codec == audioEncoderInner_->encoder_) {
            ...
            if (audioEncContext_->innerCache.remainLen inputBufferInfoQueue.pop();
            uint8_t *inputBufferAddr = OH_AVBuffer_GetAddr(reinterpret_cast(bufferInfo.buffer));
            ...
            if (audioEncContext_->micCache.remainLen inputBufferInfoQueue.pop();
            uint8_t *inputBufferAddr = OH_AVBuffer_GetAddr(reinterpret_cast(bufferInfo.buffer));
...
void CAVScreenCaptureToStream::AudioEncOutputThread()
{
    while (g_isRunning) {
        ...
        std::unique_lock lock(audioEncContext_->outputMutex);
        ...
        if (bufferInfo.codec == audioEncoderInner_->encoder_) {
            // 内录编码音频写入内录音频轨道
            muxer_->WriteSample(muxer_->GetInnerTrackId(), reinterpret_cast(bufferInfo.buffer),
                                bufferInfo.attr);
            ...
            // 外录编码音频流写入外录音频轨道
            muxer_->WriteSample(muxer_->GetMicTrackId(), reinterpret_cast(bufferInfo.buffer),
                                bufferInfo.attr);
...
- 录制完成后，调用ffmpeg指令编辑录制生成的音频文件，混合内录和外录音频轨道，生成内录与外录混合后的音频文件。
```text
/**
 * 调用ffmpeg指令混合两路音频
...
struct CodecBufferInfo {
    ...
    CodecBufferInfo(uint32_t argBufferIndex, OH_AVBuffer *argBuffer, OH_AVCodec *codec)
        : bufferIndex(argBufferIndex), buffer(reinterpret_cast(argBuffer)), codec(codec)
    {
...
private:
    ...
    std::unique_ptr audioEncInputThread_ = nullptr;
    std::unique_ptr audioEncOutputThread_ = nullptr;
    std::unique_ptr muxer_ = nullptr;

    SampleInfo sampleInfo_;
    std::atomic isStarted_{false};
...
std::unique_ptr audioEncoderInner_ = nullptr;
std::unique_ptr audioEncoderMic_ = nullptr;
