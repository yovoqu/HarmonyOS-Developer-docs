...
在启动录屏前，通过[OH_AVScreenCapture_SetDataCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h#oh_avscreencapture_setdatacallback)接口设置原始视频数据处理回调方法。
```text
OH_AVScreenCapture_SetDataCallback(g_avCapture, OnBufferAvailable, nullptr);
```

- 通过[OH_AVScreenCapture_StartScreenCapture](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h#oh_avscreencapture_startscreencapture)接口以Buffer模式启动录屏。
```text
// 以Buffer模式启动屏幕录制
...
- 启动录屏后，通过设置的[OH_AVScreenCapture_OnBufferAvailable](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-base-h#oh_avscreencapture_onbufferavailable)回调函数来监听是否有视频数据产生并获取屏幕录制的视频数据（RGBA格式）。
```text
// 录屏原始数据获取回调
...
创建视频编码器，并通过[OH_VideoEncoder_GetSurface](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-videoencoder-h#oh_videoencoder_getsurface)接口获取编码器Surface模式的输入[OHNativeWindow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-nativewindow-nativewindow)。
```text
...
- 通过[OH_AVScreenCapture_StartScreenCaptureWithSurface](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avscreen-capture-h#oh_avscreencapture_startscreencapturewithsurface)接口以Surface模式启动录屏，传入视频编码器的输入OHNativeWindow对象。
```text
...
- 屏幕录制的视频数据自动输入视频编码器进行视频编码，可以通过监听视频编码器的输出回调[OH_AVCodecOnNewOutputBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-avcodec-base-h#oh_avcodeconnewoutputbuffer)获取编码视频码流数据（H264或H265格式）。
```text
// 视频编码器编码帧输出回调
