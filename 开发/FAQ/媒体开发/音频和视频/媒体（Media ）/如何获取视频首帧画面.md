...
从本地视频获取首帧有两种方案，分别是通过PhotoAsset类的getThumbnail()方法获取首帧；通过AVImageGenerator选取视频开始时间获取首帧，详细内容请参考选取视频帧作为缩略图。

此外，如果是从沙箱路径获取视频首帧，需要获取视频文件的文件描述符fd，详细内容请参考应用文件访问(ArkTS)。
