...
- 需要获取产物名称，根据产物配置对应的模块参数，因此应在最外层配置hvigorfile.ts文件。
- 根据如下代码获取应用配置参数，获取其中配置的产物名称。
```text
...
- 使用subNodes接口获取所有模块的环境信息，然后通过getContext接口分别获取hap包配置信息。
```text
// 获取hap模块上下文信息
const hapContext = hapNode.getContext(OhosPluginId.OHOS_HAP_PLUGIN) as OhosHapContext;
```

- 根据第2步获取的包名和第三步获取到的环境信息，使用getModuleJsonOpt获取对应模块下的module.json5配置，修改后，使用setModuleJsonOpt将修改后的配置信息写入。
```text
