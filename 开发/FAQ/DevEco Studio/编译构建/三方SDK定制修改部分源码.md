...
新增"Static Library"模块A，要求模块A与提供方的SDK同packageName(oh-package.json5的name字段)和moduleName(module.json5的name字段)，并将模板A中的MainPage.ets源码文件删除。如下所示，拷贝提供方SDK中所有的声明文件、需要修改的源码文件、abc文件、和资源相关文件到模块A中。1. 源码：
- 声明文件：将src/main/ets目录拷贝到模块A中，保持原目录结构不变。
...
- 资源文件：使用提供方SDK中的resources目录覆盖模块A中的resources目录。

  
![](assets/三方SDK定制修改部分源码/file-20260515130226355-2.png)

- 配置文件：1. 将提供方SDK的依赖配置（模块A的oh-package.json5中的dependencies字段）拷贝到模块A中的依赖配置中。
...
- abc文件：将提供方产物中的modules.abc文件拷贝至模块A目录下，并在模块级build-profile.json5文件，buildOption->packingOptions字段中新增customizedOptions，增加basePackage字段指定abc的位置。
...
- 需要将MainPage加入到混淆白名单，需要将需要修改的文件名配置白名单。
