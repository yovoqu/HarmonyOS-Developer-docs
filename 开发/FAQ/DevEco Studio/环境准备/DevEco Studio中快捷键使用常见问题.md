...
打开快捷键设置：通过菜单栏进入：DevEco Studio>Preferences>Keymap（macOS路径：Settings>Keymap）。
- 修改全局搜索快捷键：2.1在搜索框输入Find in Files。
...
- 若依旧未成功，重新依照上述步骤，在搜索中输入delete line，选择edit目录下的快捷键，右键移除快捷键Ctrl+Y即可。
 ...
 - 场景三：
打开DevEco Studio，点击菜单栏DevEco Studio-Preferences。
- 选择Keymap进入快捷键配置界面，在搜索框输入关键词"Finder"或"Reveal in"。
- 分配快捷键右键点击目标操作，选择Add Keyboard Shortcut然后保存即可。
