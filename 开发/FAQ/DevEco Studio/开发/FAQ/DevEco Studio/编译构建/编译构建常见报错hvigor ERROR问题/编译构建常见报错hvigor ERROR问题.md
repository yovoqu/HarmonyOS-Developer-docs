...
- **场景一：**
  ...
  此报错说明编译器在处理文件时，文件扩展名与编译器内部标记的脚本类型不一致，常见于IDE或构建工具的缓存问题，尝试清除缓存后重新构建。1. Build -> Clean Project。
...
- **场景三：**1. 更新最新的DevEco Studio，新版本hvigor依赖内置，不需要添加依赖。

2. 若更新DevEco Studio还报错，进行如下配置：
在hvigor-config.json5中配置："modelVersion": "6.0.2"。

3. build-profile.json5文件里的compatibleSdkVersion配置需要平台版本和API版本相匹配，如："compatibleSdkVersion": "6.0.2(22)"。
