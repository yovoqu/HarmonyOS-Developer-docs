...
- macOS环境变量设置方法：1. 打开终端，执行以下命令，根据输出结果执行不同命令。
...
2. 如果输出结果为/bin/zsh，则执行以下命令以打开.zshrc文件。
```bash
vi ~/.zshrc
```


3. 按字母“i”，进入**Insert**模式。

4. 输入以下内容，添加OHOS_HDC_SERVER_PORT端口信息。
```text
...
5. 编辑完成后，单击**Esc**键退出编辑模式，然后输入“:wq”并单击**Enter**键保存。
...
7. 如果[步骤1](#zh-cn_topic_0000001166752005_li1264071053012)打开的是.zshrc文件，请执行如下命令：
```bash
source ~/.zshrc
```


8. 环境变量配置完成后，关闭并重启DevEco Studio。
