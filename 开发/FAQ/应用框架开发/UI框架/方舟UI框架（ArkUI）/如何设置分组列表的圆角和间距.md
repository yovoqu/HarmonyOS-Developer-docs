...
struct ListItemGroupExample {
  ...
  build() {
    Column() {
      List({ space: 20 }) { <em>// Set the spacing of the grouping list</em>
        ForEach(this.timeTable, (item: TimeTable) => {
          ListItemGroup({ style: ListItemGroupStyle.CARD }) {<em> // Set the rounded corners of the grouping list</em>
