...
struct ScreenTest {
  ...
  aboutToAppear() {
    ...
    try {
      display.on("change", callback); <em>// Listen for screen state changes</em>
    } catch (exception) {
  ...
  setOrientation() {
    try {
      window.getLastWindow(this.context.getHostContext(), (err, data) => { <em>// 获取window实例</em>
        if (err.code) {
        ...
        let orientation = window.Orientation.AUTO_ROTATION; <em>// 设置窗口方向为传感器自动旋转模式。</em>
        try {
