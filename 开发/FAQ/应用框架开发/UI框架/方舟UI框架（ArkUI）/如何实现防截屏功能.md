    ...
    Navigation(this.pageStack) {
      Column() {
        Button('pushPath', { stateEffect: true, type: ButtonType.Capsule })
          ...
          .margin(20)
          .onClick(() => {
            this.pageStack.pushPath({ name: 'PageOne' }) <em>// Push the page info of specified NavDestination into the stack</em>
          })
      }
    }
    .navDestination(this.PagesMap)
    .onNavBarStateChange((isVisible: boolean) => {
      <em>// Callback triggered when navigation bar display state changes</em>
...
struct PageOne {
  ...
  build() {
    ...
    .title('pageOne')
    .onBackPressed(() => {
      const popDestinationInfo = this.pageStack.pop(); <em>// Pop the top element from the navigation stack</em>
      return true;
    })
  }
}
