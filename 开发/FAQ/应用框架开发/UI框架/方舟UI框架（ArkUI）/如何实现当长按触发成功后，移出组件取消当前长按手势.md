...
struct CancelLongPressEventWhenRemovingComponents {
  ...
  build() {
    Column() {
    <em><strong>  // Touchable area.</strong></em>
      Stack() {
  ...
  <em>// Handling touch events.</em>
  private handleTouchEvent(event: TouchEvent) {
    const touchX = event.touches[0].x;
    const touchY = event.touches[0].y;
   <em> // Calculate whether the touch point is within the component boundary.</em>
    ...
    switch (event.type) {
      case TouchType.Down:
        ...
        break;
      case TouchType.Move:
        <em>// Update touch position status.</em>
        this.touchInBounds = inBounds;
        <strong>// If moving out of the boundary and long pressing, cancel.</strong>
