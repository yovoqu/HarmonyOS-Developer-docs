  ...
  build() {
    Stack({ alignContent: Alignment.Bottom }) {
      Navigation(this.pageInfos) {
        Image($r('app.media.ic_banner01')).width('100%').height(200)<em> // The image is stored in the media folder</em>
      }
...
struct PageTransition2 {
  ...
  build() {
    Stack({ alignContent: Alignment.Bottom }) {
      Navigation(this.pageInfos) {
        Image($r('app.media.ic_banner02')).width('100%').height(200) <em>// The image is stored in the media folder</em>
      }
    }.height('100%').width('100%')
  }
