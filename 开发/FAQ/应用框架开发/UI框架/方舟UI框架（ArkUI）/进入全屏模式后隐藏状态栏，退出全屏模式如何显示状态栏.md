...
当应用进入全屏模式时调用setWindowLayoutFullscreen接口会导致状态栏隐藏，返回非全屏页面时需要重新显示状态栏。

解决措施

退出页面时，需要调用setSpecificSystemBarEnabled启用状态栏,再调用setWindowLayoutFullscreen接口刷新布局使设置生效。

参考代码

首先获取窗口对象，可参考：Interface (Window)。
