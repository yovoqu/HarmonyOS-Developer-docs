...
export class DrawViewModel {
  public settings: RenderingContextSettings = new RenderingContextSettings(
    true,
  );
  public context: CanvasRenderingContext2D = new CanvasRenderingContext2D(
    this.settings,
  );
  ...
  moveUpdate(x: number, y: number) {
    ...
    this.points.push({ x: x, y: y });
    this.drawPath.quadraticCurveTo(
      (x + lastPoint.x) / 2,
      (y + lastPoint.y) / 2,
      x,
      y,
