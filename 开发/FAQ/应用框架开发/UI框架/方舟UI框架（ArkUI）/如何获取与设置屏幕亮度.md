...
Button('获取屏幕亮度')
  ...
  .onClick(() => {
   <em> // Get screen brightness through the getValueSync() method.</em>
...
Button('获取屏幕亮度')
  ...
  .onClick(() => {
    try {
     <em> // By retrieving window properties using getWindowProperties(), the screen brightness can be obtained.</em>
