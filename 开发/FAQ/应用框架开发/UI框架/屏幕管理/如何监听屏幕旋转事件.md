...
const ORIENTATION: Array = ['垂直', '水平', '反向垂直', '反向水平'];
...
struct Scene3 {
  ...
  onDegree(callback: base.Callback): void {
    sensor.on(sensor.SensorId.GRAVITY, (data: sensor.GravityResponse) => {
      let degree: number = -1;
      degree = this.CalDegree(data.x, data.y, data.z);
      if (degree >= 0 && (degree = 330)) {
        this.rotation = 'ROTATION_0';
      } else if (degree >= 60 && degree = 150 && degree = 240 && degree  // 3为有效_增量_角度_阈值_系数
    if ((x * x + y * y) * 3 = 0 ? degree % 360 : degree % 360 + 360;
  }
