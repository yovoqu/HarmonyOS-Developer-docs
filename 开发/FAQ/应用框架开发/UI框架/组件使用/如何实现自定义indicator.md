...
- 自定义indicator需要先不启用导航点指示器，即设置Swiper属性indicator(false)。
- 然后根据需要实现的效果设置相应的布局。比如利用Stack布局把Swiper和自定义indicator堆叠显示。
- 自定义设置indicator样式，并通过onChange等事件获取Swiper显示的页面索引，设置对应的indicator显示。
...
- **场景二**：自定义指示器动效。
自定义导航点动效。第1步：设置一个指示器遮罩层，通过控制遮罩层的大小和位置变换实现动画效果。
...
- 数字指示器动效。相较于导航点指示器，数字指示器动效相对简单可以通过滚动与滑动组件实现翻页滚动动效。以下示例通过Swiper组件实现数字指示器滚动效果。
...
- 通过本文档实现自定义的指示器并修改颜色。参考场景一，可以在自定义导航点指示器的最外层Row({space: 5})组件通过修改[backgroundColor](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-background#backgroundcolor)属性，实现导航点背景颜色的修改。
- 在API15中，新增加[Indicator组件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-swiper-components-indicator)，该组件可以绑定Swiper组件的indicator指示器，且支持部分[通用属性](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-component-general-attributes)设置。实现方式参考：[API示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-swiper-components-indicator#示例)。
...
- 通过设置itemHeight或itemWidth调整宽高达到类似效果。API19之前，存在默认间距和间距，在API19之后可以通过设置indicator忽略默认的32vp高度，以及设置space属性重新设置间距等效的宽高控制效果。
- 通过本文档的自定义方式实现指示器的宽高控制。
