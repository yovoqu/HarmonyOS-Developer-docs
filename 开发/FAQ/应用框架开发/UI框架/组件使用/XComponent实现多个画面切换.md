...
- 使用层叠布局，创建2个XComponent组件，组件id设置唯一，通过height，margin设置组件的位置布局。
- 创建AVPlayer实例对象和控制器XComponentController，获取不同组件的id，实现音频播放。
- 设置按钮切换横屏，实时改变XComponent组件的宽高和位置，通过setPreferredOrientation实现多个画面切换。
...
class AvPlayerInstance {
  ...
  private objects = new Mapstring, media.AVPlayer>();
