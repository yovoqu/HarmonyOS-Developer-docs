...
- bindPopup创建的弹窗本身具有点击弹窗内部区域后弹窗不会消失的特性。
- 将autoCancel参数设置为true，可以实现点击弹窗外部使Popup消失。该参数不进行设置时，默认为true。
- 给弹窗内部的按钮添加点击逻辑，改变用于控制Popup显隐的show的值。
