...
- 自定义弹窗创建。如下日志，进入主界面page后，基于NavDestination创建dialog并显示。焦点由page切换到dialog也可说明当前弹窗逻辑。
```text
...
- 侧滑返回后弹窗消失。日志显示侧滑返回后触发NavDestination的onBackPressed回调，之后dialog正常关闭，且页面焦点由dialog切换到page。
```text
...
[route_map.json配置如下](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-navigation#系统路由表)：
