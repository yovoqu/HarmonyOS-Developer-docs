...
在EntryAbility中的onForeground()与onBackground()回调里，通过AppStorage设置一个全局状态标志，用以标识应用的前后台状态。
- 在具体的组件中使用[@StorageLink](https://developer.huawei.com/consumer/cn/doc/cangjie-guides/cj-appstorage#storagelink)绑定此状态，并利用[@Watch](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-watch)装饰器监听其变化，从而自动调用VideoController的start()或pause()方法，以同步控制视频的播放与暂停。具体实现代码可参考该[指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs/faqs-media-29)。

 - 场景二：滚动容器可视区域内自动播放控制，可通过监听Video组件在滚动容器中的可见区域比例变化，实现滚动页面时“可见即播放，不可见即暂停”的播放控制。具体实现思路：
使用onVisibleAreaChange或onVisibleAreaApproximateChange回调监听组件的可见区域比例变化，并设置一个可见比例阈值（如0.5）。
- 当组件可见比例逐渐变大并且超过阈值时，调用VideoController.start()开始播放；当逐渐变小并低于阈值时，调用VideoController.pause()暂停播放。具体实现代码：
