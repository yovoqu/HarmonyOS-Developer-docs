...
- 实现功能时将GridItem内容抽取为独立组件，为item内容设置点击事件，通过点击时状态变量的变化控制item背景色变化。
- 想要获取选中的item，则需要通过item的点击事件将点击时获取到的index插入到数组中，并通过indexOf判断点击内容是否已存在数组中，当不存在时，将选中内容插入数组；当存在时，则不做插入操作。
...
struct GridHighLight {
  ...
  clickThings: Arraystring> = [];
  @Provide selectIndexList: Arraystring> = [];
...
export default struct ToDoItem {
  ...
  @Consume selectIndexList: Arraystring>;
