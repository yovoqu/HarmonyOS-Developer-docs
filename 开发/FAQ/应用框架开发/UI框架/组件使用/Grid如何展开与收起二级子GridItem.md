...
- 通过设置变量进行判断是否展开子级。
- 通过设置的列数，计算一级GridItem总行数、点击GridItem所在行。
- 向当前点击GridItem的下一行(获取当前行最末端的索引)添加一条数据到一级GridItem中，用于展示二级GridItem。
- 修改GridLayoutOptions中的regularSize和irregularIndexes参数用于实现添加数据独占一行。
...
export class CollapseMenu {
  ...
  firstMenu: Array = []; // 一级菜单
  secondMenu: Array = []; // 二级菜单
  ...
  // 初始化一级数据
  public setFirstMenu(arr: Array) {
    this.firstMenu = arr;
  };


  // 初始化二级数据
  public setSecondMenu(arr: Array) {
    this.secondMenu = arr;
  };
...
struct Index {
  ...
  child: Array = ['child1', 'child2', 'child3', 'child4', 'child5'];
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ee/v3/keCDCza4T-666dGzQyupEQ/zh-cn_image_0000002628554904.png?HW-CC-KV=V1&HW-CC-Date=20260701T025543Z&HW-CC-Expire=86400&HW-CC-Sign=582E8C5A40F3431A9CC60FD906490033F95DF84A6FD0D52DFF7642737E976412)
