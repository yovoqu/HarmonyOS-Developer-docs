...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b5/v3/uDcc9cNfQpGhuXECUjtApQ/zh-cn_image_0000002628595452.png?HW-CC-KV=V1&HW-CC-Date=20260701T025606Z&HW-CC-Expire=86400&HW-CC-Sign=481C0DE6C43ABE83357FCDFD549A3350F60E2EB6CACD816A0918D4AE7ECD48C4)
...
- 日志搜索Focus，查看组件聚焦情况，发现两次拉起输入法时，输入框的组件ID不同，可以判断组件发生了重新渲染，当组件重新渲染时，输入框会失焦，导致输入法收起。
```text
...
- 代码搜索onChange，检查代码逻辑，当输入变化时，onChange调用自定义函数并导致组件重新渲染。
```text
TextArea.onChange(() => {
  // 自定义函数包含组件重新渲染逻辑
});
