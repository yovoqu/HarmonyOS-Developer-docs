...
- 添加变量imageList存放所有图片的名称信息。
- 使用aboutToIMEInput方法监听输入内容是否为“@”，当输入“@”时赋值标志符flag为true。
- 当输入空格时结束输入图片名，并判断当前图片是否存在。
- 使用deleteSpans删除当前“@XXX”，并使用addImageSpan添加对应图片内容。
...
struct RichEditorReplaceImageDemo {
  ...
  // 存放图片名称
  imageList: Arraystring> = ['logo', 'startIcon'];
