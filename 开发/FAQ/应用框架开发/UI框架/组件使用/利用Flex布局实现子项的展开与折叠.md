...
通过子组件高度、Flex的属性、以及折叠状态下展示的行数计算，获取折叠状态下Flex组件的高度；
- 通过屏幕宽度计算Flex组件宽，从而得到每一行Flex组件布局时的可用宽度，使用measureText方法计算文本宽度，获取子组件宽度；
- 根据子组件占用的行数进行判断是否展示展开/折叠按钮。当子组件折叠时按照1中得到的高度设置Flex高度，展开时高度设置为'auto'，实现展开/折叠。
  ...
  getShowButtonValue() {
    ...
    this.textList.forEach((item) => {
      if (line // 行数还未达到需要折叠的行，继续计算布局
        ...
        let itemWidth =
          (textWidth + 16)  this.lineWidth ? this.lineWidth : (textWidth + 16));
        if (countWidth === 0) {
          countWidth = itemWidth; // 第一个子组件Flex没有主轴方向子组件间距
        } else {
          if (countWidth + itemWidth + 10 // 换行
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/fe/v3/IcC0rj5US6CiZsifnadJjQ/zh-cn_image_0000002628753484.png?HW-CC-KV=V1&HW-CC-Date=20260701T025605Z&HW-CC-Expire=86400&HW-CC-Sign=9F25CEEEBAAFD92121A1F3B87CD1CBA75147B6D386A3677EC9C5379AC5AA7507)

 
 
- 方案二：获取Flex展示的最后一个子组件索引，通过部分渲染的方式实现展开与折叠。
通过屏幕宽度计算Flex组件宽，从而得到每一行Flex组件布局时的可用宽度，使用measureText方法计算文本宽度，获取子组件宽度；
- 对折叠前几行进行计算，当子组件换行时超过折叠行数记录当前子组件的索引-1，获取折叠状态下最后一个子组件索引；
- 在Flex组件中渲染子组件时添加判断条件。当Flex状态为展开状态时，渲染全部子组件。切换为折叠状态时，渲染记录索引前的组件。
    ...
    Column() {
      Flex({
        ...
        ForEach(this.textList, (item: string, index: number) => {
          if (this.flexState === '展开' || index // 展开状态全展示，折叠状态展示部分
            TextItem({ text: item });
          }
        });
      }
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c4/v3/3TV2EJ5VSIu4kZvvH4Qlqg/zh-cn_image_0000002658952797.png?HW-CC-KV=V1&HW-CC-Date=20260701T025605Z&HW-CC-Expire=86400&HW-CC-Sign=00EAE9E96ACE82D1E5E95228C1EE1BDAC814F8A59449E1EE85EF4324C4F269F3)
