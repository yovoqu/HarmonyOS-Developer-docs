...
- sticky属性设置：通过.sticky(StickyStyle.Header)启用列表的头部吸顶特性。
- 组件区域变化监听：在自定义Header组件中，使用onAreaChange事件监听组件位置变化。
- 坐标判断逻辑：通过判断事件参数中的oldValue.position.y === 0 && newValue.position.y === 0时表示未吸顶。
