...
- 将一个不可见的组件（如高度宽度为0的组件）用于绑定bindContextMenu菜单。
- 由实际接收点击事件的组件控制isShown状态，并在点击时通过事件对象获取当前点击位置的屏幕坐标。
- 将获取的坐标值通过菜单配置参数（如ContextMenuOptions中的offset属性）动态设置菜单弹出位置，从而实现菜单跟随点击位置弹出。
```text
@Entry
@Component
struct BindMenu {
