...
struct WindowProp {
  ...
  onPageShow(): void {
    ...
    try {
      ...
      promise.then((data) => {
        windowClass = data;
        try {
          <em>// 通过调用getWindowProperties接口返回一个类型为WindowProperties的属性，该属性中的windowRect属性返回一个Rect类型的参数，该参数可获取到当前窗口的宽度（width）和高度（height）及窗口的左边界（left）和上边界（top）信息。</em>
  ...
  build() {
    Column({ space: 20 }) {
      ...
      <em>    // 通过windowRectRes的left属性获取窗口的左边界（x轴坐标值）</em>
        ...
        Row() {
          Text('窗口Y轴坐标值：')
            .fontSize(15)
            .fontWeight(FontWeight.Bold);
         <em> // 通过windowRectRes的top属性获取窗口的上边界（y轴坐标值）</em>
      ...
      Row({ space: 20 }) {
        Row() {
          Text('窗口宽度：')
            .fontSize(15)
            .fontWeight(FontWeight.Bold);
       <em>   // 通过windowRectRes的width属性获取窗口的宽度</em>
        ...
        Row() {
          Text('窗口高度：')
            .fontSize(15)
            .fontWeight(FontWeight.Bold);
         <em> // 通过windowRectRes的height属性获取窗口的高度</em>
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c2/v3/X9uFYXlYQROqFWcskuCCaw/zh-cn_image_0000002658804079.png?HW-CC-KV=V1&HW-CC-Date=20260730T072424Z&HW-CC-Expire=86400&HW-CC-Sign=AF2FB52456F5AB3583A2E027FF411A3518D2EDB9489B4FC08B9EE00CEBBF7AF5)
...
static napi_value getWindowPropertiesByID(napi_env env, napi_callback_info info)
{
  ...
  <em>// </em><em>根据窗口id获取窗口信息，返回的windowProperties实例中的windowRect属性返回一个WindowManager_Rect类型的参数，该参数可获取到当前窗口的宽度（width）和高度（height）及窗口的x轴（posX）和y轴（posY）的信息</em>
