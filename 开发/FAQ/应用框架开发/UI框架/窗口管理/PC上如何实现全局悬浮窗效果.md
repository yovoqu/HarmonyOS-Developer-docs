...
- 新建独立窗口的代码“src/main/ets/pages/pageTwo.ets”并在配置文件“src/main/resources/base/profile/main_pages.json”中新增路径：
```text
@Entry
@Component
struct PageTwo {
...
- 在“src/main/module.json5”中配置窗口全局置顶权限：
```text
"requestPermissions": [
  {
...
export default class FloatWindowAbility extends UIAbility {
  ...
  private async setTopmost(win: window.Window, isWindowTopmost: boolean): Promisevoid> {
    try {
...
- 在应用配置文件“src/main/module.json5”的abilities属性中，新增FloatWindowAbility配置项以及supportWindowMode属性为floating：
```ArkTS
{
...
- 在主界面配置拉起FloatWindowAbility窗口按钮：
```text
