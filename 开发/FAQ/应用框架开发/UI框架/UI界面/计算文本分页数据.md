...
- 获取导航条和状态栏的高度，在EntryAbility.ets的onWindowStageCreate()中设置。
```text
...
- 每页能展示的行数：获取页面的真实高度后，每页能展示的行数即为总高度/每行的高度（通过MeasureText.measureTextSize获取）。
```text
// 每页能展示的行数
heightCalculator() {
...
- 页面的数量：通过getLineCount方法获取组件内容的总行数，再用总行数/每页展示的行数即为总页数。
```text
// 计算页面数量
pageNum() {
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ce/v3/qNcuOUZUROeMugv1qLgDcw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T025702Z&HW-CC-Expire=86400&HW-CC-Sign=91D0F1D9F189EC20469DFF7FB5F9A6B656973D91E2EF3CE0D3A4DD1A1F1CE7B0)
