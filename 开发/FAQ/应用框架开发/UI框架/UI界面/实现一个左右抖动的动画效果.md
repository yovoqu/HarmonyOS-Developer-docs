...
- 创建抖动容器组件：使用keyframeAnimateTo关键帧动画通过控制translateX属性，让动画对象在X轴进行平移，实现左右抖动效果。
- 设置全局状态：AppStorage.setOrCreate&lt;number&gt;('shakeCount', 0);，便于全局调用该状态。
- 将需要设置抖动的页面添加抖动容器组件作为根组件，通过按钮点击事件调用AppStorage.set&lt;number&gt;('shakeCount', count + 1)改变shakeCount的值，触发页面抖动效果。
    ...
    Column() {
      ShakeContainer() {
        ...
        Column() {
          ...
          Button('点击').onClick(() => {
            let count = AppStorage.get('shakeCount');
            if (count !== undefined) {
              AppStorage.set('shakeCount', count + 1);
            }
          })
        }
      }
