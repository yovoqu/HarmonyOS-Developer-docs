...
replacePathByName的执行顺序是：先让目标页面B入栈并准备显示，然后再销毁当前页面A。因此，在页面跳转(A -> B)过程中，生命周期的实际触发顺序是：
 1. B页面的aboutToAppear。
2. A页面的aboutToDisappear。**关键点**：这意味着页面B的构建(aboutToAppear)发生在页面A销毁(aboutToDisappear)之前。
 
数据变量处理建议：
 1. 页面级数据：通常无需特殊处理。当从页面A跳转到页面B时，页面A的数据变量会在其aboutToDisappear后被销毁，不会影响B页面的数据。这是预期行为。
2. 全局数据（globalData问题）：**问题根源**：由于页面B的aboutToAppear在页面A的aboutToDisappear之前执行，在页面B中访问globalData时，页面A中初始化globalData的代码(aboutToDisappear)尚未执行。
