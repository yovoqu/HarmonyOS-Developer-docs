...
- 闪屏页在定时器setTimeout回调中通过pop返回首页，此时闪屏页被销毁。
...
闪屏页使用Entry页面，设置定时或用户同意隐私协议后通过[pushUrl](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-router#pushurl)、[replaceUrl](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-router#replaceurl)等方法跳转Navigation主页。
- 若使用pushUrl方法跳至Navigation页面，则需要[onPageShow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-custom-component-lifecycle#onpageshow)生命周期中使用[clear](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-router#clear)清除闪屏页。
- Router闪屏页代码：
```text
@Entry
@Component
