...
- 使用fs.access判断文件/目录是否存在；
- 使用fs.mkdir创建文件夹（当参数recursion指定为true时，可递归创建多文件夹）；
- 使用fs.open(filePath, fs.OpenMode.CREATE | fs.OpenMode.READ_WRITE)创建文件并以读写模式打开；
- 文件读写则使用fs.read、fs.write方法。
