...
- 数据库建表时声明BigInt数据类型为：UNLIMITED INT。
- 构造数据插入表中，使用BigInt()生成BigInt型数据。
- 查询数据库，使用[resultSet.getValue()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-data-relationalstore-resultset#getvalue12)获取BigInt型数据。
    ...
    Column({ space: 20 }) {
      ...
      Button('插入bigInt类型数据')
        ...
        .onClick(() => {
          ...
          let dataList = new ArrayrelationalStore.ValuesBucket>();
