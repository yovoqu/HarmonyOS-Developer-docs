...
- 将旧表重命名。
- 创建新表结构，使用UNLIMITED INT类型替换需要转换的字段。
- 将旧表数据迁移到新表，使用CAST(原列AS UNLIMITED INT)确保数据无损转换。
- 删除旧表。
...
struct Index {
  ...
  async insert(valueBucketArray: ArrayrelationalStore.ValuesBucket>) {
    // 数据插入
    if (this.rdbStore) {
      ...
      Column() {
        Button('插入数据')
          ...
          .onClick(() => {
            ...
            let valueBucketArray = new ArrayrelationalStore.ValuesBucket>(count);
