...
- 获取媒体库动态照片对象，将动态照片的图片内容和视频内容分别保存在沙箱文件中。具体可参考：[获取媒体库动态照片对象](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/photoaccesshelper-movingphoto#获取媒体库动态照片对象)。
```text
async function pickMovingPhoto(phAccessHelper: photoAccessHelper.PhotoAccessHelper, context: Context) {
  try {
    ...
    for (let i = 0; i  // 获取uri对应的PhotoAsset资产。
...
- 使用[ImagePacker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagepacker)类的[packToFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagepacker#packtofile11)接口压缩动态照片的图片内容。具体可参考：[图片编码进文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/image-encoding#图片编码进文件)。
```text
Button('Pack Image')
  .width('100%')
  .onClick(async () => {
...
- 使用[AVTranscoder](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avtranscoder)对动态照片的视频内容进行编码转换压缩。具体可参考：[使用AVTranscoder实现视频转码](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-avtranscoder-for-transcodering)。
```text
Button('Pack Video')
  .width('100%')
  .onClick(async () => {
...
- 根据压缩后的图片和视频文件重新加载成MovingPhoto对象。具体可参考：[获取应用沙箱动态照片对象](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/photoaccesshelper-movingphoto#获取应用沙箱动态照片对象)。
```text
...
- 根据压缩后的图片和视频文件保存动态照片对象到系统图库。具体可参考：[保存动态照片资源](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/photoaccesshelper-movingphoto#保存动态照片资源)。
```text
SaveButton({
