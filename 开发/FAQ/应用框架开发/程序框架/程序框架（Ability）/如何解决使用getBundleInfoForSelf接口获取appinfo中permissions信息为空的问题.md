...
- 调用bundleManager.getBundleInfoForSelf接口获取应用包信息需要传入bundleFlags参数，根据传入的bundleFlags参数不同，获取的appinfo信息不同，检查bundleFlags参数是否设置正确。
- bundleFlags参数设置可以参考[BundleFlag](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-bundlemanager#bundleflag)。
- 获取应用包申请权限信息需要添加包信息标志GET_BUNDLE_INFO_WITH_REQUESTED_PERMISSION，未添加此标志值则无法获取应用包申请权限信息。
