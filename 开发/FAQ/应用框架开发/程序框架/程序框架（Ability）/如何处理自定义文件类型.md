...
- 在当前应用entry\src\main\resources\rawfile\arkdata\utd\目录下新增utd.json5文件。其中的**TypeId属性由应用bundleName和具体类型名组成**，例如com.example.myapplication.kml，其中com.example.myapplication为应用bundleName。其余字段详情介绍可参考[约束限制](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/uniform-data-type-descriptors#约束限制)。代码示例参考如下：
```text
{
...
- 修改应用的module.json5文件，声明相关文件的打开能力，代码示例参考如下：
```text
{
  "module": {
    // ...
...
- 修改EntryAbility（UIAbility），在onNewWant生命周期回调当中，接收文件，处理逻辑，代码示例参考如下：
```text
