...
应用可以监听页面加载异常的相关事件如onErrorReceive、onHttpErrorReceive和onSslErrorEventReceive等，在对应的回调中按需实现业务逻辑，如使用loadurl加载自定义错误页；本文以onErrorReceive为例对主资源报错的场景进行处理，加载本地错误页面资源文件。
