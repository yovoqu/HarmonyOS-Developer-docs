...
- 检查预览参数配置是否正确。Preview Kit支持的文件类型只有文件后缀类型与mimeType类型相匹配时，才能正常预览文件。ofd文件的mimeType类型为general.ofd，也可以传入空值由Preview Kit判断文件类型。
  ...
  copyFile() {
    ...
    try {
      ...
      while (len) {
        ...
        if ((srcFileDescriptor.length - readLen) // 关闭文件或目录
      if (file) {
        fs.closeSync(file);
      }
