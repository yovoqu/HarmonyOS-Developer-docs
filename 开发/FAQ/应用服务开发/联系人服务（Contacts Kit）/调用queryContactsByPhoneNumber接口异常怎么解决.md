...
- 检查ACL受限权限是否申请成功，生成Profile时是否进行勾选对应受限权限。然后检查手动签名是否更新了Profile文件等签名信息。
- 检查接口使用是否正确，接口返回值是否有name字段。
...
- ohos.permission.READ_CONTACTS是受限开放权限，授权方式是用户授权（user_grant）。该问题是由于默认应用等级为normal，只能使用normal等级的权限，如果使用了system_basic、system_core等级或受限开放的权限，将导致报错。
- [contact.queryContactsByPhoneNumber](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-contact#contactquerycontactsbyphonenumber10-2)接口仅返回联系人信息中的id、key、phoneNumbers属性。
...
- contact.queryContact有多个入参不同的同名接口，详情参考[@ohos.contact API](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-contact)。其中入参含有attrs联系人的属性列表的接口需要注意，attrs不传默认查询所有联系人属性，如果传了就需要包含contact.Attribute.ATTR_NAME接口才会返回name。
