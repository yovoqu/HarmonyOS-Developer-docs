...
- 获取签名后的描述文件（如manifest.json5），根据[验签工具](https://gitee.com/arkin-internal-testing/internal-testing)说明，执行如下命令：java -jar manifest-sign-tool-1.0.0.jar -operation verify -inputFile C:\xxx\manifest.json5 -keystore C:\xxx\internaltest.p12 -keystorepasswd internaltest

  其中：

2. operation为操作类型，验证签名时为verify；

3. inputFile为需要生成签名的manifest.json5文件路径；

4. keystore为p12密钥文件路径，对应Store file(*.p12)；

5. keystorepasswd为密钥文件密码，对应Store password；

6. manifest-sign-tool-1.0.0.jar可通过[验签工具](https://gitee.com/arkin-internal-testing/internal-testing)获取。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/72/v3/8-4lL0LuTqi5ndzW-qKpDw/zh-cn_image_0000002640108094.png?HW-CC-KV=V1&HW-CC-Date=20260723T013847Z&HW-CC-Expire=86400&HW-CC-Sign=CEE1352BC82D3CBCF8758296A9B8A2105F09C212DEDFDDDA16A8520F326A0343)
...
A：请自行排查是否使用构建Deeplink实现下载应用方式安装应用，具体请参考[指定设备发布应用](https://developer.huawei.com/consumer/cn/doc/app/agc-help-internal-test-release-app-0000002260691994)。
