...
使用Huawei CBG Root CA G2证书对证书链进行验证。
- 校验叶子证书的OID：1.3.6.1.4.1.2011.2.415.1.1（固定值）。
- 证书校验通过则从叶子证书获取到PublicKey。
- 使用Header指定的算法和获取到的PublicKey进行JWT验签，服务端验签参考文档：[JWS解码和验签示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/iap-verifying-signature#section139121421071)。
