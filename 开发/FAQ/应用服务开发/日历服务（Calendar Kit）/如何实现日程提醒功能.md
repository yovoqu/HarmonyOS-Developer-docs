...
- 权限声明，module.json5中配置权限声明：获取写日历使用权限：[ohos.permission.WRITE_CALENDAR](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/permissions-for-all-user#ohospermissionwrite_calendar)。
...
- 配置权限：module.json5文件中配置日历的读写权限。
- 在使用前获取用户授权；获取权限后获取日历管理类。
- 设置日历账户信息和配置信息，并创建日历对象（参见Index.ets中的aboutToAppear()方法）。
- 根据需要新增、删除、更新、查找日历事件。
