...
- 应用A（BundleName：com.hw.mycontinuea），入口ability为EntryAbility：
...
onContinue(wantParam: Record) {
  ...
  // 兼容性校验
  if (targetVersion // 建议在校验版本兼容性失败后，提示用户拒绝迁移的原因
  ...
  try {
    ...
    if (launchParam.launchReason === AbilityConstant.LaunchReason.CONTINUATION) {
      ...
      if (want.parameters !== undefined) {
        ...
        AppStorage.setOrCreate('message', continueInput);
...
```
 - 应用B（BundleName：com.hw.mycontinueb），入口ability为ProductAbility：
在module.json5的abilities标签中增加如下配置：
```text
"continuable": true,
"continueBundleName": [
