...
- 查看对应Web页面网址，在浏览器打开对应网址调试。检查Web页面代码中Text的fontsize设置的值是否为固定值。当尺寸设置为固定值时，会导致在平板上展示时出现过大或者过小的情况。
- 建议检查Web页面有没有采用相对单位（%、em、rem、vw/vh）、媒体查询（media）、监听窗口变化事件（通过JavaScript注册resize事件）等多设备适配能力进行页面开发。
- 查看对应Web网页中的head标签是否包含适配移动端的视口元素。
```text
