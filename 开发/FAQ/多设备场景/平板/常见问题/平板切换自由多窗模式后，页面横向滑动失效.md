...
- 建议检查代码中PanGesture滑动手势事件中onActionUpdate和onActionEnd方法中设定的偏移量阈值是否是固定的。
- 如果设定为固定值，检查应用有没有通过on('windowSizeChange')方法实现对窗口尺寸大小变化的监听。在窗口变化时无法及时动态改变滑动阈值，会导致自由多窗情况下滑动不到设定的距离，无法完成滑动返回上一页的操作。
