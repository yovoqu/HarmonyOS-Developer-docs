...
- 应用中排查display.getDefaultDisplaySync或者windowSizeChange方法，确定应用有无监听屏幕尺寸的变化，并对布局进行调整。
- 应用中排查视频全屏播放时，有无通过setWindowLayoutFullScreen设置窗口为全屏模式。
- 排查视频播放组件是否设置了RenderFit属性，对视频画面填充方式进行了设置。
...
- module.json5配置文件中[abilities标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#abilities标签)下orientation设置为auto_rotation，随传感器旋转，在设备横竖屏切换时，窗口会随之旋转，示例如下：
```ArkTS
{
  "module": {
    "name": "entry",
...
- 通过display.getDefaultDisplaySync或者windowSizeChange方法监听屏幕最新的尺寸变化，对视频播放界面的布局进行调整，示例如下：
```text
// 监听窗口尺寸变化
...
- 视频全屏播放时，通过setWindowLayoutFullScreen设置窗口为全屏模式，示例如下：
```text
setWindowLayOut() {
  // 监听全屏事件，设置沉浸式窗口
...
- 视频播放组件设置RenderFit属性，对视频画面填充方式进行设置，避免四周有大量留白，示例如下：
```text
XComponent({
...
struct Index {
  ...
  private readonly OPERATE_STATE: Array = ['prepared', 'playing', 'paused', 'completed'];
