...
- 在EntryAbility内，主窗口获取并保存UIContext：
```text
...
- Index.ets页面,使用startAbility唤起小窗口。
```text
...
- 新建KeyboardPage.ets，通过StorageProp获取在主窗口保存的UIContext，可以通过该UIContext对TextInput获取焦点使能。
```text
@Entry
@Component
struct KeyboardPage {
...
- 新建小窗口的Ability命名为KeyboardAbility，并通过loadContent加载KeyboardPage页面，并且指定小窗口的大小和位置。
```text
import { UIAbility } from '@kit.AbilityKit';
...
- 将KeyboardAbility添加至模块配置的module.json5中abilities里。
```ArkTS
{
  "module": {
    "name": "entry",
