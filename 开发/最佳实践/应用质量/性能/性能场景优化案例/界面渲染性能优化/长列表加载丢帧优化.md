...
> [!NOTE]
> 建议开发者优先使用 代码Code Linter检查 工具进行代码扫描，重点关注 @performance/hp-arkui-set-cache-count-for-lazyforeach-grid 规则。若扫描结果中出现该规则相关问题，可参考本章节提供的优化建议进行调整。
...
在应用列表界面，创建DataSourcePrefetching和[BasicPrefetcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-prefetcher#basicprefetcher)对象。BasicPrefetcher是[IPrefetcher](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-prefetcher#iprefetcher)接口的基础实现，提供了一种智能数据预取算法，以根据屏幕上可见区域的实时变化和预取持续时间的变化来决定应预取哪些数据项。在List的onScrollIndex回调中，调用BasicPrefetcher的[visibleAreaChanged()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-prefetcher#visibleareachanged-1)方法，传入List的可见区域起始坐标。在可见区域变化时，实时更新预取的范围。
...
![](assets/长列表加载丢帧优化/file-20260525085459447-001.gif)
...
> [!WARNING]
> 需要注意的是复用组件中有@Builder自定义构建函数时，状态变量推荐使用 按引用传递 。@Builder装饰的函数默认按值传递，当传递的参数为状态变量时，状态变量的改变不会引起@Builder方法内的UI刷新。
...
![](assets/长列表加载丢帧优化/file-20260525085459448-005.png)
...
**图17 **丢帧耗时分析     **
![](assets/长列表加载丢帧优化/file-20260525085459448-006.png)


图18 **对丢帧部分的放大分析（整体耗时13.430ms）     

![](assets/长列表加载丢帧优化/file-20260525085459448-007.png)
...
![](assets/长列表加载丢帧优化/file-20260525085459448-008.png)
...
![](assets/长列表加载丢帧优化/file-20260525085459449-009.png)
...
**图21 **布局优化前后的层级变化      **
![](assets/长列表加载丢帧优化/file-20260525085459449-011.png)
...
![](assets/长列表加载丢帧优化/file-20260525085459449-012.png)
