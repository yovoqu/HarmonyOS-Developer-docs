...
目前并行化改造的主要实现方案有两种：一是使用TaskPool或Worker创建子线程，将耗时逻辑迁移至子线程执行；二是使用Sendable加速数据传递，避免跨线程间的数据拷贝耗时。
...
> [!NOTE]
> 相关使用说明与注意事项：
>  TaskPool注意事项Worker简介并发支持的可序列化数据类型Sendable使用约束和限制Sendable支持的数据类型
...
![](assets/并行化性能优化/file-20260515115041868-0.png)
...
export class LightArtDataComponentType implements ISerializableType<LightArtDataComponent> {
  ...
  public static instance: LightArtDataComponentType =
    new LightArtDataComponentType();
}
```
...
![](assets/并行化性能优化/file-20260515115041868-6.png)
...
本案例主要描述子线程范围2中使用全局对象信息调用网络库请求网络数据的场景。以Axios(基于Promise的网络请求库，需要安装后使用)为例，进行相关并行化改造。
...
export interface IRequestInterceptor extends lang.ISendable {
  handle(
    data: InternalAxiosRequestConfig<object>,
  ):
...
export interface IResponseInterceptor extends lang.ISendable {
  handle(
    data: AxiosResponse<object, object>,
  ): AxiosResponse<object, object> | Promise<AxiosResponse<object, object>>;
  handleError(error: object): object;
}
