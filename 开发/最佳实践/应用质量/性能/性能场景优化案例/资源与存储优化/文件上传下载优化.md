...
本文基于实现上传和下载功能示例中的后台上传场景，给出了部分断点续传的示例代码，具体可以参考该工程。


### 文件上传


本文使用request模块中的request.agent()任务托管接口，自动实现暂停、继续、重试等操作，无需手动分片和记录分片信息。流程图如下：
...
![](assets/文件上传下载优化/file-20260515115035016-3.png)
...
```ts
request.agent
  .create(context, this.downloadConfig)
