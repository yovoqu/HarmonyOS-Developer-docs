...
![](assets/Image白块解决方案/file-20260515115040984-0.png)
...
![](assets/Image白块解决方案/file-20260515115040984-1.png)
...
export async function httpRequest() {
  fs.access(fileUrl, fs.AccessModeType.READ).then((res) => {
    ...
    if (!res) {
      // If the address does not exist in the sandbox, re-request the network image resource
      http
        .createHttp()
...
下面使用trace对优化前后性能进行对比分析。开发者可以通过使用DevEco Profiler工具来抓捕trace图并识别分析应用中的性能问题。
...
![](assets/Image白块解决方案/file-20260515115040984-2.png)
