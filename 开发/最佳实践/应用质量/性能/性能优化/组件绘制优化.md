...
当组件的宽高不需要自适应时，建议在UI描述中明确指定组件的宽高数值。如果组件外部的容器尺寸发生变化，例如在拖拽缩放等场景下，组件本身的宽高固定，理论上该组件在布局阶段不会参与Measure阶段，节点中已保存了相应的大小信息。如果组件内容较多，由于避免了整体的测算过程，性能将显著提升。具体优化案例，请参阅[利用布局边界减少布局计算](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-optimization-guidance#利用布局边界减少布局计算)、[给定List组件宽高](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-optimization-guidance#scroll嵌套list场景下给定list组件宽高)**。**
