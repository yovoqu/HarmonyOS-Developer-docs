...
控制元素显示与隐藏是一种常见的场景，使用Visibility.None、if条件判断等都能够实现该效果。其中if条件判断控制的是组件的创建、布局阶段，visibility属性控制的是元素在布局阶段是否参与布局渲染。若使用方式不当，将引起性能问题。具体案例与实验数据请参阅[合理控制元素显示与隐藏](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-optimization-guidance#合理控制元素显示与隐藏)。
