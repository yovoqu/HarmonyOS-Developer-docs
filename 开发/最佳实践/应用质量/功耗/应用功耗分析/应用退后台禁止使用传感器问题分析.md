...
2. hilog：https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hilog
3. 判断应用内所有界面退后台后，是否存在传感器没有关闭的行为
方法一：通过DevEco 软件日志栏中实时过滤：(bundleName).*open the sensor|(bundleName).*close the sensor
4. 方法二：本地使用命令行控制台实时过滤日志：hdc shell hilog | grep -i "(bundleName).*open the sensor\|(bundleName).*close the sensor"
5. 判断应用在后台是否存在长时任务
过滤关键词：suspend_manager.*(bundleName)
应用后台不存在长时任务
6. 应用后台存在长时任务
7. 如果存在应用退后台后还存在没有关闭的传感器，并且没有长时任务保活，需要进行优化，合理使用资源
...
过滤日志关键词：suspend_msg.*(bundleName).*state
state 2 表示应用处于前台
2. state 4 表示应用处于后台
3. 通过所有日志查看哪些传感器在退后台后没有关闭。
