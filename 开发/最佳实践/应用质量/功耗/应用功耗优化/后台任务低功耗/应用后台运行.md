...
![](assets/应用后台运行/file-20260525085655579-001.png)
...
![](assets/应用后台运行/file-20260525085655580-002.png)
1. 定义短时任务信息SuspendTaskInfo()接口，包括短时任务的ID和获取对应短时任务的剩余时间delayTime。export interface SuspendTaskInfo {
  id: number; // Short-time task ID
  delayTime: number; // The remaining time of this request short assignment
}
2. 在信息发送的场景中，当应用处于前台时，在onAppear()回调函数中启动一个定时器，每隔2秒发送一条消息，模拟后台业务。NavDestination() {
  // ...
}
...
3. 在信息发送的场景中，通过ApplicationContext.on('applicationStateChange')注册对当前应用前后台变化的监听。当应用退至后台时，触发onApplicationBackground()回调函数，在此回调函数中申请短时任务。// Apply front - and back-end status monitoring
...
5. 应用返回前台后，调用backgroundTaskManager.getRemainingDelayTime()接口，获取对应短时任务的剩余时间。async getRemainingDelayTime(id: number): Promise&lt;number&gt; {
...
6. 应用退至后台，可调用backgroundTaskManager.cancelSuspendDelay()接口取消短时任务。cancelSuspendDelay(id: number): boolean {
  try {
...
**实现效果**
- **系统息屏场景/应用置于后台场景：**前台应用在自动息屏后，会被识别为置于后台。此时，应用可以申请短时任务，剩余时长上限为3分钟（如下图所示，delayTime为180000ms）。

![](assets/应用后台运行/file-20260525085655580-003.png)
- 当短时任务的剩余时间不足时，系统会触发回调，停止任务。

![](assets/应用后台运行/file-20260525085655580-004.png)
...
![](assets/应用后台运行/file-20260525085655580-005.png)
...
2. 应用跳转到定位功能页面，系统请求定位权限和网络访问权限。// Apply for location-related permissions
...
3. 应用需获取位置信息，使用geoLocationManager.on('locationChange')接口，开启位置变化订阅并发起定位请求。locationCallback = async (location: geoLocationManager.Location) => {
...
4. 应用退至后台需持续运行时，应调用backgroundTaskManager.startBackgroundRunning()接口申请长时任务。// Start a long task
startLongTask(): void {
...
5. 在定位和导航类的应用场景中，当应用退出时，需调用geoLocationManager.off('locationChange')接口，关闭位置变化订阅。geoLocationManager.off('locationChange');
6. 应用退出时，调用backgroundTaskManager.stopBackgroundRunning()接口，取消长时任务。// Stop a long task
stopLongTask(): void {
...
![](assets/应用后台运行/file-20260525085655580-006.png)
- 在定位和导航应用场景中，应用退至后台持续运行，控制台日志定时打印位置信息。

![](assets/应用后台运行/file-20260525085655580-007.png)
