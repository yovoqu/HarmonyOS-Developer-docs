...
![](assets/CppCrash类问题案例/file-20260525085713231-001.png)
...
![](assets/CppCrash类问题案例/file-20260525085713231-002.png)
...
![](assets/CppCrash类问题案例/file-20260525085713232-003.png)
运行时在GC过程中IterateWeakEcmaGlobalStorage，会对无callback的WeakNode调用DisposeGlobalHandle操作，也对其进行释放，如下图。

![](assets/CppCrash类问题案例/file-20260525085713232-004.png)
...
![](assets/CppCrash类问题案例/file-20260525085713232-005.png)
...
![](assets/CppCrash类问题案例/file-20260525085713232-006.png)
...
![](assets/CppCrash类问题案例/file-20260525085713232-007.png)
...
分析步骤：
1. 根据pc寄存器地址找到对应的汇编指令，确定在哪条指令执行时发生崩溃。 在CppCrash日志文件中找到栈顶的PC地址，并反汇编对应的二进制。 例如在执行00000000000a87e4地址对应的指令时发生崩溃，反汇编查看ld-musl-aarch64.so.1文件0xa87e4偏移地址显示的信息： xxx/../../third_party/optimized-routines/string/aarch64/memcpy.S:175 <- 源码行号
a87e4：a94371aa ldp x10, x11, [x1, #48]
地址： 值 汇编指令 ldr指令是加载多数据指令（LDP-Load Pair），用于从内存中同时加载两个64位的数据到两个不同的寄存器中。 ldp x10, x11, [x1, #48]
ldp 目标寄存器1, 目标寄存器2, <源地址> 从内存中指定位置（由寄存器x1中地址加上48字节偏移量确定）读取两个连续的64位数据，并将它们分别存储到寄存器x10和x11中。 根据反汇编显示的源码文件位置175行，查看对应memcpy.S源文件代码： L(loop64):
...
2. 根据寄存器值，结合上下文确定哪个对象导致了问题。 非类成员函数x0寄存器加载的是函数第1个参数，x1加载的是第2个参数，x2加载的第3个参数，依次类推；类成员函数，x0加载的是类实例对象的指针，其后x1、x2、x3为参数，注意函数参数超过5个会压入栈中。栈顶函数void* memcpy(void* restrict dest, void* restrict src, size_t n)参数，x0是dest（目的地址）, x1是src（源地址），x2是n（拷贝字节数）。 Register:
...
3. 结合崩溃地址附近的内存特征确定问题类型。 通过CppCrash日志中Memory near registers查看寄存器附近内存地址值： x1(xxxx):
...
4. 跟踪出问题对象的参数来源，结合代码与流水日志确定问题。 排查参数对象的有效性、范围是否合法，例如源内存的实际大小是否与传入的n一致。 排查参数对象的生命周期是否合法，例如源内存是否已被释放，是否存在多线程操作对象。 根据函数的上下文，排查参数的不合理操作逻辑，例如跟踪buf和bufsize的操作逻辑，增加调试信息，锁定不合理操作逻辑，上下文片段如下。 static int xxxFunc(const uint8_t *buf, uint32_t bufSize)
{
 // ...
...
1. 根据地址定位到行号发现崩溃在autia汇编指令。char g_ori[12] = "hello!";
...
2. 通过反汇编查看汇编指令，汇编指令片段如下:0000000000001954 &lt;main&gt;:
...
19cc: d65f03c0 ret 0x19a8地址对应的指令是autia x8, x9。autia是armv8.3芯片上的一个指针校验指令，第一个参数x8为函数指针，第二参数x9为用于指针校验的key值，key在编译时期自动生成。从崩溃日志中发现x8寄存器中的地址0x5a00000000是异常的，所以导致autia指令执行失败。向上查找汇编指令寻找x8寄存器的值怎么来的，发现0x19a0地址处ldr x8, [sp, #24]，x8寄存器值是从sp+24地址处加载得到，再往上查找发现0x197c地址处指令str x8, [sp, #24]，将x8的寄存器的值存入内存地址sp+24。排查0x197c到0x19a0地址之间是否有写栈内存的操作，排查发现memcpy函数第一个参数x0为sp+16，第二个参数x1为9056，第三个参数x2为12，即往a数组首地址sp+16拷贝12个字节内存。但a数组缓冲区大小只有8个字节，所以在拷贝时写入内存超过缓冲区边界，造成sp+24地址处内存值被非法改写，该地址处原本存的是函数指针。
