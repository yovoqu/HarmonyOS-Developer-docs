...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-0.png)


资源泄漏日志获取方法与日志规格详见Resource Leak（资源泄漏）检测。
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-1.png)


开发态-JS泄漏分析方法（适用于开发过程中特定场景下调优）

使用DevEco Studio Snapshot能力可抓取两次heapdump，请参考使用Snapshot模板基本操作，通过比较两次heapdump的对象delta进行分析：
...
2. 找到和业务相关的对象；
3. 展开业务代码根节点，通过Distance观察引用路径，值越小表示离GC Root的距离越近，展开节点依次找到最小的值，看一下这些业务对象个数是否符合当前这个页面的预期，是否存在循环引用或者闭包问题导致泄漏。 找到应用自己的包名的对象。
4. 查看reference分组的每个对象。
5. 展开这个对象，找到对象的成员，看每个成员的distance，找到其中最小的。
6. 重复b和c步骤，直到找到distance为1的为止，这个就是GC Root根节点。


![](assets/资源泄漏类问题分析方法/file-20260515115118229-2.png)
具体操作方法可见JS内存泄漏问题检测方法。
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-3.png)
除以上方法外，JS泄漏分析方法还可参考分析ArkTS/JS内存。

![](assets/资源泄漏类问题分析方法/file-20260515115118229-4.png)
...
对于开发过程中遇到的泄漏问题，开发者可以参考分析native内存抓取维测并分析占用最大的内存块及其调用栈排查泄漏点。
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-5.png)
2. 分析smaps日志
...
对于堆内存泄漏，开发者可以基于[NMD](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/performance-analysis-kit-terminology#nmd)（堆内存快照，判定泄漏时，堆内存布局的快照）和profiler分析： “LOGGER_MEMCHECK_SAMPLE_NMD_INFO”与“LOGGER_MEMCHECK_DETIAL_INFO”中记录抓取进程native日志时内存的快照信息，主要关注其size和allocated两列，是否有比较突出的内存占用，日志格式如下。
![](assets/资源泄漏类问题分析方法/file-20260515115118229-6.png)
如果有，假设size为a（如128），就去调用栈中查找size为a的内存申请，重点分析这行调用栈，极可能是泄漏点。 如果单次抓取的NMD信息无法确定具体的size，那么可以通过比较两次LOGGER_MEMCHECK_SAMPLE_NMD_INFO来判断5分钟内增长最大的size。
...
3. 分析profiler日志 除了通过smaps日志对内存占用进行初步分析，开发者还可以通过对profiler进行解析来对泄漏问题作进一步分析，日志解析主要有以下两种方法
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-7.png)
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-8.png)
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-9.png)
...
> [!NOTE]
> [@ohos.multimedia.image (图片处理).setMemoryNameSync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap#setmemorynamesync13) 和 [OH_PixelmapNative_SetMemoryName()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-pixelmap-native-h#oh_pixelmapnative_setmemoryname)。
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-10.png)
...
```
4. 目前因为采用了统一渲染机制，大部分ION内存都是在Render_service进程分配使用的，如果发现应用使用ION超标了，那么按照历史经验，高概率怀疑是PixelMap C++对象泄漏。 开发者首先排查是否存在如下可能： **step1：**开发者的应用使用了Node-API在native层创建并使用decode()解码获取PixelMap，但是可能存在申请未释放、把PixelMap缓存到应用生命周期的容器类中、引用计数多加等可能。 **step2：**如果应用根本就没有使用Node-API实现C++代码，那么排查是否使用JS层的PixelMap，可能存在JS对象泄漏或者缓存太多导致PixelMap大量占用，可使用IDE抓两次snapshot看一下对象的增量分析，操作方法见下图。
![](assets/资源泄漏类问题分析方法/file-20260515115118229-11.png)
**step3: ** **【推荐】**pixmap使用的ION内存，应用自定义绑定pixmap名字，当出现ION泄漏，快速根据ION的buffer名锁定哪张图片存在问题，反推至对应的问题组件。 如果上述两种情况都不存在，那么很有可能ArkUI组件内部实现存在PixelMap泄漏。
5. 从HarmonyOS 6.0.0开始，如果应用未自行打标签，系统支持自动为ion内存打tag标签信息，可通过查看维测日志[memleak-kernel-[module]-0-[timestamp].txt]中进程的ion内存信息中的buf_name、leak_type等列定界可疑泄漏组件，组件分类如下：
...
4. 分析profiler日志 开发者可以将获取到的profiler文件（[内存栈](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/resource-leak-guidelines#section18411753155019)）导入DevEco Studio Profiler插件中进行分析，导入后会在界面展示进程的内存分配情况及其调用栈。按照如下步骤将解析结果展开，按照前置分析框选怀疑泄漏的泳道，选择Created & Existing，按照内存申请大小来排查可疑的泄漏点，并通过调用栈进一步确认泄漏位置。
![](assets/资源泄漏类问题分析方法/file-20260515115118229-12.png)
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-13.png)
...
1	/proc/
```
3. 根据不同泄漏句柄类型，按照对应的分析方法**初步分析**，日志格式可参考[句柄泄漏日志规格](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/resource-leak-guidelines#section1120134982114)。 **文件句柄泄漏** 文件句柄泄漏，主要原因是：应用TS调用API间接打开了某个文件，或者C++代码直接调用open()接口，从日志内可以看到完整的文件的entry信息，基于文件路径和文件名可以反推对应的使用场景，在哪里调用的，建议关注对应的对象或者fd的生命周期，及时释放防止并发场景下fd过载导致出现功能甚至稳定性异常。 如下日志，可以看到/data/storage/el1/bundle/entry.hap这个文件的fd有5663，且是一个hap，找到对应open的位置，发现是应用内不断open 这个文件，句柄未及时释放。
```text
Leaked fd Top 10:
...
2	/dev/binder
```
**[ashmem类型句柄](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/resource-leak-guidelines#section328913422256)或****[dmabuf类型句柄](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/resource-leak-guidelines#section328913422256)** 如果泄漏日志中top句柄报的是ashmem、dmabuf（ION），可按照ashmem、ION泄漏问题进行分析：
```text
*****************************
Leaked fd Top 10:
5000	dmabuf
...
```
分析方法可参考[ashmem泄漏](#section2825227501)和[ION泄漏](#section5493141412410)，判断是这两种句柄类型泄漏后同样会打印下述分析方法中的维测信息。 **[socket类型句柄](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/resource-leak-guidelines#section328913422256)** 由于打印的维测信息是整机的，因此首先在日志中根据进程pid或者进程名搜索，查看本应用所占用的socket句柄。
4. 查看inode和对端进程，两方进程通信持有的inode相同，PeerTid一般就是对端进程的PID。
5. 关注应用代码中是否使用socket()等接口创建socket句柄，排查其生命周期是否存在泄漏。
...
![](assets/资源泄漏类问题分析方法/file-20260515115118229-17.png)
