...
4. 目前因为采用了统一渲染机制，大部分DMA(ION)内存都是在Render_service进程分配使用的，如果发现应用使用DMA(ION)超标了，那么按照历史经验，高概率怀疑是PixelMap C++对象泄漏。开发者首先排查是否存在如下可能：
...
5. 从HarmonyOS 6.0.0开始，如果应用未自行打标签，系统支持自动为DMA(ION)内存打tag标签信息，可通过查看维测日志[memleak-kernel-[module]-0-[timestamp].txt]中进程的DMA(ION)内存信息中的buf_name、leak_type等列定界可疑泄漏组件，组件分类如下：
...
4. 分析profiler日志开发者可以将获取到的profiler文件（[内存栈](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/resource-leak-guidelines#内存栈-1)）导入DevEco Studio Profiler插件中进行分析，导入后会在界面展示进程的内存分配情况及其调用栈。按照如下步骤将解析结果展开，按照前置分析框选怀疑泄漏的泳道，选择Created & Existing，按照内存申请大小来排查可疑的泄漏点，并通过调用栈进一步确认泄漏位置。
...
TOP10泄漏线程可初步定位泄漏时主要占用的线程名和对应的个数。
