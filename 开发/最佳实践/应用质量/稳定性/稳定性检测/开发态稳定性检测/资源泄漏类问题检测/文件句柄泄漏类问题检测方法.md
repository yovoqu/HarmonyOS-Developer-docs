...
![](assets/文件句柄泄漏类问题检测方法/file-20260525085706148-001.png)
...
1. 修改应用代码，做一个测试按钮，点击这个按钮枚举下进程的所有句柄。import { fileIo, ListFileOptions } from '@kit.CoreFileKit';
...
2. 尝试复现问题，然后点击一下上述代码对应的测试按钮，分析日志输出，看多了哪些句柄。如果句柄增加不明显，可以反复重复本步骤。 这个演示代码只能打印出应用沙箱目录的文件路径，其他系统路径的文件因为权限问题可能会打印不出来，所以只能定位沙箱目录下文件句柄的泄漏问题。