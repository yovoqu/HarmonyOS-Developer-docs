...
1. Linux编译环境搭建（如果已有对应版本的Linux开发环境，可跳过Linux环境搭建过程）：详细指导见以下链接。[使用 WSL 在 Windows 上安装 Linux](https://learn.microsoft.com/zh-cn/windows/wsl/install)。 [Ubuntu分发版本获取及安装说明](https://learn.microsoft.com/zh-cn/windows/wsl/install-manual)。 编译环境目前主要支持Ubuntu18.04和Ubuntu20.04。
2. HarmonyOS SDK镜像下载：从HarmonyOS官网门户选择Linux版本的Command Line Tools下载即可。 [下载链接](https://developer.huawei.com/consumer/cn/download/)。
3. 安装构建工具depot_tools并添加到环境变量。任意位置创建工作目录depot_tools，cd到自己创建的目录，拉取工具（需要网络环境）：
```text
mkdir depot_tools
cd depot_tools
...
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-0.png)
...
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-1.png)
 判断python是否安装成功： 输入python显示python版本即可。
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-2.png)


## GN构建工程适配流程


![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-3.png)


1. 新增HarmonyOS平台的宏定义。
2. 配置HarmonyOS平台的工具链核心信息，涵盖clang工具链路径，sysroot系统根目录以及clang版本等关键参数。
3. 在toolchain目录下，为各架构分别配置对应的ohos_clang_toolchain。
4. 扩充gcc_toolchain模版功能，补充HarmonyOS启动引导程序所需的.o文件相关配置。
...
1. **添加HarmonyOS平台宏定义**这里主要在build/config/BUILDCONFIG.gn文件中适配HarmonyOS的default_compiler_configs和_default_toolchain。在GN工程里面，BUILDCONFIG.gn是第一位被解析的，里面定义的变量相当于全局变量，可以被后续所有的.gn文件使用。编译过程中可能会配置一些编译选项以及一些头文件搜索路径。default_compiler_configs指向的文件里面会包括一些默认的编译选项以及头文件搜索路径等等。_default_toolchain指向了一个工具链相关的函数。具体修改点如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-4.png)
2. **设置HarmonyOS平台clang工具链相关路径**不同平台的工具链会有一些差别，所以需要使用HarmonyOS的工具链。这里主要修改config/clang/clang.gni文件。.gni文件类似于GN的头文件，会被import到各个.gn文件中使用其定义的一些变量。该文件中的核心修改点在于配置指向HarmonyOS SDK的工具链路径。另外还需修改clang_use_chrome_plugins的值为false，HarmonyOS中默认clang_use_chrome_plugins值为false，不设置可能会报错find-bad-constructs文件找不到。 此处ohos_sdk_native_root的值需要对应修改为自己本地HarmonyOS SDK中的native的路径。具体修改点如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-5.png)
3. **设置HarmonyOS平台sysroot路径**这里主要修改build/config/sysroot.gni文件，sysroot里面包含了许多头文件搜索路径，配置了sysroot之后，编译过程中会去该目录下搜索需要的头文件。SDK里面会提供大量的头文件，这些头文件都会放在sysroot目录下，所以需要引入HarmonyOS对应的sysroot。具体修改点如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-6.png)
4. **修改HarmonyOS平台clang版本**这里主要修改build/toolchain/toolchain.gni文件，在该文件中配置HarmonyOS对应的clang版本号。具体修改点如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-7.png)
5. **设置各个架构的ohos_clang_toolchain**这里主要是在build/toolchain路径下新建一个ohos/BUILD.gn文件，用于配置ohos_clang_toolchain，里面主要配置了HarmonyOS用于启动引导程序的.o文件。同时设置HarmonyOS不同架构(主要包括ohos_clang_arm、ohos_clang_arm64、ohos_clang_x86_64)的ohos_clang_toolchain配置信息。具体添加内容如下：
```text
...
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-9.png)

![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-10.png)
...
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-11.png)
7. **设置HarmonyOS的一些编译参数，将其加入到BUILDCONFIG.gn中**这里需要在build/config路径下新建一个ohos/BUILD.gn文件，该文件主要是定义了一个config("compiler")，该config会被注册到所有的编译目标，该config里面主要设置了基础的编译选项、宏定义等。 此处ohos_clang_base_path  的值需要对应修改为自己本地HarmonyOS SDK中的llvm的路径。具体添加内容如下：
```text
import("//build/config/sysroot.gni")
assert(is_ohos)
...
```
8. **build/config/compiler/BUILD.gn中增加对is_ohos的判断**保证可以正确走HarmonyOS支持的编译分支。这里主要是为了防止clang版本号校验失败导致异常。具体修改如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-12.png)
9. **未适配HarmonyOS的三方库走linux编译配置**当前部分三方库还未适配HarmonyOS，涉及到时可以先走linux的编译配置，例如：需要获取config.h文件时。 修改modules/video_capture的BUILD.gn。具体修改如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-13.png)
 修改third_party/zlib的BUILD.gn。具体修改如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-14.png)
 修改third_party/libevent中的BUILD.gn。HarmonyOS SDK中没有queue.h头文件，需要使用compat dir目录下的queue.h头文件。具体修改如下：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-15.png)
10. **编译**先通过GN命令生成对应的ninja文件，然后使用ninja编译命令进行编译。
```text
...
1. **Assertion failed. Unsupported ARM OS****问题详情：**
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-16.png)
 **问题原因/解决措施：** 三方库内部没有做对is_ohos的判断，导致走到错误分支。当前很多业务模块还未适配HarmonyOS，暂时可以走linux分支以保证正常编译。 **具体修改：** 修改third_party/zlib的BUILD.gn文件。
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-17.png)
2. **python找不到pkg-config文件：No such file or directory: 'pkg-config'****问题详情：**
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-18.png)
 **问题原因/解决措施：** 缺少pkg-config插件，安装该插件。 **具体指令：**
```text
sudo apt-get install pkg-config
```
3. **Unknown command line argument '-split-threshold-for-reg-with-hint=0'****问题详情：**
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-19.png)
 **问题原因/解决措施：** 编译过程中会提示部分配置不识别，需要将这些配置项删除。 **具体修改：** 在build/config/compiler/BUILD.gn中删除以下配置。
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-20.png)
4. **WARN类型导致的ERROR****问题详情：**
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-21.png)
...
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-22.png)
 **具体修改：**
- 添加编译指令配置项treat_warnings_as_errors （建议使用）
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-23.png)
- 修改源代码，在build/config/compiler/BUILD.gn中删除以下配置。
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-24.png)
5. **error: reinterpret_cast from 'pthread_t' (aka 'unsigned long') to 'rtc::PlatformThreadId' (aka 'int') is not allowed****问题详情：**
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-25.png)
 **问题原因/解决措施：** rtc_base/platform_thread_types.cc未识别到is_ohos导致内部走错分支导致异常。目前HarmonyOS支持的接口是gettid()，rtc_base/platform_thread_types.cc需要识别到is_ohos然后调用gettid()。由于当前很多业务模块还未进行识别，暂时需要走linux分支，故需要保留linux的定义。 **具体修改：**
- 首先需要在根目录的BUILD.gn中配置识别HarmonyOS系统的变量is_ohos：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-26.png)
- 修改rtc_base/platform_thread_types.cc业务代码：
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-27.png)
6. **fatal error: 'config.h' file not found****fatal error: 'sys/queue.h' file not found** **问题详情：**
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-28.png)

![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-29.png)
...
![](assets/GN构建工程配置HarmonyOS编译工具链/file-20260515115135309-30.png)
