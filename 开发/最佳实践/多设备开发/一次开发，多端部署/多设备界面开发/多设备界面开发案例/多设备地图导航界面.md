...
> [!NOTE]
> 阅读本章节前，读者需熟悉 Map Kit简介 ，并参考 应用开发准备 及 开发准备 开通相关服务。下文将详细介绍地图导航的“一多”开发实践。
...
| 2 | 面板 | 使用Stack组件，在地图上层嵌套Column组件实现面板，并借助栅格监听断点变化，设置在不同断点下面板的不同形态。在sm断点下面板底部展示，在md断点下悬浮展示。并通过绑定PanGesture实现在宽屏设备上的居左或居右变换。 |

| 3 | 拖动区域 | 通过绑定PanGesture实现面板高度变换，当前支持三个档位变换调节。 |

| 4 | 搜索框 | 使用TextInput组件实现搜索框效果，通过onSubmit()事件实现搜索功能。 |
...
| 1 | 搜索框 | 搜索内容后，右侧按钮变为取消。 |

| 2 | 搜索结果列表 | 面板处于中档位时采用Swiper实现延伸能力，高档位时采用List实现延伸能力。 |
...
| 1 | 输入区域 | 通过判断当前面板高度更换按钮及输入区域布局，Row组件配合layoutWeight实现拉伸能力。 |

| 2 | 方案页签 | 使用Tabs组件实现延伸能力。 |
...
> [!NOTE]
> 阅读本章节前，读者需熟悉 Form Kit（卡片开发服务） 以及 ArkTS卡片开发（推荐） 。下面将详细介绍ArkTS卡片在“一多”开发中的实践。
...
| 1 | 静态卡片 | 在多端均采用2*4的八宫格静态卡片。可参考创建ArkTS卡片在入口模块创建一个静态卡片，并配置ArkTS卡片的配置文件。 |
...
> [!NOTE]
> 阅读本章节前，读者需熟悉 Live View Kit简介 及 实况窗支持对接的场景 ，并根据读者的开发场景进行 开发准备 。下面将详细介绍实况窗在“一多”开发中的实践。
