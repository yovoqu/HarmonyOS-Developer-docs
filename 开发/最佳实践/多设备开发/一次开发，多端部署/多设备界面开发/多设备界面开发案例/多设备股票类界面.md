...
- [UX设计](#section18423159193410)：介绍股票类应用的交互逻辑和通用设计要点，对于类似的设计要点，开发者可以直接参考。
- [工程管理](#section251720249266)：推荐“一多”项目采用分层架构，明确各层逻辑。同时，介绍股票类应用适用的三层架构配置。
- [移动端页面](#section824510452191)、[电脑端页面](#section2092619161616)：遵循实际应用开发流程，以页面为基本单元，分别讲解移动端、电脑端页面在窗口适配、页面开发和功能开发的设计思路与实现方法。
...
股票类应用的UX设计可参考金融理财类多设备响应式设计指南的[自选股](https://developer.huawei.com/consumer/cn/doc/design-guides/responsive-design-examples6-0000001793536905#section0732192812391)章节，设计参考图如下所示。
...
在多股比价场景中，当在应用内进行分屏操作时，新增窗口应保留当前浏览的股票信息，而主窗口则应回到股票列表。为实现这一功能，建议在新窗口的启动生命周期中触发事件，原窗口通过监听该事件并执行退栈操作。1. 在分屏程序的入口SplitScreenAbility.ets中的onCreate()和onNewWant()生命周期中进行事件触发。
```ArkTS
...
export struct TopTitleBar {
  ...
  private callBack: Callback = (eventData: emitter.EventData) => {
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8b/v3/t926ei6iTj-vwSTf4yCojw/zh-cn_image_0000002610071557.png?HW-CC-KV=V1&HW-CC-Date=20260624T020954Z&HW-CC-Expire=86400&HW-CC-Sign=3AF78CEF9B4FDE89C984E689DBEAB5B05C7D80C463AEFB0E02874AD315387E16)
...
| 2 | 指数 | 同移动端自选股页面对应区域的布局实现方案一致。 |
| 3 | 股票列表-工具栏 | 同移动端自选股页面对应区域的布局实现方案一致。 |
| 4 | 股票列表 | 同移动端自选股页面对应区域的布局实现方案一致。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b7/v3/zu-nfBb3QOG840XdR9INfQ/zh-cn_image_0000002579631780.png?HW-CC-KV=V1&HW-CC-Date=20260624T020954Z&HW-CC-Expire=86400&HW-CC-Sign=A7981311182A055A23C772BA0C4C729E4873825DCDE69BEC1765E450B02FF7BC)
...
| 2 | 标题 | 同移动端股票详情页对应区域的布局实现方案一致。 |
| 3 | 行情列表数据 | 同移动端股票详情页对应区域的布局实现方案一致。 |
| 4 | 中间Tab | 同移动端股票详情页对应区域的布局实现方案一致。 |
| 5 | 曲线图和柱状图 | 同移动端股票详情页对应区域的布局实现方案一致。 |
