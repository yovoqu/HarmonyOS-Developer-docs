...
FolderStack是系统提供的ArkTS组件，继承自层叠布局Stack。在Stack组件的基础上，FolderStack提供监控设备是否进入悬停态并进行重新布局的能力。
...
![](assets/折叠屏悬停态/file-20260515114840021-2.png)
...
try {
  ...
  hilog.error(
    0x0000,
    TAG,
    'Failed to register onFoldStatusChange callback. Code: ' +
...
```
 视频控制组件位于下半屏，无需调整。 顶部返回组件应移动到屏幕下半部分的顶部。
```ts
Row() {
  // ...
}
