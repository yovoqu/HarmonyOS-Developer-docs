...
![](assets/折叠屏悬停态/file-20260525090121048-001.png)
...
![](assets/折叠屏悬停态/file-20260525090121054-003.png)
...
![](assets/折叠屏悬停态/file-20260525090121054-004.png)
...
![](assets/折叠屏悬停态/file-20260525090121055-005.png)
...
1. 悬停态通过状态变量isHover进行监听。当折叠屏的折叠状态变化时，判断当前是否为悬停态并更新isHover的值。定义监听折叠状态变化回调方法。 private onFoldStatusChange: Callback&lt;display.FoldStatus&gt; = (data: display.FoldStatus) => {
  try {
 // ...
...
2. 当设备处于悬停状态（isHover为true）时，页面内组件需要获取折痕区的大小和位置，方法如下。static getFoldCreaseRegion(): void {
  try {
...
3. 根据折痕区的大小和位置调整布局。视频播放组件将上移至屏幕上方。 Column() {
  XComponent({
...
.height(this.isHover ? this.creaseRegion[0] : '100%') 视频控制组件位于下半屏，无需调整。 顶部返回组件应移动到屏幕下半部分的顶部。 Row() {
  // ...
}
.width('80%')
.height('24vp')
...
![](assets/折叠屏悬停态/file-20260525090121056-006.png)
