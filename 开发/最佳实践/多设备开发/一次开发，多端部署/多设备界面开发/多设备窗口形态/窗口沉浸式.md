...
2. 组件级：作用于当前组件，可针对单个页面内的多个组件分别配置。

3. 避让处理：避免页面内容与避让区的系统信息（如电量、时间）、交互功能（如导航条手势）或窗口控制键（如关闭、最小化）发生遮挡和冲突。
...
> [!NOTE]
> backgroundColor() ：背景颜色限定在安全区内。 background() ：背景颜色扩展至顶部状态栏及底部导航条区域。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/69/v3/wYc5cwJTTfmpbELCB9xdbg/zh-cn_image_0000002531334143.jpg?HW-CC-KV=V1&HW-CC-Date=20260624T020954Z&HW-CC-Expire=86400&HW-CC-Sign=FEA03C28D510CDE58AEC5A114C0953D29DEA132FDEAEEFDD1EE9F2D8D1BBA87A)
...
3. 通过[Window.getWindowAvoidArea()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#getwindowavoidarea9)方法获取状态栏和导航条高度，并用状态变量avoidSystem记录。
```ArkTS
// Get the avoid area.
...
4. 使用[on('avoidAreaChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#onavoidareachange9)事件监听避让区域的变化，在监听回调中更新状态变量avoidSystem。
```ArkTS
...
5. 设置组件的上下padding分别为状态栏和导航条的高度，使内容避让状态栏和导航条，仅在安全区域显示。
```ArkTS
Column() {
  // ...
}
...
5. 挖孔区在左侧，则左侧避让 = 挖孔区距屏幕左侧边缘的距离 + 挖孔区域的宽度。
6. 挖孔区在右侧，则右侧避让 = 屏幕宽度 - 挖孔区距屏幕左侧边缘的距离。
7. 挖孔区在底部，则底部避让 = 屏幕高度 -  挖孔区距屏幕顶部距离。
8. 设置组件padding，进行避让。
```ArkTS
Column() {
  // ...
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5c/v3/UIsWi7z_Q_GvVEi3TPRDdQ/zh-cn_image_0000002531334153.jpg?HW-CC-KV=V1&HW-CC-Date=20260624T020954Z&HW-CC-Expire=86400&HW-CC-Sign=623AB32F88FAFBD4510FA79A42297F38F922FF341BA47305D544FD47CE43436E)
