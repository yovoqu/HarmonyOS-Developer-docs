...
> [!NOTE]
> 鼠标左键的事件响应，推荐使用 onClick 点击事件，其他按键通过 鼠标事件 处理。
...
> [!NOTE]
> onKeyEvent事件默认是冒泡的，在onKeyEvent事件的回调函数中，若事件已被处理，建议开发者返回true，表示已消费该事件。这可以阻止事件继续冒泡，避免上层节点重复响应，从而防止按键事件被触发多次。 灵犀手写笔不响应 onHover 事件，上表数据仅适用于其他型号的手写笔设备。
...
4. 键盘：键盘按下空格键。空格键的键码为KEYCODE_SPACE，其他按键的键码可通过[keyCode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-keycode#keycode)枚举查询，例如回车键对应KEYCODE_ENTER。应用可根据实际交互需求，灵活适配不同按键。
```ArkTS
.onKeyEvent((event?: KeyEvent) => {
