...
![](assets/响应式布局/file-20260525090121206-001.png)
**图2 **响应式布局示意图2

![](assets/响应式布局/file-20260525090121206-002.png)
...
**通过主动监听断点变化刷新UI**
1. 使用自定义窗口信息类WindowInfo保存窗口断点信息。@Observed
...
2. WindowUtil类中创建窗口信息类对象，使用[getWindowWidthBreakpoint()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-uicontext#getwindowwidthbreakpoint13)与[getWindowHeightBreakpoint()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-uicontext#getwindowheightbreakpoint13)获取当前窗口横向断点与纵向断点。通过[on('windowSizeChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#onwindowsizechange7)开启窗口尺寸变化的监听，并在监听回调中重新获取断点。export class WindowUtil {
  // ...
...
3. 在EntryAbility的onWindowStageCreate()生命周期中，获取应用窗口实例，对WindowUtil进行初始化操作，并保存至AppStorage中。在loadContent加载完页面后，完成断点监听注册、断点更新等操作。完成上述步骤后，当窗口尺寸发生变化时，更新横纵断点。onWindowStageCreate(windowStage: window.WindowStage): void {
  // ...
  try {
...
4. 使用@StorageLink装饰器获取窗口管理类WindowUtil对象，子组件初始化时传递窗口信息类对象，使子组件可以获取当前窗口的断点信息。@Component
...
5. 使用@ObjectLink装饰器接收窗口信息类对象，根据获取的窗口信息，对不同断点区间进行针对性布局。如下示例代码完成了在不同断点下，Grid组件显示不同数量列的功能。@Component
...
![](assets/响应式布局/file-20260525090121209-004.png)
**类方屏旋转方案**
...
> [!NOTE] 说明
> 应用设置窗口旋转方案需要结合多种条件一起判断，详情可参考为多设备配置旋转策略。Pura X外屏默认不支持窗口旋转。
...
2. 使用@Watch装饰器监听状态变量isFullScreen的变化，以判断视频是否全屏播放，并在显示或隐藏时同步修改窗口方向。全屏播放时，未使用断点的窗口设置逻辑如下：需要将窗口设置为AUTO_ROTATION_LANDSCAPE属性的情况包括手机、双折叠屏（X 系列）的折叠态与半折态。图4 手机效果图 图5 双折叠屏（X系列）半折态 反例： if (this.isFullScreen) {
...
} 在进入全屏页时，通过isFoldable、deviceType和getFoldStatus三个值共同判断，这种方式可读性和维护性较差。随着鸿蒙生态的拓展，不同设备上可能会出现各种异常情况。 正例： 双折叠屏（X系列）展开态与平板，支持旋转。双折叠屏（X系列）的横向断点为md，纵向断点为md；平板横向持握时横向断点为lg，竖向持握时横向断点为md，纵向断点为lg。将窗口显示方向设置为AUTO_ROTATION_RESTRICTED。手机与双折叠屏（X系列）折叠态竖屏时的横向断点为sm，纵向断点为lg，全屏播放时支持横向旋转。将窗口显示方向设置为AUTO_ROTATION_LANDSCAPE_RESTRICTED。手机与双折叠屏（X系列）折叠态横屏时的横向断点为md，纵向断点为sm，此时如果退出全屏播放，则竖屏展示布局。将窗口显示方向设置为PORTRAIT。 本案例未覆盖部分手机（如Pocket 2、Pura 70Pro）横屏时横向断点落入lg的情况。 // entry/src/main/ets/page/VideoExample.ets
...
1. 对通过媒体查询监听[断点](#section1532120147301)的功能做简单的封装，方便后续使用。import { mediaquery } from '@kit.ArkUI';
...
2. 在页面中，通过媒体查询，监听应用窗口宽度变化，获取当前应用所处的断点值。import { BreakpointSystem, BreakpointState } from './Breakpoint';
...
![](assets/响应式布局/file-20260525090121210-006.png)
...
> [!NOTE] 说明
> ArkUI在API version 9对栅格组件做了重构，推出了新的栅格组件GridRow和GridCol，同时原有的GridContainer组件及栅格设置已经废弃。本文中提到的栅格组件，如无特别说明，都是指GridRow和GridCol组件。
...
![](assets/响应式布局/file-20260525090121211-007.gif)
...
![](assets/响应式布局/file-20260525090121212-008.png)
...
![](assets/响应式布局/file-20260525090121214-009.png)
