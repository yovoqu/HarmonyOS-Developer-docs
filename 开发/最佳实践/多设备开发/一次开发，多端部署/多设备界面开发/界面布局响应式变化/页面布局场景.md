...
HarmonyOS基于“一次开发，多端部署”的理念设计了响应式布局，旨在帮助开发者高效构建适应不同设备的应用界面。系统通过统一的UI框架、响应式布局能力——[断点](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-device-responsive-layout#section1532120147301)和[栅格](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-device-responsive-layout#section1061332817545)，让应用页面能够根据代码的差异化实现自动适应从手机、折叠屏、平板到电脑等各种终端形态。
...
设置不同横向断点下，[List](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-list)组件的lanes、space属性实现目标效果。
...
设置不同横向断点下，[WaterFlow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-waterflow)组件的columnsTemplate属性实现目标效果。
...
设置不同横向断点下，[Swiper](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-swiper)组件的displayCount、prevMargin、nextMargin和indicator属性实现目标效果。
...
> [!NOTE]
> 网格布局将容器按行和列划分为规则的网格，每个子组件严格对齐，实现均分和占比能力，详情请参考 创建网格 (Grid/GridItem) ；瀑布流布局子组件高度支持自定义，无需对齐，适合展示高度不一的内容，详情请参考 创建瀑布流（WaterFlow） 。

 
**实现方案**
 
设置不同横向断点下，[Grid](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-grid)组件的columnsTemplate属性实现目标效果。在不设置Grid组件行数的情况下，行数 = 展示元素数量 / 列数。Grid组件其他布局模式，请参考[rowsTemplate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-grid#rowstemplate)。
...
在不同横向断点下，动态控制[SideBarContainer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-sidebarcontainer)组件的showSideBar和sideBarWidth属性实现目标效果。
...
设置不同横向断点下，[Navigation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation)组件的mode属性实现目标效果。
...
使用[Navigation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation)组件，动态控制其[NavigationMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation#mode9)，在路由时切换单双栏显示。聊天列表作为Navigation导航栏，通过单击不同列表条目实现右侧Navigation内容区的路由控制，这种分栏路由模式为NavigationMode.Split。单击商品链接时，设置全屏变量isNavFullScreen为true，切换Navigation为单栏展示，此时路由模式为NavigationMode.Stack。路由返回时，isNavFullScreen变量改为false，路由重新切换为NavigationMode.Split。
 
对于聊天页面的双栏路由模式切换，开发者可以抽象为：
 

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/23/v3/HaZXarGmRZ2vioWxbA-VwQ/zh-cn_image_0000002509292755.png?HW-CC-KV=V1&HW-CC-Date=20260813T095912Z&HW-CC-Expire=86400&HW-CC-Sign=90F83A82D0801537E044049C9466B518A6F668A9A587D6B758560DE0F71D6ADD)
...
使用[Navigation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation)组件动态控制[NavigationMode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation#mode9)，根据设备宽度变化切换单栏或双栏展示。当设备宽度≥840vp、高宽比≥0.5且<1.2、isShowComment为true时，采用双栏展示，左侧为视频界面，右侧为评论区内容，路由模式为NavigationMode.Split。单击关闭评论区按钮时，设置isShowComment为false并返回上一路由，路由模式切换为NavigationMode.Stack，全屏显示视频。再次单击评论区按钮，设置isShowComment为true并加载评论区路由，路由模式变为NavigationMode.Split。当设备宽度或高宽比不满足边看边评显示条件时，路由模式切换为NavigationMode.Stack，导航栏上方为视频区，下方为评论区，此时评论区无法打开或关闭。
 
对于边看边评的多分栏模式变化，开发者可参考下图：
 

![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/7/v3/f05s-ei6RdiHkZ-hnCJO2w/zh-cn_image_0000002669546586.png?HW-CC-KV=V1&HW-CC-Date=20260813T095912Z&HW-CC-Expire=86400&HW-CC-Sign=EBA2E6426E3B4EE799B29257B735ED75B00CFF7DC14FE0CC182AEF123367481B)
...
在不同横向断点下，动态控制[SideBarContainer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-sidebarcontainer)组件的showSideBar、sideBarWidth属性，和[Navigation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation)组件的mode、navBarWidth属性实现目标效果。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b1/v3/KFjelww7QuGpU-lt0ELUbg/zh-cn_image_0000002509132789.png?HW-CC-KV=V1&HW-CC-Date=20260813T095912Z&HW-CC-Expire=86400&HW-CC-Sign=9F8B3DF5A9BFE1F500CD7170B1D050E2BC84B8CD2716F85E34EF7933E5F71CAF)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b2/v3/h7Tx5u_QSGa-X9noDcE4TQ/zh-cn_image_0000002509292759.png?HW-CC-KV=V1&HW-CC-Date=20260813T095912Z&HW-CC-Expire=86400&HW-CC-Sign=31D041D1F340F1D4177713CB91A403A6E6C9AE26F981B9B95E8C19FE643DF103)
...
设置不同横向断点下，[GridRow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridrow)组件的columns、breakpoints属性，和[GridCol](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridcol)组件的span属性实现目标效果。
...
设置不同横向断点下，[Tabs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-tabs)组件的barPosition、vertical、barHeight、barWidth和barMode属性实现目标效果。
...
在xl断点下或使用电脑设备，首页调用SideBarContainer组件，传入侧边栏区域组件和内容区域组件。此时可以结合[@State](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state)和[@Link](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-link)装饰器，同步所需要的参数信息，如当前选中的一级目录索引和二级目录索引，可以用于呈现与页签强相关的页面内容。
...
设置不同横向断点下，[GridRow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridrow)组件的columns、breakpoints属性和[GridCol](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridcol)组件的span、offset属性实现目标效果。
