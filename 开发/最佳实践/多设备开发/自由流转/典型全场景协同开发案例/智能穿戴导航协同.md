...
本章节从UX设计和UI交互两个维度，详细阐述智能穿戴设备协同导航的体验设计方案。
...
![](assets/智能穿戴导航协同/file-20260515114919017-0.jpg)
...
本章节将详细解析智能穿戴设备协同导航的核心功能开发，重点阐述地图操作、互联通信、消息通知和振感提示四大核心功能的实现方案，为开发者提供明确的技术指导。


### 地图操作


地图操作主要涉及地图展示以及地图绘制两个功能，地图展示能力开发者可以参考显示地图。

智能穿戴设备协同导航的地图绘制功能通过MapComponentController实现，具体包含以下核心操作：

地图点击（目的地标记）：

图1 地图点击操作时序图

![](assets/智能穿戴导航协同/file-20260515114919017-1.jpg)
...
```
2. 监听destination_Position变化，调用[addMarker()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/map-map-mapcomponentcontroller#section0810361284)动态更新目的地地图标记。
```ts
// Destination location information obtained based on the map click event.
@StorageLink('destination_Position') @Watch('changeDestinationPositionMark') destination_Position: mapCommon.LatLng =
{
...
Button($r('app.string.DrawRoute'))
  ...
  .onClick(async () => {
    ...
    drawRouteUtil.drawRoute(
      this.mapController,
      result.routes[0].steps[0].roads,
    );
  });
```
