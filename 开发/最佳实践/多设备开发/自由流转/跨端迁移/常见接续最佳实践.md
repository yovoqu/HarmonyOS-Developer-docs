...
![](assets/常见接续最佳实践/file-20260525090230334-001.png)
...
1. 启用接续 在module.json5文件的abilities中，将continuable标签配置为“true”，表示该UIAbility可以被迁移。配置为“false”的UIAbility将被系统识别为不可迁移，且该配置的默认值为“false”。 {
  "module": {
 // ...
 "abilities": [
 {
...
2. 源端保存迁移数据 当对端点击接续图标时，源端将触发UIAbility中的onContinue()接口。在此接口中，开发者可以将需要迁移的数据以键值对形式保存至wantParam中，并返回AbilityConstant.OnContinueResult.AGREE，标识应用同意迁移，从而将数据迁移至对端。 async onContinue(wantParam: Record<string, Object>): Promise&lt;AbilityConstant.OnContinueResult&gt; {
...
3. 对端恢复数据 在源端保存数据并同意迁移后，对端可启动应用，并在UIAbility中的onCreate()或onNewWant()生命周期回调中恢复数据。如果Ability的启动原因为LaunchReason.CONTINUATION，开发者可以从want.parameters中获取保存的键值对数据。 onCreate(want: Want, launchParam: AbilityConstant.LaunchParam): void {
...
} 可将恢复数据的方法提取为公共方法，以便于在UIAbility的onCreate()或onNewWant()中调用。 continueRestore(want: Want) {
  if (!want.parameters) {
...
![](assets/常见接续最佳实践/file-20260525090230334-002.gif)
...
![](assets/常见接续最佳实践/file-20260525090230334-003.jpg)
1. [启用接续](#li6149192715494)。
2. 在Scroll组件的onDidScroll()接口中监听长列表的浏览进度变化。 Scroll(this.scroller) {
  // ...
...
5. 在onDidBuild()事件中恢复浏览状态。 onDidBuild(): void {
...
![](assets/常见接续最佳实践/file-20260525090230334-004.gif)
媒体播放接续的内容主要包括播放列表中的集数、播放状态和进度。此外，还可以接续其他播放设置，以进一步提升用户体验。

![](assets/常见接续最佳实践/file-20260525090230334-005.jpg)
1. [启用接续](#li6149192715494)。
2. 使用avPlayer.on('timeUpdate')接口来监听媒体播放进度的变化。 this.avPlayer.on('timeUpdate', (time: number) => {
...
5. 在avPlayer初始化完成后，判断当前为接续状态，调用封装的调整视频进度方法videoSeek()，恢复至接续前的播放状态。 if (this.continue) {
...
![](assets/常见接续最佳实践/file-20260525090230334-006.gif)
...
![](assets/常见接续最佳实践/file-20260525090230334-007.jpg)
1. [启用接续](#li6149192715494)。
...
5. 在onPageEnd()回调中调用runJavaScript()接口以恢复进度。 Web({ src: this.pageUrl, controller: this.controller })
// ...
  .onPageEnd(async () => {
 // ...
