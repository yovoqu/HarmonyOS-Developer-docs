...
> [!NOTE] 说明
> 只有监听了onDrop()事件，拖拽在组件范围内移动时，才会触发onDragMove()事件。
...
![](assets/统一拖拽/file-20260525090252964-001.gif)
**实现原理**
...
**开发步骤**
1. 将拖出方Image组件的[draggable()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-drag-drop#draggable)属性设置为true。Image(\$rawfile('river.png'))
  // ...
  .draggable(true)
2. 在拖出对象的[onDragStart()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondragstart)接口中，获取图像信息并将其转换成[PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap)。Image(\$rawfile('river.png'))
  // ...
...
3. 将图片绘制到[Canvas](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-components-canvas-canvas)画布上，并获取拖拽时间作为水印绘制到画布上的指定位置，得到添加水印的图像。Image(\$rawfile('river.png'))
  // ...
  .onDragStart((event: DragEvent) => {
 // ...
...
4. 将图像打包保存在文件中，调用[DragEvent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#dragevent7)的[setData()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#setdata10)接口将水印图像设置为拖拽数据。Image(\$rawfile('river.png'))
  // ...
  .onDragStart((event: DragEvent) => {
 // ...
...
2. 将自定义组件转换成PixelMap，作为拖拽过程中显示的图片。private getComponentSnapshot(): void {
...
}  由于CustomBuilder需要离线渲染之后才能使用，存在一定的性能开销和时延，因此推荐开发者优先使用DragItemInfo中的PixelMap方式返回背板图。
3. 在拖出对象的[onPreDrag()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#onpredrag12)接口中，预先创建背板图。Image(\$r('app.media.mount'))
// ...
...
4. 在拖出对象的[onDragStart()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondragstart)接口中，将回调的PixelMap作为拖拽过程中的背板图。Image(\$r('app.media.mount'))
// ...
  .onDragStart(() => {
 let dragItemInfo: DragItemInfo = {
 pixelMap: this.pixelMap,
...
**开发步骤**
1. 在拖拽释放区域的[allowDrop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-drag-drop#allowdrop)接口中设置允许拖入的数据类型为[uniformTypeDescriptor.UniformDataType.IMAGE](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-data-uniformtypedescriptor#uniformdatatype)。Column() {
...
.allowDrop([uniformTypeDescriptor.UniformDataType.IMAGE])
// ...
2. 在拖入对象的[onDrop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondrop)接口中，调用[DragEvent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#dragevent7)的[getData()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#getdata10)接口获取拖拽数据。Column() {
  Text(this.textContent)
  // ...
}
// ...
...
3. 将拖拽数据转换成颜色数据格式为BGRA_8888的[PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap)类型的视觉信息。Column() {
  Text(this.textContent)
  // ...
}
// ...
...
4. 调用系统文字识别能力textRecognition获取拖拽数据中的文字信息。Column() {
  Text(this.textContent)
  // ...
}
// ...
...
2. 设置拖出方Image组件的[draggable()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-drag-drop#draggable)属性为true。Image('https://www-file.huawei.com/-/media/corp2020/home/banner/12/pura-x-1.jpg')
// ...
  .draggable(true)
  // ...
3. 在落入方Image组件的[onDrop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondrop)回调中获取图片的uri，绑定到落入方组件上，触发UI刷新。Column() {
...
4. 在落入方Image组件的[onDrop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondrop)回调里调用[request](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-request#导入模块)的[downloadFile()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-request#requestdownloadfile9)方法下载图片资源。Column() {
  Image(this.targetImage)
  // ...
}
// ...
...
**实现步骤：**
1. 设置拖出方Video组件的[draggable()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-drag-drop#draggable)属性为true。Video({
...
![](assets/统一拖拽/file-20260525090252965-007.gif)
**实现原理**
...
> [!NOTE] 说明
> 使用RichEditor组件时，需要在onDrop()回调里手动调用DragEvent的setResult()接口设置拖拽结果，否则会执行系统默认的onDrop()接口导致文字落入两次。参见onDrop()接口。

#### 基于UDMF多Entry的图文混排拖拽
**运行效果如下所示**
**图8 **多Entry图文混排拖拽

![](assets/统一拖拽/file-20260525090252966-008.gif)
**实现原理**
在拖出方手动构造多Entry数据进行拖拽，这种方式灵活性较高，但是由于需要用户手动去构造拖拽数据，因此代码开发更加复杂。
**开发步骤**
1. 在拖出方RichEditor组件的[onDragStart()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondragstart)回调中，根据选中的内容构造拖拽数据。RichEditor({ controller: this.sourceController })
// ...
 ...
 hilog.error(0x0000, TAG, `%{public}s`, err.code, err.message);
  ...
  // ...
2. 封装buildUnifiedRecords()函数，构造图片和文字内容的拖拽数据。buildUnifiedRecords(selection: RichEditorSelection) {
  try {
...
3. 封装getOriginImageUri()函数，getOriginPixelMap()函数，和getSelectedText()函数。getOriginImageUri(): string {
...
4. 在落入方[onDrop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-events-drag-drop#ondrop)回调里获取拖拽数据，调用[getEntry()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-data-unifieddatachannel#getentry15)接口读取所需要格式的数据。RichEditor({ controller: this.targetController1 })
// ...
...
![](assets/统一拖拽/file-20260525090252966-009.gif)
...
![](assets/统一拖拽/file-20260525090252966-010.gif)
...
![](assets/统一拖拽/file-20260525090252966-011.gif)
**图12 **将数据拖入中转站

![](assets/统一拖拽/file-20260525090252966-012.gif)
