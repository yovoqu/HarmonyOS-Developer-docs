...
- 如果应用是横屏应用，如游戏类应用，启动时默认为横屏，存在以下两种情况：仅支持横屏，建议配置landscape为默认旋转策略。支持在横屏和反向横屏中切换，建议配置为auto_rotation_landscape。
...
![](assets/视频类应用横竖屏切换/file-20260525090537798-003.png)
...
![](assets/视频类应用横竖屏切换/file-20260525090537798-004.png)
...
![](assets/视频类应用横竖屏切换/file-20260525090537799-005.png)
...
![](assets/视频类应用横竖屏切换/file-20260525090537799-006.png)
> 调用setPreferredOrientation()接口会改变窗口的显示方向。因此，如果在进入视频播放页时手动调用此接口，将窗口旋转至横屏，那么在退出页面时也应调用此接口，将窗口恢复为竖屏。
...
![](assets/视频类应用横竖屏切换/file-20260525090537799-007.png)
...
![](assets/视频类应用横竖屏切换/file-20260525090537799-008.png)
...
![](assets/视频类应用横竖屏切换/file-20260525090537800-009.png)
> 需要注意的是，对于视频全屏效果，建议采用沉浸式开发。沉浸式效果的实现，可参考开发应用沉浸式效果。
...
![](assets/视频类应用横竖屏切换/file-20260525090537800-010.png)
开发者应考虑以下三种情况以实现上述功能：
...
![](assets/视频类应用横竖屏切换/file-20260525090537800-011.png)
...
> [!NOTE] 说明
> BuildNode提供了组件预构建的能力，对于初始化耗时较长的声明式组件（如XComponent），可以有效减少初始化时间，提高组件加载效率。因此，推荐使用BuildNode挂载视频播放组件。

为了实现视频在不同页面连续播放，可使用NodeContainer挂载视频播放节点，并通过NodeController移除原页面的视频节点，迁移到新页面。具体实现可分为以下四步：
1. 使用[XComponent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-xcomponent)组件创建视频播放组件。步骤如下：创建XComponent组件和XComponentController控制器对象，并将控制器对象绑定至XComponent组件。在XComponent的onLoad()事件中初始化AVPlayer，并传入XComponent对应Surface的ID，实现视频的播放，可参考在ArkTS侧使用SurfaceId进行渲染绘制。通过全局自定义构建函数封装Video自定义组件，创建videoBuilder构建函数。 @Component
struct VideoNode {
...
2. 将视频播放组件挂载至BuildNode，并通过[NodeController](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-nodecontroller)管理自定义节点。步骤如下：使用wrapBuilder封装videoBuilder构建函数。通过调用BuildNode节点的build()接口创建组件树。在NodeController的makeNode()回调方法中返回组件树的实体节点（FrameNode）。当NodeController绑定的NodeContainer组件创建时，自动将返回的节点挂载至NodeContainer组件。 export class VideoNodeController extends NodeController {
...
3. 在竖屏页面中，将NodeController与NodeContainer绑定。@Component
...
} 在横屏页面中，将NodeController与NodeContainer绑定。 @Component
...
4. 页面切换完成时，调用NodeController的自定义方法onRemove()移除原页面的视频节点。// This method is called when the page transition animation is completed.
private doFinishTransition(): void {
  // ...
  // Remove the video node from the original page.
  this.nodeController.onRemove();
} 在自定义方法onRemove()中，将isRemove标志位设置为true，表示移除节点，然后调用rebuild()接口通知NodeContainer组件，将null挂载至NodeContainer，实现节点移除。操作完成后，重置isRemove标志位，保证新页面的节点正常生成。 onRemove(): void {
...
#### 自定义页面转场动画
1. 创建Navigation自定义转场动画配置类CustomTransition，用于注册、获取和删除页面的动画回调，包含以下方法：getInstance()：获取转场动画配置类的实例，使用静态方法在页面间共享。static delegate = new CustomTransition();
// Return the CustomTransition instance.
static getInstance() {
  return CustomTransition.delegate;
} registerNavParam()：当页面加载完成时，注册当前页面的动画回调。// Register the transition animation callback of the current page.
...
} unRegisterNavParam()：当页面卸载完成时，删除当前页面的动画回调。// Unregister the transition animation callback of the current page.
unRegisterNavParam(name: string): void {
  customTransitionMap.delete(name);
} getAnimateParam()：在自定义转场动画回调方法中，通过此方法获取原页面和新页面的动画回调。// Obtain the transition animation callback.
...
2. 在Navigation（导航页面）的[customNavContentTransition()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation#customnavcontenttransition11)事件中实现自定义转场动画回调方法，自定义竖屏页面与横屏页面的导航转场动画。具体步骤如下：检查原页面与新页面是否存在，其中一个不存在则返回undefined。返回值为undefined表示执行默认动画。// If the parameters related to jumping are not defined, no custom animation will be performed.
...
if (!isCustomTransitionEnable(from.name, to.name)) {
  return undefined;
} 通过创建Navigation自定义转场动画配置类实现的getAnimateParam()方法获取原页面与新页面的动画回调方法。// It is necessary to check whether the transition page has registered an animation
...
if (!fromParam.animation || !toParam.animation) {
  return undefined;
} 创建自定义转场动画协议对象（NavigationAnimatedTransition），定义Navigation路由跳转时的转场动画。  // After all judgments are made, construct customAnimation for the system to call
...
} 在Navigation（导航页面）的customNavContentTransition()事件中，传入自定义转场动画回调方法。build() {
...
3. 在NavDestination的[onReady()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navdestination#onready11)事件中获取[NavPathStack](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-navigation#navpathstack10)对象，并在横屏页面的onReady()事件中注册当前页面的动画回调。build() {
  // ...
...
this.uiContext.animateTo({
 ...
 AppStorage.setOrCreate('videoBackgroundColor', Color.Black);
  ...
  this.translateY = 0;
  this.rotate = 0;
}) 从横屏页面返回竖屏页面时，计算animateTo()接口闭包内视频节点的可动画属性。动画执行完成后，移除横屏页面的视频节点。 let notFullCenterX: number = nodeInfoPx.top + nodeInfoPx.height / 2;
...
2. 在页面的转场动画回调方法中执行视频旋转动画。当设备状态为完全展开或半折叠时，执行doAnimationFoldable()动画，否则，执行doAnimation()动画。// Register the animation callback of the current page.
CustomTransition.getInstance().registerNavParam(this.pageId,
  (isPush: boolean, isExit: boolean, transitionProxy: NavigationTransitionProxy) => {
 if (WindowUtils.isExpandedOrHalfFolded()) {
 // Play animation, foldable screen in unfolded state
...
3. 使用[window.getLastWindow()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-f#windowgetlastwindow9)接口获取应用内层级最高的窗口对象，通过[on('windowSizeChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#onwindowsizechange7)接口开启窗口尺寸变化的监听。当折叠屏的折叠状态切换时，视频节点宽高同步调整。window.getLastWindow(this.getUIContext().getHostContext()).then((windowClass) => {
...
4. 在横屏页面折叠屏的折叠状态切换时，使用[display.on('foldStatusChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-display#displayonfoldstatuschange10)接口监听并适配UI界面。当折叠屏由展开态切换为折叠态时，调用pop()接口返回到竖屏页面，并调用竖屏页面传入的prePageDoFinishTransition()方法移除横屏页面的视频节点。开发者可根据实际业务场景返回到竖屏页面或者将显示方向设置为横屏显示。 当折叠屏由折叠态切换为展开态时，仍使用竖屏的方式播放视频，因此将显示方向设置为PORTRAIT。 // Handling UI adaptation for video playback with collapse and expand functionality
foldStatusChangeUI() {
  try {
