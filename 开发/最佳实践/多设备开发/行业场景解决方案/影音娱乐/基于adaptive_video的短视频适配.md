...
![](assets/基于adaptive_video的短视频适配/file-20260525090537858-001.png)

视频宽高比处于9:16区间的短视频在完整断点区间上的沉浸效果图如下（红色部分为视频显示区域）：

![](assets/基于adaptive_video的短视频适配/file-20260525090537858-002.png)

视频宽高比处于非9:16区间的短视频在窗口宽为320vp以上设备的沉浸效果如下（虚线框内为视频可能显示的区域）：

![](assets/基于adaptive_video的短视频适配/file-20260525090537858-003.png)

#### 开发步骤

> [!NOTE] 说明
> adaptive_video三方库中的视频自适应沉浸组件支持网络视频链接，因此三方库使用了以下权限。权限设置详情参考声明权限。 ohos.permission.INTERNET：允许应用访问互联网。 以下代码默认应用已经安装并导入了adaptive_video三方库，详细的三方库安装教程可以参考如何使用ohpm引入三四方库。

- 沉浸模式要求将视频页面的内容扩展至状态栏和导航栏，因此需设置窗口为全屏模式，此时组件的布局范围将从安全区域延伸至整个窗口。// Set full screen
...
- 从adaptive_video三方库导入AdaptiveImmersion自适应沉浸工具并初始化。import { AdaptiveImmersion } from '@hadss/adaptive_video';
// ...
...
- 调用AdaptiveImmersion的接口，根据视频宽高及底部Tab导航栏高度计算页面沉浸信息，并根据计算结果调整页面布局（包括视频组件的宽高、视频组件的位置等），以实现页面的沉浸式布局。build() {
  RelativeContainer() {
 Stack() {
 Column() {
 XComponent({
...
}   短视频滑动播放页面通常懒加载了多个视频播放组件，这些组件包括不同宽高比例的视频。在视频切换时，需要重新调用AdaptiveImmersion的接口来计算当前页面的沉浸式布局；此外，当设备窗口发生变化时，也需要重新计算页面的沉浸式布局。有关AdaptiveImmersion接口的详细说明，请参阅adaptive_video使用说明。
...
#### 开发步骤
- 从adaptive_video三方库导入AdaptiveRotation自适应旋转工具、ScreenModeNotifier页面布局变换工具并初始化。导入后开发者可直接通过调用AdaptiveRotation的接口设置视频页面旋转属性，并通过ScreenModeNotifier监听页面布局变换通知。import { AdaptiveRotation, ScreenMode, ScreenModeNotifier } from '@hadss/adaptive_video';
// ...
...
- 定义屏幕布局模式变化回调函数并注册。aboutToAppear(): void {
  // ...
...
// ...
- 页面进入全屏播放时。// Play the video in full screen mode
adaptiveRotation?.setOrientationFullScreen({
  width: oriWidth, height: oriHeight
}); // Input the width and height of the current video, control the rotation attribute, and horizontal full-screen playback effect
- 页面进入非全屏播放时。// Exit the full-screen mode
adaptiveRotation?.setOrientationNotFullScreen({
  width: oriWidth, height: oriHeight
}) // Input the width and height of the current video to be played, control the rotation attribute, and play the video vertically
- 退出视频播放界面时，注销布局模式通知相关的监听。// Unregister the screen mode listener
screenModeNotifier.offScreenModeChange(this.onScreenModeChange);   短视频滑动播放页面通常懒加载了多个视频播放组件，这些组件包括不同宽高比例的视频。在视频切换时，需要重新调用AdaptiveRotation的接口来设置当前页面视频对应的旋转属性。
