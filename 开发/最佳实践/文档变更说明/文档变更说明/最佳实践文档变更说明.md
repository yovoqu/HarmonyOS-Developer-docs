...
**行业场景解决方案**
 ...
 - [基于PixelMap实现图片编辑](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-pixelmap-image-editing)：介绍了如何使用PixelMap对图片进行编辑处理，如获取图片信息、裁剪、缩放、偏移、旋转、翻转、设置透明度、读写像素数据等。
 - [基于PixelMap与Canvas实现图片编辑](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-pixelmap-canvas-image-editing)：介绍了如何基于PixelMap和Canvas实现图片编辑处理，如获取图片信息、裁剪、缩放、偏移、旋转、翻转、设置透明度、读写像素数据、图片水印添加等。


**稳定性**

 - [开发态快速定位应用崩溃](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-locate-application-crashes)：介绍了以内存越界访问为典型案例，构建数组越界示例工程。
 - [运维态高效处理GPU内核泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-handling-gpu-kernel-leaks)：介绍了如何通过APMS智能分析平台定位问题的全部流程。
 - [运维态高效处理FD资源泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-efficiently-handle-fd-leaks)：介绍了在运维态下如何通过应用质量管理（APMS）平台进行FD资源泄漏的监控、分析、定位与修复。
...
**一次开发，多端部署**
 ...
 - [页面布局场景](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-device-page-layout)：新增边看边评典型场景，优化双分栏判断逻辑与比例。


**多端设备支持**

 - [电脑应用开发](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-pc-guide)：新增梦露电脑硬件参数、产品示意图、手写笔适配内容。
...
**行业场景解决方案**
 ...
 - [基于List和Scroller实现双向滚动列表](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-horizontal-vertical-scrolling-list)：介绍了如何基于List组件和Scroller控制器，封装核心组件[DualScrollTable](https://gitcode.com/HarmonyOS_Samples/DualScrollList/blob/master/tableview/src/main/ets/components/DualScrollTable.ets)（双向滚动列表组件），实现满足基础功能的双向滚动列表解决方案。
 - [K歌解决方案](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-karaoke-singing)：介绍了如何基于[AudioCapturer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer)、[AudioRenderer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiorenderer)、[AudioLoopback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audioloopback)、[AudioRoutingManager](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audioroutingmanager)、[AVPlayer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avplayer)开发一个K歌应用。


**稳定性**

 - [CppCrash故障检测机制及规格说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-cppcrash-fault-detection-mechanism)：介绍了CppCrash故障检测机制及规格说明，为开发者提供详细的维测日志以辅助故障定位。
 - [SIGSEGV无效内存访问故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-cppcrash-sigsegv-fault-mode)：介绍了SIGSEGV无效内存访问故障模式说明，为开发者提供详细的问题定位及处理指导。
 - [SIGABRT进程主动终止故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-cppcrash-sigabrt-fault-mode)：介绍了SIGABRT进程主动终止故障模式说明，为开发者提供详细的问题定位及处理指导。
 - [SIGBUS非法内存访问故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-cppcrash-sigbus-fault-mode)：介绍了SIGBUS非法内存访问故障模式说明，为开发者提供详细的问题定位及处理指导。
 - [SIGILL非法指令故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-cppcrash-sigill-fault-mode)：介绍了SIGILL非法指令故障模式说明，为开发者提供详细的问题定位及处理指导。
 - [SIGTRAP断点或陷阱异常故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-cppcrash-sigtrap-fault-mode)：介绍了SIGTRAP断点或陷阱异常故障模式说明，为开发者提供详细的问题定位及处理指导。
 - [应用主线程阻塞故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-main-thread-block)：介绍了如何通过分析应用冻屏故障日志中的主线程堆栈，定位主线程阻塞类型的应用冻屏（AppFreeze）问题。
 - [应用主线程繁忙故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-main-thread-busy)：介绍了如何通过分析应用冻屏故障日志中的主线程堆栈，定位主线程繁忙类型的应用冻屏（AppFreeze）问题。
 - [ArkWeb GPU进程卡死故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-arkweb-gpu-freeze)：介绍了如何定位主线程阻塞在ArkWeb堆栈类型的应用冻屏（AppFreeze）问题。
 - [ArkWeb Render进程卡死故障模式说明](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-arkweb-render-freeze)：介绍了如何定位因render卡死导致的应用冻屏（AppFreeze）问题。
 - [开发态快速定位ArkTS泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-arkts-leak-in-develop)：介绍了如何通过常见泄漏场景和ArkTS内存泄漏分析案例，快速定位应用中的内存泄漏问题。
 - [开发态快速定位DMA泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-dma-leak-in-develop)：介绍了如何通过高频泄漏场景和DMA内存泄漏分析案例快速定位应用DMA泄漏的问题
 - [开发态快速定位Native泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-native-leak-in-develop)：介绍了如何通过高频泄漏场景和Native内存泄漏分析案例，定位应用中的Native内存泄漏问题。
 - [开发态快速定位AppFreeze冻屏指导](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-freeze-in-develop)：介绍了如何通过高频冻屏场景及分析案例，快速定位冻屏问题。
 - [运维态高效处理应用崩溃](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-crash-in-operation)：介绍了在运维态下如何通过[APMS](https://developer.huawei.com/consumer/cn/doc/app/agc-help-apms-0000002235870062)平台对应用崩溃事件进行标准化排查、分析、定位和修复的闭环流程。
 - [运维态高效处理ArkTS泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-arkts-leak-in-operation)：介绍了在运维态下如何通过[APMS](https://developer.huawei.com/consumer/cn/doc/app/agc-help-apms-0000002235870062)平台进行ArkTS泄漏事件的标准化排查、分析、定位和修复闭环流程。
 - [运维态高效处理Native泄漏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-native-leak-in-operation)：介绍了在运维态下如何利用[APMS](https://developer.huawei.com/consumer/cn/doc/app/agc-help-apms-0000002235870062)平台完成Native内存泄漏的监控、分析、定位及修复全流程。
 - [运维态高效处理应用冻屏](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-freeze-in-operation)：介绍了在运维态下如何通过[APMS](https://developer.huawei.com/consumer/cn/doc/app/agc-help-apms-0000002235870062)平台对应用冻屏事件进行标准化排查、分析、定位和修复的闭环流程。
...
**自由流转**
 ...
 - [碰一碰文件分享](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-application-knock-file-share)：新增精准碰一碰章节。
...
**组件封装与复用**
 ...
 - 组件动态创建：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 学习UI范式基本语法 > 自定义组件 > 自定义组件复用 > 自定义组件复用开发实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-component_reuse)。
 - 组件封装：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 学习UI范式基本语法 > 自定义组件 > 自定义组件复用 > 自定义组件复用开发实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-component_reuse)。
 - 组件复用：已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 学习UI范式基本语法 > 自定义组件 > 自定义组件复用 > 组件复用问题诊断分析”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-diagnosis-component-reuse-issues)。
 - 组件复用问题诊断分析：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 学习UI范式基本语法 > 自定义组件 > 自定义组件复用 > 组件复用问题诊断分析”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-diagnosis-component-reuse-issues)。


**布局与弹窗**

 - 自定义弹窗选型与开发：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架) > UI开发 (ArkTS声明式开发范式) > 使用弹窗 > 自定义弹窗选型与开发”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/custom-dialog-select-develop)。
 - 常见列表流：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 列表与网格 > 常见列表流”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-common-list-flow)。
 - 文本展开折叠：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 使用文本 > 文本展开折叠”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-text-unfolding-and-folding)。
 - 富文本显示的选型与开发：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架）> UI开发 (ArkTS声明式开发范式) > 使用文本 > 富文本显示的选型与开发”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-selection-and-development-of-rich-text-display)。
 - 布局优化指导：内容已迁移至[“指南 > 应用框架 > ArkUI（方舟UI框架) > UI开发 (ArkTS声明式开发范式) > 组件布局 > 布局优化指导”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-layout-optimization-guidance)。


**动画与转场**

 - 页面间转场：指南中已有相似文档，详见[页面转场动画 (不推荐)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-page-transition-animation)。


**应用框架**

 - 同层渲染原生组件：内容已迁移至[“指南>应用框架>ArkWeb（方舟Web）>同层渲染原生组件”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/same-layer-rendering-native-component)。


**网络**

 - 网络信息查询与连接管理：内容已迁移至[“指南>系统>网络>Network Kit（网络服务）>连接网络>连接网络开发实践>网络信息查询与连接管理”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/network-information-query-connection-management)。
 - 应用网络重连：内容已迁移至[“指南>系统>网络>Network Kit（网络服务）>连接网络>连接网络开发实践>应用网络重连”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-network-reconnection)。


**媒体**

 - 基于AVPlayer播放长视频实践：内容已迁移至[“指南 > 媒体 > Media Kit（媒体服务）> 媒体开发指导(ArkTS) > 媒体开发实践 > 基于AVPlayer播放长视频实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/avplayer-long-video)。
 - 基于AVPlayer播放短视频实践：内容已迁移至[“指南 > 媒体 > Media Kit（媒体服务）> 媒体开发指导(ArkTS) > 媒体开发实践 > 基于AVPlayer播放短视频实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/avplayer-short-video)。
 - 基于AVPlayer播放嵌入式短视频实践：内容已迁移至[“指南 > 媒体 > Media Kit（媒体服务）> 媒体开发指导(ArkTS) > 媒体开发实践 > 基于AVPlayer播放嵌入式短视频实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/avplayer-embeded-short-video)。
 - 基于AVPlayer播放网络视频实践：内容已迁移至[“指南 > 媒体 > Media Kit（媒体服务）> 媒体开发指导(ArkTS) > 媒体开发实践 > 基于AVPlayer播放网络视频实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/avplayer-network-video)。
 - 在线视频播放卡顿优化：内容已迁移至[“指南 > 媒体 > Media Kit（媒体服务）> 媒体开发指导(ArkTS) > 媒体开发实践 > 在线视频播放卡顿优化”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/online-video-playback-lags-optimize)。
 - 音画同步：内容已迁移至[“指南 > 媒体 > AVCodec Kit（音视频编解码服务）> 音视频编解码开发实践 > 音画同步”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/audio-video-synchronization)。
 - 图片获取与保存实践：内容已迁移至[“指南 > 媒体 >Media Library Kit（媒体文件管理服务）> 图片获取与保存实践”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/images-get-preservation-practice)。
 - 自定义相机拍照：内容已迁移至[“指南>媒体>Camera Kit（相机服务）>开发相机应用基础能力(ArkTS)>拍照(ArkTS)”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-shooting)。
 - 自定义相机录像：内容已迁移至[“指南>媒体>Camera Kit（相机服务）>开发相机应用基础能力(ArkTS)>录像(ArkTS)”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-recording)。
 - 自定义相机预览：内容已迁移至[“指南>媒体>Camera Kit（相机服务）>开发相机应用基础能力(ArkTS)>预览(ArkTS)”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-preview)。
 - 相机预览花屏解决方案：内容已迁移至[“指南>媒体>Camera Kit（相机服务）>开发相机应用基础能力(ArkTS)>相机预览花屏解决方案”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/camera-preview-glitch-solution)。


**NDK开发**

 - Native侧子线程与UI主线程通信：内容已迁移至[“指南 > NDK 开发 > 编译工具链 > Native侧子线程与UI主线程通信”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/native_subthread-to-uimain)。
 - 跨语言调用复杂参数传递：内容已迁移至[“指南 > NDK 开发 > 编译工具链 > 跨语言调用复杂参数传递”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cross-language-complex-param)。


**应用安全**

 - 加解密跨平台数据兼容性：内容已迁移至[“指南>系统>安全>Crypto Architecture Kit（加解密算法框架服务）>实现跨平台加解密数据兼容性”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-development-practice)。
 - 网络连接安全配置：内容已迁移至[“指南>系统>网络>Network Kit（网络服务）>连接网络>连接网络开发实践>网络连接安全配置”](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/network-connection-security-configuration)。
...
**技术创新**
 ...
 - [智感握姿](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-smart-reach)：介绍了如何在HarmonyOS应用中接入智感握姿能力，具体包括组件原生适配与自定义握持感知两种方案。
 - [沉浸光感](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-spatiality-immersive)：介绍了如何在HarmonyOS应用中快速接入沉浸光感能力。包括沉浸光感的基础概念，首眼沉浸光感、自适应悬浮导航与智感握姿智能交互三大应用场景。
 - [防窥保护](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-antipeep-protection)：介绍了如何基于dlpAntiPeep能力，智能检测到旁窥风险时，系统对隐私数据的安全操作，从而保护用户安全隐私。
 - [冷启网络预建链最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-pre-connect)：介绍了远场通信预键链特性在网络请求获取资源的原理及特点、实现流程和约束与限制。
 - [星盾机密风控引擎](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-risk-control-engine)：介绍了星盾机密风控引擎的功能原理及特点，如何在涉诈场景中应用和使用该特性的约束与限制。
 - [弱网直播优化](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-live-streaming-optimization)：介绍quic特性在弱网直播场景下原理及特点、实现流程和约束限制。
 - [互动卡片开发实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-live-form-development-practice)：介绍了互动卡片的使用场景、开发方案和常见问题。


**场景创新**

 - [实现修图软件的波轮菜单功能](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-wavewheel-for-image-editor)：介绍了如何使用波轮菜单SDK提供的完整的波轮菜单功能封装，实现在应用中集成波轮菜单功能的开发指导。
 - [全链路盯盘开发实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-always-on-market-watch)：介绍了如何实现桌面盯盘、防窥保护、锁屏盯盘、待机屏保盯盘的功能开发。
 - [游戏预启动开发实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-game-prelaunch-practice)：介绍了如何实现游戏预启动的功能开发。
...
**媒体**
 ...
 - [音频焦点管理解决方案](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-audio-focus-management)：优化文章描述和示例代码。


**一次开发，多端部署**

 - [多设备音乐界面](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-music-app-overview)：文档架构调整，整篇优化。
 - [多设备购物比价界面](https://developer.huawei.com/consumer/cn/doc/best-practices/multi-shopping-price-comparison)：文档架构调整，整篇优化。
 - [多设备社区评论界面](https://developer.huawei.com/consumer/cn/doc/best-practices/multi-community-app)：文档架构调整，整篇优化；实现的一多社区评论界应用，补充适配三折叠、阔折叠、Hope、电脑。
 - [多设备短视频界面](https://developer.huawei.com/consumer/cn/doc/best-practices/multi-short-video-app)：文档架构调整，整篇优化；增加了三折叠、阔折叠、电脑、智慧屏、智能穿戴的适配，增加视频播放、视频切换、展开评论区、短视频自适应等功能开发指导。


**自由流转**

 - [常见接续最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-application-continue-progess)：更新流程图和部分设备运行动图，更新文章章节结构。
 - [社交通讯全场景协同最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-continue)：扩展垂域案例，增加碰一碰添加图片、碰一碰/隔空传送分享内容的功能。


**稳定性**

 - [使用GWP-ASan检测内存错误](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-gwpasan-detection)：GWP-ASan常见案例完善，从代码实例、定位思路、修复建议等维度补充介绍各类异常检测类型。
...
**应用框架**
 ...
 - [基于AbcKitTS实现字节码插桩](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-abckitts-implements-instrumentation)：介绍了在实际业务场景中，AbcKitTS的切面编程中的高频应用模式和基本使用方法。


**性能**

 - [Web帧率问题分析](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-web-frame-rate-performance-analysis)：介绍了三方应用在滑动时产生丢帧现象的常见原因，并提供了相应的解决方案。


**稳定性**

 - [JS Crash类问题案例](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-scenario-stability-jscrash)：介绍了当前开发者所遇到的高频JS Crash故障案例。
...
新增文档6篇。
 ...
 - **媒体**
 - [基于Video组件播放长视频](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-video-component-long-video)：介绍了如何基于Video组件实现长视频播放，实现基本播控、视频首帧显示、全屏播放、跳转播放、前台小窗播放、点击按钮选择倍速、长按视频倍速、循环播放、音量设置、接入播控中心等功能。
 - [基于Video组件播放短视频](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-video-component-short-video)：介绍了如何使用Video组件实现短视频播放，实现基础播控、自定义进度条、全屏播放、跳转播放、播放倍速调节、自动连播、音量设置及前后台状态感知等功能。


**功耗**

 - [应用UI进程空跑问题分析](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-ui-skip-analysis)：介绍了什么是应用UI进程空跑、应用UI进程空跑的分析思路和常见故障根因。
 - [自渲染图层未使用硬件合成问题分析](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-hwc-self-rendering-layer-analysis)：介绍了如何实现自渲染图层使用硬件合成、问题定位和常见问题根因。
 - [应用退后台禁止使用传感器问题分析](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-background-sensors-baned-analysis)：介绍了为何应用退后台禁止使用传感器和问题定位流程。
...
**多端设备体验提升**
 ...
 - [阔折叠应用开发](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-purax-guide)：在原有Pura X阔折叠基础上，补充Puira X Max的相关开发指导。


**一次开发，多端部署**

 - [多设备长视频界面](https://developer.huawei.com/consumer/cn/doc/best-practices/multi-video-app)：新增阔折叠、三折叠、PC和智慧屏适配，按照最新案例优化内容，简化功能描述。
 - [窗口方向](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-device-window-direction)：分类介绍了18种窗口横竖屏旋转枚举的效果，以窗口旋转策略实现的高频场景为载体，帮助开发者选择更适合应用的窗口旋转模式。
...
**媒体**
 ...
 - 基于AVPlayer播放嵌入式短视频实践：介绍如何基于[AVPlayer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avplayer)系统播放器实现嵌入式短视频播放。
...
图1 **最佳实践目录变更前后对比      **
![](assets/最佳实践文档变更说明/file-20260515115146911-0.png)
...
![](assets/最佳实践文档变更说明/file-20260515115146911-1.png)
...
![](assets/最佳实践文档变更说明/file-20260515115146911-2.png)
...
**功能开发**      
 ...
 - Native侧子线程与UI主线程通信：介绍Native侧子线程与UI主线程通信开发的两种方案，即如何基于线程安全函数机制实现和基于libuv异步库的uv_async_send方法实现。
...
**HarmonyOS特征**
 ...
 - [多设备移动支付界面](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-mobile-payment)：介绍“一多”在移动支付应用开发中的应用。主要包含支付、收款、扫码等。
