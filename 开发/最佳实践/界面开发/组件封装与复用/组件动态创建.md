...
![](assets/组件动态创建/file-20260515114422784-0.jpg)
...
![](assets/组件动态创建/file-20260515114422784-1.png)
...
![](assets/组件动态创建/file-20260515114422784-2.png)
...
![](assets/组件动态创建/file-20260515114422784-3.png)
...
export class AdNodeController extends NodeController {
  ...
  initAd(uiContext: UIContext, id: string, adType: string) {
    ...
    this.adNode.build(wrapBuilder(adBuilder), {
      id: id,
      isVideo: adType === 'video',
    });
    this.rootNode
      .getRenderNode()
      ?.appendChild(this.adNode.getFrameNode()?.getRenderNode());
  }
...
export const getAdNodeController = (
  uiContext: UIContext,
  id: string,
): AdNodeController | undefined => {
...
```ts
Button($r('app.string.text_dialog_shield')).onClick(() => {
...
![](assets/组件动态创建/file-20260515114422784-4.png)
2. 以上示例场景在FrameNode扩展模式下的完成时延为6.1ms（根据设备和场景不同，数据会有差异，本数据仅供参考），如下图所示。
![](assets/组件动态创建/file-20260515114422784-5.png)
