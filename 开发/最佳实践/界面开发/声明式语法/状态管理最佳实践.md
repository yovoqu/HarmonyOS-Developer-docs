...
![](assets/状态管理最佳实践/file-20260515114514719-0.png)
...
> [!NOTE]
> 建议开发者优先使用 Code Linter扫描工具 进行代码检查，重点关注 @performance/hp-arkui-remove-redundant-state-var 规则。若扫描结果中出现该规则相关问题，可参考本章节提供的优化建议进行调整。
...
> [!NOTE]
> 建议开发者优先使用 Code Linter扫描工具 进行代码检查，重点关注 @performance/hp-arkui-remove-unchanged-state-var 规则。若扫描结果中出现该规则相关问题，可参考本章节提供的优化建议进行调整。
...
**图2 **HMOS世界App主题选择交互图**
![](assets/状态管理最佳实践/file-20260515114514719-11.jpg)
...
- 父子组件间共享状态：如下图，”父组件”和其子组件”子组件A”、”子组件B”共享状态loading。
  ...
  图3 **父子组件间共享状态场景**
![](assets/状态管理最佳实践/file-20260515114514719-12.png)
...
- 不同子树上组件间共享状态：如下图，祖先组件的左子树上”孙子组件AAA”和右子树上”孙子组件BAA”共享状态loading。
  ...
  图4 **不同子树上组件间状态共享场景**
![](assets/状态管理最佳实践/file-20260515114514719-13.png)

- 不同组件树间共享状态：如下图，组件树A内”子组件AA”和组件树B内”孙子组件BAA”共享状态loading。

  图5 **不同组件树间共享状态的场景**
![](assets/状态管理最佳实践/file-20260515114514719-14.png)
...
图6 **HMOS世界App界面组件示意图**
![](assets/状态管理最佳实践/file-20260515114514719-15.gif)

 

![](assets/状态管理最佳实践/file-20260515114514719-16.gif)
...
![](assets/状态管理最佳实践/file-20260515114514719-2.jpg)
...
![](assets/状态管理最佳实践/file-20260515114514719-3.jpg)
...
![](assets/状态管理最佳实践/file-20260515114514719-4.png)
...
![](assets/状态管理最佳实践/file-20260515114514719-5.png)
...
![](assets/状态管理最佳实践/file-20260515114514719-6.png)

 
1. @State+@Prop组合方案：
- @Prop装饰器支持接收Object、class、string、number、boolean、enum类型，以及这些类型的数组。

2. @Prop装饰的变量是对父组件传入状态值的深拷贝，当@Prop装饰器装饰的变量为复杂Object、class或其类型数组时，会增加状态创建时间以及占用大量内存。

3. @Prop装饰的变量和父组件是单向绑定的关系。当父组件数据源发生变化时，接收该数据源的@Prop所在组件的实例会重新渲染。 当该组件内被@Prop装饰的变量被修改时，父组件数据源不会变化，父组件实例也不会重新渲染。

4. @State+@Link组合方案：
@Link装饰器支持接收Object、class、string、number、boolean、enum类型，以及这些类型的数组。

5. @Link装饰器修饰的变量是对父组件传入状态的引用的拷贝，两者指向同一个地址。当状态是简单数据类型或简单Object类型时，@Link和@Prop在状态创建时间和内存的占用方面区别不大。当状态为复杂的Object、class或其类型数组时，@Link相较@Prop能明显减少状态创建时间和内存的占用。

6. @Link装饰器的变量和父组件是双向绑定的关系。当父组件数据源发生变化时，接收该数据源的@Link所在组件的实例会重新渲染。 当该组件内被@Link装饰的变量被修改时，父组件数据源会同步修改，父组件实例也会重新渲染。

7. @State+@Observed+@ObjectLink组合方案：
@ObjectLink只支持接收被@Observed装饰的class实例及继承Date或者Array的class实例。

8. @ObjectLink装饰的变量是只读的，不支持对状态重新赋值。

9. @ObjectLink必须配合@Observed使用，它的设计是为了解决对嵌套类对象属性变化的监听，如需要观察对象数组中单个数据项的属性值变化，或嵌套对象的对象类型属性的子属性变化。
...
![](assets/状态管理最佳实践/file-20260515114514719-7.jpg)

 

![](assets/状态管理最佳实践/file-20260515114514719-8.jpg)
...
> [!NOTE]
> 过分追求状态结构拆分可能在某些场景导致组件设计过度，不利于维护。此时，可以将对象或类上经常一起改变的几个属性聚合成一个新的对象或类模型，并使用@Observed装饰器修饰，再作为属性挂载到之前的对象或类上。通过此方法，当属性变化时ArkUI只会通知变化给新的对象或类，不会通知最上层的对象。这样既可以有效的减少无用渲染次数，又能使代码更好维护。 如类ClassA上存在属性b、c、d。其中c和d经常一起发生变化，即当c的状态修改时同时也要修改d的状态。  CODE8  此时，将c和d组合在一起做为新的类ClassE的属性并使用@Observed装饰器修饰。对于ClassA去掉c、d属性，新增属性e且其类型为ClassE，设计如下：  CODE9  使用此方案，在AppStorage中存入数据结构为ClassA的变量。当ClassA实例的属性e中的属性c的值变化时，ArkUI框架会通知使用ClassE实例的组件重新渲染，不会通知所有使用AppStorage中ClassA实例的组件更新，即只使用了ClassA实例b属性的组件不会重新渲染。
...
> [!TIP]
> 建议开发者优先使用 Code Linter扫描工具 进行代码检查，重点关注 @performance/multiple-associations-state-var-check 规则。若扫描结果中出现该规则相关问题，可参考本章节提供的优化建议进行调整。
