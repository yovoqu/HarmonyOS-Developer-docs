...
> [!NOTE]
> sheet类型弹窗须调用 setComponentTargetId(targetCompId ) 以实现页面级弹窗，并且保证绑定的组件id存在。 当前示例主要面向直板机等窄屏场景使用。因此对于如PC、三折叠展开态等屏幕宽度大于600vp的场景，会自动隐藏对应入口。
...
- DialogHub提供的其他类别弹窗，如CustomDialog，在弹窗弹出时父页面的焦点默认不会转移到弹窗上。开发者可以配置弹窗的requestFocusWhenShow属性实现：弹窗弹出时，将页面的焦点转移到弹窗中。进而实现[会避让键盘的弹窗](#section395448164119)的效果。
...
- DialogMode.OverlayManager（默认）模式下，返回手势会优先作用于页面，由页面消费该事件。
  ...
  处理方法如下：1. 在页面的onBackPress()中调用dispatchBackPressToDialog()方法将事件传递给弹窗。
