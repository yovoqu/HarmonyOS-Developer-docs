  ...
  getTextFontPreference(context: Context) {
    try {
      ...
      hilog.error(
        0x0000,
        TAG,
        `create preference failed. code: ${error.code}, message:${err.message}`,
      );
    }
  }
  ...
  getFont(): string {
    ...
    try {
      ...
      hilog.error(
        0x0000,
        TAG,
        `getSync font preference data failed. code: ${error.code}, message:${err.message}`,
      );
    }
    return textFont;
  }
...
export function registerMyFont(uiContext: UIContext) {
  try {
    ...
    hilog.error(
      0x0000,
      TAG,
      `registerFont failed. code: ${error.code}, message:${err.message}`,
    );
  }
}
```
...
```ts
Toggle({ type: ToggleType.Switch, isOn: this.toggleState }).onChange(
  (isOn: boolean) => {
...
export function fp2pxUtil(fp: number): string {
  ...
  try {
    ...
    hilog.error(
      0x0000,
      TAG,
      `get densityDPI failed. code: ${error.code}, message:${err.message}`,
    );
  }
  return pxVal + pxStr;
}
