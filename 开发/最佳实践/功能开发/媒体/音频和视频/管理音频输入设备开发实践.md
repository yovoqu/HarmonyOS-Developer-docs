...
AudioRoutingManager提供管理全局音频输入设备的能力，包括查询设备信息、监听设备连接状态变化等。
...
使用AudioSessionManager管理音频输入设备。通过该组件，可默认将蓝牙设备设为音频输入源，同时支持动态切换不同的媒体输入设备。
...
在PC设备上，通过系统提供的录音设备选择组件AVInputCastPicker切换音频输入设备。


![](assets/管理音频输入设备开发实践/file-20260515114755073-3.gif)
...
AudioCapturer.on('inputDeviceChange')可以监听音频流输入设备变化并返回切换后的设备信息。应用可以根据切换后的新设备做对应的处理。


### 开发步骤


使用AudioCapturer.on('inputDeviceChange')监听到音频流输入设备变化时，显示新设备信息。
```ts
...
API20提供了AudioStreamManager.isRecordingAvailable(capturerInfo: AudioCapturerInfo)接口，设置AudioCapturerInfo.source为Audio.SourceType.SOURCE_TYPE_MIC，然后根据返回值判断麦克风状态。如果返回true，表明可以使用麦克风进行录制；如果返回false，表明麦克风可能已被占用。
