...
具体开发步骤可参考最佳实践《基于AVPlayer基础播控实践》。
...
AVCodec模块中视频解码的Native API接口，可以完成视频解码功能，即将媒体数据在系统侧解码成YUV文件并送显至应用上。
...
使用Interface (CameraManager)+Interface (AVRecorder)录制HDR Vivid视频，与录制普通视频的区别主要在于：

- AVRecorderAVRecoder需要配置isHdr参数为true，对应的编码格式必须为video/hevc。


- Camera相机创建video output实例时，选择yuv 10bit profile（CAMERA_FORMAT_YCRCB_P010）。
- HDR录像需要相机支持视频防抖功能。
...
let previewProfile: undefined | camera.Profile = previewProfilesArray.find(
...
try {
  ...
  Logger.error(
    TAG,
    `Failed to create the videoOutput instance. error: ${JSON.stringify(err)}`,
  );
}
```
...
try {
  this.captureSession = this.cameraManager.createSession(
...
try {
  ...
  Logger.info(
    TAG,
    `isVideoStabilizationModeSupported: ${JSON.stringify(isSupported)}`,
  );
} catch (error) {
...
if (isSupported) {
  try {
    ...
    let activeVideoStabilizationMode =
      this.captureSession.getActiveVideoStabilizationMode();
    Logger.info(
      `activeVideoStabilizationMode: ${activeVideoStabilizationMode}`,
    );
  } catch (error) {
    let err = error as BusinessError;
    Logger.warn(
...
if (isSupported) {
  ...
  try {
    ...
    let err = error as BusinessError;
    Logger.error(
      `The getSupportedColorSpaces call failed. error code: ${err.code}`,
    );
  }
  ...
  if (isSupportedColorSpaces) {
    ...
    try {
      let activeColorSpace: colorSpaceManager.ColorSpace =
        this.captureSession.getActiveColorSpace();
      Logger.info(`activeColorSpace: ${activeColorSpace}`);
    } catch (error) {
      let err = error as BusinessError;
      Logger.error(
        `getActiveColorSpace failed, code=${err.code}, message=${err.message}`,
      );
    }
  } else {
...
this.videoOutput.start((err: BusinessError) => {
  if (err) {
    Logger.error(
      TAG,
      `Failed to start the video output. error: ${JSON.stringify(err)}`,
    );
    return;
  }
  Logger.info(
...
![](assets/HDR%20Vivid视频播放与录制开发实践/file-20260515114743185-3.png)


使用Interface (CameraManager)+AVCodec录制HDR Vivid视频，与录制普通视频的区别主要在于：

- AVCodec视频编码器AVCodec需要选择HEVC格式，并配置profile为HEVC_PROFILE_MAIN_10的相机底层。
- 编码器AVCodec配置颜色相关参数为COLOR_PRIMARY_BT2020。
- Camera相机在创建video output实例时，选择yuv 10bit profile。
...
```ts
export function videoProfileCheck(
  cameraManager: camera.CameraManager,
  cameraData: CameraDataModel,
): undefined | camera.VideoProfile {
  ...
  let videoProfile: undefined | camera.VideoProfile = videoProfiles.find(
    (profile: camera.VideoProfile) => {
...
```ts
let videoProfile: undefined | camera.VideoProfile = videoProfileCheck(
  cameraManager,
  params,
);
if (!videoProfile) {
...
encoderVideoOutput = cameraManager.createVideoOutput(
  videoProfile,
  params.surfaceId,
);
...
function isVideoStabilizationModeSupported(
  ...
  mode: camera.VideoStabilizationMode,
): boolean {
  let isSupported: boolean = false;
  try {
  ...
  if (isSupported) {
    ...
    try {
      ...
      hilog.error(
        0x0000,
        TAG,
        `setVideoStabilizationMode catch error, code: ${error.code}, message: ${error.message}`,
      );
    }
  } else {
...
function setColorSpaceBeforeCommitConfig(
  ...
  try {
    let colorSpace: colorSpaceManager.ColorSpace = isHdr
      ? colorSpaceManager.ColorSpace.BT2020_HLG_LIMIT
      : colorSpaceManager.ColorSpace.BT709_LIMIT;
    let colorSpaces: Array<colorSpaceManager.ColorSpace> =
      getSupportedColorSpaces(session);
    let isSupportedColorSpaces = colorSpaces.indexOf(colorSpace) >= 0;
    if (isSupportedColorSpaces) {
    ...
    hilog.error(
      0x0000,
      TAG,
      `setColorSpace catch error, code: ${error.code}, message: ${error.message}`,
    );
  }
}
```
6. 创建并配置相机会话。
```ts
let XComponentPreviewProfile: camera.Profile | undefined =
  previewProfileCameraCheck(cameraManager, params);
...
try {
  videoSession = cameraManager.createSession(
    camera.SceneMode.NORMAL_VIDEO,
  ) as camera.VideoSession;
} catch (error) {
  let err = error as BusinessError;
  Logger.error(
    TAG,
    `Failed to create the session instance. error: ${JSON.stringify(err)}`,
  );
}
// ...
...
方案2：参考《基于AVPlayer播放长视频实践》中亮度控制章节。
