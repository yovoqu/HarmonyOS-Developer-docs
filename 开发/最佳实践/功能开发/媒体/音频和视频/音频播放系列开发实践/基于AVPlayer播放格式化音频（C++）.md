...
| 场景名称 | 描述 | 实现方案 |
| --- | --- | --- |
| [基础播控](#section1764813377511) | 音频资源的加载、播放、暂停、退出等操作。 | 使用[avplayer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#概述)接口实现。 |
...
| [静音播放](#section125715278533) | 点击按钮设置静音播放。 | 使用avplayer的[OH_AVPlayer_SetVolume()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_setvolume)接口设置音量为0，进入静音状态。 |
| [切换歌曲播放](#section590418431566) | 点击上一首或下一首或歌单列表中的歌曲进行不同歌曲播放。 | 使用[OH_AVPlayer_Reset()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_reset)接口重置播放器状态，给avplayer的fd或fdSrc属性赋值为新的歌曲资源，实现播放不同的歌曲功能。 |
| [倍速设置](#section189460361122) | 滑动调节面板调节播放速度。 | 使用[OH_AVPlayer_SetPlaybackRate()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_setplaybackspeed)接口设置播放倍速。 |
| [音量设置](#section88718617116) | 滑动调节面板调节播放音量。 | 使用[OH_AVPlayer_SetVolume()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_setvolume)设置播放音量。 |
...
通过avplayer接口实现核心音频播放控制能力，包括音频资源加载、播放、暂停、停止及退出等操作。
...
核心原理是使用avplayer接口实现播放、暂停等功能，需要特别注意的是，播放器在执行不同的操作前，必须要保证此时处于正确的状态，比如执行播放操作前，只有当前状态在prepared/paused/completed时，才能正确执行，否则系统可能会抛出异常或生成其他未定义的行为。AVPlayer的播放状态和不同接口间的关系参考使用AVPlayer播放音频一节中的播放状态变化示意图。
...
第4步设置完url、fdSrc等属性后，播放器并不是就立刻进入initialized状态；第5步执行完[OH_AVPlayer_Prepare()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_prepare)接口，播放器也不是立刻进入prepared，都是需在[OH_AVPlayer_SetOnInfoCallback()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_setoninfocallback)注册状态变化的回调中，监听到播放器成功触发至initialized状态后，才能执行下一步的操作，否则接口会执行异常。
 ...
 7. 执行[OH_AVPlayer_Pause()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_pause)接口，暂停音频资源。 8. 执行[OH_AVPlayer_Stop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_stop)接口，停止播放音频资源。 9. 执行[OH_AVPlayer_Release()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-avplayer-h#oh_avplayer_release)接口，销毁播放资源。


### 开发步骤


1. 通过OH_AVPlayer_Create()创建一个AVPlayer实例。
...
2. 使用OH_AVPlayer_SetOnInfoCallback()注册状态变化的回调，对状态变化做出响应。
...
3. 使用OH_AVPlayer_SetOnErrorCallback()设置异常信息的回调，发生异常后，监听错误事件。
...
4. 通过OH_AVPlayer_SetFDSource()设置播放资源。
...
5. 执行OH_AVPlayer_Prepare()接口准备播放音频。
...
6. 执行OH_AVPlayer_Play()接口，播放音频资源。
...
7. 执行OH_AVPlayer_Pause()接口，暂停播放音频。
...
8. 执行OH_AVPlayer_Stop()接口，停止播放音频资源。
...
9. 执行OH_AVPlayer_Release()接口，销毁播放资源。
...
使用Slider组件实现进度条，在onChange()回调中触发进度调节获取目标时间，使用AVPlayer的seek()接口，跳转到目标时间。


### 开发步骤


使用avplayer的OH_AVPlayer_Seek()接口，跳转到目标时间。
...
使用avplayer的OH_AVPlayer_SetVolume()接口设置音量为0，进入静音模式状态。


### 开发步骤


调用avplayer的OH_AVPlayer_SetVolume()接口，如果确定开启静音模式，则将音量设置成0。
...
使用OH_AVPlayer_Reset()接口重置播放器状态，给AVPlayer的fd或fdSrc属性赋值为新的歌曲资源，实现播放不同歌曲的功能。
...
2. 用OH_AVPlayer_Reset()接口重置播放器状态。
...
3.使用OH_AVPlayer_SetFDSource()设置播放资源。
...
通过调节面板获取目标速度值，传入OH_AVPlayer_SetPlaybackRate()接口中，实现设置播放速度的功能。


### 开发步骤


1. 通过调节面板获取速度值，传入OH_AVPlayer_SetPlaybackRate()接口中。
...
2. 使用OH_AVPlayer_SetPlaybackRate()接口设置播放倍速。
...
通过调节面板获取目标音量值，输入到OH_AVPlayer_SetVolume()接口中，实现设置播放音量的功能。


### 开发步骤


1. 通过调节面板获取音量值，传入OH_AVPlayer_SetVolume()接口中。
...
2. 使用OH_AVPlayer_SetVolume()设置播放音量。
...
在调用AVPlayer的OH_AVPlayer_Prepare()、OH_AVPlayer_Play()、OH_AVPlayer_Stop()等方法时，会执行失败，返回错误码3。如以下场景。
...
通过OH_AVErrCode错误码查出，错误码3对应的信息为“AV_ERR_INVALID_VAL ”，可能得原因是AVPlayer的当前状态不支持此操作。AVPlayer播放器在执行不同的操作前，必须要保证此时处于正确的状态，比如执行播放操作前，只有当前状态在prepared/paused/completed时，才能正确执行。针对问题现象中举例的两种场景，其错误的原因可能如下。
...
1.  先了解在AVPlayer的不同播放状态下，可以执行哪些接口。熟悉AVPlayer的播放状态和不同接口间的关系，可以参考使用AVPlayer播放音频一节中的播放状态变化示意图一节中的播放状态变化示意图。

2. 保证在在正确的播放状态下，执行对应的接口。建议开发者务必使用OH_AVPlayer_SetOnInfoCallback()注册状态变化的回调，当监听到AVPlayer的播放状态到达目标状态时，执行对应的接口。当监听到AVPlayer处于initialized状态时，再执行OH_AVPlayer_Prepare()接口，监听到AVPlayer处于prepared状态时，再执行OH_AVPlayer_Play()接口。
