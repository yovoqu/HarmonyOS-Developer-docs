...
在使用动态模糊对图片组件进行处理时，若检测结果异常，动态模糊绘制过程中出现丢帧情况。建议改用静态模糊，以优化图片模糊性能。动态模糊与静态模糊的概念及使用场景，请参阅相关文档：使用场景。


![](assets/图像模糊卡顿问题分析/file-20260515114804551-2.png)


例如，从检测报告中的可能故障原因中，点击组件所在源码文件，可跳转定位至TestStructPage.ets页面中的Column组件处。在示例代码中，使用了blur动态模糊API对图片组件进行模糊处理，导致在检测中总耗时过长和丢帧。
...
针对图片模糊场景，建议使用静态模糊。通过静态模糊和动态模糊性能效果对比，可以发现在模糊效果类似的条件下，静态模糊的性能要优于动态模糊。建议开发者在组件背景和内容无需实时更新的场景中，优先使用静态模糊，可以减少应用卡顿与丢帧，提升用户体验。
...
例如，从检测报告中可能故障原因中，点击组件所在源码文件，可跳转定位至ReduceBlurRadius.ets页面中的Text组件处。在示例代码中，使用了backdropBlur()对Text组件进行背景模糊，其模糊半径为2。
