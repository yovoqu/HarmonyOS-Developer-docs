...
在安防监控、车载回放等场景中，录像回放是核心功能。用户通过滑动时间轴，查看不同时间节点的历史视频。本文基于Canvas绘图能力和视频组件，提供功能完备的录像回放时间轴解决方案，封装了核心组件TimeBarView，助力开发者快速实现时间轴功能。该组件集成了时间轴绘制、单指滑动浏览、双指缩放切换时间粒度等功能。
...
TimeBarView组件采用分层绘制，依次为刻度线、中间线、视频区域及顶层自定义绘制区域。通过滑动和缩放手势事件，实时更新状态并触发时间轴重绘。详细实现参考本章开发流程。
...
const middleLineDuration =
  (viewWidth / 2) * (TimeMsUnit.TEN_MINUTE / this.intervalWidth);

// Time at the far left edge
this.leftTime = this._currentTime - middleLineDuration;
```
**绘制刻度线** 以10分钟为最小单位生成刻度，在小时模式（MODE_HOUR）下，每60分钟绘制一次大刻度及时间文本；在分钟模式（MODE_MINUTE）下，将10分钟划分为10个细分刻度，因此循环10次绘制刻度线。drawTickLine()方法用于绘制大、小刻度，drawTimeText()方法用于绘制大刻度对应的时间文本。
```ts
// Find the first tick (aligned to 10 minutes)
let minuteNum = Math.floor(this.leftTime / TimeMsUnit.TEN_MINUTE);
let xPosition =
  (minuteNum * TimeMsUnit.TEN_MINUTE - this.leftTime) *
  (this.intervalWidth / TimeMsUnit.TEN_MINUTE);
...
for (let i = 0; i < scaleNum; i++) {
  ...
  if (this.divisorMode == ScaleMode.MODE_HOUR) {
    ...
    const tickLength = isMajorTick
      ? this.timeScaleOption.scaleLineHeight * 2
      : this.timeScaleOption.scaleLineHeight;
    ...
    for (let j = 0; j < 10; j++) {
      const positionX =
        j === 0 ? currentX : currentX + j * this.intervalWidth * 0.1;
...
```
2. 中间指示线绘制中间线是时间轴上的当前时间标记，固定在画布中央，用于标识当前选中或播放的时间点。
```ts
...
```
3. 视频区域绘制**获取图片pixelMap对象** 视频区域用于显示视频片段的时间分布（如录制片段、有效视频区间），采用离屏预渲染，避免滑动缩放时重复计算绘制。
...
const offSettings: RenderingContextSettings = new RenderingContextSettings(
  false,
);
const offCanvas: OffscreenCanvas = new OffscreenCanvas(
  videoAreaWidth,
  this.timeBarOption.height,
);
...
segArr.forEach((recordSegment) => {
  ...
  const videoAreaHeight =
    this.videoAreaOption.topOffset + this.videoAreaOption.height >
    this.timeBarOption.height
      ? this.timeBarOption.height - this.videoAreaOption.topOffset
      : this.videoAreaOption.height;
  offPaint.fillRect(
    leftX,
    this.videoAreaOption.topOffset,
    width,
    videoAreaHeight,
  );
});

const data = offPaint.getPixelMap(
  0,
  0,
  videoAreaWidth,
  this.timeBarOption.height,
...
```
**实时绘制** 缩放时，通过矩阵变换（缩放）将预渲染图适配当前画布比例，避免重复绘制。
```ts
...
const nextTime =
  this._currentTime -
  effectiveDelta * (TimeMsUnit.TEN_MINUTE / this.intervalWidth);
...
this._onTimeBarMoveListener?.onMoveScaleCallback(
  this._currentTime,
  PlayStatus.PLAYING,
);
```
...
- 模式切换：根据intervalWidth阈值切换显示模式（小时/分钟）。intervalWidth 边界限制：设置最小（10vp）和最大（180vp）缩放阈值，避免过度缩放。
图2 缩放时间轴示意图
...
- 边界滑动：拖动至视频范围最右侧（最后一段视频结束时间），时间轴无法继续右滑，视频停留在最后一帧。计算目标视频位置（时间映射与边界处理）从时间轴拖动的currentTime（选中时间）出发，通过“时间偏移计算→片段定位→进度校正”，确定视频需跳转的目标进度。核心代码如下：
```ts
...
const info = findSegmentIndexByTimeline(
...
const playEnd =
  nextIndex === this.videosInfo.length - 1 && clampedOffset === totalTimelineMs;

// Compute the preview/target progress inside the segment (seconds)
const videoProgressSec = this.computeProgressSeconds(
  clampedOffset,
  segStartMs,
  segDurMs,
  info,
...
```ts
if (
  Math.abs(offsetX - this.lastOffsetX) < 0.5 &&
  Math.abs(offsetY - this.lastOffsetY) < 0.5
) {
  return;
}
