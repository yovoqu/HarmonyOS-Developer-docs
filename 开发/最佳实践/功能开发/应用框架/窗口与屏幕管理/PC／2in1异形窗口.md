...
// entry/src/main/syscap.json
{
  ...
  "development": {
    "addedSysCaps": ["SystemCapability.Window.SessionManager"]
  }
}
```
...
if (windowStage === null) {
  hilog.error(
    0x0000,
    'Sample',
    'Failed to create the subwindow. Cause: windowStage is null',
  );
  return;
}
...
try {
  ...
  hilog.error(
    0x0000,
    'Sample',
    'Failed to create sub window. Cause: ' + JSON.stringify(exception),
  );
}
```
...
async function getCircleMask(
  width: number,
  height: number,
): Promise<number[][]> {
  const radius = Math.min(width, height) / 2;
  const maskArray: number[][] = new Array(height)
    .fill(null)
...
async function getTriangleMask(
  width: number,
  height: number,
): Promise<number[][]> {
  const maskArray: number[][] = new Array(height)
...
```ts
async function setWindowShape(
  win: window.Window,
  width: number,
  height: number,
  getMaskFunc: (w: number, h: number, picPath?: string) => Promise<number[][]>,
  picPath?: string,
): Promise<void> {
 ...
 */
export async function setWindowCircleShape(
  win: window.Window,
  width: number,
  height: number,
): Promise<void> {
 ...
 * @param height The original window height
 */
export async function setWindowTriangleShape(
  win: window.Window,
  width: number,
  height: number,
): Promise<void> {
...
![](assets/PC／2in1异形窗口/file-20260515114612017-0.png)
 ...
 */
export async function image2PixelMap(
  Context: Context,
  icon: string,
  w: number,
  h: number,
  ...
  try {
    const rawFileDescriptor: resourceManager.RawFileDescriptor =
      Context.resourceManager.getRawFdSync(icon);
...
```ts
export async function getPicMask(
  Context: Context,
  width: number,
  height: number,
  picPath: string,
  ...
  const maskArray: number[][] = new Array(height)
    .fill(null)
    .map(() => new Array(width).fill(0));
  const pixelMap: image.PixelMap = await image2PixelMap(
    Context,
    picPath,
    width,
    height,
 ...
 */
export async function setWindowPicShape(
  win: window.Window,
  Context: Context,
  width: number,
  height: number,
