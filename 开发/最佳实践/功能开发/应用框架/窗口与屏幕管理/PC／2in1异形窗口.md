...
1. 为使用setWindowMask()等接口，需要额外配置syscap.json文件，在模块/src/main目录下，创建syscap.json文件，填入以下内容：// entry/src/main/syscap.json
{
  "devices": {
 "general": [
 "2in1"
...
#### 设置窗口形状
1. 根据要实现的异形窗口形状，计算二维数组掩码windowMask。以下是以原矩形中点为圆心、直径为500的圆。最终效果见圆形子窗口效果。function fillCircle(array: number[][], x: number, y: number, radius: number): void {
...
async function getCircleMask(width: number, height: number): Promise<number[][]> {
  ...
  return maskArray;
} 以下以原矩形宽为底边、原矩形高为高的等腰三角形。最终效果见三角形子窗口效果。function fillTriangle(array: number[][], base: number, height: number): void {
...
2. 使用setWindowMask(windowMask)接口，设置窗口形状。async function setWindowShape(win: window.Window, width: number,
...
3. 使用showWindow()接口显示异形子窗口。subWindow.showWindow();
...
![](assets/PC／2in1异形窗口/file-20260525090644009-002.png)
...
![](assets/PC／2in1异形窗口/file-20260525090644009-003.png)
...
2. 从pixelMap数据中取出颜色信息数组，将每个像素的透明度单独提取至allPixels，再转化生成为对应形状的二维数组掩码maskArray。export async function getPicMask(Context: Context,width: number, height: number, picPath: string): Promise<number[][]> {
  const maskArray: number[][] = new Array(height).fill(null).map(() => new Array(width).fill(0));
...
3. 使用setWindowMask(windowMask)接口，设置窗口形状。async function setWindowShape(win: window.Window, width: number,
  height: number, getMaskFunc: (w: number, h: number, picPath?: string) => Promise<number[][]>,
  picPath?: string): Promise&lt;void&gt; {
  const windowMask = await getMaskFunc(width, height, picPath);
  if (canIUse('SystemCapability.Window.SessionManager')) {
...
4. 使用showWindow()接口显示异形子窗口。subWindow.showWindow();

#### 实现效果
**图4 **不规则形状子窗口效果

![](assets/PC／2in1异形窗口/file-20260525090644009-004.png)
