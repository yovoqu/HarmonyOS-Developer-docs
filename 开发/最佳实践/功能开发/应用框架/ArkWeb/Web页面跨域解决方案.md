...
1. 应用文件目录通过[Context.filesDir()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-context#context)获取，其子目录示例如下：/data/storage/el2/base/files/example/data/storage/el2/base/haps/entry/files/example
2. 应用资源目录通过[Context.resourceDir()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-context#context)获取，其子目录示例如下：/data/storage/el1/bundle/entry/resources/resfile/data/storage/el1/bundle/entry/resources/resfile/example
...
- 代理请求：在WebView中设置拦截器拦截Web页面发起的跨域请求，使用[rcp](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/remote-communication-rcp)请求代理请求到目标远程服务器。RCP请求与远程服务器的通信不受浏览器限制，因此可以接收到服务器的响应结果，但将结果传回WebView时仍需配置跨域响应头来解决跨域问题。图1 代理请求方案流程图
...
2. 使用RCP请求作为跨域请求的代理请求。 export class HttpProxy {
  session?: rcp.Session;
...
3. 新建响应头，将跨域响应头配置到新响应头上，将新响应头和RCP请求的结果都放到拦截器的[resourceHandler](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview-webresourcehandler)中，通知Web组件被拦截的请求已经完成。private handleResponse(res: rcp.Response, resourceHandler: webview.WebResourceHandler,
...
#### 开发步骤
1. 在Web前端访问服务器时，设置withCredentials: true。const response = await axios.post('http://www.b.harmonyos:8080/api/getCookies', {}, {
  withCredentials: true
});
2. 在ArkWeb中，使用[WebCookieManager](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview-webcookiemanager)的相关接口，完成允许携带Cookie、获取A域名下的Cookie、将Cookie设置到B域名下的操作。webview.WebCookieManager.putAcceptCookieEnabled(true);
...
3. 服务器端在响应头中添加Access-Control-Allow-Credentials: true，将Access-Control-Allow-Origin设置为具体域名（非通配符“*”）。appB.use(cors({
  origin: 'http://www.a.harmonyos:8080',
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization'],
  credentials: true
...
#### 开发步骤
1. 为自定义协议app://设置拦截器。this.schemeHandler.onRequestStart((request: webview.WebSchemeHandlerRequest,
...
2. 拦截WebView组件加载页面资源时的自定义协议资源，读取对应的本地资源文件进行替换，创建响应头，将响应头和读取的本地资源设置到拦截器的resourceHandler中。let pathArray = request.getRequestUrl().split('app://');
...
3. 拦截自定义协议接口，调用显示系统弹窗接口，提示自定义协议已被拦截，返回true拦截该请求。if (request.getRequestUrl().includes('openDialog')) {
