...
4. scan：配置定义点的扫描范围，指定模块、路径、类和指定方法。
...
4. scan：配置调用点的扫描范围，指定模块和路径。

5. api：配置目标API，包括模块、导入名称和方法名称。
