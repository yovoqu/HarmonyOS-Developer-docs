...
网络数据传输是应用的核心功能场景之一，而传输的性能直接决定着用户体验——例如高清晰度视频/图片的加载、大文件的上传下载、设备间多文件的同步迁移等场景，有限的网络传输性能导致的卡顿、长时间等待往往会导致用户使用感受变差。针对此类性能敏感的场景，Network Boost Kit的多网并发能力提供了网络加速的解决方案。
...
1. 开发前准备多网并发能力在设备类型、网络状态及开发方案的选择上存在一定限制，具体接入要求可参考[多网发起和释放](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/networkboost-netmultipath-request-release)，开发者也可以在调试过程中通过[多网请求错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section8508131819279)分析。
2. 发起多网并发(1) 权限配置：应用使用多网并发等网络加速能力，连接迁移能力部分接口需要ohos.permission.LINKTURBO权限，权限配置方法参见[申请权限步骤](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/networkboost-preparations#section1648815561221)。同时此权限为受限ACL权限需要特别配置和申请，具体操作步骤参考[配置签名](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/networkboost-preparations#section94731428358)和[受限ACL权限申请](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/networkboost-preparations#section13192154213515)。 (2) 发起流程：多网并发能力发起的核心步骤包括[netBoost.setSceneDesc()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-netboost#section13106021163)设置多网场景、[netHandover.getMultiPathQuotaStats()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section47829529513)查询多网配额、[netHandover.requestMultiPath()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section8508131819279)请求发起多网以及[netHandover.on('multiPathStateChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section7919111205211)多网状态监听，详细实现可参考[多网发起与接入](#li4874151714524)。
3. 获取多网NetHandle并绑定Socket(1) 成功发起多网请求后，可以在[netHandover.on('multiPathStateChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section7919111205211)回调中监听并获取到可用的[NetHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#nethandle)数据网络句柄对象。 (2) 针对需要在多网路径上传输的数据，创建对应的[TCP Socket](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-socket#tcpsocket)/[UDP Socket](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-socket#udpsocket)，通过[bindSocket()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#bindsocket9)方法将Socket对象绑定到NetHandle对应的网络上。
4. Socket发起网络传输任务绑定网络后，TCP/UDP Socket进行[数据传输](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/socket-connection#应用通过tcp-socket-server进行数据传输)时将会使用对应的网络通路，多个Socket使用不同的网络通路同时发起网络传输任务就可以起到多网并发的效果。
...
![](assets/多网并发网络加速/file-20260515114628479-0.png)
...
export function setUploadSceneDesc(enter: boolean): boolean {
  try {
    let sceneDesc: netBoost.SceneDesc = {
      scene: 'upload',
      sceneEvent: enter
        ? netBoost.SceneEvent.SCENE_EVENT_ENTER
        : netBoost.SceneEvent.SCENE_EVENT_LEAVE,
    };
    ...
    hilog.error(
      DOMAIN,
      TAG,
      `[setUploadSceneDesc] Error:${error.code},${error.name}`,
    );
    return false;
  }
}
...
export function getMultiPathQuota(): netHandover.MultiPathQuota | undefined {
  try {
    let quota: netHandover.MultiPathQuota =
    ...
    hilog.error(
      DOMAIN,
      TAG,
      `[getMultiPathQuota] Error:${error.code},${error.name}`,
    );
    return;
  }
}
  ...
  return new Promise((resolve) => {
    try {
      netHandover.requestMultiPath(
        (data: netHandover.MultiPathRequestResult) => {
          if (
            data &&
            data.result ===
              netHandover.MultiPathErrorResult.MULTIPATH_ERROR_NONE
          ) {
            resolve(true);
          } else {
            resolve(false);
      ...
      hilog.error(
        DOMAIN,
        TAG,
        `[requestMultiPath] Error:${error.code},${error.name}`,
      );
    }
  });
}
  ...
  try {
    netHandover.on(
      ...
      (data: netHandover.MultiPathStateInfo) => {
        if (
          data.multiPathState ===
            netHandover.MultiPathState.MULTIPATH_CREATED &&
          data.netHandle.netId >= CommonConst.MIN_USEFUL_NET_ID
        ) {
          netHandleChange?.onMultiNetSuccess(data.netHandle);
        } else if (
          data.multiPathState === netHandover.MultiPathState.MULTIPATH_CREATING
        ) {
          hilog.info(
            DOMAIN,
            TAG,
            `[setOnMultiPathStateChange.multiPathStateChange] ${data.netHandle.netId} Creating`,
          );
        } else {
    ...
    hilog.error(
      DOMAIN,
      TAG,
      `[setOnMultiPathStateChange] Error:${error.code},${error.name}`,
    );
  }
}
```
2. 多网通路绑定Socket封装网络查询函数，通过[connection.getAllNetsSync()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#connectiongetallnetssync10)获取当前所有网络信息。由于网络传输任务一般放在子线程实现，而当前无法跨线程传递Socket对象和NetHandle对象，为了在子线程实现网络与Socket绑定需要获取所有网络信息，再通过之前请求多网后记录的NetHandle对象的ID找到指定网络在执行绑定操作；如果应用业务不涉及多线程问题可直接绑定上一步多网状态监听中获取到的NetHandle。
```ts
/**
 * Get all netHandles.
  ...
  try {
    ...
    for (let i = 0; i < netHandles.length; i++) {
      hilog.info(
        DOMAIN,
        TAG,
        `[getAllNetHandles] success [${i}]>>>${netHandles[i].netId}`,
      );
    ...
    hilog.error(
      DOMAIN,
      TAG,
      `[getAllNetHandles] Error:${error.code},${error.name}`,
    );
    return undefined;
  }
}
 ...
 * */
export async function bindSpecifiedNet(
  socketInstance: socket.TCPSocket,
  netId: number,
  port: number,
) {
  ...
  if (netHandles) {
    for (let i = 0; i < netHandles.length; i++) {
      if (netHandles[i].netId === netId) {
        try {
          const connectionProperties = connection.getConnectionPropertiesSync(
            netHandles[i],
          );
          if (
            connectionProperties &&
            connectionProperties.linkAddresses &&
            ...
            hilog.error(
              DOMAIN,
              TAG,
              `[socketBindNet] Get ConnectionProperties error`,
            );
          }
        } catch (err) {
          const error: BusinessError = err as BusinessError;
          hilog.error(
            DOMAIN,
            TAG,
            `[socketBindNet] Error occurred:${error.code},${error.name}`,
          );
        }
        break;
      }
...
```
3. 文件分片处理封装文件处理类，设置分片文件数据结构，包括当前文件片段数量、开始位置、分片大小、片段重试次数等信息。
```ts
/**
...
function getChunkSize(fileSize: number): number {
  ...
  hilog.info(
    DOMAIN,
    TAG,
    `[getChunkSize] FileSize:${fileSize},ChunkSize:${res}`,
  );
  return res;
}
```
...
```
4. 多Socket并发上传不同文件片段封装分片文件上传类，提供初始化分片上传接口，通知服务端待上传文件片段对应的存储路径。此处代码片段仅供参考，开发者可以根据自己业务的实际实现方式调整通信格式、请求参数等。
```ts
/**
...
```
5. 通知服务端整合分片文件全部文件片段上传完毕后，通知服务端文件片段顺序，进行文件整合。
```ts
/**
...
```
6. 释放资源通过[netHandover.releaseMultiPath()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section196405414517)释放多网，注销相关的监听方法。
```ts
/**
 * Stop multi net path.
 * */
export function releaseMultiPath() {
  try {
    ...
    hilog.error(
      DOMAIN,
      TAG,
      `[releaseMultiPath] Error:${error.code},${error.name}`,
    );
  }
}
...
export function setOffMultiPathStateChange() {
  try {
    ...
    hilog.error(
      DOMAIN,
      TAG,
      `[setOffMultiPathStateChange] Error:${error.code},${error.name}`,
    );
  }
}
```
...
![](assets/多网并发网络加速/file-20260515114628479-1.png)
