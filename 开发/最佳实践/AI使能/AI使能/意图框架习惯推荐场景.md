...
![](assets/意图框架习惯推荐场景/file-20260515114945350-0.png)
...
![](assets/意图框架习惯推荐场景/file-20260515114945350-1.png)
...
3. 调用[shareIntent()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/intents-arkts-api-insightintent#shareintent)接口将意图对象输入到HarmonyOS，用于学习用户的行为规律。成功共享后“小艺建议”会展示对应应用的音乐模板卡片。展示效果如图所示。
![](assets/意图框架习惯推荐场景/file-20260515114945350-2.png)
...
4. 配置PROJECT_HOME/entry/src/main/ets/entryability/InsightIntentExecutorImpl.ets文件，定义onExecuteInUIAbilityForegroundMode()方法。点击卡片，会拉起对应的应用并触发onCreate()和onExecuteInUIAbilityForegroundMode()等方法。意图调用如果会拉起应用界面，采取前台模式，如果不需要拉起采取后台模式。应用在foreground模式和background模式下会触发不同的生命周期与方法：
...
```
 如果想删除掉意图，可以调用[deleteIntent()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/intents-arkts-api-insightintent#deleteintent)，在注册文件只有一个意图的情况下卡片入口会消失。运行效果如下图所示。

  
![](assets/意图框架习惯推荐场景/file-20260515114945350-3.gif)
