...
1、通过relationalStore.getRdbStore()接口创建或打开已有关系型数据库。

2、通过execute()接口执行建表操作，包含id、subject text、content text、image_text text、attachment_names text等字段。
...
依次读取原始数据文件，将读出的数据使用execute()接口插入到新建的数据库中。
...
1、初始化请求，配置请求参数，在header中需要配置生成的API生成的key，extraData中需要传入相关的提问和模型参数，参考请求参数说明设置frequency_penalty频率奖惩、presence_penalty新词语奖惩、top_p浮点数累计概率控制确保大模型回答偏向简短精准，提升系统的回答速度和准确度。
...
2、使用requestInStream()根据URL地址和相关配置项，发起HTTP网络请求并返回流式响应。
...
2、构建检索配置retrieval.RetrievalConfig，配置向量数据库和倒排索引数据库的参数，并将这些配置绑定到相应的检索通道中，构建了一个用于多数据源检索的配置对象retrievalConfig。这个配置对象可以被后续的检索操作使用，以从指定的数据库中检索数据。
...
3、配置检索条件retrieval.RetrievalCondition，从多个数据源（基于逆向索引和向量相似度）召回相关数据，并通过RRF方法对召回结果进行重排，最终返回5条最相关的记录，在信息检索系统中提升召回率和精确率。
...
4、配置LLM会话，在HTTP流式响应数据接收事件中进行数据解析，将问题的答案和是否结束封装在answer中传入回调。此步骤会因大模型的选型受到影响，开发中根据实际选择的大模型进行调整。
...
export default class MyChatLlm extends rag.ChatLLM {
  ...
  async streamChat(
    query: string,
    callback: Callback<rag.LLMStreamAnswer>,
    ...
    try {
        ...
        (data) => {
          try {
            ...
            if (resultStr.startsWith('{"error_code"')) {
              ...
              try {
                ...
                if (
                  ...
                  obj['error_msg'] &&
                  obj['error_code'] &&
                  obj['error_msg'] === 'Invalid authorization header.'
                ) {
                ...
                hilog.error(
                  0,
                  'MyChatLlm',
                  'Parse json failed. String: ' + resultStr,
                );
              }
            ...
            if ((obj as object)?.['choices'][0]['delta']['reasoning_content']) {
              chunk = (obj as object)?.['choices'][0]['delta'][
                'reasoning_content'
              ];
            } else {
            ...
            hilog.error(
              0,
              'MyChatLlm',
              `BusinessError, error code: ${err.code}, error message: ${err.message}`,
            );
          }
        },
      ...
      hilog.error(
        0,
        'MyChatLlm',
        `Request HuaweiYun failed, error code: ${err.code}, error message: ${err.message}`,
      );
      if (err.code === 2300028) {
...
5、在完成检索条件、检索配置、LLM会话的配置后，即可配置rag.Config。
...
1、使用rag.createRagSession()接口创建会话，需要传入应用上下文context和rag.Config。
...
rag
  .createRagSession(this.context, sessionCfg)
  .then((data: rag.RagSession) => {
  ...
  .catch((err: BusinessError) => {
    hilog.error(
      DOMAIN,
      'testTag',
      `createRagSession failed, code is ${err.code},message is ${err.message}.`,
    );
  });
```

2、使用streamRun()接口开始会话，需要传入提问的问题、提问选项和处理的动作，接收返回的字符串进行拼接形成答案。
...
hilog.info(
  0,
  TAG,
  `Before streamRun, time: ${this.streamRunStartTime.getTime()}`,
);
let ragSession: rag.RagSession = AppStorage.get<rag.RagSession>(
  'RagSessionObject',
) as rag.RagSession;
