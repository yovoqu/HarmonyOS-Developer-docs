...
3. 减少使用存储权限。过多的存储访问权限可能会导致用户隐私数据的泄露，应该减少使用存储权限，仅请求访问应用程序所需的数据，可以用[Picker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-picker)来减少对用户存储数据的访问权限。
4. 动态申请敏感权限。申请敏感权限需满足最小化要求，只申请必需的信息或资源权限，减少权限滥用和敏感数据泄露。
5. 数据加密处理。技术上保证数据处理的安全性，包括加密存储和安全传输，应默认开启或采取安全保护措施。 存储敏感数据应该进行加密处理，具体可以参考《[应用数据安全](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-data-security)》。
...
![](assets/应用隐私保护/file-20260515114947563-0.png)
...
![](assets/应用隐私保护/file-20260515114947563-1.png)
...
geoLocationManager
  ...
  .then((result) => {
    Logger.info(
      `geoLocation current location:error: ${JSON.stringify(result)}`,
    );
    // ...
  })
  .catch((error: BusinessError) => {
    Logger.error(
      `geoLocation getCurrentLocation: error: ${JSON.stringify(error)}`,
    );
  });
```
...
![](assets/应用隐私保护/file-20260515114947563-2.png)
