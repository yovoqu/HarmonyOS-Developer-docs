...
| 混淆选项 | -enable-property-obfuscation | 属性混淆 |
| 混淆选项 | -enable-toplevel-obfuscation | 顶层作用域名称混淆 |
| 混淆选项 | -enable-filename-obfuscation | 文件名混淆 |
| 混淆选项 | -enable-export-obfuscation | export导出名称与属性混淆 |
| 混淆选项 | -compact | 代码压缩 |
| 混淆选项 | -remove-log | 删除console.*方法 |
| 混淆选项 | -print-namecache filepath | 指定路径输出namecache.json文件及内容 |
| 混淆选项 | -apply-namecache filepath | 复用指定的名称缓存文件 |
| 混淆选项 | -remove-comments | 删除注释 |
| 保留选项 | -keep-property-name | 保留属性名 |
| 保留选项 | -keep-global-name | 保留顶层作用域和导出元素的名称 |
| 保留选项 | -keep-file-name | 保留指定的文件/文件夹的名称 |
| 保留选项 | -keep-dts | 读取指定.d.ts文件中的名称作为白名单 |
| 保留选项 | -keep-comments | 保留编译生成的声明文件中class, function, namespace, enum, struct, interface, module, type及属性上方的JsDoc注释 |
| 保留选项 | -keep | 保留指定相对路径中的所有名称（例如变量名、类名、属性名等） |
| 保留选项 | 通配符 | 名称类和路径类的保留选项支持通配符 |
