...
**任何 HTML→Markdown 转换都必须使用 `fetch_docs.py` 中的完整 `html_to_markdown()` 函数。严禁编写简化版转换器。**

2026-07-01 的教训：`pull_new_docs.py` 中编写了简化版 `html_to_markdown_simple()`，仅从 `<h1>` 提取 `device-type`，导致 3,429 篇文档的子标题（h2/h3/h4）设备信息全部丢失。最终需要用完整转换器全部重拉。

简化版每次都会遗漏不同细节（device-type、note callout、rowspan、代码块保护顺序等），唯一安全的做法是始终使用经过验证的完整实现。
...
- **来源**：官网完整 URL
- **支持设备**（仅 API 参考文档）：从 `<h1 device-type="...">` 提取文档级别
- **支持设备（子标题）**：从 `<h2>/<h3>/<h4 device-type="...">` 提取段落级别，每个带属性的子标题后插入独立 `**支持设备：**` 行
...
### 4.7 表格行间空行（2026-08-13 教训）

**问题**：部分文档（实测 637 个文件）的表格**数据行之间有空行**，Markdown 渲染时会把表格拆散成多个独立表格或破坏对齐。

```markdown
| 接口 | 描述 |
| --- | --- |
| OH_Drawing_PenCreate (void) | 用于创建一个画笔对象。 |
                                            ← 错误：行间空行
| OH_Drawing_PenSetColor (...) | 用于设置画笔颜色 |
```

**检测**：扫描 `表格行 + 空行 + 表格行` 模式，排除代码块内内容。

**修复规则**（必须区分"同表行间空行"与"两个独立表格的合法间隔"）：

| 情况 | 处理 |
|------|------|
| 空行两侧都是表格行，且同列数 / 含分隔线 / 分隔线已在前方出现 | **删除空行**（同表） |
| 空行后的行是新表头（其后 1 行内有分隔线） | **保留空行**（不同表格） |
| 空行后的行后跟文字/列表（非表格行） | **删除空行**（同表中段） |
| 行位于代码块（``` 包裹）内 | 不处理 |
...
# b = 空行后的表格行
nxt = 第一个非空行(b 之后)
b_is_new_header = nxt.startswith('|')   # 后跟表格行 = 新表头
if not b_is_new_header:
    # b 是同表数据行 → 删除空行
```
...
- [ ] API 参考文档及子标题是否包含 `**支持设备：**`（h1/h2/h3/h4 的 device-type 均需提取）
- [ ] API 元数据项之间是否有空行分隔
